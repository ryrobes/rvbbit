...
(defn pool-create
  [jdbc-body name]
  (delay (let [base  {:idle-timeout 600000
...
(def flows-db
  {:datasource @(pool-create
                  {:jdbc-url
                     ;"jdbc:sqlite:file:./db/flow.db?cache=shared&journal_mode=WAL&auto_vacuum=FULL" ;&transaction_mode=IMMEDIATE&journal_mode=WAL"
                   "jdbc:sqlite:file:./db/flow.db?cache=shared&journal_mode=WAL&busy_timeout=5000&locking_mode=NORMAL";
                   :idle-timeout 600000
                   :max-lifetime 1800000
                   :auto_vacuum "FULL"
                   :cache "shared"}
...
(def ghost-db
  {:datasource @(pool-create
                  {:jdbc-url
                     "jdbc:sqlite:file:./db/ghost.db?cache=shared&journal_mode=WAL&mode=memory" ;&transaction_mode=IMMEDIATE&journal_mode=WAL"
                   :idle-timeout 600000
                   :max-lifetime 1800000
                   :cache "shared"}
                  "ghost-db")})
...
(defn insert-error-row-OLD!
  ...
  (jdbc/with-db-connection [sdb system-db] ;; ?
                           (let [ins (-> {:insert-into [:errors]
                             ...
                             (try (jdbc/db-do-commands sdb ins)
                                  (catch Exception e
...
(defn sql-query
  ...
  (jdbc/with-db-connection ;; for connection hygiene - atomic query
    ...
    (try (wrap-maps
           query
           (jdbc/query t-con query {:identifiers keyword :as-arrays? true :result-set-fn doall}))
         (catch Exception e
...
(defn sql-exec2
  ...
  (jdbc/with-db-transaction [t-con db-spec {:isolation :read-uncommitted}]
                            (try (jdbc/db-do-commands t-con query)
                                 (catch Exception e
                                   (do (ut/pp (merge
                                                {:sql-exec-fn-error e :db-spec db-spec :query query}
                                                (if extra {:ex extra} {})))
                                       (insert-error-row! db-spec query e))))))
...
(defn worker-loop-sql
  ...
  (loop []
    (when @running-sql
      (let [queue (@task-queues-sql keyword) task (when queue (.take queue))] (when task (task))))
    (recur)))

(defn start-workers-sql
  [keyword num-workers]
  (reset! running-sql true)
  (swap! workers-sql assoc
         keyword
         (doall (map (fn [_] (future (worker-loop-sql keyword))) (range num-workers)))))

(defn enqueue-task-sql
  [keyword task]
  (let [queue
        (or (@task-queues-sql keyword)
...
(defn recycle-workers-sql
  [keyword num-workers]
  (stop-workers-sql keyword)
  (start-workers-sql keyword num-workers))


(defn sql-exec
  [db-spec query & [extra]]
  (enqueue-task-sql db-spec (fn []
...
(defn to-sql
  ...
  (let [sql-dialect (when key-vector
                      (cond (cstr/includes? (cstr/lower-case (first key-vector)) "microsoft")
                              :sqlserver
        ...
        value-mapping (if value-keys?
                        (into {} (for [v value-keys] {v (lookup-value key-vector v system-db)}))
                        {})
        hm (walk/postwalk-replace value-mapping honey-map) ; (walk/postwalk {} honey-map)
        fixed-honey-map
          (cond (and (= sql-dialect :sqlserver) (get hm :limit))
                  (let [limit           (get hm :limit)
                        select-distinct (get hm :select-distinct)
                        select          (get hm :select)]
                    (cond select-distinct (dissoc (dissoc (assoc hm
                                                            :select-distinct-top
                                                              (vec (cons [limit] select-distinct)))
                                                    :select-distinct)
                                            :limit)
                ...
                (and (= sql-dialect :oracle) (get hm :limit))
                  (let [limit (get hm :limit)] (dissoc (assoc hm :fetch limit) :limit))
                :else hm)]
    (try (-> fixed-honey-map
             (honey/format (merge {:pretty true :inline true}
                                  (if (nil? sql-dialect) {} {:dialect sql-dialect}))))
         (catch Exception e
...
(defn fetch-all-tables
  ...
  (let [;table-names-query (to-sql {:select [:name]
        ...
        table-data  (doall (walk/keywordize-keys
                             (into {} (map #(hash-map % (fetch-table %)) table-names))))]
    table-data))
...
(defn safe-sql-exec-helper
  ...
  (try (sql-exec-risk db query)
       (catch Exception e
         (println "Error during SQL execution, retrying..."
                  (.getMessage e)
                  " query: "
                  (hash query)
                  query)
