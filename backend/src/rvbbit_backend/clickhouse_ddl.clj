...
(def create-errors
  ...
  EXISTS
  errors
  (db_conn String sql_stmt String error_str String ts DateTime DEFAULT now ())
  ENGINE
  =
  MergeTree
  ()
...
(def create-rules-table1
  "
      ...
      NOT
      EXISTS
      rule_maps_attributes
      (connection_id String run_id Int32 attribute_name String when_logic String)
      ENGINE
      =
      MergeTree
      ()
...
(def create-rules-table2
  "
      ...
      NOT
      EXISTS
      rule_maps_derived_fields
      (connection_id String run_id Int32 field_name String when_logic String calc_logic String)
      ENGINE
      =
      MergeTree
      ()
...
(def create-logs
  "
      ...
      EXISTS
      logs
      (booted String ts Float64 hts String caller String type_key String log_str String)
      ENGINE
      =
      MergeTree
      ()
...
(defn create-attribute-sample
  ...
  (try
    (let [field-types (cstr/join "
      ...
      ddl)
    (catch Exception e
      (ut/pp [:error-creating-ddl-for-sample-based-on-rowset sample-table-name-str :error (str e)
              :rowset rowset]))))
