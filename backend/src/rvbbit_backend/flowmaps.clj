  ...
  (let [bd            (try (get-in @block-defs [flow-id (nth keypath 2)]) (catch Exception _ nil))
      ...
      (do ;; ^ again. *not* redundant here.
        (swap! block-dump assoc-in
          ...
          (conj (if rewind-limit?
                  (vec (take-last rewind (sort-by :start (get-in @block-dump [flow-id :block-history block-id] []))))
    ...
    (when (and ts bd)
      (swap! waffle-data assoc
        flow-id
        (conj (if rewind-limit? (vec (take-last rewind (sort-by :start (get @waffle-data flow-id)))) (get @waffle-data flow-id))
  ...
  (try (let [deadline (+ (System/currentTimeMillis) timeout-ms)]
         (loop []
           (let [events (get @channel-history flow-id)
                 event  (first (filter #(and (= (get % :channel) channel) (>= (get % :end) start-time)) events))]
             (cond event                                   event ;(get event :value) ; Found the desired event, return it
                   (> (System/currentTimeMillis) deadline) (throw (Exception. (str "Timeout waiting for event to hit channel "
                                                                                     channel
                                                                                   " in flow " flow-id)))
                   :else                                   (do (Thread/sleep 300) ; Wait for a short duration before polling
                                                                                  ; again
                                                               (recur))))))
...
(defn- end-chain-msg
  ...
  (let [fp-vals          (vec (for [f the-path] [f (get-in @results-atom [flow-id f])]))
    ...
    (when last-in-chain?
      (do (when debug?
            (ut/pp [chains-run :chain-done :done-hop ff :steps (count fp-vals) :final-result result :value-path fp-vals chains-run
                    :of chains :flow-chains-completed]))
          (swap! chains-completed conj the-path))
      (when (and all-chains-done? last-in-chain?)
        (when debug?
          (ut/pp [:all-chains-done! :resolved-paths (vec (distinct (get @resolved-paths flow-id))) ff :fp fp :results-atom
                  (get @results-atom flow-id)]))))))
  ...
  (doseq [[k c] (get @channels-atom flow-id)]
    (try #_{:clj-kondo/ignore [:redundant-do]}
         (do ;(ut/pp [:closing-channel k c])
           (try (when (not (nil? c)) (do (swap! channels-atom ut/dissoc-in [flow-id k]) (async/close! c)))
                (catch Throwable e
                  (do (swap! channels-atom assoc-in [flow-id k] c) (ut/pp [:error-closing-channel-inner e k c])))) ;; close
                                                                                                                   ;; channel
...
(defn close-channel!
  ...
  (let [ch (get-in @channels-atom [flow-id channel-id])]
    (try (when (not (nil? ch)) (do (swap! channels-atom ut/dissoc-in [flow-id channel-id]) (async/close! ch)))
      ...
      (async/go-loop [pending-chs input-chs]
        (when (seq pending-chs)
          (let [wstart                 (System/currentTimeMillis) ;; start on channel (for
            ...
            (when (= ff :done) ;; process any :done stuff, debug or not. ends this
              (let [done-channel? valid-output-channels? ;(ut/chan? done-ch)
                ...
                (cond done-atom?                    (reset! done-ch data-val)
                      ...
                      :else                         nil)
                (when close-on-done? (close-channels! flow-id)) ;; kill all channels for this flow. We done.
                (pp [:done-block-reached
                     (cond done-atom?    [:sending-to-atom done-ch]
                           done-channel? [:sending-to-channel done-ch]
                           :else         [:ending-flow-chain
                                          (if debug? [:debug-keeping channels :channels-open] [:closing channels :channels])])])))
            (when is-accumulating-ports? (swap! port-accumulator assoc ff (merge (get @port-accumulator ff {}) data-val))) ;; ff
            ...
            (if (or (and (not (ut/namespaced? ff)) (not (ut/namespaced? data-from))) (not (some nil? (vals @channel-results))))
              (if (or (get data-val :error) (= data-val :done)) ;; receiving :done/error keyword as a value is a
                (do (pp [:chain-done! ff :done-signal]) (end-chain-msg flow-id fp ff nil the-path debug?))
                (let [block-fn-raw? (fn? block-function)
                                   ...
                                   (try (cond (not pre-when?)                              [:failed-pre-check :w-input pre-in] ;; failed
                                              (and fully-resolved? is-subflow?) ;(and is-subflow?
                                                (let [a (atom nil)] ;; use temp atom to hold subflow
                                                  (flow block-function
                                                        {:flow-id (str flow-id "/" (ut/unkeyword ff)) :debug? debug?}
                                                        a ;output-chs
                                                        subflow-overrides)
                                                  (while (nil? @a) (Thread/sleep 100))
                                                  @a)
                                              (and fully-resolved? is-accumulating-ports?) (apply (get block-function :fn)
                                                                                             resolved-inputs)
                                              block-fn-raw?                                (block-function data-val)
                                              block-is-port?                               {:port-in? true ff data-val} ;; just
                                              :else                                        ((get block-function :fn) data-val))
                                        (catch Exception e
                                          (do (ut/pp [:chain-dead! :exec-error-in ff :processing data-val :from data-from
                                                      :block-fns block-function :error e]) ; :done
                                              {:error (str e)}))))
                      expression-dbgn
                        (delay
                          (try ;; testing - to be refactored away shortly w macros
                            (cond (not pre-when?)                              [:failed-pre-check :w-input pre-in] ;; failed
                                  is-subflow?                                  (let [a (atom nil)] ;; use temp atom to hold
                                                                                 ...
                                                                                 (while (nil? @a) (Thread/sleep 100))
                                                                                 @a)
                                  (and fully-resolved? is-accumulating-ports?) (dx/dbgt_ (apply (get block-function :fn)
                                                                                           resolved-inputs))
                            ...
                            (catch Exception e
                              (do (ut/pp [:chain-dead! :exec-error-in ff :processing data-val :from data-from :block-fns
                                          block-function :error e])                                                    ; :done
                                  {:error (str e)}))))
                      {:keys [result fn-start fn-end elapsed-ms dbgn-output]}
                        (merge (ut/timed-exec @expression) (if debux? {:dbgn-output (with-out-str @expression-dbgn)} {}))
                      condis (into {}
                                   (for [[path f] (get block-function :cond)]
                                     {path (try (f result)
                                                (catch Exception e (do (ut/pp [:block ff :cond path :condi-error e]) false)))}))
                      ...
                      view (if error? ;; if error, we'll force a view to better surface it in
                             (fn [x] [:re-com/v-box :style {:font-family "Poppins" :font-size "16px"} :padding "4px" :children
                                      [[:re-com/box :align :center :justify :center :style
                                        {:font-size "30px" :font-weight 700 :color "red"} :child (str "error:")]
                                       [:re-com/box :padding "6px" :style {:color "orange"} :child (str (get x :error))]]])
                      ...
                      view-out (when (fn? view)
                                 (try (let [vv (view result)] ;; if view isn't hiccup and just
                                        (if (string? vv)
                                          [:re-com/box :width :width-px :height :height-px :child vv :align :center :justify
                                           :center :padding "10px" :size "none" :style
                                           {:font-size "18px" :margin-top "30px" :font-weight 700 :color :vcolor}]
                                          vv))
                      ...
                      output-chs (remove nil?
                                   (cond ;; output channel, condis, or regular output switch
                                     ...
                                     :else                                       output-chs))
                      web-val-map (merge (cond (and fully-resolved? is-accumulating-ports?) {:v     (limited result flow-id) ;; since
                                                                                                                             ;; it's
                                                                                                                             ;; going
                                                                                             :input (limited resolved-inputs
                                                                                                             flow-id)}
                                               (and block-is-port? (ut/namespaced? ff)) ;; only true
                                                 {:v       (limited data-val flow-id)                                        ;; since
                                                                                                                             ;; it's
                                                  :port-of (first next-hops)}
                                               :else                                        {:v     (limited result flow-id) ;; since
                                                                                                                             ;; it's
                                                                                             :input (limited data-val flow-id)})
                                         (if #_{:clj-kondo/ignore [:not-empty?]} (not (empty? condis)) {:cond condis} {})
                  ...
                  (when post-print
                    (ut/pp [flow-id ff (str (java.time.LocalDateTime/now)) :post
                            (try (post-print result) (catch Exception e [:post-print-error (str e)]))]))
                  (when (and web-push? (not (= ff :done))) ;; :done block is hidden, so no pushing data
                  ...
                  (try (pp [:block ff [data-from ff] :has-received (limited (value-only data-val) flow-id) :to-web
                            (limited (get web-val-map :v) flow-id) :from data-from :next-channel output-chs :has-output
                            (limited (value-only result) flow-id)])
                         ...
                         (do (if (seq output-chs)
                               (doseq [oo output-chs] ;; forward processed data to the output
                                 (try (async/>! oo
                                      ...
                                      (catch Exception e
                                        (ut/pp [:caught-in-push-loop e :outputs (count output-chs) :this oo :output-chs
                                                output-chs]))))
        ...
        (let [flow-id (or (get opts-map :flow-id) (ut/generate-name))
          ...
          (try
            (let [{:keys [components connections]} network-map
                  opts-map                         (merge {:debug? true :debux? false :close-on-done? true} opts-map) ;; merge w
                                                                                                                      ;; defaults,
                                                                                                                      ;; pass
              ...
              (pp [:gen-connections gen-connections :connections connections :fp fp :comps components :coords-map coords-map
                   :condi-connections condi-connections])
              (when web-push? (push! flow-id :none [flow-id :network-map] (get @working-data flow-id)))
              (when web-push?
                ...
                (do
                  (doseq [[bid v] components
                    ...
                    (do (swap! block-defs assoc-in [flow-id bid] block-def) (push! flow-id bid [flow-id :blocks bid] block-def)))
                ...
                (let [start             (System/currentTimeMillis)
                  ...
                  (if (and (not (fn? from-val)) ;; not a raw fn
                    ...
                    (let [text-input? (true? (some #(= :text-input %) (flatten [from-val])))
                      ...
                      (when web-push? (push! flow-id from [flow-id :blocks from :body] (str {:v (limited from-val flow-id)})))
                      (pp (vec (remove nil?
        ...
        (for [flow-id (keys @resolved-paths)]
          {flow-id (let [res (dissoc (into {}
                                           (for [p (vec (distinct (get @resolved-paths flow-id)))]
                                             {p (for [pp p] [pp (get-in @results-atom [flow-id pp])])}))
...
(defn schedule!
  ...
  (let [[opts chan-out override] args
    ...
    (chime/chime-at times ;; [] chime time seq, https://github.com/jarohen/chime#recurring-schedules
                    (fn [time] (let [opts (merge opts {:schedule-started (str time)})] (flow flowmap opts chan-out override)))
...
(def my-network
  {:description "a simple example flow: addition and integers"
   ...
   :connections [[:comp1 :adder/in1] [:comp2 :adder/in2] [:comp3 :adder-one/in] [:adder-one :add-one] [:adder :simple-plus-10]
                 [:simple-plus-10 :done]]
...
(def test-flow-map-1
  {:components  {:comp1 10
                 ...
                 :counter {:fn   #(count %)
                           :view (fn [x] [:re-com/box :child (str x " loops") :align :center :justify :center :padding "10px"
                                          :style {:color "yellow" :font-weight 700 :font-size "100px"}])
                           :h    225
                           :w    725
                           :x    1981
                           :y    915}
                 :conjer {:fn   (fn [x] (defonce vvv (atom [])) (do (swap! vvv conj x) @vvv))
                          :view (fn [x] [:vega-lite
                                         {:data       {:values (map-indexed (fn [index value] {:index index :value value}) x)}
                          ...
                          :x    1217
                          :y    848}
                 :add-one4 {:fn #(do (Thread/sleep 120) (+ 45 %)) :x 595 :y 1087 :cond {:condicane2 #(> % 200)}}
                 :whoops {:fn #(str % ". YES.") :x 1627 :y 1337}
                 :condicane {:fn   #(str % " condicane!")
                             :view (fn [x] [:re-com/box :child (str "conditional path that never gets run or seen" x) :align
                                            :center :justify :center :padding "10px" :style
   ...
   :connections [[:comp1 :adder/in1] ;; channels directly to port
                 [:comp2 :adder/in2] [:adder :simple-plus-10] [:condicane2 :whoops] [:add-one4 :add-one2] [:add-one4 :conjer]
                 [:conjer :counter] [:whoops :done] [:simple-plus-10 :add-one] [:add-one :add-one2] [:add-one2 :add-one3]
                 [:add-one3 :add-one4]]})
