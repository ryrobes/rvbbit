...
(def kit-fns (into {} (for [[k v] kits]
                        (into {} (for [[kk vv] (get v :packages)]
                                   {kk (merge (merge (dissoc v :packages) {:kit-name kk :package-name k}) vv)})))))

(def settings (try (read-string (slurp "./config.edn"))
