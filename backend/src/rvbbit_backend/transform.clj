...
;; (defn vals-by-pivot-key [vals pivot-key]
;;   (let [pivots (distinct (map #(get % pivot-key) vals))]
;;     (zipmap pivots
;;             (map
;;              (fn [pivot]
;;                (filter #(= pivot (get % pivot-key)) vals))
;;              pivots))))
...
(defn transform
  ...
  (let [{:keys [transform-select from pivot-by nest-by]} dsl
        agg-specs (filter #(vector? (try (first %) (catch Exception _ nil)))
                          transform-select)
        dim-specs (filter #(not (vector? (try (first %) (catch Exception _ nil))))
                          transform-select)
                              ...
                              (fn [pivot-val-group]
                                (let [color (get (first pivot-val-group) pivot-key)
                                      ...
                                      agg-maps (calculate-aggregations agg-specs
                                                                       pivot-val-group)]
                                  ;(println [pivot-val-group pivot-key color agg-maps])
                                 ; (ut/pp (last pivot-val-group) )
                                 ; (ut/pp pivot-val-group)
                                 ; (ut/pp [pivot-key color agg-maps])
                                 ; (into {}
...
;; (defn selects
;;   []
;;   [(get dsl-pivot-rep2 :transform-select)
;;    (get dsl-pivot-rep :transform-select)
;;    (get dsl-nested-category :transform-select)
;;    (get dsl-pivot-region :transform-select)])

;(transform dsl-pivot-region data)

;(transform dsl-nested-category data)

;(transform dsl-pivot-rep data)

;(transform dsl-pivot-rep2 data)
