(ns rvbbit-backend.cruiser
  (:require ;[datalevin.core :as d]
    ...
    [rvbbit-backend.sql        :as    sql
                               :refer [sql-exec sql-exec-no-t sql-exec2 sql-query sql-query-meta
                                       sql-query-one system-db insert-error-row! to-sql
                                       pool-create]]
  ...
  (when (ut/ne? rowset)
    (let [rando-name                  (str "rowset" (rand-int 12345567)) ;; prevent
          rowset-type                 (cond (and (map? (first rowset)) (vector? rowset)) :rowset
                                            (and (not (map? (first rowset))) (vector? rowset))
                                              :vectors)
          columns-vec-arg             (first columns-vec)
          columns-vec-arg-underscores (edn/read-string
                                        (cstr/replace (str columns-vec-arg) #"-" "_")) ;; ugly,
          ...
          asort                       (fn [m order]
                                        (let [order-map (apply hash-map (interleave order (range)))]
                                          (conj (sorted-map-by #(compare (order-map %1)
                                                                         (order-map %2))) ; empty
                                                (select-keys m order))))
  ...
  (let [k                     key-vector
        ...
        postwalk-map          {:field      (keyword field-name) ;(if (not (= data-type
                               :table      (cond (= db-schema "none") (keyword table-name)
                                                 (and (cstr/includes? (cstr/lower-case db-type)
                                                                      "mysql")
                                                      (= db-schema "none"))
    ...
    (into
      ...
      (doall
        (for [[k v] tests-w-params] ;; filter out table tests during field loops and vice versa
          (let [table-sniff-and-*?     (and (= (get granularity-map k) :table)
                                            (= (keyword field-name) :*))
                field-sniff-and-not-*? (and (= (get granularity-map k) :field)
                                            (not (= (keyword field-name) :*)))
            ...
            (let [test-value            (:val test-values)
                  ...
                  test_val_string       (when (and (not sample?)
                                                   (or (string? test-value)
                                                       (cstr/includes? (str (type test-value))
                                                                       "DateTime") ;; find
                                                       (cstr/includes? (str (type test-value))
                                                                       "Date")))
                                          (str test-value))
                  ...
                  stmt                  (to-sql
                                          {:insert-into [:tests]
                                           :columns     [:db_type :connection_id :table_type
                                                         :db_schema :db_catalog :table_name
                                                         :field_name :field_type :data_type
                                                         :test_sql :test_raw_val :derived_calc
                                                         :derived_name :key_hash :is_sample
                                                         :test_name :test_val_string
                                                         :test_val_integer :test_val_float
                                                         :context_hash :run_id]
                                           :values      [[db_type connection_id table_type db_schema
                                                          db_catalog table_name field_name
                                                          field_type data_type test-sql
                                                          (str test-value) (str derived_calc)
                                                          (str derived_name)
                                                          (if sample?
                                                            sample-table-name-str
                                                            key_hash) (if sample? 1 0) test_name
                                                          test_val_string test_val_integer
                                                          test_val_float context_hash
                                                          this-run-id]]})]
              ...
              (if sample? ;; meaning the sniff test returns a rowset the needs it own table
                (try
                  (let [create-stmt           (when has-results? ;; has to be hardcoded
                                                                 ;; Sqlite-ddl!!!
                                                (sqlite-ddl/create-attribute-sample
                                                  sample-table-name-str
                                                  (walk/postwalk-replace {:case :case_stmt}
                                                                         test-value)))
                        drop-old-sample-table (str "drop table if exists "
                                                   sample-table-name-str
                                                   ";")
                  ...
                  (for [[k v] attributes] ;; TODO
                    (let
                      [;attribute-logic (read-string (cstr/replace (str v) #"-" "_"))
                       ...
                       available-single-value-types
                         (into {}
                               (for [r (sql-query system-db available-single-value-types-sql)]
                                 {(keyword (get r :test_name)) (keyword (get r :type))}))
                       available-field-tests [:db_type :table_type :table_name :field_name
                                              :field_type :data_type]
                       available-connection-tests [:database_name :database_version :user_name]
                       available-single-value-tests
                         (vec (for [r (sql-query system-db available-single-value-tests-sql)]
                                        ...
                                        (into
                                          ...
                                          (for [f available-single-value-tests]
                                            (let [sub-q-alias   (str "tmp_" (ut/unkeyword f))
                                                  test-val-type (get available-single-value-types
                                                                     f
                                                                     :test_raw_val)]
                       ...
                       single-value-tests-select-aliases
                         (vec (for [f available-single-value-tests]
                                [(keyword (str "tmp_" (ut/unkeyword f) ".v")) f]))
                       final-select-vec (into (into available-field-tests
                                                    available-connection-tests)
                                              single-value-tests-select-aliases)
                       ...
                       attribute-test-pivot-query-honey {:select 1
                                                         ...
                                                         :limit  1}
                       attribute? (= 1
                                     (sql-query-one system-db
                                                    (to-sql attribute-test-pivot-query-honey)
                                                    {:hs attribute-test-pivot-query-honey
                                                     :fn [field_name derived_name]
                                                     :c  "get-attributes"}))
                    ...
                    (to-sql {:delete-from [:attributes] ;; remove any previous taste test (no
                             :where       (cons :and
                                                (vec (doall (for [[k v] ;(merge
                                                                    (select-keys
                                                                      attribute-map
                                                                      [:connection_id :db_schema
                                                                       :db_type :attribute_name
                                                                       :derived_calc :field_name
                                                                       :table_name :table_type
                                                                       :key_hash])]
                                                              [:= k v]))))}))
          (when (> (count attribute-map) 0)
            (sql-exec system-db
                      (to-sql {:insert-into [:attributes]
                    ...
                    (for [[derived-key v] field-tests] ;; TODO
                      (let
                        [;derived-field-logic (read-string (cstr/replace (str v) #"-" "_")) ;;
                         ...
                         available-attributes-sql
                           (to-sql {:select-distinct [:attribute_name] ;; add another set
                                    :from            [:attributes]
                                    :where           [:and [:= :connection_id connection_id]
                                                      [:= :table_name table_name]
                                                      [:= :db_schema db_schema]
                         ...
                         available-single-value-tests
                           (vec (for [r (sql-query system-db available-single-value-tests-sql)]
                                  (keyword (get r :test_name))))
                         available-attributes (vec (for [r (sql-query system-db
                                                                      available-attributes-sql)]
                                                     (keyword (get r :attribute_name))))
                         merged-join
                           (read-string
                             (str
                               "[" ;; weird workaround for honeysql not allowing a vector of
                               (cstr/join
                                 ""
                                 (drop-last
                                   (subs
                                       ...
                                       (merge
                                         ...
                                         (into
                                           ...
                                           (for [f available-attributes]
                                             (let [sub-q-alias (cstr/replace (str "tmp_"
                                                                                  (ut/unkeyword f))
                                                                             "?"
                                                                             "")]
                         ...
                         attributes-select-aliases
                           (vec (for [f available-attributes]
                                  [(keyword
                                     (cstr/replace (str "tmp_" (ut/unkeyword f) ".v") "?" ""))
                                   (keyword (cstr/replace (str (ut/unkeyword f)) "?" ""))]))
                         final-select-vec (into (into (into available-field-tests
                                                            available-connection-tests)
                                                      single-value-tests-select-aliases)
                                                attributes-select-aliases)
                         pivot-honey {:select-distinct final-select-vec
                         ...
                         attribute-test-pivot-query-honey {:select 1
                                                           ...
                                                           :limit  1}
                         valid?
                           (= 1 (sql-query-one system-db (to-sql attribute-test-pivot-query-honey)))
                         new-derived-name (-> (str derived-key "_" field_name)
                                              (clojure.string/replace " " "_")
                                              (clojure.string/replace ":" "")
                                              (clojure.string/replace "-" "_"))
                         derived-vector-key [db_type connection_id table_type db_schema db_catalog
                                             table_name field_name "derived" "derived" calc-field
                                             new-derived-name]]
...
(defn insert-connection-data!
  ...
  (do (sql-exec system-db
                (to-sql {:delete-from [:connections]
                         :where       [:= :connection_id (get connect-meta :connection_id)]}))
      (sql-exec system-db
                (to-sql {:insert-into [:connections]
                         :columns     (conj (conj (conj (vec (keys connect-meta)) :run_id)
                                                  :original_connection_str)
                                            :metadata_filter)
                         :values      [(conj (conj (conj (vec (vals connect-meta)) this-run-id)
                                                   (str target-db))
                                             (str sql-filter))]}))))
  ...
  (let
    [connection_id (:connection_id context-map)
     ...
     available-single-value-tests-sql
       (to-sql {:select-distinct [:test_name]
                ...
                :where           [:and [:= :connection_id connection_id] [:= :table_name table_name]
                                  [:= :db_schema db_schema] [:= :is_sample 0]]})
     available-single-value-types-sql
       (to-sql
         {:select-distinct
            [:test_name
     ...
     available-single-value-types (into {}
                                        (for [r (sql-query system-db
                                                           available-single-value-types-sql)]
                                          {(keyword (get r :test_name)) (keyword (get r :type))}))
     field-meta-fields [:db_type :table_type :table_name :f.field_name :field_type :data_type
                        :key_hash :f.connection_id :db_schema :db_catalog :context_hash
                        :derived_name :derived_calc]
     available-connection-tests [:database_name :database_version :user_name]
     available-single-value-tests (vec (for [r (sql-query system-db
                                                          available-single-value-tests-sql)]
                                         (keyword (get r :test_name))))
     ...
     merged-join
       (read-string
         (str
           "[" ;; weird workaround for honeysql not allowing a vector of vectors for join key..
           (cstr/join
             ""
             (drop-last
               (subs
     ...
     attributes-select-aliases (vec (for [f available-attributes]
                                      [(keyword
                                         (cstr/replace (str "tmp_" (ut/unkeyword f) ".v") "?" ""))
                                       (keyword (cstr/replace (str (ut/unkeyword f)) "?" ""))]))
     final-select-vec (into (into (into field-meta-fields available-connection-tests)
                                  single-value-tests-select-aliases)
                            attributes-select-aliases)
     pivot-honey {:select-distinct final-select-vec
                  :from            [[:fields :f]]
                  ...
                  :left-join       merged-join
                  :where           [[:and [:= :f.connection_id connection_id]
                                     [:= :f.table_name table_name] [:= :f.db_schema db_schema]]]}
...
(defn insert-found-fields!
  ...
  (let [connection_id   (:connection_id context-map)
        ...
        db_schema       (:db_schema context-map)
        table_name      (:table_name context-map)
        insert-keys     [:db_type :connection_id :table_type :db_schema :db_catalog :table_name
                         :axes_key :shape_name :field_name :key_hash :context_hash :logic_map
                         :derived_calc :derived_name] ;; fields coming from found-fields are
                                                      ;; arbitrary except
        rowset          (vec (for [r fields-rowset]
                               (merge {:logic_map (pr-str r)}
                                      (merge shape-keys (select-keys r insert-keys)))))
...
(def target-db2
  {:classname "org.sqlite.JDBC" :subprotocol "sqlite" :subname "./data/boston-crime-data.db"})
...
(def target-db7
  "jdbc:sqlserver://localhost:1433;databaseName=imdb;encrypt=false;username=sa;password=yourStrong(!)Password") ;"jdbc:sqlserver://sa:yourStrong(!)Password@localhost:1433;database=imdb")
    ...
    (doall
      (for [f     field-vectors
        ...
        (let [query                 (get @sql/query-history (nth f 5))
              ...
              selects-non-aliased   (into {}
                                          (for [s selects]
                                            (if (vector? s) {(first s) (last s)} {s s})))
              ...
              is-group-by?          (if (not no-group-by?)
                                      (true? (try (some #(= (keyword (nth f 6)) %)
                                                        materialize-group-bys)
                                                  (catch Exception _ false)))
                                      true)]
          (sql-exec system-db
                    (to-sql {:delete-from [:fields]
                             :where       [:and [:= :db_type (nth f 0)]
                                           [:= :connection_id (nth f 1)] [:= :table_type (nth f 2)]
                                           [:= :db_schema (nth f 3)] [:= :db_catalog (nth f 4)]
                                           [:= :table_name (nth f 5)] [:= :field_name (nth f 6)]
                                           [:= :derived_calc
                                            (try (str (nth f 9)) (catch Exception _ nil))]
                                           [:= :derived_name
                                            (try (str (nth f 10)) (catch Exception _ nil))]]}))
          (sql-exec system-db
                    (to-sql {:insert-into [:fields]
                             :columns     [:db_type :connection_id :table_type :db_schema
                                           :db_catalog :table_name :field_name :field_type
                                           :data_type :derived_calc :derived_name :key_hash
                                           :context_hash :is_group_by]
                             :values      [[(nth f 0) (nth f 1) (nth f 2) (nth f 3) (nth f 4)
                                            (nth f 5) (nth f 6) (nth f 7) (nth f 8)
                                            (try (str (nth f 9)) (catch Exception _ nil))
                                            (try (str (nth f 10)) (catch Exception _ nil))
                                            (keyhash-from-field-vector f)
                                            (contexthash-from-field-vector f) is-group-by?]]})))))
    (catch Exception e
      (do (insert-error-row! target-db
                             (str (str e))
                             {:sql-filter sql-filter :field-vectors field-vectors})
  ...
  (vec
    (for [vector-key field-vectors]
      (let [db_type        (nth vector-key 0)
            ...
            honey-map      (walk/postwalk-replace
                             {:table (cond (= db_schema "none") (keyword table_name) ;; dupe
        ...
        [db_type connection_id table_type db_schema db_catalog table_name field_name field_type
         post-data-type derived_calc derived_name]))))

(defn process-field-vectors
  [system-db target-db field-vectors this-run-id sniff-tests-map field-attributes-map
   derived-field-map & derived?]
  ...
  (insert-attribute-flag-results!
    ...
    this-run-id)
  (when (not derived?)
    (let [derived-field-vectors        (get-possible-derived-fields system-db
                                                                    field-vectors
                                                                    this-run-id
                                                                    derived-field-map)
...
(defn find-all-viz-fields!
  ...
  (ut/pp [:finding-fields-for-viz-shapes
          (if sql-filter {:where (first sql-filter)} :all-in-database)])
  (let [base-honey {:select-distinct [:connection_id :db_schema :table_name] :from [:fields]}
        honey-map  (if sql-filter
                     (assoc base-honey :where (first (sqlize-keywords sql-filter)))
                     base-honey)
...
(defn find-all-viz-combos!
  ...
  (let [base-honey {:select-distinct [:context_hash :shape_name :axes_key :key_hash :derived_name
                                      :derived_calc]
                    :from            [:found_fields]}
        honey-map  (if sql-filter
                     (assoc base-honey :where (first (sqlize-keywords sql-filter)))
          ...
          (doall
            (doseq [[shape-name axes] ctxval]
              (let [axes-keys          (vec (keys axes))
                    ...
                    valid?             (= (count axes-keys)
                                          (count (keys (get-in viz-shape
                                                               [(rekey shape-name) :axes]))))
                    cartesians         (apply combo/cartesian-product combo-vecs)
                    filtered-carts     (remove nil?
                                         (for [c cartesians]
                                           (when (= (count axes-keys) (count (distinct c))) c)))
                        ...
                        (try
                          (let [;lookup-field-data-map
                                ...
                                sample-key-vec (let [f sample-lookup]
                                                 [(:db_type f) (:connection_id f) (:table_type f)
                                                  (:db_schema f) (:db_catalog f) (:table_name f) nil
                                                  nil nil])
                                ...
                                parent-keys (select-keys sample-lookup
                                                         [:db_type :db_schema :db_catalog
                                                          :table_name :table_type :connection_id])
                                is-mysql? (and (cstr/includes? (cstr/lower-case db-type) "mysql")
                                               (= db-schema "none"))
                                      ...
                                      (for [[k v] c]
                                        (when (not (nil? (:derived_name (lookup-field-data-map
                                          ...
                                          {(keyword (str (ut/unkeyword k) "-field"))
                                             (walk/postwalk-replace
                                               {:field (keyword (:field_name (lookup-field-data-map
                                                                               (str v))))}
                                ...
                                aliased-selects3 (into {}
                                                       (apply concat
                                                         (for [[k a] aliased-selects]
                                                           {(first (flatten k))
                                                              (first (let [tt (walk/postwalk-replace
                                                                                aliased-selects2
                                                                                a)]
                                ...
                                query-replacement-map
                                  (merge (into
                                           {} ;; alias and logic - for selected clause
                                           (for [[k v] c]
                                             {(keyword (str (ut/unkeyword k) "-field"))
                                                (let [calc?      (not (nil? (:derived_name
                                                                              (lookup-field-data-map
                                                                                (str v)))))
                                                      base-field (:field_name (lookup-field-data-map
                                                                                (str v)))]
                                                  (if calc?
                                                    [(walk/postwalk-replace
                                                       {:field (keyword base-field)}
                                                       (read-string (:derived_calc
                                                                      (lookup-field-data-map
                                                                        (str v)))))
                                                     (keyword (:derived_name (lookup-field-data-map
                                                                               (str v))))]
                                                    (keyword base-field)))}))
                                         field-replacements)
                                query-replacement-map2
                                  (merge (into
                                           {} ;; only the alias name / field name
                                           (for [[k v] c]
                                             {(keyword (str (ut/unkeyword k) "-field"))
                                                (let [calc?      (not (nil? (:derived_name
                                                                              (lookup-field-data-map
                                                ...
                                                (merge q
                                                       {:select   (if true ; (not (empty? (get (hash
                                                                    ...
                                                                    (walk/postwalk-replace
                                                                      query-replacement-map
                                                                      (get q :select {})))
                                                        :vselect  (walk/postwalk-replace
                                                                    query-replacement-map2
                                                                    (get q :vselect {}))
                                                        :group-by (walk/postwalk-replace
                                                                    query-replacement-map3
                                                                    (get q :group-by {}))}))
                                ...
                                combo-map {:context-hash ctx
                                           ...
                                           :query-map-str
                                             (to-sql (first query-map) sample-key-vec system-db)
                                           :viz-map (str viz-map)
                                           :key-hashes (pr-str (into {} (sort-by key c)))
                                           :key-hashes-hash
                                             (str (hash (pr-str (into {} (sort-by key c)))))
                                           :h h
                                           :w w
                                           :selected-view (str (get-in viz-shape
                                                                       [(rekey shape-name)
                                                                        :selected-view]))
                                           :combo-edn
                                             (pr-str
                                                 ...
                                                 (sort (for [v (vals c)]
                                                         (let [calc? (not (nil?
                                                           ...
                                                           (if calc?
                                                             ...
                                                             base-field))))))}
                                cid (hash combo-map)
                                sel-recos (filter #(= (get % :connection_id) connection_id)
                                            @em/selected-recos)
                                calc-score0
                                  (fn [k v]
                                    (let [cache (get @score-atom0 [k v])]
                                      (if cache
                                        cache
                                        (let [res (for [_ (filter #(and (= (get % :shape_name)
                                                                           shape-name)
                                                                        (= (get % :field_name)
                                                                           (:field_name
                                                                             (lookup-field-data-map
                                    ...
                                    (into
                                      ...
                                      (for [[k v] c]
                                        (let [talias (keyword (str (ut/unkeyword k) "-field"))]
                                          (->
                                            (merge (merge parent-keys
                                                          {:context-hash ctx
                                                           :combo-hash   (str combo-hash)
                                                           :shape-name   shape-name})
                                                   (select-keys (lookup-field-data-map (str v))
                                                                [:field_name :field_type :data_type
                                                                 :key_hash :derived_calc
                                                                 :derived_name]))
                                            (assoc :axes k)
                                            ...
                                            (assoc :walk1 (pr-str (select-keys query-replacement-map
                                                                               [talias])))
                                            (assoc :walk2 (pr-str (select-keys
                                                                    query-replacement-map2
                                                                    [talias])))
                                            (assoc :walk3 (pr-str (select-keys
                                                                    query-replacement-map3
                                                                    [talias])))
                                            (assoc :walka
                                                     (pr-str
                                                         ...
                                                         (for [[qidx qq] aliased-selects3]
                                                           {qidx (into
                                                                   ...
                                                                   (filter
                                                                     ...
                                                                     qq))}))))))))]
                              (sql-exec system-db
                                        (to-sql {:insert-into [:combo-rows] :values combo-rows})))
                            (sql-exec system-db
                                      (to-sql {:insert-into [:combos] :values [combo-map]})))
                          (catch Exception e
...
(defn insert-current-rules!
  [system-db connection_id this-run-id sniff-tests-map field-attributes-map derived-field-map
   viz-shape-map flow-functions-map]
  (sql-exec system-db
            (to-sql {:delete-from [:rule_maps_tests] :where [:= :connection_id connection_id]}))
  (doseq [[rk r] sniff-tests-map]
    (sql-exec system-db
              (to-sql
                {:insert-into [:rule_maps_tests]
                 :columns     [:connection_id :run_id :test_name :sql_map :when_logic :fetch_one]
                 :values      [[connection_id this-run-id (ut/unkeyword rk) (str (get r :sql-map))
                                (str (get r :when)) (str (get r :fetch-one?))]]})))
  (sql-exec system-db
            (to-sql {:delete-from [:rule_maps_attributes]
  ...
  (doseq [[rk r] field-attributes-map]
    (sql-exec system-db
              (to-sql {:insert-into [:rule_maps_attributes]
                       ...
                       :values      [[connection_id this-run-id (ut/unkeyword rk)
                                      (str (get r :when))]]})))
  (sql-exec system-db
            (to-sql {:delete-from [:rule_maps_derived_fields]
                     :where       [:= :connection_id connection_id]}))
  (doseq [[rk r] derived-field-map]
    (sql-exec system-db
              (to-sql {:insert-into [:rule_maps_derived_fields]
                       ...
                       :values      [[connection_id this-run-id (ut/unkeyword rk)
                                      (str (get r :when)) (str (get r :calc))]]})))
  (sql-exec system-db
            (to-sql {:delete-from [:rule_maps_viz_shapes]
                     :where       [:= :connection_id connection_id]}))
  (doseq [[rk r] viz-shape-map]
    (sql-exec system-db
              (to-sql {:insert-into [:rule_maps_viz_shapes]
                       :columns     [:connection_id :run_id :shape_name :axes_logic :sql_maps
                                     :library_shapes :base_score :selected_view]
                       :values      [[connection_id this-run-id (ut/unkeyword rk)
                                      (str (get r :axes)) (str (get r :sql-maps))
                                      (str (get r :library-shapes)) (get r :base-score)
                                      (str (get r :selected-view))]]})))
  (sql-exec system-db
            (to-sql {:delete-from [:flow_functions] :where [:= :connection_id connection_id]}))
  (doseq [{:keys [category name description file-path inputs icon types]} (ut/flatten-map
                                                                            flow-functions-map)]
    (sql-exec system-db
              (to-sql {:insert-into [:flow_functions]
                       :columns     [:connection_id :run_id :category :name :full_map :description
                                     :inputs :icon :input_types :output_types :file_path]
                       :values      [[connection_id this-run-id (str category) (str name)
                                      (str (get-in flow-functions-map [category name])) ;; full
                                      (str description) (str inputs) (str icon)
                                      (str (vec (vals (dissoc types :out)))) (str (get types :out))
                                      (str file-path)]]}))))


(defn lets-give-it-a-whirl
  [f-path target-db system-db sniff-tests-map field-attributes-map derived-field-map viz-shape-map &
   sql-filter]
  (doall
    (try
      (let [connect-meta      (surveyor/jdbc-connect-meta target-db f-path)
            ...
            field-vectors     (if (and sql-filter ;; filter them out of field-vectors-all ...
                                ...
                                (let [col-names       [:db-type :connection-id :table-type
                                                       :db-schema :db-catalog :table-name
                                                       :field-name :field-type :data-type]
                                      full-sql-filter {:select col-names
                                                       :from   :rowset
                                                       :where  sql-filter}]
        ...
        (if (and sql-filter #_{:clj-kondo/ignore [:not-empty?]} (not (empty? sql-filter)))
          (find-all-viz-fields! system-db
                                viz-shape-map
                                connection_id
                                [:and [:= :connection-id connection_id] (first sql-filter)])
          (find-all-viz-fields! system-db
                                viz-shape-map
                                connection_id
                                [:= :connection-id connection_id]))
        (if (and sql-filter #_{:clj-kondo/ignore [:not-empty?]} (not (empty? sql-filter)))
          (find-all-viz-combos! system-db
                                viz-shape-map
                                connection_id
                                [:and [:= :connection-id connection_id] (first sql-filter)])
          (find-all-viz-combos! system-db
      ...
      (catch Exception e
        (let [sw (java.io.StringWriter.)
              pw (java.io.PrintWriter. sw)]
          (.printStackTrace e pw)
          (ut/pp [:error-running-whirl-full f-path sql-filter target-db (str e)
                  (.toString sw)]))))))
...
(def tmp-db-src1
  {:datasource @(pool-create
                  {;:jdbc-url
                   :jdbc-url
                     "jdbc:sqlite:file:tmpsniffdb1?mode=memory&cache=shared&auto_vacuum=FULL"
                   :auto_vacuum "FULL"
                   :cache "shared"}
                  "tmp-db-src1")})
...
(def mem-db-sqlite3
  {:classname "org.sqlite.JDBC" :subprotocol "sqlite" :cache "shared" :subname ":memory:"})
