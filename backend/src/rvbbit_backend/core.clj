(ns rvbbit-backend.core
  (:require
    ...
    [rvbbit-backend.sql :as    sql
                        :refer [flows-db insert-error-row! pool-create sql-exec sql-query
                                sql-query-meta sql-query-one system-db to-sql]]
...
(defn get-last-modified-time
  ...
  (let [path          (Paths/get f-path (into-array String []))
        instant       (.toInstant (Files/getLastModifiedTime
                                    path
...
(def valid-to-invalid-keywords
  (zipmap (vals invalid-to-valid-keywords) (keys invalid-to-valid-keywords)))
(defn convert-to-valid-edn
  [s]
  (reduce (fn [s [invalid valid]] (clojure.string/replace s invalid valid))
    s
...
(defn convert-back-to-original
  ...
  (clojure.walk/postwalk
    (fn [x] (if (and (keyword? x) (valid-to-invalid-keywords x)) (valid-to-invalid-keywords x) x))
    m))
...
(defn update-screen-meta
  ...
  (let [;file-path "./screens/"
                           ...
                           (assoc screen-data
                             :panels (into {}
                                           (for [[k v] (get screen-data :panels)]
                                             {k (assoc v
                                                  :queries (select-keys resolved-queries
                                                                        (keys (get v
                                                                                   :queries))))})))
                           screen-data)
        ...
        theme-name       (if (and has-theme? (not (nil? (get theme-map :theme-name))))
                           ...
                           "")
        blocks           (remove nil?
                           (conj (keys (get-in screen-data [:panels])) (when has-theme? :*theme*)))
        ...
        board-map        (into {}
                               (for [b boards]
                                 {b (into {}
                                          (for [[k v] (get-in screen-data [:panels])
                                                :when (= (get v :tab) b)]
                                            {k v}))}))
    ...
    (doseq [b blocks] ;; collect keywords from all views for autocomplete - might get
      (doseq [[_ vv] (get-in screen-data [:panels b :views])]
        (reset! wss/autocomplete-view-atom (vec (filter #(and (not (cstr/includes? (str %) "/"))
                                                              (keyword? %))
                                                  (distinct (into (vec (distinct (ut/deep-flatten
                                                                                   vv)))
    ...
    (sql-exec system-db
              (to-sql
                {:insert-into [:blocks]
                 :columns     [:file_path :screen_name :block_key :block_name :views :queries
                               :view_names :query_names :block_data :tab_name]
                 :values      (for [b blocks]
                                (let [theme?     (true? (= b :*theme*))
                                      ...
                                      views      (if theme?
                                                   theme-map
                                                   (get-in screen-data [:panels b :views]))
                                      block-name (str (cond theme? (str "(meta: this screen's theme"
                                                                        theme-name
                                                                        ")")
                                                            board? (str "board: " b)
                                                            :else  (get-in screen-data
                                                                           [:panels b :name])))]
                                  [f-path screen-name (str b) block-name (count (keys views))
                                   (count (keys queries))
    ...
    (wss/enqueue-task3 ;; since out base sqlite db hate concurrency, with a "real" db, this can
      (fn []
        (let [connection_id "system-db"]
          (sql-exec system-db
                    (to-sql {:delete-from [:flow_functions]
                             :where       [:and [:= :name name] [:= :category category]]}))
          (sql-exec system-db
                    (to-sql {:insert-into [:flow_functions]
                             :columns     [:connection_id :run_id :category :name :full_map
                                           :description :inputs :icon :input_types :output_types
                                           :file_path]
...
(defn update-all-conn-meta
  ...
  (let [prefix "connections/"
    ...
    (doseq [f    files
            :let [f-path    (str prefix f)
                  conn-name (str (cstr/replace (cstr/lower-case (last (cstr/split f-path #"/")))
                                               ".edn"
                                               ""))
                  ...
                  conn      (if poolable?
                              (try {:datasource @(pool-create conn (str conn-name "-pool"))}
                                   (catch Exception e
                                     (do (ut/pp [:connection-error conn-name e])
                                         {:datasource nil})))
                              conn)]]
      (try ;; connection errors, etc
        ...
        (catch Exception e
          (do (swap! wss/conn-map dissoc conn-name) (ut/pp [:sniff-error conn-name e])))))))
        ...
        state     (if (.exists file)
                    (with-open [rdr (io/reader file)]
                      (try (edn/read (java.io.PushbackReader. rdr))
                           (catch Exception e
                             (do (ut/pp [:flow-results-thaw-atom-error!!!! file e])
                                 (System/exit 0)))))
                    {})]
...
(defn start-services
  ...
  (let [flows (vec (filter #(not (cstr/includes? (str %) "-solver-flow-"))
                     (keys @flow-db/results-atom)))]
    (reset! flow-db/results-atom (select-keys (ut/replace-large-base64 @flow-db/results-atom)
                                              flows))))
...
(defn -main
  ...
  (add-watch wss/rules-atom
             ...
             (fn [_ _ old-state new-state] (ut/pp [:rules-defs-changed :reloading....])))
  ...
  (cruiser/insert-current-rules! system-db
                                 "system-db"
                                 0
                                 cruiser/default-sniff-tests
                                 cruiser/default-field-attributes
                                 cruiser/default-derived-fields
                                 cruiser/default-viz-shapes
                                 (merge cruiser/default-flow-functions
  ...
  (let [signals  (vec (keys @wss/signals-atom))
        signalsv (vec (apply concat
                        (for [[k v] @wss/signals-atom]
                          (for [e (filter #(and (keyword? %)
                                                (or (some (fn [x] (= x %)) signals)
                                                    (cstr/includes? (str %) "/")))
          ...
          (for [k     kks ;(keys @flow-db/results-atom)
            ...
            (let [;; _ (ut/pp [:flow-status-atom-all! k (get @flow-db/results-atom k)
                  ...
                  running-blocks  (vec (for [[k v] (get @flow-db/tracker k)
                                         ...
                                         k))
                  not-started-yet (vec (cset/difference (set blocks)
                                                        (set (into running-blocks done-blocks))))
                  running-blocks  (vec (cset/difference (set blocks)
                                                        (set (into done-blocks not-started-yet))))
                  res             (get-in @flow-db/results-atom [k :done] :nope)
                  running?        (or (= res :nope) (ut/chan? res) (= res :skip))
                  done?           (not (= res :nope))
                  error?          (try (some #(or (= % :timeout) (= % :error))
                                             (ut/deep-flatten res))
                                       (catch Exception _ false))
                  _ (when error? (swap! wss/temp-error-blocks assoc k not-started-yet))
                  start           (try (apply min
                                         (or (for [[_ v] (get @flow-db/tracker k)] (get v :start))
                                             [-1]))
                  ...
                  elapsed         (try (- end start)
                                       (catch Throwable e
                                         (do (ut/pp [:elapsed-error?? k e :start start :end end])
                                             0)))
                  ...
                  _ (when (and done? (not error?))
                      (swap! wss/times-atom assoc
                        k
                        (conj (get @wss/times-atom k []) (int (/ elapsed 1000)))))
                  _ (when run-sql?
                      (try (let [row        {:client_name     client-name
                             ...
                             (wss/enqueue-task3 ;; temp remove to test some shit 3/9/24
                               (fn [] (sql-exec flows-db (to-sql insert-sql)))))
      ...
      (let [orig-tracker (get @flow-db/tracker flow-id)
            tracker      (into {}
                               (for [[k v] orig-tracker ;; remove condi non-starts. dont send
                                 ...
                                 {k v}))]
        (swap! wss/tracker-history assoc
          flow-id
          (vec (conj (get @wss/tracker-history flow-id []) tracker))) ;; for
      )))
          ...
          (let [;kks-string (-> (cstr/join "-" kks) (cstr/replace " " "_") (cstr/replace "/"
            ...
            (doseq [flow-id kks
              ...
              (let [data        [[(ut/millis-to-date-string mst) blocks]
                                 {:values (select-keys (ut/replace-large-base64
                                                         (get-in @flow-db/results-atom [flow-id]))
                                                       blocks)
                                  :diff   diffy}]
  ...
  (defn heartbeat
    [dest]
    {:components
       {:kick-1            {:inputs [:destination :name :sub-task :thread-id :thread-desc
                                     :message-name]
                            :fn     '(fn
                                      [destination name sub-task thread-id thread-desc message-name
                                       & args]
     ...
     :connections [[:kick-1 :done] [:open-input :kick-1/name] [:open-input :kick-1/sub-task]
                   [:open-input :kick-1/thread-id] [:open-input :kick-1/thread-desc]
                   [:open-input :kick-1/message-name] [:kick-1destination :kick-1/destination]]})
  (wss/schedule!
    [:seconds 15]
    (heartbeat :all)
