...
(defn get-test-conn-meta [db-conn]
  (into []
        (for [t
              (vec (jdbc/with-db-metadata
                     [m db-conn]
          ...
          (do ;(ut/ppln t)
            (merge t ;(select-keys t [:table_schem :table_name :table_type])
                   {:fields (for [f (jdbc/with-db-metadata
                                      [m db-conn]
                                      (jdbc/metadata-result
                                       (.getColumns m
                                                    (:table_cat t) ;nil ;(get db-conn :subname) 
                                                    (:table_schem t) ;nil 
                                                    (:table_name t)
                                                    nil)))]
                              (do ;(ut/ppln f)
                                ;(select-keys f [:column_name :type_name])
                                f))})))))
              ...
              (vec (jdbc/with-db-metadata
                     ...
                     (jdbc/metadata-result
                      (.getTables m
                                  ...
                                  nil ;; schema pattern 
                                  nil ;; table name 
                                  (into-array ["TABLE" "VIEW"]) ;; types of objects 
                                  ))))]
          (do ;(ut/ppln t)
            (merge t ;(select-keys t [:table_schema :table_name :table_type])
                   {:fields (for [f (jdbc/with-db-metadata
                                      ...
                                      (jdbc/metadata-result
                                       (.getColumns m
                                                    (:table_cat t) ;nil ;(get db-conn :subname) ;; catalog name pattern match
                                                    (:table_schem t) ;nil ; schema name  pattern match
                                                    (:table_name t) ;; table name  pattern match
                                                    nil ;; field pattern match
                                                    )))]
                             ; (do ;(ut/ppln f)
                             ;   (select-keys f [:column_name :type_name :table_schem :table_cat]))
                              f)})))))

(defn jdbc-connect-meta [db-conn f-name]
  ;; LOTS more here - https://docs.oracle.com/en/java/javase/11/docs/api/java.sql/java/sql/DatabaseMetaData.html
  (let [user-name (jdbc/with-db-metadata [m db-conn] (jdbc/metadata-result (.getUserName m)))
        ...
        conn-id (if (cstr/ends-with? (cstr/lower-case conn-id) ".csv") "imported" conn-id)] ;; bizarre workaround. TODO
    {:user_name user-name
     :database_name database-name
     :database_version database-version
     :connection_str connection-str
     ;:connection_id (str (hash connection-str))
...
(defn flatten-jdbc-conn-meta [connect-meta db-meta]
  (let [;db-meta (jdbc-connect-meta connect-meta)
        ...
        db-meta-with-* (vec (for [t db-meta] ;; adding a * so table-level granularity sniffs only get run once per table sniff, not for each field...
                              (assoc t :fields (merge (:fields t)
                                                      {:column_name "*"
                                                       :type_name "special"}))))]
    (ut/ppln db-meta-with-*)
    (vec (flatten
          (for [table-map db-meta-with-*]
            (for [field (:fields table-map)]
              {[conn-type conn-id (:table_type table-map)
                (if (nil? (:table_schem table-map)) "none" (:table_schem table-map))
                (:table_cat table-map)
