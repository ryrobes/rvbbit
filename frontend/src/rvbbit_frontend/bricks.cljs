             ...
             (fn [canvas]
               (let [dataUrl (.toDataURL canvas "image/jpeg" 0.75)]
                 (when (not (nil? dataUrl))
                   (ut/tracked-dispatch [(if save? ::http/save-screen-snap ::http/save-snap)
                                         dataUrl]))))
...
(re-frame/reg-sub ::theme-pull-sub
...
(defn code-mirror-theme
  ...
  (let [ccolor (fn [x] (get (theme-pull :theme/data-colors db/data-colors) x))]
    {;; Editor styles
     ".cm-s-rvbbit-dynamic.CodeMirror"
       {:background "inherit" :color "#b3b1ad" :font-size "inherit" :line-height "inherit"}
     ".cm-s-rvbbit-dynamic .CodeMirror-selected" {:background
                                                    (theme-pull :theme/editor-outer-rim-color nil)} ;; "#ff4aff"
     ...
     ".cm-s-rvbbit-dynamic .CodeMirror-line::-moz-selection, .cm-s-rvbbit-dynamic .CodeMirror-line > span::-moz-selection, .cm-s-rvbbit-dynamic .CodeMirror-line > span > span::-moz-selection"
       {:background "#27374763"} ;; {:background "rgba(39, 55, 71, 99)"}
     ".cm-s-rvbbit-dynamic .CodeMirror-gutters"
       {:background "inherit" :border-right "0px" :z-index "-1" :opacity "0.9"}
     ...
     ".cm-s-rvbbit-dynamic .CodeMirror-cursor"
       {:border-left (str "2px solid "
                          (ut/invert-hex-color (theme-pull :theme/editor-outer-rim-color nil)))} ;; {:border-left
     ...
     ".cm-s-rvbbit-dynamic span.cm-property, .cm-s-rvbbit-dynamic span.cm-attribute" {:color
                                                                                        "#f386bf"}
     ...
     ".cm-s-rvbbit-dynamic .CodeMirror-matchingbracket"
       {:text-decoration "underline"
        :color           (str (theme-pull :theme/editor-outer-rim-color nil) " !important")} ;; "#ff4aff
     ".cm-s-rvbbit-dynamic ::-webkit-scrollbar-corner"
       {:background "rgba(0,0,0,0) !important" "background-color" "rgba(0,0,0,0) !important"}
...
(re-frame/reg-event-db ::shift-down
                       (fn [db [_]] (if (not (get db :shift?)) (assoc db :shift? true) db)))
...
(re-frame/reg-sub
  ::get-flow-subs
  (fn [db _]
    (let [create-runner-listeners (fn [obody]
          ...
          runstreams              (vec (for [{:keys [flow-id]} @(rfa/sub ::runstreams)]
                                         (keyword (str "flow/" flow-id ">*running?"))))
          panels-map              (get db :panels) ;; this could get funky... TODO since we
                                              ...
                                              (filter keyword?
                                                (into (ut/deep-flatten panels)
                                                      (ut/deep-flatten (get-all-values
                                                                         (get db :click-param))) ;; do
                                                                                                 ;; we
                                                )))
                                            current-flow-open)))
          warren-item             (get db :selected-warren-item)
          solver-open?            (and (= (get @db/flow-editor-system-mode 0) "signals")         ;; signals
                                       (get db :flow?) ;; flow panel open
                                  )
          solver                  (if (and (get-in db [:solvers-map warren-item :data])
                                           solver-open?)
                                    [(keyword (str "solver/"
                                                   (ut/replacer (str warren-item) ":" "")))
                                     (keyword (str "solver-meta/"
                                                   (ut/replacer (str warren-item) ":" "")
                                                   ">extra"))
                                     (keyword (str "solver-meta/"
                                                   (ut/replacer (str warren-item) ":" "")
          ...
          solvers                 (if (and solver-open? (some #(= % "solvers") @db/selectors-open))
                                    (vec (for [ss (keys (get db :solvers-map))]
                                           (keyword (str "solver-meta/"
                                                         (ut/replacer (str ss) ":" "")
                                                         ">extra"))))
                                    [])
          signals-mode?           (and (= (get @db/flow-editor-system-mode 0) "signals") ;; signals
                                       (get db :flow?) ;; flow panel open
                                  ) ;; signal subbox open
          signals-box-open?       (some #(= % "signals") @db/selectors-open)
          signal-ui-refs          (when signals-box-open?
                                    (vec (for [ss (keys (get db :signals-map))]
                                           (keyword (str "signal/"
                                                         (ut/replacer (str ss) ":" ""))))))
          signal-ui-part-refs     (when (and warren-item signals-mode?)
                                    (let [pps (ut/where-dissect
                                                (get-in db [:signals-map warren-item :signal]))]
                                      (vec (for [pp pps]
                                             (keyword (str "signal/part-" (ut/replacer
                                                                            (str warren-item)
                                                                            ":"
                                                                            "")
                                                           "-"            (.indexOf pps pp)))))))
          signal-hist             (if (and (get-in db [:signals-map warren-item :signal])
                                           ...
                                           signals-mode?)
                                    [(keyword (str "signal-history/"
                                                   (ut/replacer (str warren-item) ":" "")))]
                                    [])
          signal-subs             (if signals-mode?
                                    (vec (into signal-hist
                                               (into signal-ui-refs signal-ui-part-refs)))
                                    [])
      ...
      (filterv #(not (cstr/includes? (str %) "||")) ;; live running subs ignore
        (vec (distinct (concat signal-subs
                               solver
                               solvers
                               drop-refs
                               runstream-refs
...
(re-frame/reg-sub ::stale-flow-subs?
                  (fn [db _]
                    (let [;;new @(ut/tracked-subscribe [::get-flow-subs])
                          ...
                          deads (filterv #(not (cstr/includes? (str %) "||")) deads)] ;; dont mess
                                                                                      ;; with
                      (ut/ne? deads))))
...
(re-frame/reg-event-db ::unsub-to-flows
                       (fn [db _]
                         (let [;;new @(ut/tracked-subscribe [::get-flow-subs])
                               ...
                               deads (vec (cset/difference (set old) (set new)))
                               deads (filterv #(not (cstr/includes? (str %) "||")) deads) ;; dont
                                                                                          ;; mess
                                                                                          ;; with
                               _ (ut/tapp>> [:flow-unsub-change! (get db :client-name)
                                             {:old old :new new :removing deads}])]
                           (doall (doseq [n deads]
                                    (ut/tracked-dispatch [::http/unsub-to-flow-value n])
                                    (ut/tracked-dispatch
                                      [::conn/declick-parameter
                                       (vec (map keyword
                                              (ut/splitter (ut/replacer (str n) ":" "") #"/")))]) ;; remove
                                  ))
...
(re-frame/reg-sub ::get-new-flow-subs
                  (fn [db _]
                    (let [;;flow-refs @(ut/tracked-subscribe [::get-flow-subs])
                          ...
                          subbed    (filterv #(not (cstr/includes? (str %) "||")) subbed)] ;; dont
                                                                                           ;; mess
                                                                                           ;; with
                      (vec (cset/difference (set flow-refs) (set subbed))))))

(re-frame/reg-sub ::new-flow-subs? (fn [_ _] (ut/ne? @(rfa/sub ::get-new-flow-subs {}))))
...
(re-frame/reg-event-db ::refresh-runstreams
                       (fn [db _]
...
(re-frame/reg-sub ::workspace
                  (fn [db [_ keypath]]
                    (let [merged-keypath (cons :panels keypath)] (get-in db merged-keypath))))
...
(defn sql-alias-replace-sub [query] @(rfa/sub ::sql-alias-replace-sub {:query query}))
  ...
  (fn [{:keys [db]} _]
    (let [flow-id              @(ut/tracked-subscribe [:rvbbit-frontend.flows/selected-flow])
          ...
          resolved-queries     (into {}
                                     (for [[qk q] (apply merge
                                                    (for [[_ p] (get db :panels)]
                                                      (get p :queries)))]
      ...
      (ut/tracked-dispatch
        [::http/insert-alert
         [:box :child
          (str "sent " (ut/replacer (str ttype) ":" "") " save request to server... Please wait.")
...
(re-frame/reg-sub ::matching-snapshots
                  (fn [db _]
                    (let [snaps (get-in db [:snapshots :params] {})
                          curr  @(rfa/sub ::current-params {})]
                      (for [[k v] snaps
                            :let  [block-states     (get v :block-states)
                                   valid-state-keys (vec (cset/intersection ;; since we are diffing,
                                                                            ;; we only
                                                           (set (keys block-states)) ;; otherwise we
                                                                                     ;; get a diff
                                                           (set (keys (get curr :block-states)))))] ;; which
                                                                                                    ;; are
                            :when (= (assoc (select-keys v [:params])
                                       :block-states (select-keys (get v :block-states)
                                                                  valid-state-keys))
                                     (dissoc (assoc curr
                                               :block-states (select-keys (get curr :block-states)
                                                                          valid-state-keys))
                                       :selected-tab))]
                        k))))
...
(re-frame/reg-sub ::current-tab-blocks
                  (fn [db _]
                    (let [blocks (try (map name
                                        (keys (into {}
                                                    (filter #(= (get db :selected-tab)
                                                                (get (val %) :tab ""))
                                                      (get db :panels)))))
                                      (catch :default _ []))]
                      (or blocks []))))
...
(defn partition-keywords
  [keywords]
  (let [str-keywords (map str keywords)] (group-by #(cstr/includes? % "||") str-keywords)))

(re-frame/reg-sub ::partitioned-subs ;; in order to abuse re-frame subscription caching
                  (fn [db _] (partition-keywords (get db :flow-subs []))))

(re-frame/reg-sub ::flow-watcher-subs :<- [::partitioned-subs] (fn [subs _] (get subs true [])))

(re-frame/reg-sub ::server-subs :<- [::partitioned-subs] (fn [subs _] (get subs false [])))

(re-frame/reg-sub ::flow-watcher-subs-grouped
                  :<-
                  [::flow-watcher-subs]
                  (fn [subs _]
                    (if (empty? subs)
...
(re-frame/reg-sub ::tabs
                  (fn [db]
                    (let [seen-tabs     (distinct (for [[_ pv] (get db :panels)] (get pv :tab))) ;; prevents
                          ...
                          tabs          (vec (remove empty?
                                               (distinct (into explicit-tabs seen-tabs))))]
                      tabs)))
                       ...
                       (fn [db [_ tab-name]]
                         (let [tabs     (get db :tabs)
                           ...
                           (-> db
                               ...
                               (assoc :panels (into {}
                                                    (filter #(not (= tab-name (get (val %) :tab)))
                                                      (get db :panels))))))))
...
(re-frame/reg-event-db ::rename-tab
                       (fn [db [_ old new]]
                         (-> db
                             ...
                             (assoc :tabs (vec (map (fn [item] (if (= item old) new item))
                                                 (get db :tabs))))
...
(re-frame/reg-event-db ::add-tab
                       (undoable)
                       (fn [db [_ new-tab]] (assoc db :tabs (conj (vec (get db :tabs)) new-tab))))
...
(re-frame/reg-sub
  ...
  (fn [_]
    (and @db/auto-refresh? ;; auto-refresh killswitch bool
         (or (= @db/editor-mode :status) (= @db/editor-mode :vvv) (= @db/editor-mode :meta)))))

(re-frame/reg-sub ::update-flow-statuses?
                  (fn [db]
...
(re-frame/reg-sub ::flow-channels-open?
                  (fn [db [_ flow-id]] (get-in db [:flow-statuses flow-id :channels-open?] false)))
...
(re-frame/reg-event-db ::toggle-external
                       (fn [db [_]]
                         (assoc-in db [:external?] (not (get-in db [:external?] false)))))

(re-frame/reg-event-db ::toggle-flow
                       (fn [db [_]] (assoc-in db [:flow?] (not (get-in db [:flow?] false)))))

(re-frame/reg-event-db ::toggle-flow-editor
                       (fn [db [_]]
                         (assoc-in db [:flow-editor?] (not (get-in db [:flow-editor?] false)))))

(re-frame/reg-event-db ::toggle-session-modal
                       (fn [db [_]]
...
(re-frame/reg-event-db ::toggle-flow-gantt
                       (fn [db [_]]
...
(re-frame/reg-event-db ::toggle-lines
                       (fn [db [_]] (assoc-in db [:lines?] (not (get-in db [:lines?] false)))))

(re-frame/reg-event-db ::toggle-peek
                       (fn [db [_]] (assoc-in db [:peek?] (not (get-in db [:peek?] false)))))

(re-frame/reg-event-db ::toggle-kick-alert
                       (fn [db [_]] (reset! db/kick-alert (not @db/kick-alert)) db))

(re-frame/reg-event-db ::toggle-auto-run
                       (fn [db [_]]
                         (assoc-in db [:auto-run?] (not (get-in db [:auto-run?] false)))))

(re-frame/reg-event-db ::toggle-no-ui
                       (fn [db [_]] (assoc-in db [:no-ui?] (not (get-in db [:no-ui?] false)))))
...
(re-frame/reg-event-db
  ...
  (fn [db [_ theme-map]]
    (assoc-in db
      [:click-param :theme]
      (merge (apply dissoc (get-in db [:click-param :theme]) (vec (keys db/base-theme)))
             theme-map))))
                       ...
                       (fn [db [_ board-map]]
                         ...
                         (-> db
                             (assoc-in [:click-param :param]
                                       (merge (get-in db [:click-param :param])
...
(re-frame/reg-event-db ::refresh-all
                       (fn [db _]
                         (-> db
  ...
  (fn [db {:keys [query-key]}]
    (let [asl               (get db :panels)
          ...
          valid-body-params (filter #(not (and (cstr/ends-with? (str %) "-sys")
                              ...
                              (ut/deep-flatten @(rfa/sub ::valid-body-params-in {:body asl})))
          vbp-bools         (try (into {}
          ...
          params            (into {}
                                  (apply concat
                                    (for [[p v] (get-in db [:click-param])
                                          :when (and (not (= p :theme))
                                                     (not (cstr/ends-with? (str p) "-sys")))]
                                      (for [e (keys v)]
                                        (let [;asl @(ut/tracked-subscribe [::all-sql-calls])
                                              comb-key (keyword (str (ut/safe-name p)
                                                                     "/"
                                                                     (ut/safe-name e)))]
          ...
          both-maps         (into
                              ...
                              (for [[[k1 k2] b] (merge params vbp-bools)
                                    :let        [k2 (if (cstr/ends-with? (str k1) ".*")
                                                      [k2
                                                       (get-in db [:click-param cell-query-key k2])]
                                                      k2)]]
                                {[k1 k2] b}))
          matchers          (vec (map last
                                   (keys (filter #(and (or (last %) (vector? (last (first %))))
                                                       (cstr/starts-with? (str (first (first %)))
                                                                          (str query-key)))
                                           both-maps))))]
      matchers)))
...
(re-frame/reg-sub ::last-heartbeat
                  (fn [db] (get-in db [:status-data :heartbeat :kick :data 0 :at] "none!")))

(re-frame/reg-event-db ::set-screen-name
                       (fn [db [_ screen-name]]
                         (http/change-url (str "/" screen-name))
                         (assoc-in db [:screen-name] (str screen-name))))

(re-frame/reg-sub ::screen-name
                  (fn [db] (get db :screen-name (ut/safe-name (get db :client-name)))))


(re-frame/reg-event-db ::dispatch-keepalive
                       (fn [db [_]]
                         (do (conn/sql-data [:keepalives]
                                            {:select [(rand-int 123433333335)]
                                             :limit  [(rand-int 123333333345)]})
                             (assoc-in db [:keepalives] (+ (get-in db [:keepalives] 0) 1)))))

(re-frame/reg-event-db ::refresh-status
                       (fn [db [_]]
...
(re-frame/reg-event-db ::update-workspace
                       (undoable)
                       (fn [db [_ keypath value]] (assoc-in db (cons :panels keypath) value)))
...
(re-frame/reg-event-db ::update-workspace-noundo
                       (fn [db [_ keypath value]] (assoc-in db (cons :panels keypath) value)))

(re-frame/reg-event-db ::update-selected
                       (undoable)
                       (fn [db [_ value]]
                         (ut/tapp>> [:updating-selected [:panels (get db :selected-block)] :with
                                     value])
...
(re-frame/reg-event-db ::update-selected-key
                       (undoable)
                       (fn [db [_ key value]]
                         (ut/tapp>> [:updating-selected [:panels (get db :selected-block) key] :with
                                     value])
...
(re-frame/reg-event-db ::clean-up-previews
                       (fn [db [_]]
                         (let [pp (doall (for [r (filter #(cstr/starts-with? (str %)
                                                                             ":reco-preview")
...
(re-frame/reg-event-db ::quick-delete-panel
                       (fn [db [_ panel-key]] (ut/dissoc-in db [:panels panel-key])))
...
(re-frame/reg-sub ::lookup-connection-id-by-query-key
                  (fn [db [_ query-key]]
                    (get-in db
                            [:panels
                             (first (remove nil?
...
(re-frame/reg-sub ::selected-block (fn [db] (get-in db [:selected-block] "none!")))

(re-frame/reg-sub ::page-num
                  (fn [db [_ panel-key query-key]]
...
(re-frame/reg-sub ::selected-block?
                  (fn [db {:keys [panel-key]}]
                    (true? (= panel-key (get-in db [:selected-block] "none!")))))

(re-frame/reg-sub ::root (fn [db [_ panel-key]] (get-in db [:panels panel-key :root])))

(re-frame/reg-sub ::zindex (fn [db [_ panel-key]] (get-in db [:panels panel-key :z] 0)))

(re-frame/reg-sub ::size
                  (fn [db [_ panel-key]] [(get-in db [:panels panel-key :h])
                                          (get-in db [:panels panel-key :w])]))

(re-frame/reg-sub ::selected-block-map (fn [db] (get-in db [:panels (get db :selected-block)])))
...
(re-frame/reg-sub ::drop-type ;; experiment
                  (fn [db] (get db :drop-type)))


(re-frame/reg-event-db ::get-memory-usage
                       (fn [db]
...
(re-frame/reg-sub ::memory (fn [db _] (get db :memory)))

(re-frame/reg-sub ::runstreams
                  (fn [db _]
...
(re-frame/reg-event-db ::add-runstream
                       (undoable)
                       (fn [db [_ flow-id]]
                         (ut/tapp>> [:adding-flow-id-to-runstream flow-id (get db :client-name)])
                         (assoc-in db [:runstreams flow-id] {:open? true})))

(re-frame/reg-event-db ::remove-runstream
                       (undoable)
                       (fn [db [_ flow-id]]
                         (ut/tapp>> [:removing-flow-id-from-runstream flow-id
                                     (get db :client-name)])
                         (ut/dissoc-in db [:runstreams flow-id])))

(re-frame/reg-sub ::runstreams-lookups (fn [db _] (get db :runstreams-lookups)))
...
(re-frame/reg-sub ::query
                  (fn [db [_ panel-key query-key]]
                    (get-in db [:panels panel-key :queries query-key])))

(re-frame/reg-sub ::find-query
                  (fn [db [_ query-key]]
                    (let [kpth (or (vec (first (filter #(= (last %) query-key)
                                                 (ut/kvpaths (get db :panels)))))
                                   [:nope])]
    ...
    (if (get db :flow?)
      ...
      (let [panel-id     (get db :selected-block)
            ...
            query-key    (cond (and (nil? selected-tab) (not (empty? qkeys))) (first qkeys)
                               (and (not (nil? selected-tab)) (some #(= % selected-tab) qkeys))
                                 selected-tab ;(first
                               :else nil)
        ...
        (if col-mode?
          (let [modfields          (get db :col-names)
                ...
                fields-left        (if (>= (- field-idx 1) 0)
                                     ...
                                     fields)]
            (cond (= dir :left)  (-> db
                                     (assoc-in [:panels panel-id :queries query-key :select]
                                               move-left-selectc)
    ...
    (if (get db :flow?)
      (let [bid   (get db :selected-flow-block)
        ...
        (condp = dir
          ...
          :narrower (when (>= (- w chunk) (* 4 chunk))
                      (assoc-in db [:flows (get db :selected-flow) :map bid :w] (- w chunk)))
      ...
      (let [panel-id      (get db :selected-block)
            ...
            curr-width    (get-in db
                                  [:panels (get db :selected-block) :queries query-key :col-widths
                                   curr-field]
                                  def-col-width)
            col-mode?     (true? (not (nil? curr-field)))
        ...
        (if col-mode?
          ...
          (condp = dir
            ...
            :narrower (when (>= (- w 1) 2)
                        (assoc-in db [:panels (get db :selected-block) :w] (- w 1)))
            :longer   (assoc-in db [:panels (get db :selected-block) :h] (+ h 1))
            :shorter  (when (>= (- h 1) 2)
                        (assoc-in db [:panels (get db :selected-block) :h] (- h 1)))))))))
...
(re-frame/reg-event-db ::esc-unselect-current
                       (fn [db [_]] ;; if col is selected, first unselect that and then on
                                    ;; subsequent, unselect
                         (reset! mad-libs-view nil)
                         (reset! db/editor-mode :meta)
                         (cond (and (not (nil? (get db :selected-flow-block))) (get db :flow?))
      ...
      (if flow?
        ...
        (let [panel-keys     (filter #(and (not (cstr/starts-with? (str %) ":reco-preview"))
                               ...
                               (keys (doall
                                       (sort-by
                                         (fn [r]
                                           (let [rr (get r :root) [int2 int1] rr] (+ int1 int2)))
                                         (into {}
...
(re-frame/reg-sub ::build-grid
                  (fn [_ {:keys [bh bw start-h start-w]}]
                    (vec (for [x (range start-w bw) y (range start-h bh)] [x y]))))
...
(re-frame/reg-sub ::is-grid?222
                  (fn [db [_ panel-key view]]
                    (true? (some #(= % :grid)
                                 (ut/deep-flatten (get-in db [:panels panel-key :views view]))))))
    ...
    (let [start-x       (.-clientX evt)
          ...
          translate     (when false ;icon?
                          (let [containers            @(ut/tracked-subscribe [::containers])
                                ...
                                tab-over              (or (first (is-inside? [x1 y1] containers))
                                                          curr-tab)
                                base-tab?             (= curr-tab tab-over)]
                            (cond (and started-in-container? (not base-tab?)) [x1 y1 tab-over]
                                  (and started-in-container? base-tab?)
                                    (let [[offx offy] (for [[x y w h [t]] containers
                                                            :when         (= t tab-over)]
                                                        [x y])]
      ...
      (when (not (= selected-root [x y]))
        (if (and icon? translate)
          (do (when (not (= @dragging-block block)) (reset! dragging-block block))
              (ut/tracked-dispatch-sync [::update-workspace-noundo [block :root]
...
(defn insert-new-block
  ...
  (when (not @over-flow?)
    (let [new-key (str "block-" (rand-int 12345))
          ...
          is-view? (and (cstr/ends-with? (str param-key) "-vw")
                        (cstr/starts-with? (str param-key) ":flow"))
          multi-param? (true? (cstr/includes? (str param-key) ".*/"))
          base-map
            (if body ;; do we have incoming panel data or not?
              (cond (= (get-in (first body) [:drag-meta :type]) :param)
                      {:h       (get-in (first body) [:h])
                       ...
                       :views   (if multi-param?
                                  {:multi-param-vals [:box :align :center :justify :center :padding
                                                      "13px" :style {:font-size "30px"} :child
                                                      [:h-box :gap "9px" :children param-key]]}
                                  {:param-val [:box :align :center :justify :center :padding "13px"
                                               :style {:font-size "45px"} :child
                    ...
                    (= (get-in (first body) [:drag-meta :type]) :viewer-pull)
                      {:h       (get-in (first body) [:h])
                       ...
                       :queries {}}
                    (= (get-in (first body) [:drag-meta :type]) :open-input) ;; middle click
              ...
              {:h             height
               ...
               :selected-view :hi
               :name          new-key
               :views         {:hi [:box :align :center :justify :center :attr
                                    {:id (str new-keyw "." :hi)} :style
                                    {:font-size    "106px"
                                     :font-weight  700
                                     :padding-top  "6px"
                                     :padding-left "14px"
...
(defn insert-rec-preview-block
  ...
  (let [new-key       (str "reco-preview" (if (not single?) combohash ""))
        ...
        table-name    (if (keyword? table-name)
                        (ut/replacer (ut/replacer (str table-name) ":" "") "-" "_")
                        table-name)
                              ...
                              (let [randy (rand-int 999)]
                                {c                                         (keyword
                                                                             (str "c" randy
                                                                                  "-" (ut/safe-name
                                                                                        c)))
                                 (keyword (str "condi/" (ut/safe-name c))) (keyword
                                                                             (str "condi/c" randy
                                                                                  "-" (ut/safe-name
                                                                                        c)))})))
        queries       (into {}
        ...
        selected-view (if panel?
                        (try ;; weirdness here.. why was blowing up as seq? def just a keyword
                          (let [;sv @(ut/tracked-subscribe [::conn/clicked-parameter-key
                                sv @(ut/tracked-sub ::conn/clicked-parameter-key-alpha
                                                    {:keypath [:recos-sys/selected_view]})]
                            (when (ut/ne? sv) sv))
                          (catch :default _ nil))
                        (when (ut/ne? (get mad-libs-row :selected_view))
                          (get mad-libs-row :selected_view)))
        selected-view (when selected-view
                        (try (keyword (ut/replacer selected-view #":" ""))
                             (catch :default _ selected-view)))
...
(defn draggable
  ...
  [(reagent/adapt-react-class rdnd/Draggable)
   {:type          type
    ...
    :data          (pr-str data)} [re-com/box :size "auto" :child element :style {:cursor "grab"}]])
     ...
     {:types types-vec
      :on-drop
        #(let [incoming    (js->clj %)
               ...
               ok-new?     (or (and (not @on-block?)
                                    (or (false? @dyn-dropper-hover) (nil? @dyn-dropper-hover)))
                               (= dm-type :viz-reco))]
             ...
             (let [;cc @(ut/tracked-subscribe [::conn/clicked-parameter [:recos-sys]])
               ...
               (ut/tracked-dispatch [::wfx/request :default
                                     {:message {:kind        :selected-reco
                                                ...
                                                :client-name client-name}
                                      :timeout 50000}])))
           (ut/tapp>> [:dropped type :ok-new? :drag-meta-type dm-type
                       [@on-block? @dyn-dropper-hover] ok-new? rooted-data root])
           (reset! dragging? false)
           (cond (= dm-type :meta-screens) (ut/tracked-dispatch [::http/load
                                                                 (get rooted-data :file-path)])
                 (= dm-type :meta-theme)   (ut/tracked-dispatch [::overwrite-theme
                                                                 (apply dissoc
                                                                   rooted-data
                                                                   [:block-key :drag-meta :root
                                                                    :file-path])])
                 (= dm-type :meta-board)   (ut/tracked-dispatch [::add-board
                                                                 (apply dissoc
                                                                   rooted-data
                                                                   [:block-key :drag-meta :root
                                                                    :file-path])])
...
(defn mouse-move-handler-new
  ...
  (fn [evt]
    (let [start-x    (.-clientX evt)
          ...
          start-root [(js/Math.floor (/ orig-x brick-size)) (js/Math.floor (/ orig-y brick-size))]
      ...
      (reset! new-block-atom {:root [(first start-root) (last start-root)]
                              :w    (+ drag-w 1)
                              :h    (+ drag-h 1)})
...
(defn resize-mouse-move-handler
  ...
  (fn [evt]
    (let [block      @(ut/tracked-sub ::selected-block {})
          ...
          wmax       (if (< wf 2) 2 wf)
          hmax       (if (< hf 2) 2 hf)]
      (when (not (= hmax selected-h))
        (ut/tracked-dispatch [::update-workspace-noundo [block :h] hmax]))
      (when (not (= wmax selected-w))
        (ut/tracked-dispatch [::update-workspace-noundo [block :w] wmax])))))
              ...
              (when (< line (.lineCount doc))
                (when (clojure.string/includes? (.getLine doc line) (first code-lines))
                  (let [start-line         line
                                             ...
                                             (when (< line (.lineCount doc))
                                               (if (clojure.string/includes? (.getLine doc line)
                                                                             (last code-lines))
                                                 line
                                                 (recur (inc line)))))
                        ...
                        code-part-start-ch (clojure.string/index-of (.getLine doc start-line)
                                                                    (first full-code-parts))
                        code-part-end-ch   (+ (clojure.string/index-of (.getLine doc start-line)
                                                                       (last full-code-parts))
                                              (count (last full-code-parts)))]
                    (loop [code-parts code-parts
                           start-ch   0]
                      (when (seq code-parts)
                        (let [code-part (first code-parts)
                              start-ch (clojure.string/index-of (.getLine doc start-line)
                                                                code-part
                                                                start-ch)
                                ...
                                (try
                                  (let [node (js/document.createElement "span")
                                                          ...
                                                          (str "0 0 10px 5px "
                                                               (ut/invert-hex-color
                                                                 (theme-pull
                                                                   ...
                                                                   nil)))}))))
                                        _ (.appendChild node
                                                        (js/document.createTextNode code-part))]
                                    (.addEventListener
                                      node
                                      "click"
                                      (fn []
                                        (let [this-token-idx (.indexOf full-code-parts code-part)
                                              token-parts    (subvec full-code-parts
                                                                     0
                                                                     this-token-idx)
                                              seeded-token   (str (first token-parts)
                                                                  "/"
                                                                  (cstr/join ">"
                                                                             (rest token-parts)))]
                                          (.setCursor editor
                                    ...
                                    (.addEventListener
                                      ...
                                      "mouseover"
                                      (fn []
                                        (let [code   (try (edn/read-string code)
  ...
  (fn [db _]
    (let [;;codes [:theme/base-font :font-family :height]
          ...
          codes         (vec (apply concat
                               [server-params view-codes themes flow-subs click-params])) ;; not
         ]
      codes)))
...
(defn autocomplete-suggestions
  ...
  (if (not (cstr/ends-with? input ">"))
    (let [regs   (first (filter #(and (not= % input) (clojure.string/starts-with? % input))
                          all-keywords))
...
(re-frame/reg-sub ::panel-code-up?
                  (fn [db _]
                    (let [editor?            (get db :editor? false)
                          ...
                          view-code-hash     (hash [(get-in db [:panels (get db :selected-block)])
                                                    part-kp])
...
(re-frame/reg-event-db
  ...
  (fn [db _]
    (let [flow-subs      (get db :flow-subs)
      ...
      (if (not value-spy?)
        (highlight-codes-only codes
                              {;:color "white"
                               :background-color (str (ut/invert-hex-color
                                                        (get (theme-pull :theme/data-colors nil)
                                                             "keyword")))
                               :filter           "invert(1.2)"
                               :cursor           "pointer"
                               :border-radius    "5px"})
        (highlight-codes-values
          (into {}
                (for [c codes]
                  {c @(ut/tracked-sub ::conn/clicked-parameter-key-alpha
                                      {:keypath [(try (edn/read-string c)
                                                      (catch :default _ c))]})}))
          {;:color "white"
           :background-color (str (ut/invert-hex-color (get (theme-pull :theme/data-colors nil)
                                                            "keyword")))
           :filter           "invert(1.2)"
           :cursor           "pointer"
           :border-radius    "5px"})))
    ...
    [re-com/box :size "auto" :width (px (- width-int 24)) :height (px (- height-int 24)) :style
     ...
     [(reagent/adapt-react-class cm/UnControlled)
      {:value   (ut/format-map (- width-int 24) (str value))
       :onBlur  #(ut/tracked-dispatch-sync [::update-selected-field kp
                  ...
                  (fn [db {:keys [param-name]}]
                    (let [psplit    (ut/splitter (ut/safe-name (str param-name)) "/")
                      ...
                      (cond (= table :condi)             ;; condis are special
                              {:data-type (ut/data-typer ;; todo: change this hardcoding when we
                                                         ;; move to global
                                            (condp = (get-in (get-in db [:post-condi field]) [0 :v])
                                              1 true
                                              0 false
                                              :else (get-in (get-in db [:post-condi field])
                                                            [0 :v])))}
...
(defn click-param-browser
  ...
  (let [ph                  @param-hover ;; reaction hack
    ...
    (if false ;@db/param-code-hover
      [re-com/v-box :padding "6px" :width (px (- width-int 33)) :align :center :justify :between
       :gap "10px" :children
       [[re-com/box :style
         {:font-size        "14px"
          :font-weight      700
          :border-radius    "5px"
          :color            (ut/invert-hex-color (get (theme-pull :theme/data-colors nil)
                                                      "keyword")) ;; (theme-pull
          :padding          "4px"
          :background-color (get (theme-pull :theme/data-colors nil) "keyword")} :align :center
         :justify :center :child (str @db/param-code-hover)]
        (let [code @db/param-code-hover
          ...
          [re-com/box :size "auto" :width "100%" :style {:font-size "15px"} :child
           [map-boxes2 vv nil "" [] nil nil] :style
           {;:color "white"
            :border-radius "4px"}])]]
      [re-com/v-box :children
       [[re-com/h-box :padding "6px" :height "33px" :width (px (- width-int 33)) :align :center
         :justify :between :children
         (if @db/cm-focused?
           [[re-com/box :size "auto" :align :center :justify :center :style
             {:color "#ffffff99" :font-size "14px" :font-weight 500} :child
             "clover params used in this view"]]
           [[re-com/input-text :src (at) :model searcher-atom :width "93%" :on-change
             #(reset! searcher-atom (let [vv (str %) ;; (cstr/trim (str %))
                                         ]
                                      (if (empty? vv) nil vv))) :placeholder "(search parameters)"
                                        ...
                                        (filter
                                          #(and ;; filter out from the top bar toggle first....
                                             ...
                                             (not (cstr/starts-with?
                                                    (str (first %))
                                                    (if (get pf :theme) ":theme/***" ":theme/")))
                                             (not (cstr/starts-with?
                                                    (str (first %))
                                                    (if (get pf :user) ":param/***" ":param/")))
                                             (not (cstr/starts-with?
                                                    (str (first %))
                                                    (if (get pf :condis) ":condi/***" ":condi/")))
                                             (if (get pf :this-tab? true)
                                               (some (fn [x]
                                                       (cstr/starts-with? (str (first %))
                                                                          (str ":" x)))
                                                     current-tab-queries)
                                               true)
                 ...
                 (for [[k v] filtered-params]
                   (let [meta        @(ut/tracked-sub ::meta-from-param-name {:param-name k})
                         ...
                         is-image?   (and (or (cstr/ends-with? (cstr/lower-case (str param-value))
                                              ...
                                              (cstr/ends-with? (cstr/lower-case (str param-value))
                                                               ".gif")
                                              (cstr/includes? (cstr/lower-case (str param-value))
                                                              "/images/")
                                              (cstr/includes? (cstr/lower-case (str param-value))
                                                              "/image/"))
                                          (cstr/starts-with? (cstr/lower-case (str param-value))
                                                             "http"))
                     ...
                     (draggable ;(sql-spawner-filter :param [k table field])
                       ...
                       [re-com/v-box :size "auto" :attr
                        {:on-mouse-enter #(reset! param-hover [k table field])
                         :on-mouse-leave #(reset! param-hover nil)} :style
                        ...
                        [[re-com/h-box :justify :between :children
                          [[re-com/box :child
                            ...
                            {:color         dcolor
                             ...
                             :padding-right "4px"}]
                           (if hovered?
                             [re-com/md-icon-button :md-icon-name "fa-regular fa-trash-can" ;; "zmdi-close"
                                                                                            ;; ;; <i
                              :style
                              {:color        dcolor
                               :padding      "0px"
                               ...
                               :margin-right "3px"
                               :font-size    "14px"
                               :height       "15px"} :on-click
                              #(ut/tracked-dispatch [::conn/declick-parameter [table field]])]
                         ...
                         [re-com/box :child
                          (if (scrub/hex-color? v)
                            [re-com/h-box :gap "7px" :children
                             [(str v)
                              [re-com/box :src (at) :child " " :size "auto" :width "15px" :height
                               "15px" :style
                               {:background-color (str v)
                                :margin-top       "2px"
                                :padding-left     "3px"
                                :padding-right    "3px"}]]]
                            (if (nil? v)
                          ...
                          (merge {:color          (str (theme-pull :theme/editor-font-color nil) 99) ; "#ffffff99"
                                  ...
                                  :padding-right  "4px"
                                  :padding-bottom "2px"}
                                 (if (nil? v) {:font-style "italic" :opacity 0.3} {})) :align
                          :end]]])))])))]]]]))) ;]
...
(defn screen-query-browser
  ...
  (let [ph                  @query-hover ;; reaction hack
    ...
    [re-com/box :size "none" :height (px (- height-int 70)) :style
     {:overflow-y "auto" :overflow-x "hidden" :margin-left "-22px"} :child
     [re-com/v-box :style
        ...
        (let [;meta @(ut/tracked-subscribe [::meta-from-query-name k])
          ...
          [re-com/v-box :attr
           ...
           {:border           (cond
                                ...
                                :else
                                  (str "1px solid " (theme-pull :theme/editor-font-color nil) 33))
...
(re-frame/reg-sub ::panel-name-only
                  (fn [db {:keys [panel-key]}]
...
(defn sql-spawner
  ...
  (let [sql-name-base        (ut/replacer (str (ut/safe-name target-id) "-drag-" (rand-int 45))
                                          #"_"
                                          "-")
        ...
        general-field        {:h             (if agg? 4 5)
                              ...
                              :queries       (if agg?
                                               {sql-name {:select [[[:sum target-id]
                                                                    (keyword (str (ut/safe-name
                                                                                    target-id)
                                                                                  "_sum"))]]
                                                          :from   [[parent-sql-alias
                                                                    parent-sql-sql-alias]]}}
                                               {sql-name {:select   [target-id [[:count 1] :rowcnt]]
                                                          :from     [[parent-sql-alias
                                                                      parent-sql-sql-alias]]
                                                          :group-by [target-id]
...
(defn sql-spawner-where
  ...
  (let [sql-name-base (ut/replacer (str (ut/safe-name field-name) "-drag-" (rand-int 45)) #"_" "-")
        ...
        new-flow-map (when flow-item?
                       (let [pval   @(ut/tracked-subscribe [::conn/clicked-parameter
                                                            [:flow-fn-sys]])
        ...
        general-field
          {:h             6
           ...
           :flow-item     (select-keys flow-item
                                       [:category :name :type :icon :defaults :types :style
                                        :selected-style :inputs :expandable? :required])
           ...
           :connection-id (get panel-map :connection-id)
           :name          (str "drag-from-" (get panel-map :name))
           :queries       {sql-name {:select selected-fields ;[:*]
        ...
        general-field (if flow-item?
                        (let [inputs    (vec (keys (get-in new-flow-map [:ports :in])))
                              flow-item (assoc (get-in new-flow-map [:data :flow-item])
                                          :inputs inputs)
...
(defn sql-spawner-chat
  ...
  (let [sql-name-base (ut/replacer (str "chat-drag-" (rand-int 45)) #"_" "-")
        ...
        general-field {:h             (or h 6)
                       :w             (or w 5)
                       :drag-meta     {:type :chat :connection-id (get panel-map :connection-id)}
                       :connection-id (get panel-map :connection-id)
...
(defn sql-spawner-meta
  ...
  (let [selected-meta-db      @(ut/tracked-subscribe [::conn/clicked-parameter [:connections-sys]]) ;;
        ...
        rando                 (rand-int 45)
        sql-name-base         (cond (= type :meta-fields) (ut/replacer
                                                            (str (ut/safe-name table-name)
                                                                 "-drag-" (ut/safe-name field-name)
                                                                 "-"      rando)
                                                            #"_"
                                                            "-")
                                    :else                 (ut/replacer (str (ut/safe-name
                                                                              table-name)
                                                                            "-drag-"
                                                                            rando)
                                                                       #"_"
        ...
        drag-meta             {:target           (keyword field-name)
                               ...
                               :table-fields     (if (= type :meta-tables)
                                                   selected-fields
                                                   [(keyword field-name)])
                               :connection-id    connection-id
        ...
        qual-table-name       (cond (cstr/includes? db-type "vertica") (str (get selected-meta-table
                                                                                 :db_schema)
                                                                            "."
                                                                            table-name)
                                    :else                              table-name)
        ...
        meta-fields-panel     {:h             5
                               ...
                               :queries       {sql-name {:select   [(keyword field-name)
                                                                    [[:count 1] :rowcnt]]
                                                         :from     [[(keyword qual-table-name)
                                                                     parent-sql-sql-alias]]
                                                         :group-by [(keyword field-name)]
                                                         :order-by [[:rowcnt :desc]]}}}
        meta-screen-load      {:file-path (get @(ut/tracked-subscribe [::conn/clicked-parameter
                                                                       [:files-sys]])
                                               :file_path)
                               :drag-meta {:type type}}
        meta-block-load       (merge
                                (read-string @(ut/tracked-sub ::conn/clicked-parameter-key-alpha
                                                              {:keypath [:blocks-sys/block_data]}))
                                (let [bk (get @(ut/tracked-subscribe [::conn/clicked-parameter
                                                                      [:blocks-sys]])
                                              :block_key)
                                      bn (get @(ut/tracked-subscribe [::conn/clicked-parameter
                                                                      [:blocks-sys]])
                                              :block_name)]
                                  {:file-path (get @(ut/tracked-subscribe [::conn/clicked-parameter
                                                                           [:files-sys]])
                                                   :file_path)
                                   :block-key bk
                                   :drag-meta {:type (cond (= bk ":*theme*") :meta-theme
                                                           (cstr/starts-with? (str bn) "board: ")
                                                             :meta-board
                                                           :else type)}}))
        meta-fields-agg-panel {:h             5
                               ...
                               :connection-id connection-id
                               :name          (str "select-" field-name "-" table-name)
                               :queries       {sql-name {:select [[[:sum (keyword field-name)]
                                                                   (keyword (str field-name
                                                                                 "_sum"))]]
                                                         :from   [(keyword qual-table-name)]}}}]
    (cond (= type :viz-reco)
...
(defn item-subscription-spawner-meta
  ...
  (let [main-row-selected @(ut/tracked-subscribe [::conn/clicked-parameter [:searches-rows-sys]]) ;;
        ...
        shortcode (try (edn/read-string (get main-row-selected :value))
                       (catch :default _ "shortcode-error!"))
        rando (rand-int 45)
        {:keys [item_type item_sub_type item_key display_name sample is_live block_meta]}
          main-row-selected
        ...
        selected-fields (vec (keys (get @(ut/tracked-subscribe [::conn/sql-metadata [name]])
                                        :fields)))
        ...
        parent-sql-sql-alias (ut/gen-sql-sql-alias)
        sql-name-base
          (cond :else (ut/replacer (str (ut/safe-name table-name) "-clone-" rando) #"_" "-"))
        sql-name (keyword sql-name-base)
        connection-id (get panel-map :connection-id)
        drag-meta {;:source-table     table-name
                   :type ttype}
        new-panel (cond
                    (cstr/ends-with? (str ttype) "-views")   {:h         (get panel-map :h 5)
                                                              :w         (get panel-map :w 7)
                                                              :drag-meta drag-meta
                                                              :views     {:vw1 [:box :size "auto"
                                                                                :child shortcode]}
...
(defn sql-spawner-cloner
  ...
  (let [panel-map            @(ut/tracked-sub ::workspace-alpha {:keypath [source-panel-id]})
        selected-fields      (vec (keys (get @(ut/tracked-sub ::conn/sql-metadata-alpha
                                                              {:keypath [name]})
                                             :fields)))
        selected-fields      (if (empty? selected-fields) [:*] selected-fields)
        table-name           name
        view-data            (if (= name :view)
                               (get panel-map :views)
                               (get-in panel-map [:views name]))
        rando                (rand-int 45)
        parent-sql-sql-alias (ut/gen-sql-sql-alias)
        sql-name-base        (cond ;(= type :meta-fields)
                               :else (ut/replacer (str (ut/safe-name table-name) "-clone-" rando)
                                                  #"_"
                                                  "-"))
        sql-name             (keyword sql-name-base)
        connection-id        (get panel-map :connection-id)
...
(re-frame/reg-sub ::subq-used
                  (fn [db [_ panel-id]] ;; filter has got to be faster than for loops. TODO for
                                        ;; other subs im
                    (let [queries-used    (filter #(cstr/includes? (str %) ":query/")
                                            (ut/deep-flatten (get-in db
                                                                     [:panels panel-id :queries])))
                          root-query-keys (for [k queries-used]
                                            (keyword (last (ut/splitter (ut/safe-name k) "/"))))]
                      root-query-keys)))
                                  ...
                                  (for [[k v] panels]
                                    {k (merge
                                         {:uses
                                            (apply concat
                                              (for [[_ v1] (merge (get v :queries) (get v :views))]
                                                (for [qq (filter #(or (cstr/includes? (str %)
                                                                                      ":query/")
                                                                      (some #{%} all-sql-call-keys))
                                                           (flatten
                                                             (conj
...
(re-frame/reg-sub
  ...
  (fn [db [_ panel-id]] ;; filter has got to be faster than for loops. TODO for other subs im
    (let [panels            (into {}
          ...
          query-sources     (into {}
                                  (remove empty?
                                    (flatten (for [[k v] panels]
                                               (flatten (for [kk (keys (get v :queries))]
                                                          {kk k}))))))]
      (vec (remove #(or (nil? %) (= (get db :selected-block) %))
...
(re-frame/reg-sub
  ...
  (fn [db [_ panel-id]] ;; filter has got to be faster than for loops. TODO for other subs im
    (let [queries-used    (filter #(cstr/includes? (str %) ":query/")
                            (ut/deep-flatten
                              (into {} (for [[k v] (get db :panels)] (get v :queries)))))
                        ...
                        [;[re-com/box :child (str kp " :: " v)]
                         [re-com/h-box :justify :between :children
                          [(when (not flow?)
                           ...
                           (when (not flow?)
                             [re-com/h-box :gap "5px" :style {:font-size "11px" :padding-top "3px"}
                              :children
                           ...
                           [re-com/h-box :gap "5px" :children
                            [(when (scrub/hex-color? v)
                               [re-com/box :src (at) :child " " :size "none" :width "18px" :height
                                "18px" :attr
                                {:on-click #(reset! scrub/color-picker-enabled?
                                      ...
                                      (for [[k v] queries]
                                        [re-com/v-box :children
                                         [;[re-com/box :child (str k)]
                                          [shape/map-boxes2 v panel-key (str k) [] panel-key
                                           "map"]]])]))
                          flat-scrubber-boxes)]
     (cond canvas? [re-com/v-box :size "auto" :width "100%" :height "100%" :gap "3px" :children
                    final-children]
...
(re-frame/reg-sub ::resolve-sql-alias
                  (fn [db {:keys [sql-key]}]
                    (let [q (get (into {} (for [[_ v] (get db :panels)] (into {} (get v :queries))))
                                 sql-key)]
...
(defn starts-with-fragment?
  ...
  (and (<= (count fragment) (count vec))
       (every? true? (map = fragment (subvec vec 0 (count fragment))))))
...
(defn sql-alias-replace
  ...
  (let [deep-flat-query     (ut/deep-flatten query)
        vpaths              (try (let [kpaths (ut/kvpaths query)
                                       vpaths (get (into {} (for [k kpaths] {(get-in query k) k}))
                                                   :*code*)
                                       vpaths (assoc vpaths (- (count vpaths) 1) 1)
                                       vpaths (for [kp (filter #(starts-with-fragment? vpaths %)
                                                         kpaths)]
                                                (get-in query kp))]
                                   vpaths)
                                 (catch :default _ []))
                                         ...
                                         {x @(ut/tracked-subscribe
                                               [::generate-all-clicked
                                                {:query-key (-> x
                                                                ...
                                                                (ut/replacer "/*.clicked" "")
                                                                keyword)}])})
                                    (filter #(cstr/includes? (str %) "/*.clicked")
                                      deep-flat-query)))
        valid-params        (into (distinct @(rfa/sub ::valid-body-params-all-condis))
                                  (vec (remove nil?
                                         (for [k deep-flat-query]
                                           (when (and (keyword? k)
                                             ...
                                             k)))))
        valid-params        (cset/difference (set valid-params)
                                             (set (filter ut/process-key vpaths)))
        ...
        value-walks         (into {}
                                  (for [k value-walks-targets] ;all-sql-call-keys]
                                    (let [fs    (ut/splitter (ut/safe-name k) "/")
                                      ...
                                      {k (get-in @(rfa/sub ::conn/sql-data-alpha {:keypath [ds]})
                                                 [row field])})))
        ...
        sql-params          (into {}
                                  (for [k valid-params]
                                    (if (cstr/includes? (str k) ":query/")
                                      {k (dissoc @(rfa/sub ::resolve-sql-alias
                                                           {:sql-key (keyword (ut/replacer (str k)
                                                                                           ":query/"
                                                                                           ""))})
                                           :style-rules)}
...
(re-frame/reg-event-db
  ...
  (fn [db [_ panel-key query-key clause drop-data]]
    (let [clause-conform       (cond (or (= clause :where<>) (= clause :where=)) :where
                                     ...
                                     (or (= clause :order-by-asc) (= clause :order-by-desc))
                                       :order-by
                                     :else clause)
      ...
      (cond ;p = clause-conform
        (some #(= clause-conform %) drops)
          (let [;dd @(ut/tracked-subscribe [::drop-details clause-conform])
                ...
                param-key    (keyword
                               (str (ut/unkey src-table) "/" (ut/unkey param-field) "." row-num))
                param-value  (get-in drop-data [:drag-body :param-full])
                target-block (get-in drop-data [:target-panel])
                cell-drop    (ut/safe-key (keyword (str "cell-drop-"
                                                        (ut/replacer (str clause-conform) ":" ""))))
                md           (get-in drop-data [:drag-body :data-type])
                new-view     [:box :size "auto" :style
                              {:font-size  [:auto-size-px param-key]
                               :transition "all 0.6s ease-in-out"} :child
                              [clause-conform param-key]]]
            (if (not (and row-num src-table param-field))
              (do (ut/tapp>> [:cant-drop-text-view :missing-parts src-table "/" param-field "."
                              row-num])
                  db)
              (-> db
        ...
        (= clause-conform :runstream-overrides)
          (let [{:keys [source target]}                                      (get drop-data
                                                                                  :drop-data)
        ...
        (cstr/starts-with? (str clause-conform) ":weave/")
          (let [weave-by             (keyword (ut/replacer (str clause-conform) ":weave/" ""))
                ...
                view-body            (get-in db
                                             [:panels source-panel-key :views source-view]
                                             [:div "where's the beef!"])
                fxky                 (fn [k]
                                       (if (cstr/starts-with? (str k) ":query/")
                                         (keyword (ut/replacer k #":query/" ""))
                                         k))
                data-key-path        (or (first (filter #(= (last %) :data) (ut/kvpaths view-body)))
                                         [:nope])
                view-height          (or (first (filter #(= (last %) :height)
                                                  (ut/kvpaths view-body)))
                                         [:nope])
                view-width           (or (first (filter #(= (last %) :width)
                                                  (ut/kvpaths view-body)))
                                         [:nope])
                data-key             (or (first (filter #(= (last %) :dataKey)
                                                  (ut/kvpaths view-body)))
                                         [:nope]) ;; temp
                ...
                view-body            (-> view-body
                                         ...
                                         (assoc-in (conj (vec (drop-last data-key))
                                                         :isAnimationActive)
                                                   false)
                ...
                modded-query         (if (get base-query :where)
                                       (assoc base-query
                                         :where [:and (get base-query :where)
                                                 [:= weave-by weave-by-placeholder]])
        ...
        (not (nil? (get clause-conform :layout-drop)))
          (let [target             (get clause-conform :layout-drop)
                ...
                incoming-block-src (if (= (get-in drop-data [:drag-body :type]) :query)
                                     :query
                                     incoming-block-src) ;; override with query keyword prefix
                compound-key       (try (keyword (str "view/" (name incoming-block-src)
                                                      "."     (name incoming-view-src)))
            ...
            (ut/tapp>> [:adding-view-to-layout target-kp compound-key (conj target-kp compound-key)
                        exists? (conj target-kp target) (get-in db (conj target-kp target))])
            (if exists?
        ...
        (= clause-conform :where) (let [exist?     (not (empty? existing-query-part))
                                        ...
                                        stmt       (if param?
                                                     [:*when kw-alias ;; no need to wrap literals in
                                                                      ;; :*when
        ...
        (= clause-conform :highlight-rows)
          (let [exist           (get-in db [:panels panel-key :queries query-key :style-rules])
                ...
                stmt            {:logic [:*if clicked-keyword [:*all= clicked-keyword group-bys]
                                         false]
                                 :style {:background-color (str (theme-pull
                                                                  :theme/editor-outer-rim-color
                                                                  nil)
                                                                45) ;;"#008b8b66"
        ...
        (or (cstr/starts-with? (str clause-conform) ":pivot-on/")
          ...
          (let [pivot-field  (keyword (last (ut/splitter (str clause-conform) #"/")))
                ...
                new-query    (-> existing-q
                                 ...
                                 (assoc :select (ut/remove-key (get existing-q :select)
                                                               pivot-field))
                                 (assoc :group-by (ut/remove-key (get existing-q :group-by)
                                                                 pivot-field)))
                new-query    (if spicy?
                               (-> new-query)
                               new-query)]
        ...
        (cstr/starts-with? (str clause-conform) ":join-on/")
          (let [add-alias                     (fn [alias field]
                                                (keyword (str (ut/safe-name alias)
                                                              "/"
                                                              (ut/safe-name field))))
                ...
                existing-alias-walk           (into {}
                                                    (for [e existing-table-fields]
                                                      {e (add-alias existing-table-alias e)}))
                ...
                incoming-table-fields-dupes   (cset/intersection (set incoming-table-fields)
                                                                 (set existing-table-fields))
                incoming-table-fields-final   (cset/difference (set incoming-table-fields)
                                                               incoming-table-fields-dupes)
                incoming-table-fields-aliased (vec (for [e incoming-table-fields-final]
                                                     (add-alias incoming-table-alias e)))
                subquery?                     (not (= (get-in drop-data
                                                              [:drag-body :source-panel-key])
                                                      "none!"))
                incoming-table                (if subquery?
                                                (keyword (str "query/"
                                                              (ut/safe-name incoming-table)))
                                                incoming-table)
                                                  ...
                                                  (if existing-joins?
                                                    (vec (concat
                                                           ...
                                                           [[incoming-table incoming-table-alias]
                                                            [:=
                                                             (add-alias incoming-table-alias
                                                                        join-field)
                                                             (first (filter #(cstr/includes?
                                                                               (str %)
                                                                               (str "/"
                                                                                    (name
                                                                                      join-field)))
                                                    ...
                                                    [[incoming-table incoming-table-alias]
                                                     [:= (add-alias incoming-table-alias join-field)
                                                      (add-alias existing-table-alias
                                                                 join-field)]]))
                aliased-q                     (assoc aliased-q
                                                :select (vec (apply merge
                                                               (get aliased-q :select)
                                                               (vec
                                                                 incoming-table-fields-aliased))))]
            (ut/tapp>> [:join-debug existing-selects (add-alias incoming-table-alias join-field)
                        (name join-field)
                        (first (filter #(cstr/includes? (str %) (str "/" (name join-field)))
                                 existing-selects))])
          ...
          (let [src-table      (get-in drop-data [:drag-body :source-table])
            ...
            (-> db
                ...
                (assoc-in [:panels panel-key :queries new-q-key]
                          (sql-alias-replace-sub {:select   [target-field [[:count 1] :rowcnt]] ;; sql-alias-replace
        ...
        (= clause-conform :remove)
          (let [fname      (get-in drop-data [:drag-body :target])
                ...
                exists-in? (fn [name entry]
                             (or (= name entry) (some #(= name %) (ut/deep-flatten entry))))
                new-select (vec (remove nil? (for [e select] (when (not (exists-in? fname e)) e))))
                new-order  (vec (remove nil?
                                  (for [e order-by] (when (not (exists-in? fname e)) e))))
                new-group  (vec (remove nil?
                                  (for [e group-by] (when (not (exists-in? fname e)) e))))]
            (-> db
                ...
                (ut/dissoc-in [:panels panel-key :queries query-key
                               (if (empty? new-order) :order-by :nevermind!)]) ;; dirty hack
                (ut/dissoc-in [:panels panel-key :queries query-key
                               (if (empty? new-group) :group-by :nevermind!)]))) ;; dirty hack
        ...
        (or (= clause-conform :delete-query) (= clause-conform :delete-view))
          (let [source-block      (get-in drop-data [:drag-body :source-panel-key])
            ...
            (when (= clause-conform :delete-query)
              (ut/tracked-dispatch [::unalias-query source-tab])) ;; dont
            (-> db
        ...
        (or (= clause-conform :copy-into)
            (= clause-conform :move-into)
            (= clause-conform :reference))
          (let [source-block      (get-in drop-data [:drag-body :source-panel-key])
                ...
                new-tab-name      (if (= clause-conform :move-into)
                                    ...
                                    (keyword
                                      (str (ut/safe-name source-tab) "-copy-" (rand-int 99))))]
            (cond
              (and view? (= clause-conform :reference)) (-> db
                                                            (assoc-in [:panels dest-block source-key
                                                                       new-tab-name]
                                                                      (keyword
                                                                        (str "view/" (ut/safe-name
                                                                                       source-block)
                                                                             "." (ut/safe-name
                                                                                   source-tab)))))
              query?                                    (if (= clause-conform :copy-into)
                                                          (-> db
                                                              (assoc-in [:panels dest-block
                                                                         source-key new-tab-name]
                                                                        new-tab-data)
                                                              (assoc-in [:panels dest-block
                                                                         :connection-id]
                                                                        connection-id))
                                                          (-> db
                                                              ...
                                                              (assoc-in [:panels dest-block
                                                                         :connection-id]
                                                                        connection-id)
                                                              (ut/dissoc-in [:panels source-block
                                                                             source-key source-tab])
                                                              (ut/dissoc-in (if last-tab? ;; if we
                                                                                          ;; are
                                                                                          ;; moving
                                                                                          ;; out of
                                                                                          ;; the
                                                                              ...
                                                                              [:panels source-block
                                                                               :nevermind!]))))
              :else                                     (if (= clause-conform :copy-into) ;; for
                                                                                          ;; views
                                                                                          ;; DONT
                                                                                          ;; overwrite
                                                                                          ;; the
   ...
   {:types   types-vec
    :on-drop #(let [incoming (js->clj %)
                ...
                (ut/tapp>> [:dropped-pill type data
                            [::execute-pill-drop (get drop-data :target-panel)
                             (get drop-data :target-query) (get drop-data :clause) drop-data]])
                (reset! dragging? false)
                (reset! on-block? false)
                (reset! dragging-size [])
                (ut/tracked-dispatch [::execute-pill-drop (get drop-data :target-panel)
                                      (get drop-data :target-query) (get drop-data :clause)
                                      drop-data]))} [re-com/box :child element]])
...
(defn dynamic-spawner-targets
  ...
  (let [drag-body (get dbody :drag-meta)
        ...
        view-data-ref (get-in dbody
                              (or (first (filter #(= (last %) :data) (ut/kvpaths dbody))) [:nope]))
        viz-for-each-fields
          (when (and (keyword view-data-ref) (not (nil? query-key)))
            (let [fxky              (fn [k]
                  ...
                  base-query-fields (cond (map? base-query-from)     (get base-query-from :select)
                                          (keyword? base-query-from) (get @(ut/tracked-subscribe
                                                                             [::find-query
                                                                              (fxky
                                                                                base-query-from)])
                                                                          :select
                                                                          [])
                                          :else                      [])
                  local-fields      (get @(ut/tracked-subscribe [::find-query query-key])
                                         :select
                                         [])
                  local-meta        (get @(ut/tracked-subscribe [::conn/sql-metadata [query-key]])
                                         :fields)
                  possibles         (vec (cset/intersection (set local-fields)
                                                            (set base-query-fields)))
                  possibles-vec     (vec (for [p     possibles
                                               :when (<= (get-in local-meta [p :distinct]) 50)]
                                           p))]
              possibles-vec))
        ...
        table-source-k
          (if (vector? (first table-source)) (first (first table-source)) (first table-source))
        ...
        param? (and (or (= drag-type :param) (= drag-type :where))
                    (not (cstr/includes? (str (get drag-body :param-full)) ".*/"))) ;; ignore
        ...
        clicked-fields (if view-drag?
                         @(ut/tracked-subscribe [::view-clicked-fields source-panel-key
                                                 source-table])
                         [])
        clicked? (ut/ne? clicked-fields)
        query-fields (if (or clicked? table?)
                       (vec (keys (get @(ut/tracked-subscribe [::conn/sql-metadata [query-key]])
                                       :fields)))
                       [])
        pivot-dim-query-fields
          (keys (into {}
                      (filter (fn [[_ v]]
                                (and (= (get v :data-type) "string") (< (get v :distinct) 50)))
                        (get @(ut/tracked-subscribe [::conn/sql-merged-metadata [query-key]])
                             :fields))))
        common-fields (cond table?   (vec (cset/intersection (set query-fields)
                                                             (set (get drag-body :table-fields))))
                            clicked? (vec (cset/intersection (set (vals clicked-fields))
                                                             (set query-fields)))
                            :else    [])
        union? (and (= (sort query-fields) (sort (get drag-body :table-fields)))
                    (ut/ne? query-fields))
        partial-union? (and (>= (count (cset/intersection (set query-fields)
                                                          (set (get drag-body :table-fields))))
                                1) ;; do they share at least 2 field names?
                            (ut/ne? query-fields))
        ...
        multi-param? (and (or (= drag-type :param) (= drag-type :where))
              ...
              (remove nil?
                (conj
                  ...
                  (when (and not-view? agg? self? (> (count pivot-dim-query-fields) 0))
                    (partition-all 2
                                   (for [f pivot-dim-query-fields]
                                     (keyword (str "pivot-on/" (ut/safe-name f))))))
                  (when (and not-view? not-sys? view-based?) [[]])
                  (when (and not-view? not-sys? self? (not table?))
                    [[:remove] [:order-by-asc :order-by-desc]])
                  ...
                  (when (and not-view? not-sys? multi-param? (not self?) (not table?))
                    [[:is-in :not-in]])
                  (when
                    (and not-view? not-sys? table? common-fields? (not self?) same-db? (not union?))
                    (let [poss-joins (vec (take 10
                                                (for [f common-fields]
                                                  (keyword (str "join-on/" (ut/safe-name f))))))
        ...
        action-typer (try (keyword (get-in dbody [:drag-meta :data-type]))
                          (catch :default _ :error!)) ;; (get-in
        ...
        block-ops (into (if (>= w 10)
                          (if view-drag?
                            [[:move-into :copy-into :reference]]
                            [[:move-into :copy-into :row-count]])
                          (if view-drag?
                            [[:move-into] [:copy-into] [:reference]]
                            [[:move-into] [:copy-into] [:row-count]]))
          ...
          (cond
            ...
            (and ;(>= w 11) (>= h 6)
              ...
              (merge (if (and not-view? not-sys? (not self?) agg? field?)
                       {:new-query
                          (if (<= w 9) [[:sum] [:avg] [:min] [:max]] [[:sum :avg] [:min :max]])}
                       {})
            ...
            (and not-sys? view-based? (not block-panel?))
              (merge (if (and (not self?) block-panel?)
                       {:block-ops block-ops}
                       (if big-enough-for-viz? {:new-viz viz-options} {}))
      ...
      (let [hov @dyn-dropper-hover]
        [re-com/h-box :attr
         {:on-drag-over  #(when (not @on-block?) (reset! on-block? true))
          :on-drag-leave #(reset! on-block? false)} :gap "5px" :size "auto" :width
         (if view-based? (px (- width-int 10)) (px (+ 20 width-int))) :height (px (- height-int 50))
         :style {:position "fixed" :font-family (theme-pull :theme/base-font nil) :z-index 100}
         :children
           ...
           [re-com/v-box :gap "5px" :size "auto" :children
            [;[re-com/box :child (str "you are: " source-table " I am: " table-source)]
             ...
             [(if (= tname :view-opts) re-com/h-box re-com/v-box) :gap "5px" :style
              ...
              (when (= tname :view-opts) (px (+ width-int 50))) :size
              (if (= tname :view-opts) "none" "auto") :children
              (for [opts toptions]
                (cond
                  (= tname :layout-map) ;; fancy drop targets for layoutmap
                    (let [pw-int (/ width-int brick-size)
                          ...
                          processed-layout
                            (lay/process-layout (get (first layout-map) :panels) pw-int ph-int)
                          brix (fn [x & [mod]]
                                 (px (if mod (+ (* brick-size x) mod) (* brick-size x))))
                          boxes
                            (vec
                              (for [[view-ref p] processed-layout]
                                (let [h (get p :h 4)
                                      ...
                                      box-map {:child (str view-ref)
                                               ...
                                               :justify :center
                                               :attr
                                                 {;:on-drag-enter #(reset! dyn-dropper-hover
                                                  :on-drag-over  #(when (not (= @dyn-dropper-hover
                                                                                [panel-key
                                                                                 view-ref]))
                                                                    (reset! dyn-dropper-hover
                                                                      [panel-key view-ref]))
                                               ...
                                               :style
                                                 (merge
                                                   {;:border "2px dashed yellow"
                                                    ...
                                                    :overflow "hidden"
                                                    :color (if (= [panel-key view-ref] hov)
                                                             "red"
                                                             "yellow")
                                                    :background-color
                                                      (if (= [panel-key view-ref] hov)
                                                   ...
                                                   pstyle)}
                                      leftovers
                                        (dissoc p :style :h :w :root :k :z :view-ref :hidden? :id)]
                                  (when root-visible?
                                    (droppable-pills
                                      ["meta-menu"]
                  ...
                  (= tname :view-opts)
                    (do
                      (ut/tapp>> [:view-opts-footer opts toptions])
                       ...
                       (for [o opts]
                         [re-com/box :child
                          (droppable-pills
                            ["meta-menu"]
                            {:clause       (if (= tname :layout-map)
                                             {:layout-drop o :kp (last layout-map)}
                                             o)
                             :drag-body    drag-body
                             :target-panel panel-key
                             :target-query query-key}
                            [re-com/box :size "none" :width
                             (px (/ (- width-int 13) (count toptions))) :align :center :justify
                             :center :attr
                             {;:on-drag-enter #(reset! dyn-dropper-hover [panel-key o])
                              ...
                              :on-click      #(reset! dragging? false)
                              :on-drag-leave #(reset! dyn-dropper-hover nil)} :child (str o)])
                          ...
                          {:border           (str "2px "
                                                  ...
                                                  (if (= tname :new-viz22) "77" ""))
                           :font-weight      700
                           :font-size        (font-size-px (str o " ")
                                                           (/ h (+ 1 (count toptions)))
                                                           (/ w 3))
                           :filter           (str "drop-shadow(0.35rem 0.35rem 0.4rem " (get
                                                                                          color-map
                                                                                          o)
                                                  55                                    " )")
                           :color            (if (= [panel-key o] hov) "#000000" (get color-map o))
                           :background-color (if (= [panel-key o] hov)
                                               (str (get color-map o) 55)
                                               "#00000055")
                           :font-family      (theme-pull :theme/base-font nil)} :size "auto"])])
                  :else
                    [re-com/h-box :gap "5px" :size "auto" :children
                     (for [o opts]
                       [re-com/box :child
                        (droppable-pills
                          ["meta-menu"]
                          ...
                          [re-com/box :size "auto" :attr
                           {;:on-drag-enter #(reset! dyn-dropper-hover [panel-key o])
                            :on-drag-over  #(when (not (= @dyn-dropper-hover [panel-key o]))
                                              (reset! dyn-dropper-hover [panel-key o]))
                            :on-click      #(reset! dragging? false)
                            :on-drag-leave #(reset! dyn-dropper-hover nil)} :child (str o)]) :align
                        ...
                        {:border           (str "2px "
                         ...
                         :font-size        (font-size-px (str o " ")
                                                         (/ h (+ 1 (count toptions)))
                                                         (/ w 3))
                         :filter           (str "drop-shadow(0.35rem 0.35rem 0.4rem " (get color-map
                                                                                           o)
                                                55                                    " )")
                         :color            (if (= [panel-key o] hov) "#000000" (get color-map o))
                         :background-color (if (= [panel-key o] hov)
                                             (str (get color-map o) 55)
                                             "#00000011")
                         :font-family      (theme-pull :theme/base-font nil)} :size
                        "auto"])]))]]])]))))
             ...
             (let [query     (-> v
                   ...
                   query     (if (not (empty? data-key))
                               (ut/postwalk-replacer
                                 {{:data data-key} {:select [:*] :from [(str k "/data-entry")]}}
                                 query)
                               query)
...
(defn formula-code-box
  ...
  (let [;sql-hint? (cstr/includes? (str value) ":::sql-string")
    ...
    [re-com/box :size "auto" :width (px (- width-int 24)) :height (px (- height-int 24)) :style
     ...
     [(reagent/adapt-react-class cm/UnControlled)
      {:value   (if (= type :honey)
                  (ut/format-map (- width-int -40) (str honey-sql))
                  ...
                  (fn [db [_ panel-key query-key]]
                    (into {}
                          (for [[[cols name] logic-map]
                                  (get-in db [:sql-source query-key :style-rules])] ;; moved to fit
                                                                                    ;; non-panel
  ...
  (fn [db [_ panel-key query-key field val styler-keys scheme depth direction]]
    (let [;other-vals (vec (distinct (for [r (get-in db [:data query-key])] (get r field))))
          ...
          local-vals   (remove nil?
                         (for [r (get-in db [:data query-key])]
                           (when (= (get r styler-field) 1) (get r field))))
                  ...
                  (fn [db [_ panel-key query-key]]
                    (when (not (nil? (get db :selected-cols)))
                      (let [c             (get db :selected-cols)
                            ...
                            sel-query-key (safe-nth c 1)
                            sel-field     (safe-nth c 2)]
                        (when (and (= panel-key sel-panel-key) (= sel-query-key query-key))
                          sel-field)))))
...
(re-frame/reg-sub ::custom-col-widths
                  (fn [db [_ panel-key query-key]]
                    (get-in db
                            [:panels panel-key :queries query-key :col-widths]
                            (get-in db [:sql-source query-key :col-widths] {})) ;; custom queries
                                                                                ;; that have no
                  ))
...
(defn clear-preview2-recos
  []
  (dorun (let [prev-preview-keys (for [k @(ut/tracked-subscribe [::preview-keys2])]
                                   (keyword (str "reco-preview" k)))]
...
(defn mad-libs-shapes
  ...
  (let [;all-sql-call-keys @(ut/tracked-subscribe [::all-sql-call-keys])
        ...
        sql-params (merge (into {}
                                (for [k [:viz-shapes0-sys2/shape :viz-shapes-sys2/combo_edn
                                         :user-dropdown-sys2/req-field]]
        ...
        sql-calls
          {:viz-tables-sys2  {:select   [:table_name [[:count 1] :recos]]
                              :from     [:viz_recos_vw]
                              :where    [:and [:not [:like :table_name "query_preview%"]]
                                         [:= :table_name query-id]] ;; changed to single from
                              :order-by [:table_name]
                              :group-by [:table_name]}
           ...
           :viz-shapes-sys2  {:select   [:combo_edn [[:count 1] :recos]]
                              ...
                              :where    [:and
                                         ...
                                         (when req-field-picked?
                                           [:like :combo_edn
                                            (str "%"
                                                 (get sql-params :user-dropdown-sys/req-field)
                                                 "%")])
           ...
           :recos-sys2       (if (not @mad-libs-top?)
                               ...
                               {:select   [:*]
                                :from     (if (or combo-picked? shape-picked?)
                                            [:viz_recos_vw]
                                            [:viz_recos_vw2])
                                :order-by [[:score :desc]]
                                :where    [:and [:= :table_name query-id]
        ...
        preview-maps (into {}
                           (for [{:keys [combo_hash shape_name combo_edn query_map score w h
                                         selected_view viz_map condis]}
                                   full-recos]
          ...
          (fn [preview-maps pk pnum]
            (try
              (let [panel-key (nth pk pnum)
                    ...
                    color-keys-which
                      (last (reverse (sort-by name (keys (get ut/colorbrewer (first color-keys))))))
                    colors1 (vec (if (= (last color-keys) :desc)
                                   (reverse (get-in ut/colorbrewer
                                                    [(first color-keys) color-keys-which]))
                    ...
                    value-to-color (fn [value-vector color-vector value]
                                     (let [min-val    (apply min value-vector)
                                           ...
                                           idx        (int (Math/floor (* normalized
                                                                          (dec (count
                                                                                 color-vector)))))]
                                       (if (and normalized (>= idx 0) (< idx (count color-vector)))
                                         (color-vector idx)
                                         nil)))
                    selected-view (try (edn/read-string (get-in preview-maps
                                                                [panel-key :selected_view]))
                                       (catch :default _ nil))
                    rounder (fn [num] (/ (js/Math.round (* num 100)) 100))
                    combo_edn (try (when (not (nil? ce)) (ut/replacer (str ce) #"\"" ""))
                                   (catch :default _ "")) ;; TODO, why bombing?
                    ...
                    body [re-com/v-box :size "none" :width (px (* ww brick-size)) :height
                          (px (* hh brick-size)) :style
                          {:zoom         0.44
                           :padding-left "10px"
                          ...
                          [[re-com/h-box :justify :between :children
                            [[re-com/h-box :gap "5px" :style {:padding-left "3px"} :children
                             ...
                             [re-com/box :padding "6px" :style
                              {:font-size "20px" :opacity 0.3 :font-weight 700} :child
                              (str (rounder score))]]]
                ...
                [re-com/box :size "auto" :child
                 (draggable (sql-spawner-meta panel-key) "meta-menu" body)])
              (catch :default _
                [re-com/box :height
                 (px (* (js/Math.floor (* (/ (/ height-int charts-high) brick-size) h-multi))
                        brick-size)) :width
                 (px (* (js/Math.floor (* (/ (/ width-int charts-wide) brick-size) w-multi))
                        brick-size)) :size "auto" :align :center :justify :center :style
    ...
    (ut/tapp>> [:full-recos recos-page preview-maps preview-keys query-id full-recos
                (select-keys preview-maps preview-keys)])
    (dorun
      (let [prev-preview-keys (for [k @(ut/tracked-subscribe [::preview-keys2])]
                                (keyword (str "reco-preview" k)))
            grid-page         @(ut/tracked-subscribe [::recos-page2])
            grid-page         (if (> (- grid-page 1) pages) 0 grid-page)
            recos             (take per-page
                                    (drop (* grid-page per-page)
                                          @(ut/tracked-subscribe [::conn/sql-data [:recos-sys2]])))
        ...
        (doseq [{:keys [combo_hash query_map viz_map condis combo_edn shape_name]} recos]
          (insert-hidden-reco-preview combo_hash
                                      viz_map
                                      query_map
                                      condis
                                      combo_edn
             ...
             (for [h (range charts-high)]
               [re-com/h-box :size "none" :children
                (for [w (range charts-wide)]
                  [rc/catch
                   [preview-container preview-maps preview-keys (+ (* h charts-wide) w)]])])]
            [re-com/v-box :size "auto" :height (px height-int) ;"380px"
             :style {:border-top "1px solid #9973e066" :overflow "hidden"} :children
             [[re-com/box :child
               [re-com/md-icon-button :md-icon-name "zmdi-trending-up" :on-click
                ...
                {:font-size  "22px"
                 ...
                 :opacity    0.5
                 :padding    "0px"
                 :margin-top "-1px"}] :height "20px" :style
               {:font-size        "12px"
              ...
              [re-com/v-box :children
               (for [c (range pages)]
                 [re-com/box :child (str (+ 1 c)) ; (if (= c recos-page) (str (+ 1 c)) "..")
                  :height "19px" :align :end :justify :center :attr
                  {:on-click #(ut/tracked-dispatch [::set-recos-page2 c])} :style
                  {:padding-right    "4px"
                   :cursor           "pointer"
                   :font-size        "10px"
                   ...
                   :background-color (if (= c recos-page)
                                       "#9973e0" ;"darkcyan"
                                       "inherit")
                   :border-bottom    "1px dashed #9973e066"}])]] :width "30px"]]])]]]]))
...
(re-frame/reg-sub
  ...
  (fn [db [_ query-key1 query-key2]]
    (let [;queries (into {} (for [p (get-in db [:panels])] (get p :queries)))
          qgb1 (vec
                 (for [[f fv] (get-in db [:meta query-key1 :fields]) :when (get fv :group-by?)] f))
          qgb2 (vec
                 (for [[f fv] (get-in db [:meta query-key2 :fields]) :when (get fv :group-by?)] f))]
      (ut/tapp>> [qgb1 qgb2])
      [])))
                  ...
                  (fn [db [_ query-key1]]
                    (let [;queries (into {} (for [p (get-in db [:panels])] (get p :queries)))
                          qgb1 (vec (for [[f fv] (get-in db [:meta query-key1 :fields])
                                          :when  (get fv :group-by?)]
                                      f))]
                      qgb1)))

(re-frame/reg-sub ::query-waitings
                  (fn [_ [_ query-key]]
                    (let [;; messages (map :message @(ut/tracked-subscribe [::wfx/pending-requests
                          waitings    (true? (some #(= % query-key)
                                                   (for [e @(rfa/sub ::http/pending-requests
                                                                     {:socket-id http/socket-id})]
    ...
    (let [query           (get-in db [:panels panel-key :queries query-key])
          ...
          heat-keys       (into {}
                                (first (for [[[k1 k2] v] style-rules
                                             :when       (and (cstr/starts-with? (str k2)
                                                                                 ":heatmap-")
                                                              (= k1 column))]
                                         {[k1 k2] v})))
  ...
  (fn [db [_ type]] ;; TODO add "who is asking" deets like panel, query, so we can do kit WHEN
    (let [all  (get-in db [:server :settings :kits])
          ...
          kits (vec (sort-by :name
                             (for [[k v] all
                                   :when (and (some #(= [(get v :package-name) (get v :kit-name)] %)
                                                    curr) ;; is enabled in params?
                                              (= (get v :run-on) type))]
                               (merge {:kit-name k} v))))]
      kits)))
...
(re-frame/reg-sub ::market-kits
                  (fn [_ _] ;; TODO add "who is asking" deets like panel, query, so we can do kit
                            ;; WHEN
                    [{:kit-name     :cool-thing1
                      :name         "Todd Howard"
                      :description  "community repo placeholder1"
                      :package-name :cool-package1}
                     {:kit-name     :sandwiches!
                      :name         "Sandwiches!"
                      :description  "sandwiches!"
                      :package-name :skyrim-sandwiches}
                     {:kit-name     :cool-thing3
                      :name         "Horse Armour"
                      :description  "community repo placeholder3"
                      :package-name :cool-package1}
                     {:kit-name     :cool-thing4
                      :name         "Gnomes"
                      :description  "community repo placeholder4"
                      :package-name :cool-package1}]))
...
(defn magic-table
  ...
  (let [viewer-sourced? (try (and (cstr/starts-with? (first data-keypath) "::")
                                  (string? (first data-keypath)))
                             (catch :default _ false))
        data-keypath (if viewer-sourced?
                       (try [(edn/read-string (ut/replacer (first data-keypath) "::" ":"))]
                            (catch :default _ nil))
        ...
        {:keys [row-height render-all? clicked-row-height]} @(ut/tracked-subscribe
                                                               [::custom-rows panel-key query-key])
        width-int (- (* (if ww ww w) brick-size) 31)
        modded-width (- width-int 8)
        height-int (* (if hh hh h) brick-size)
        ...
        selected-field-code (get-in @(rfa/sub ::panel-sql-calls {:panel-key selected-block})
        ...
        curr-row-str
          (str curr-row-start " - " (if (> full-rowcount curr-row-end) curr-row-end full-rowcount))
        ttl-pages (nf (js/Math.ceil (/ (or full-rowcount rowcount) rows-per-page)))
        rowcount-string
          (cond (= rowcount 1) "1 row"
                (= page-num -1) (str (nf rowcount) " rows (limit off!)")
                (and (or (> full-rowcount rows-per-page) (> rowcount rows-per-page))
                     (nil? full-rowcount))
                  (str curr-row-str " of " (nf rowcount) "+ rows")
                (and (or (> full-rowcount rows-per-page) (< rowcount rows-per-page))
                     (nil? full-rowcount))
                  (str (nf rowcount) " rows")
                :else (str (if (> full-rowcount rows-per-page) (str curr-row-str " of ") "")
                           (nf full-rowcount)
                           " rows"))
        ...
        has-rabbit-code? (true? (some (fn [x] (= x "rabbit-code"))
                                      (ut/deep-flatten (get metadata :fields))))
        clicked-rabbit-code-multiplier 4
        clicked-row-height (if (not (nil? clicked-row-height))
                             clicked-row-height
                             (* row-height clicked-rabbit-code-multiplier))
        ...
        default-col-widths @(rfa/sub ::column-default-widths
                                     {:panel-key panel-key :query-key query-key})
        running? single-wait?
    ...
    (if (seq @rowset) ;(and (seq @rowset) (not running?))
      (if (and callout-instead? (nil? hh))
        (let [label         (nth fields 0)
          ...
          [re-com/v-box :size "auto" :style {:overflow "hidden"} :align :center :justify :center
           :class (when (and (= panel-key (last @hover-field)) @animate?) "zoom-and-fade2")
           :children
           [(when (and overlay? (not mad-libs?))
              @(ut/tracked-subscribe [::dynamic-drop-targets table-source @dragging-body panel-key
            ...
            [re-com/box :align :center :justify :center :child formatted-val :attr
             (if agg?
               {:on-double-click
                  (fn [_]
                    (let [query-key  (keyword (str
                                                (ut/replacer (str query-key) #":" "")
                                                "->-"
                          ...
                          new-query  (cond is-sub? (try (sql-alias-replace-sub base-table)
                                                        (catch :default _
                                                          {:select
                                                             [["error in sql-alias-replace-sub1"
                                                               :err]]}))
                                           (map? base-table)
                                             (try (sql-alias-replace base-table)
                                                  (catch :default _
                                                    {:select [["error in sql-alias-replace-sub2"
                                                               :err]]}))
        ...
        [re-com/v-box :style {:margin-top "-8px"} :children
         [(when (and overlay? (not mad-libs?))
          ...
          (when (and overlay? mad-libs? (not non-panel?)) ;;; MAD LIBS
            [re-com/box :child [mad-libs-shapes query-key width-int height-int] :size "none" :align
             :center :justify :center :height (px (- height-int 40)) :width (px width-int) :style
          ...
          [re-com.core/simple-v-table :columns
           (vec
             (for [c fields]
               (let [cwidth     (cond (cstr/ends-with? (str query-key) "-sys") ;; TODO, convert to
                                        (cond (some #(= % c)
                                                    [:queries :fields :views :blocks :recos
                                                     :combo_hash :schema_cat])
                                                66
                                              (= c :database_name) 115
                                              (= c :description) 550
                                              (and (= c :name)
                                                   (cstr/includes? (str query-key) "-fn-"))
                                                145
                                              (= c :shape_name) 180
                                              (= c :block_key) 88
                                              (= c :items) 70
                                      ...
                                      (and (some #(= % c) (keys custom-col-widths)))
                                        (get custom-col-widths c) ;; :col-wdiths keys exists in
                                      :else equal-width-final)
                 ...
                 {:id c
                  ...
                  :color (get @(ut/tracked-subscribe [::conn/data-colors])
                              (get-in metadata [:fields c :data-type]))
                  :row-label-fn
                    #(if (= % clicked-row)
                       (let [row-hash   (str (hash %) c)
                             row-height (if has-rabbit-code? clicked-row-height row-height)
                             row-num    (row-num-fn %)
                             hh         (js/Math.floor (/ row-height brick-size))
                             ww         (js/Math.floor (/ cwidth brick-size))]
                         (cond
                           (= panel-key :system-tables-list*)
                             ...
                             (draggable
                               ...
                               (let [rowname (str "query-preview-block" row-hash)
                                 ...
                                 (when (and (= (get-in % [c 0]) :*render*)
                                            (empty? @(ut/tracked-subscribe [::panel-map
                                                                            (keyword rowname)])))
                                   (ut/tracked-dispatch
                                     [::update-workspace [(keyword rowname)]
                                      (ut/postwalk-replacer
                                        ...
                                        (let [render-code (get-in % [c 1])
                                              is-map?     (and (map? render-code)
                                                               (contains? render-code :view))
                                              base        {:h hh
                                                           :w ww
                                              ...
                                              body        (if is-map?
                                                            (-> base
                                                                (assoc-in [:views :a-row]
                                                                          (get render-code :view))
                                                                (assoc-in [:queries]
                                                                          (get render-code
                                                                               :queries)))
                                                            base)]
                                 ...
                                 [re-com/box :size "auto" :height (px (+ row-height 20)) ;"33px"
                                  ...
                                  (if (= (get-in % [c 0]) :*render*)
                                    [re-com/box :size "none" :width (px cwidth) :height
                                     (px row-height) :child [honeycomb (keyword rowname) :a-row]]
                                    [shape/map-boxes2 (get-in % [c 1]) :inline-render :inline-render
                                     [] nil (if (vector? (get-in % [c 1])) "vector" "map")])]))
                           :else
                             (draggable
                               (sql-spawner-where :where panel-map data-keypath c panel-key row-num)
                               "meta-menu"
                               [re-com/box :size "none" :attr
                                {;:on-mouse-enter (fn [_] (when (and (not (= @hover-field [c
                                 :on-mouse-over  (fn [_]
                                                   (when (and (not (= @hover-field [c panel-key]))
                                                              hover-field-enable?)
                                ...
                                (when (and (= panel-key (last @hover-field)) @animate?)
                                  "zoom-and-fade") :style {:color "inherit"} :child ;(str (get % c))
                                          ...
                                          body        (if is-map?
                                                        (-> base
                                                            (assoc-in [:views :a-row]
                                                                      (get render-code :view))
                                                            (assoc-in [:queries]
                                                                      (get render-code :queries)))
                                                        base)]
                                      body))]))
                             [re-com/box :size "auto" :height (px (+ row-height 20)) ;"33px" ;"30px"
                              :align :start :justify :start :style {:zoom 0.65 :padding-top "0px"}
                              :child
                              (if (= (get-in % [c 0]) :*render*)
                                [re-com/box :size "none" :width (px cwidth) :height (px row-height)
                                 :child [honeycomb (keyword rowname) :a-row]]
                                [shape/map-boxes2 (get-in % [c 1]) :inline-render :inline-render []
                                 nil (if (vector? (get-in % [c 1])) "vector" "map")])])
                         (nil? (get % c))
                           [re-com/box :child "NULL" :attr
                            {:on-context-menu
                               (fn [_]
                                 (ut/tracked-dispatch
                            ...
                            {:on-context-menu
                               (fn [_]
                                 (ut/tracked-dispatch
                                   [::conn/cell-click-parameter
                                    [(keyword (str (ut/replacer (str query-key) #":" "") ".*"))
                                     (keyword (ut/replacer (str c) #":" ""))] (get % c)]))} :child
                            (str (nf (get % c)))]
                                             ...
                                             (try (sql-alias-replace base-table)
                                                  (catch :default _
                                                    {:select [["error in sql-alias-replace-sub2"
                                                               :err]]}))
                                           :else {:select [:*] :from table-source})
                     new-query       (assoc new-query
                                       :where [:*when clicked-keyword
           ...
           (fn [row {:keys [id] :as column}]
             (let [;this-row (.indexOf @rowset row)
                   ...
                   sel?             (and selected?
                                         @(ut/tracked-subscribe [::column-selected? panel-key
                                                                 query-key id]))
                                            ...
                                            logic-kps (into {}
                                                            (for [[_ v] (into {}
                                                              ...
                                                              (let [[_ scheme depth direction] v]
                                                                {v @(ut/tracked-subscribe
                                                                      [::heatmap-color-val panel-key
                                                                       query-key id (get row id)
                                                                       heatmap-styles scheme depth
                                                                       direction])})))]
                                        (ut/postwalk-replacer logic-kps obody)))
                   styles           (apply merge
                                      (for [s styler-keys]
                   ...
                   data-type-color  (get @(ut/tracked-subscribe [::conn/data-colors])
                                         (get-in metadata [:fields id :data-type]))
                   agg?             (and (not non-panel?)
                                         (not (get-in metadata [:fields id :group-by?])))
               ...
               (merge (cond (or (= row clicked-row)
                                (and (= panel-key :recos-list*) ;; special case with viz browser
                                     (= (get row :combo_hash)
                                        @(rfa/sub ::conn/clicked-parameter-key-alpha
                                                  {:keypath [:recos-sys/combo_hash]}))))
                              (if has-rabbit-code?
                              ...
                              (let [upstream? (some (fn [x] (= x id)) child-cols)
                                ...
                                (merge
                                  {:background-color (if upstream?
                                                       (str fcolor (if deeper-upstream? 10 20))
                                                       "#00000000")
                                   :padding          "3px"
                                   :margin           "0px"}
                                  (if upstream?
                                    {;:border (str "1px solid " (str fcolor (if deeper-upstream? 15
                                     ...
                                     :border-left  (str "1px solid "
                                                        (str fcolor (if deeper-upstream? 15 39)))
           ...
           :max-rows max-rows :max-width (px (+ 25 width-int)) :table-row-line-color
           (str text-color 11) ;;"#ffffff11"
           :row-style
           (fn [row {:keys [id] :as column}]
             (let [;this-row (.indexOf @rowset row)
                   poss-stylers (filter #(cstr/starts-with? (str %) ":styler_") (keys row))
               ...
               (cond (and (= row clicked-row) has-rabbit-code?) {;:height "299px" :overflow "auto"
                     ...
                     :else {:padding "0px"}))) :table-padding 5 :fixed-column-border-color
           "#00000011" :column-header-height 39 :column-header-renderer
             ...
             [re-com/h-box :children
              (for [c cols]
                (let [w          (get c :width)
                      ...
                      typ        (get c :data-type)
                      sel?       (and selected?
                                      @(ut/tracked-subscribe [::column-selected? panel-key query-key
                                                              id]))
                      label      (get c :header-label)]
                  [re-com/box :attr
                   (if selected?
                     {:on-click #(ut/tracked-dispatch [::column-select panel-key query-key
                                                       (if sel? nil id)])}
                     {}) :child
                   (draggable
                     (sql-spawner :field panel-map data-keypath id panel-key)
                     "meta-menu"
                     [re-com/v-box :justify :center :height "39px" :style
                      (merge {;:color "#000000" ;(str color 99)
                              ...
                              :font-weight      700}
                             (if sel? (theme-pull :theme/grid-selected-column-css nil) {})) :padding
                      "5px" :children
                      [[re-com/box :child
                       ...
                       [re-com/h-box :style {:font-size "9px" :font-weight 700 :opacity 0.9} :gap
                        "5px" :children
          ...
          [re-com/h-box :padding "4px" :justify :between ;:align :center
           :children
           [;; panel-code-box [panel-id key width-int height-int value]
            (if (and (= @formula-bar data-keypath) selected? (not non-panel?))
              (let [block-h        (or hh h)
                ...
                (ut/tapp>> [:formula-bar-open? panel-key query-key block-h block-w column-selected
                            selected-field-idx ff])
                [re-com/h-box :size "auto" :style
                 {:filter (if overlay? "blur(3px) opacity(88)" "none") :margin-top "12px"} :justify
                 :between :children
                 [[re-com/v-box :size "auto" :children
                   [;; [re-com/box :height "15px"
                    [field-code-box panel-key query-key selected-field-idx label-px-width 55
                     selected-field-code]]]]])
              (let [;; waitings (filter #(and (> (count (get % :ui-keypath)) 1)
                ...
                [re-com/h-box :size "none" :height "10px" :padding "3px" :style
                 ...
                 [[re-com/h-box :gap "5px" :children
                   [[re-com/box :child rowcount-string] [re-com/gap :size "0px"] ;; the 2 gaps are
                                                                                 ;; enough
                    ...
                    (when (and (> full-rowcount 5) ;; TODO, gets messed up sometimes?
                      ...
                      [re-com/md-icon-button :md-icon-name "zmdi-toll" :on-click
                       #(ut/tracked-dispatch [::conn/run-sql-deep-meta-for panel-key query-key
                                              (sql-alias-replace-sub query)]) :attr
                       (when hover-field-enable?
                         {:on-mouse-enter
                          ...
                          :on-mouse-leave #(swap! db/context-box dissoc query-key)}) :class
                       (when waits? "rotate linear infinite") :style
                       {:font-size        "15px"
                        :cursor           "pointer"
                        :transform-origin "7.5px 11px"
                        :opacity          0.5
                  ...
                  (when (and server-kits (not non-panel?)) ;; (not non-panel?)
                    [re-com/h-box :gap "5px" :children
                     (vec
                       (cons [re-com/gap :size "6px"]
                             (vec
                               (for [{:keys [description kit-name name icon]} server-kits
                                     :let
                                       [wait?   @(ut/tracked-subscribe [::reco-running? query-key
                                                                        kit-name])
                                        queued? @(ut/tracked-subscribe [::reco-queued? query-key
                                                                        kit-name])]]
                                 [re-com/md-icon-button :md-icon-name icon :on-click
                                  ...
                                  (when hover-field-enable?
                                    {:on-mouse-enter #(swap! db/context-box assoc
                                                        query-key
                                                        (str "kit-fn: " name " - " description))
                                     :on-mouse-leave #(swap! db/context-box dissoc query-key)})
                                  :class
                                  (cond wait?   "rotate linear infinite"
            ...
            [re-com/h-box :children
             [(when col-selected?
                (let [;heat-simple (into {} (for [[[k1 _] v] (get-in query [:style-rules])] {k1
                      ...
                      has-one?  (not (empty? heat-keys))]
                  [re-com/h-box :size "none" :height "10px" :padding "3px" :style
                   {:margin-right "10px" :font-size "12px"} :gap "5px" :children
                   [[re-com/md-icon-button :md-icon-name "zmdi-fire" :on-click
                     #(do (ut/tracked-dispatch [::toggle-heatmap panel-key query-key
                                                column-selected])
                          (ut/tracked-dispatch [::column-select panel-key query-key nil])) :attr
                     (when hover-field-enable?
                       {:on-mouse-enter #(swap! db/context-box assoc
                                           query-key
                                           "toggle heatmap on this column")
                     ...
                     {:font-size        "15px"
                      ...
                      :padding          "0px"
                      :margin-top       "-3px"}]
                    [re-com/md-icon-button :md-icon-name "zmdi-delete" ;; panel-key query-key clause
                                                                       ;; drop-data
                     :on-click
                ...
                [re-com/h-box :style {:filter (if overlay? "blur(3px) opacity(88)" "none")}
                 ...
                 [(when (and has-pages? (not first-page?))
                    [re-com/md-icon-button :md-icon-name "zmdi-caret-left" :on-click
                     #(do (ut/tracked-dispatch [::change-page panel-key query-key (- page-num 1)
                                                ttl-pages])
                     ...
                     {:font-size  "22px"
                      ...
                      :margin-top "-1px"}])
                  (when has-pages?
                    [re-com/box :align :center :justify :center :style {:font-size "10px"} :child
                     (str page-num " / " ttl-pages)])
                  (if (and has-pages? (not last-page?))
                    [re-com/md-icon-button :md-icon-name "zmdi-caret-right" :on-click
                     ...
                     {:font-size  "22px"
                      ...
                      :padding    "0px"
                      :margin-top "-1px"}]
                    [re-com/box :child " " :width "22px"])]]) [re-com/gap :size "0px"]]]]]]]) ;; just
                                                                                              ;; in
                                                                                              ;; case
                                                                                              ;; so
                                                                                              ;; we
...
(re-frame/reg-sub ::sql-data-table-magic
                  (fn [_ [_ panel-key keypath]] [magic-table panel-key keypath]))

(re-frame/reg-sub ::sql-data-table-magic2
                  (fn [_ [_ panel-key keypath]] [re-com/box :child
                                                 [magic-table panel-key keypath]]))

(re-frame/reg-sub ::sql-data-table-editor
                  (fn [_ [_ panel-key keypath block-sizes block-width]] [magic-table panel-key
                                                                         keypath block-sizes
                                                                         block-width]))
...
(re-frame/reg-sub ::has-no-view?
                  (fn [db [_ panel-key]] (nil? (get-in db [:panels panel-key :views]))))

(re-frame/reg-sub
  ::dynamic-tab-selected
  (fn [db [_ panel-key]]
    (try
      (let [tab-res    (get-in db [:post-tab panel-key])
                               ...
                               {t (apply max
                                    (flatten (for [r (get-in tab-res [t])]
                                               (for [b (last r)] ;; WTF happened with this? no
                                                 (last (first b))))))}))
            dsc        (into (sorted-map-by (fn [key1 key2] (compare (key2 valids) (key1 valids))))
                             valids)
            winnah     (first (first dsc))
            query?     (some #(= winnah %) qys)
            full-alias (keyword (str (if query? "grid/" "view/")
                                     (ut/safe-name panel-key)
                                     "."
                                     (ut/safe-name winnah)))]
        (if query? [rc/catch [magic-table panel-key [winnah]]] full-alias))
      (catch :default e nil))))                          ;; just in case... TODO

(re-frame/reg-sub ::view
                  (fn [db [_ panel-key view-key]] (get-in db [:panels panel-key :views view-key])))
        ...
        (merge views ;(get-in db [:panels panel-key :views])
               (if oz-layers?
                 {:layered-viz (-> vega-lite-shell
                                   ...
                                   (assoc-in [1 :resolve] {:scale {:y "independent"}}))} ;; ex:
                                                                                         ;; "resolve":
                 {})
               {})
        views))))
...
(re-frame/reg-sub ::queries-reco-status
                  (fn [db [_ panel-key]]
                    (into {}
                          (for [[p _] (get-in db [:panels panel-key :queries] {})]
                            {p (get-in db [:status :reco p])}))))

(re-frame/reg-sub ::panel-views
                  (fn [db {:keys [panel-key]}]
                    (let [v (get-in db [:panels panel-key :views])] (if (vector? v) {:base v} v))))

(re-frame/reg-sub ::panel-view-keys
                  (fn [db {:keys [panel-key]}]
                    (let [v (get-in db [:panels panel-key :views])] (vec (keys v)))))
...
(re-frame/reg-sub ::selected-view
                  (fn [db [_ panel-key]]
                    (let [body    (get-in db [:panels panel-key :views])
                          ...
                          vq      (conj (conj (into (keys body) (keys queries)) :layered-viz)
                                        :dyn-tab)] ;; pseudo
...
(re-frame/reg-event-db ::select-view
                       (fn [db [_ panel-key view-key]]
...
(re-frame/reg-sub ::panel-connection-id
                  (fn [db [_ panel-key]] (get-in db [:panels panel-key :connection-id])))

(re-frame/reg-sub ::panel-sql-calls
                  (fn [db {:keys [panel-key]}]
                    (into {}
                          (for [[k v] (get-in db [:panels panel-key :queries])]
...
(re-frame/reg-sub ::panel-vsql-call-keys
                  (fn [db [_ panel-key]]
                    (keys (into {}
                                (for [[k v] (get-in db [:panels panel-key :queries])]
                                  (when (not (nil? (find v :vselect))) {k v}))))))
                                          ...
                                          (into {}
                                                (for [[kk _] v]
                                                  {(keyword (str (ut/safe-name k))) nil              ;; reffing
                                                   ...
                                                   (keyword
                                                     (str (ut/safe-name k) "/" (ut/safe-name kk)))
                                                     nil
                                                   (keyword (str (ut/safe-name k)
                                                                 "-parameter/"
                                                                 (ut/safe-name kk)))
                                                     nil})))))))
...
(re-frame/reg-sub ::all-vsql-calls
                  (fn [db]
                    (into {}
                          (for [[_ v] (get db :panels)]
                            (into {}
                  ...
                  (fn [db]
                    (vec (remove empty?
                           (for [[k v] (get-in db [:click-param])]
                             (into {}
                                   (for [[kk vv] v]
                                     {(keyword (str (ut/safe-name k) "/" (ut/safe-name kk)))
                                        vv})))))))

(re-frame/reg-sub
  ::all-click-params
  (fn [db]
    (let [all-condis {:condi (into {}
                                   (for [[_ v] (get db :panels)]
                                     (into {} (for [[kk vv] (get v :conditionals)] {kk vv}))))}
  ...
  (let [styles        (if (and (map? style) (not (nil? style))) style {})
    ...
    (vec (for [r data]
           [re-com/box :size "auto" :padding "4px" :attr
            (if clickable? {:on-click #(ut/tracked-dispatch [::click-parameter keypath r])} {})
            :child (str (vals r)) :style
            (merge styles
                   {:cursor           (if clickable? "pointer" "inherit")
                    :background-color (if (= r clicked-param) "grey" "inherit")})]))))
...
(defn sql-data-boxes-values2
  ...
  (let [styles        (if (and (map? style) (not (nil? style))) style {})
    ...
    (vec (for [r data]
           [re-com/box :size "auto" :padding "4px" :attr
            {:on-click #(ut/tracked-dispatch [::conn/click-parameter keypath r])} :child
            (str (vals r)) :style
            (merge styles
                   {:cursor           "pointer"
                    :background-color (if (= r clicked-param) "grey" "inherit")})]))))
...
(re-frame/reg-sub
  ...
  (fn [db [_ panel-key view-key]]
    (let [view                 (if (or (= view-key :base) (nil? view-key))
          ...
          layout-map-scrubbers @(ut/tracked-subscribe [::scrub/keypaths-of-layout-panels panel-key
                                                       view-key])
          kvpaths-vals         (into {}
                                     (for [p (sort (into layout-map-scrubbers kvpaths))]
                                       {p (let [v (get-in view p)]
                                            [v
                                             (try (scrub/get-type p v)
                  ...
                  (fn [db _]
                    (let [sched (get db :sched {})]
                      (into {}
                            (for [[k v] sched]
                              {(str [:run k])
                                 (- v (get-in db [:re-pollsive.core/polling :counter]))})))))
    ...
    (try (let [single-input? (if (nil? single-input?) false true)
               params        (get-in db
                                     (if single-input?
                                       ...
                                       [:flows (get db :selected-flow) :map bid :data :flow-item
                                        :defaults])
                                     {})
...
(re-frame/reg-sub ::valid-body-params-in
                  (fn [_ {:keys [body]}]
...
(re-frame/reg-sub ::valid-sql-params
                  (fn [db [_ panel-key]]
                    (vec (remove nil?
                           (map ut/process-key
                             (ut/deep-flatten (get-in db [:panels panel-key :queries])))))))
...
(re-frame/reg-sub ::layouts-for
                  (fn [db [_ panel-key view k]]
                    (let [;kvs (ut/kvpaths (get-in db [:panels panel-key :views view])) ;; get
                          ;nested layouts
          ...
          (into
            ...
            (for [k valid-params] ;; deref here?
              (let [deets      @(ut/tracked-subscribe [::layouts-for panel-key selected-view k])
                    body       @(ut/tracked-subscribe [::resolve-view-alias k (get deets :w)
                                                       (get deets :h)])
                    body       (if (nil? body)
                                 [re-com/box :align :center :justify :center :size "auto" :child
                                  [:div
                                   ...
                                   (str " I can't find this view. Halp.")] :width :width-px :height
                                  :height-px]
                                 body)
  ...
  (fn [db [_]]
    (let [scheds         (get db :sched)
          scheds-reverse (doall (into {}
                                      (for [vv (vals scheds)]
                                        {vv (vec (for [[k v] scheds :when (= v vv)] k))})))
...
(re-frame/reg-event-db ::insert-sql-source
...
(re-frame/reg-event-db ::prune-alert
                       (fn [db [_ alert-id]]
...
(re-frame/reg-event-db
  ...
  (fn [db _]
    ...
    (doseq [a [ut/replacer-data ut/replacer-cache ut/deep-flatten-data ut/deep-flatten-cache
               ut/clover-walk-singles-map ut/process-key-cache ut/process-key-tracker
               ut/compound-keys-cache ut/compound-keys-tracker ut/upstream-cache
               ut/upstream-cache-tracker ut/downstream-cache ut/downstream-cache-tracker
               ut/split-cache ut/split-cache-data ut/extract-patterns-data ut/extract-patterns-cache
               ut/clean-sql-atom ut/auto-font-atom ut/postwalk-replace-data-cache
               ut/postwalk-replace-cache ut/is-base64-atom ut/is-large-base64-atom
               ut/safe-name-cache ut/format-map-atom ut/body-set-atom ut/data-typer-atom
               ut/coord-cache]]
      (reset! a {}))
    db))
...
(re-frame/reg-event-db
  ...
  (fn [db _]
    (let [mem (get db :memory)
      ...
      (do (when (> (get-in atom-size-map [:ut/postwalk-replace-data-cache :mb]) 40)
...
(re-frame/reg-event-db
  ...
  (fn [db [_]]
    (let [ks1     (vec (apply concat
                         (for [k (keys (get-in db [:panels]))]
                           (keys (get-in db [:panels k :queries])))))
          ...
          run-it? (or (and (nil? @mad-libs-view)
      ...
      (if run-it?
        (-> db
            ...
            (assoc :panels (select-keys (get db :panels)
                                        (filter #(not (cstr/starts-with? (str %) ":query-preview"))
                                          (keys (get db :panels)))))
            ...
            (assoc :query-history (select-keys (get db :query-history)
                                               (filter #(not (cstr/ends-with? (str %) "-sys*"))
                                                 keepers)))
  ...
  (fn [db [_ panel-id [combo-hash table-name]]]
    (let [existing    (get-in db [:data :mad-libs-viz panel-id table-name])
          running?    (fn [kp]
                        (some ;#(= (first %) panel-id)
                          #(= % kp)
                          (for [e @(rfa/sub ::http/pending-requests {:socket-id http/socket-id})]
      ...
      (when (and (empty? (get-in db [:data :viz-shapes])) (not (running? [:viz-shapes])))
        (conn/sql-data [:viz-shapes]
                       {:select [:axes_logic :base_score :connection_id :library_shapes
                                 :selected_view :run_id :shape_name :sql_maps]
                        ...
                        :where  [:and
                                 [:in :context_hash ;; self sub-q
                                  {:select [:context_hash]
                                   ...
                                   :where  [:and [:= :combo_hash combo-hash]]} :tete]
                                 [:in :shape_name ;; self sub-q
                                  {:select [:shape_name]
                                   :from   [[:combo_rows :uu2434aa]]
                                   :where  [:and [:= :combo_hash combo-hash]]} :tete2]]
                        :from   [[:combo_rows :uu2434aa]]}))
  ...
  (fn [db [_ panel-id src-table-id-str combo-hash]]
    (let [rows          (get-in db [:data :mad-libs-viz panel-id src-table-id-str])
      ...
      {:shape_name    shape-name
       ...
       :data          (group-by (juxt :talias :field_name :walk1 :walk2 :walk3 :walka :axes)
                                rows)})))
...
(re-frame/reg-sub
  ...
  (fn [db [_ panel-id src-table-id-str shape-name axes-map]]
    (let [;hm (str (hash axes-map))
          ...
          lk-sql   {:select [:*]
                    :from   [:combos]
                    :where  [:and [:= :key_hashes (pr-str axes-map)]
                             [:= :table_name src-table-id-str] [:= :shape_name shape-name]]}]
      ...
      (if (not exists?)
        (do (reset! db/mad-libs-waiting-room axes-map)
            (go (<! (async/timeout 1000))
                (do (ut/tracked-dispatch [::post-process-combo-hash panel-id src-table-id-str
                                          shape-name kp1])
  ...
  (fn [db [_ panel-id src-table-id-str shape-name kp1]]
    (let [new-combo-row                          (get-in db kp1)
          ...
          new-base-map                           (modify-combo-viz view
                                                                   ...
                                                                   false
                                                                   src-table-id-str)
          query-keys                             (keys (get-in db [:panels panel-id :queries])) ;; existing
                                                                                                ;; keyword
                                                                                                ;; aliases
          final-map                              (final-combo-viz new-base-map panel-id query-keys)]
      (ut/tapp>> [:new-base-map new-base-map :final-combo-viz final-map :new-combo-block
                  [view query condis conn error] :new-combo-row (first new-combo-row)])
      (if (not error)
...
(re-frame/reg-sub ::get-viz-shape
                  (fn [db [_ shape-name]]
                    (first (get (group-by :shape_name (get-in db [:data :viz-shapes]))
                                shape-name))))
...
(re-frame/reg-sub ::query-reco-counts
                  (fn [db [_ query-id]]
                    (let [query-id-str (-> (str query-id)
                                           (ut/replacer "-" "_")
                                           (ut/replacer ":" ""))]
                      (get (first (filter #(= (get % :table_name) query-id-str)
                                    (get-in db [:data :reco-counts])))
                           :cnt
                           0))))
  ...
  (fn [db [_]]
    (let [panels      (into {}
      ...
      (doseq [[k v] all-queries]
        (conn/sql-deep-meta [k]
                            (sql-alias-replace-sub v)
    ...
    (let [lkp         (fn [query-key]
          ...
          all-queries (into {}
                            (for [[_ v] (get db :panels)]
                              (into {}
                                    (for [[kk vv] (get v :queries)]
                                      (when (and (nil? (find vv :vselect)) (find vv :style-rules))
                                        {kk vv})))))]
      (doseq [[k v] all-queries]
        (conn/sql-style-meta [k]
                             (sql-alias-replace-sub v)
                             (get-in db [:panels (lkp k) :connection-id]))))))
...
(re-frame/reg-event-db ::update-metadata-tabs
                       (fn [db [_]]
                         (let [tab-rules (into {}
                                               (for [[k v] (get db :panels)]
                                                 (into {}
                                                       (for [[kk vv] (get v :tab-rules)]
                                                         {kk {:logic vv :panel-key k}}))))]
                           (doseq [[[tab-name rule-name] {:keys [logic panel-key]}] tab-rules]
                             (conn/sql-tab-meta [panel-key]
                                                [[tab-name rule-name] (sql-alias-replace-sub logic)]
                                                panel-key)))))
...
(re-frame/reg-event-db
  ...
  (fn [db [_]]
    (let [all-condis (into {}
                           (for [[_ v] (get db :panels)]
                             (into {} (for [[kk vv] (get v :conditionals)] {kk vv}))))]
      (doseq [[k v] all-condis] (conn/sql-condi-meta [k] (sql-alias-replace-sub v))))))
...
(defn data-viewer-source
  ...
  (let [kps       (ut/extract-patterns obody :data-viewer 2)
        logic-kps (first (for [v kps] (let [[_ src] v] src)))]
    logic-kps))
...
(defn map-boxes2
  ...
  (let [sql-explanations (sql-explanations-kp)
        ...
        source @(ut/tracked-sub ::data-viewer-source
                                {:block-id block-id :selected-view selected-view})
                    ...
                    [re-com/h-box :children
                     [[dggbl ;;draggable
                       ...
                       [re-com/v-box :min-width (px (* (count (str kk)) 11)) ;"110px"
                        ...
                        :padding "8px"]]
                      (map-boxes2 k-val block-id selected-view keypath-in kk nil draggable?)] :style
                     keystyle]
                     ...
                     [[dggbl ;;draggable
                      ...
                      [map-boxes2
                       (if (= k-val-type "rowset")
                         ...
                         (zipmap (iterate inc 0) k-val)) block-id selected-view keypath-in kk nil
                      ...
                      [re-com/h-box :gap "6px" :children
                       [[dggbl ;;draggable
                         {:h         4
                          ...
                          :root      [0 0]
                          :drag-meta {:type        :viewer-pull
                                      :param-full  [:box :size "auto" :align :center :justify
                                                    :center :style
...
(defn transform-nested ;; do this on the server?
  ...
  (let [groups (group-by group-key data)]
    (vec (map (fn [[group vals]]
                {:id   group
                 :data (vec (map (fn [val] {:x (get val x-key) :y (get val y-key)}) vals))})
           groups))))
  ...
  (let [groups (group-by group-key data)]
    (vec (map (fn [[group vals]]
                (merge {group-key group}
                       (apply merge-with
                         +
                         (vec (map (fn [val] {(get val category-key) (get val value-key)}) vals)))))
           groups))))
...
(defn nivo-data-check
  ...
  (let [data     (get edn-mass :data)
        ...
        no-data? (nil? data)]
    (if no-data?
      [re-com/box :child "no data" :height
       (if (or (float? h) (integer? h)) (px (js/Math.floor h)) h) :width
       (if (or (float? w) (integer? w)) (px (js/Math.floor w)) w) :style
       {:font-family (theme-pull :theme/base-font nil)} :size "auto" :align :center :justify
       :center]
      try-block)))
...
(defn nivo-render-straight
  ...
  (nivo-data-check edn-mass
                   (try [(reagent/adapt-react-class nivo-fn) edn-mass]
                        (catch :default e
                          [re-com/box :child
                           (str "reagent/adapt-react-class nivo-render-straight err: " e)]))))
...
(defn nivo-render
  ...
  (let [data               (get edn-mass :data)
        ...
        edn-mass           (if node-size?
                             (assoc edn-mass
                               :nodeSize #(do (get (walk/keywordize-keys (js->clj %))
                                                   (get edn-mass :nodeSize)
                                                   14)))
                             edn-mass)
        node-size2?        (and (keyword? (get edn-mass :size))
                                (= nivo-fn nivo-swarmplot/SwarmPlot))
        edn-mass           (if node-size2?
                             (assoc edn-mass
                               :size #(do (get (walk/keywordize-keys (js->clj %))
                                               (get edn-mass :size)
                                               14)))
                             edn-mass)]
    [re-com/box :style {:overflow "hidden"} :child
     (nivo-data-check edn-mass [(reagent/adapt-react-class nivo-fn) edn-mass])]))
...
(re-frame/reg-event-db ::delete-layout-panel
                       ...
                       (fn [db [_ panel-key view-key layout-key root]]
                         (let [[layout-kp layout-map] @(ut/tracked-subscribe [::get-layout-in
                                                                              panel-key view-key])
...
(re-frame/reg-event-db
  ...
  (fn [db [_ panel-key view-key layout-key root]]
    (let [[layout-kp layout-map] @(ut/tracked-subscribe [::get-layout-in panel-key view-key])
          target-kp              (into layout-kp [:panels layout-key])
          new-kp                 (into layout-kp
                                       [:panels (keyword (str "cleared-" (rand-int 1234)))])]
  ...
  (fn [db [_ panel-key view-key layout-key root processed-layout]]
    (let [[layout-kp layout-map] @(ut/tracked-subscribe [::get-layout-in panel-key view-key])
      ...
      (-> db
          ...
          (assoc-in
            (into layout-kp [:panels (keyword (str "empty-v-" (rand-int 1234)))])
...
(defn layout
  ...
  (let [panels (get layout-map :panels {})
        ...
        brix (fn [x & [mod]] (px (if mod (+ (* brick-size x) mod) (* brick-size x))))
              ...
              [[re-com/h-box :style {:position "fixed" :left 0 :top 25} :children
                (for [i (range pw-int)]
                  [re-com/box :child (str i) :align :center :justify :center :style
                   {:font-family (theme-pull :theme/base-font nil) :font-weight 200 :opacity 0.6}
            ...
            (vec
              (for [[view-ref p] processed-layout]
                (let [h (get p :h 4) ;; if omitted, default to 4
                      ...
                      blank-map
                        {:child
                           [re-com/v-box :size "auto" :justify :center :align :center :children
                            (if scrub-borders?
                              [[re-com/v-box :align :center :justify :center :children
                                (for [e (ut/splitter (ut/replacer (str orig-key) #"view/" "") ".")]
                                  [re-com/box :style
                                   {:color "pink" :font-family (theme-pull :theme/base-font nil)}
                               ...
                               [re-com/h-box :gap "10px" :padding "5px" :justify :between :children
                                [[re-com/md-icon-button :md-icon-name "zmdi-view-agenda" :on-click
                                  #(ut/tracked-dispatch [::split-v-layout-panel panel-key view-key
                                                         orig-key (get p :root) processed-layout])
                                  :style
                                  {:font-size  "17px"
                                   :cursor     "pointer"
                                   :color      "pink"
                                   ...
                                   :margin-top "-1px"}]
                                 [re-com/md-icon-button :md-icon-name "zmdi-view-agenda" :on-click
                                  #(ut/tracked-dispatch [::split-h-layout-panel panel-key view-key
                                                         orig-key (get p :root) processed-layout])
                                  :style
                                  {:font-size  "17px"
                                   :cursor     "pointer"
                               ...
                               [re-com/h-box :gap "10px" :padding "5px" :justify :between :children
                                [[re-com/md-icon-button :md-icon-name "zmdi-grid-off" :on-click
                                  #(ut/tracked-dispatch [::clear-layout-panel panel-key view-key
                                                         orig-key (get p :root)]) :style
                                  {:font-size  "17px"
                                   :cursor     "pointer"
                                   :color      "pink"
                                   :padding    "0px"
                                 ...
                                 [re-com/md-icon-button :md-icon-name "zmdi-close" :on-click
                                  #(ut/tracked-dispatch [::delete-layout-panel panel-key view-key
                                                         orig-key (get p :root)]) :style
                                  {:font-size  "17px"
                                   :cursor     "pointer"
                                   :color      "pink"
                         ...
                         :justify :center
                         :style (merge
                                  {:border           (if scrub-borders? "2px dashed pink" "inherit")
                  ...
                  (when (and root-visible? (not hidden?))
                    (ut/mapply re-com/box
                               (merge leftovers
                                      (if (or
                                            (keyword? view-ref)
                                            (and scrubber-on? (= panel-key selected-block) editor?))
                                        blank-map
                                        box-map))))))))]
...
(defn dropdown
  ...
  (let [;;choice @(ut/tracked-subscribe [::conn/clicked-parameter-key [(try (keyword (str (name
        ...
        width  (if (string? width)
                 (try (edn/read-string (ut/replacer width "px" "")) (catch :default _ 100))
                 width)]
    [re-com/h-box :size "auto" :justify :between :align :center :children
     [[re-com/md-icon-button :on-click
       #(ut/tracked-dispatch [::conn/click-parameter [panel-key field] nil]) :style
       (merge {:font-size "19px" :width "30px" :margin-top "8px" :opacity 0.33} button-style)
       :md-icon-name "zmdi-close"]
      [re-com/single-dropdown :style (if (nil? style) {} style) :parts
       {:chosen-drop {:style {:overflow "visible"}}} :placeholder (str placeholder) :choices choices
       :model choice ;(ut/tracked-subscribe [::conn/clicked-parameter-key [(try (keyword (str
  ...
  (let [panel-key :virtual-panel ;:block-4752 ;:hello-there-brother
    ...
    (cond (= type :view)  (let [view {key data_d}
          ...
          (= type :query) (let [temp-key (get data_d :_query-id (keyword (str "kick-" (hash c))))
                            ...
                            [re-com/box :size "none" :width (px (* w brick-size)) :height
                             (px (- (* h brick-size) 30)) :child
...
(defn clover-walk-singles
  ...
  (let [kk (pr-str [panel-key client-name px-width-int px-height-int ww hh w h selected-view
                    override-view])
      ...
      (let [walk-map
              (merge
                {;:re-com/box re-com/box ;; basics
                 ...
                 :left-pad
                   (fn [[num len]]
                     (let [num-str (str num) padded-num (.padStart num-str len "0")] padded-num))
                 :>> (fn [[x y]] (true? (> x y)))
                 :<< (fn [[x y]] (true? (< x y)))
                 :string (fn [args]
                           (if (vector? args)
                             ...
                             (str args)))
                 :data-viewer (fn [x] [re-com/box :width (px (- ww 10)) :size "none" :height
                                       (px (- hh 60)) ;;"300px" ;(px hh)
                 ...
                 :progress-bar
                   (fn [[ww seconds uid]]
                     (let [progress            (or (get @progress-bars uid) (reagent.core/atom 0))
                       ...
                       (fn []
                         [re-com/progress-bar :model (if (> @progress 100) 100 @progress) :striped?
                          true :bar-style
                          {:color            (str (ut/choose-text-color
                                                    (theme-pull :theme/editor-outer-rim-color nil))
                                                  67)
                           :outline          "none"
                           :border           "none"
                          ...
                          [[re-com/md-icon-button :md-icon-name "zmdi-play" :on-click
                            (fn []
                              (when true ;(not (some (fn [x] (= x text)) @db/speech-log))
                                (do (ut/dispatch-delay 800
                                                       [::http/insert-alert (str "\"" text "\"") 13
                                                        2 8])
                                    (ut/tracked-dispatch [::audio/text-to-speech11 panel-key :speak
                                                          (str text)])))) :style
                            {:font-size "15px" :opacity 0.25 :cursor "pointer"}]
                           [re-com/box :child ""]]]]]))
                 :grid (fn [tab] [rc/catch [grid tab]]) ;;; TODO :grid was clashing with nivo
                 :read-edn (fn [x] (try (edn/read-string x) (catch :default _ x)))
                 :open-input
                   (fn [args] [re-com/box :width (px (- ww 10)) :size "none" :height (px (- hh 60)) ;;"300px"
                                                                                                    ;;;(px
                                                                                                    ;;hh)
                 ...
                 :play (fn [text & [uid]]
                         (let [uid (or uid (str "tt" (rand-int 1234) (rand-int 1234)))]
                           (when (not (some #(= % uid) @db/speech-log))
                             ...
                             (ut/tracked-dispatch [::audio/text-to-speech11 :audio :speak (str text)
                                                   true]))
                           ""))
                 :play1 (fn [[text & [uid]]]
                          (let [uid (or uid (str "tt" (rand-int 1234) (rand-int 1234)))]
                          ...
                          [[re-com/md-icon-button :md-icon-name "zmdi-play" :on-click
                            (fn []
                              (when (not (some (fn [x] (= x text)) @db/speech-log))
                                (do (ut/tracked-dispatch [::audio/text-to-speech11 panel-key :speak
                                                          (str text) true])))) :style
                            {:font-size "15px" :opacity 0.25 :cursor "pointer"}]
                           [re-com/box :child ""]]]]]))
                 :play-click-target
                           ...
                           [re-com/v-box :size "auto" :children
                            [[re-com/box :child (if view view (str [flow-id bid value]))]
                             [re-com/h-box :size "auto" :justify :between :height "10px" :style {} ;:border
                                                                                                   ...
                                                                                                   ;white"
                              :children
                              [[re-com/md-icon-button :md-icon-name
                                "fa-solid fa-location-crosshairs" ;; <i class=
                                :on-click
                                (fn []
                                  (when bid (ut/tracked-dispatch [::push-value flow-id bid value])))
                 ...
                 :dialog-push
                   (fn [[flow-id bid value & [view]]]
                     (let [bid      (try (if (cstr/starts-with? bid ":")
                                           ...
                                           (edn/read-string (str ":" bid)))
                                         (catch :default _ nil))
                           alert-id @(ut/tracked-subscribe [::lookup-alert-id :dialog-push flow-id
                                                            bid value])]
                       [re-com/box :style {:cursor "pointer"} :attr
                        {:on-click (fn []
                                     (when bid
                                       (ut/tracked-dispatch [::push-value flow-id bid value])
                                       (ut/dispatch-delay 800 [::prune-alert alert-id])))} :size
                        "auto" :child (if view view (str "push " value " to " flow-id "/" bid))]))
                 :web-cam
                   #(let [video-ref     (reagent/atom nil)
                      ...
                      (reagent/create-class
                        {:component-did-mount (fn []
                         ...
                         :reagent-render      (fn [] [re-com/box :style
                                                      (if (not running?)
                           ...
                           (when runstreamed?
                             [re-com/md-icon-button :md-icon-name "zmdi-tune" :style
                              {:font-size        "inherit"
                               ...
                               :margin-top       "2px"}])
                           [re-com/md-icon-button :md-icon-name
                            (if running? "zmdi-refresh" "zmdi-play") :class
                            (when running? "rotate linear infinite") :on-click
                            (fn []
                              (when (not running?) ;(not (some (fn [x] (= x text))
                                (let [fstr (str "running flow "
                                                flow-id
                                    ...
                                    [::wfx/request :default
                                     {:message     {:kind        :run-flow
                                                    ...
                                                    :opts        (if (map? overrides)
                                                                   ...
                                                                   base-opts)
                                                    :client-name client-name
                                                    :keypath     [:panels panel-key :views
                                                                  selected-view]}
                 ...
                 :scrubber
                   (fn [[kk pm & [opts]]]
                     [scrubber-panel true @(rfa/sub ::keypaths-in-params {:key-type :param}) kk pm
                 ...
                 :params> #(ut/tracked-dispatch [::conn/click-parameter [panel-key]
                 ...
                 :v-box re-com/v-box}
                (walk-map-sizes (/ px-width-int brick-size)
...
(defn honeycomb
  ...
  (let [;block-map panel-map ;@(ut/tracked-subscribe [::panel-map panel-key]) ;(get workspace
        ...
        all-keys (vec (into (vec (sort (into (vec (keys sql-calls)) all-view-keys)))
                            [:virtual-view]))
        selected-view @(ut/tracked-sub ::selected-view-alpha {:panel-key panel-key}) ;(or  :view)
        override-view
          (cond (= override-view :*) selected-view ;; default editor "everything"
                (and (not (nil? override-view)) (some #(= % override-view) all-keys)) override-view ;; not
                                                                                                    ;; nil
                                                                                                    ;; and
                                                                                                    ;; exists,
                                                                                                    ;; do
                                                                                                    ;; it
                (and (not (nil? override-view)) (not (some #(= % override-view) all-keys))) ; not
                  (first all-keys)
        ...
        walk-map (clover-walk-singles panel-key
                                      client-name
                                      px-width-int
                                      px-height-int
                                      ww
        ...
        sql-aliases-used @(ut/tracked-sub ::panel-sql-aliases-in-views-body
                                          {:panel-key panel-key :body body})
        ...
        vsql-calls
          (ut/namespaced-swapper "this-block" (ut/replacer (str panel-key) #":" "") vsql-calls)
        vsql-replace-map (into
                           {}
                           (for [[k v] vsql-calls]
                             (let [look-for-datasets   (cset/intersection (set (ut/deep-flatten v))
                                                                          (set all-sql-call-keys))
                                   data-subbed-rep-map (into {}
                                                             (for [ds look-for-datasets]
                                                               {ds @(rfa/sub ::conn/sql-data-alpha
                                                                             {:keypath [ds]})}))
        ...
        used-datasets (cset/union (set sql-aliases-used)
                                  (cset/intersection possible-datasets-used
                                                     (set all-sql-call-keys)))
        used-datasets
          (if replacement-all? (into used-datasets (keys replacement-query)) used-datasets)
        data-walks (into {}
                         (for [k used-datasets] ;all-sql-call-keys]
                           {k @(rfa/sub ::conn/sql-data-alpha {:keypath [k]})}))
                          ...
                          (for [k condi-walks-targets]
                            {k ;;@(ut/tracked-subscribe [::conn/condi-value (keyword (last
                               @(ut/tracked-sub ::conn/condi-value
                                                {:condi-key (keyword (last (ut/splitter
                                                                             (ut/safe-name k)
                                                                             "/")))})}))
        into-walk-map2
          (fn [obody]
          ...
          (fn [obody]
            (let [;obody (walk/postwalk-replace condi-walks orig-body)
                  ...
                  logic-kps (into {}
                                  (for [v kps]
                                    (let [[_ l this that] v]
                                      {v (if (if (vector? l) (resolver/logic-and-params l nil) l)
                                           this
                                           that)})))]
              (walk/postwalk-replace logic-kps obody)))
        when-walk-map2
        ...
        auto-size-walk-map2
          (fn [obody]
            (let [kps       (ut/extract-patterns obody :auto-size-px 2)
                  logic-kps (into {}
                                  (for [v kps] (let [[_ l] v] {v (ut/auto-font-size-px l h w)})))] ;(=
              (walk/postwalk-replace logic-kps obody)))
        onclick-walk-map2
          (fn [obody]
                          ...
                          logic-kps (into {}
                                          (for [v kps]
                                            (let [[_ & this]                   v
                                              ...
                                              {v #(ut/tracked-dispatch [::push-value flow-id bid
                                                                        value alert?])})))]
                      (walk/postwalk-replace logic-kps obody)))
        ...
        get-in-walk
          (fn [obody]
            (let [kps       (ut/extract-patterns obody :get-in 2)
              ...
              (walk/postwalk-replace logic-kps obody)))
        run-rs-flow
          (fn [flow-id flow-id-inst panel-key override-merge-map]
            (let [client-name @(ut/tracked-subscribe [::client-name])
                  base-opts   {:increment-id? false :instance-id flow-id-inst}
                  running-key (keyword (str "flow/" flow-id-inst ">*running?"))
                  running?    @(rfa/sub ::conn/clicked-parameter-key-alpha {:keypath [running-key]})
                  overrides   (merge @(ut/tracked-subscribe [::runstream-overrides flow-id])
                                     override-merge-map)]
              (when (not running?) ;(not (some (fn [x] (= x text)) @db/speech-log))
                (let [fstr (str panel-key " DROP running flow " flow-id " (with double overrides)")
                  ...
                  (ut/dispatch-delay 800
                                     [::http/insert-alert
                                      [:box :child fstr :style {:font-size "14px"}] w 0.5 5])))))
        run-drop (fn [[drop-id _] val]
                   (let [drop-deets     @(ut/tracked-subscribe [::drop-details drop-id])
                     ...
                     (when (and @(rfa/sub ::auto-run-and-connected? {})
        ...
        drop-walk-replace
          (fn [keynames obody] ;; swapped order since we need to pass a list of
            (reduce (fn [body keyname]
                      (let [kps       (ut/extract-patterns body keyname 2)
                            logic-kps (into {}
                                            (for [v kps] (let [[_ val] v] {v (run-drop v val)})))]
        ...
        body ;;(ut/timed
          (cond->> body
            true                      (ut/namespaced-swapper "this-block"
                           ...
                           (remove nil?
                             (flatten (for [[_ v] sql-calls]
                                        (when (not (cstr/includes? (str (get v :from)) ":query/"))
                                          (get-in v [:from 0])))))))
        base-table-sniffs (into {}
                                (for [t base-tables]
                                  {t {:select        [:*]
                                      :connection-id (if (some #(= t %)
                                                               ["channel_history" "fn_history"
                                                                "flows_history"])
                                                       "flows-db"
                                                       "system-db")
                                      :from          [t]
    ...
    (cond ;;replacement-all? [rc/catch [re-com/box :child (get body selected-view)]]
      (seq body)
        (cond (not (nil? override-view)) ;; for editor usage
                (if (= override-view :base)
                  ...
                  (if override-view-is-sql?
                    [rc/catch [magic-table panel-key [override-view]]]
                    (let []
                      [rc/catch
                       [re-com/box :width (px ww) :size "none" :style
                        {:filter (if overlay? "blur(3px) opacity(60%)" "none")} :child
                        (get body override-view)]])))
              selected-view-is-sql?      [rc/catch [magic-table panel-key [selected-view]]] ;; magic-table
                                                                                            ;; [panel-key
              single-view?               [rc/catch
                                          [re-com/v-box :children
                                           [(when overlay?
                                              @(ut/tracked-subscribe [::dynamic-drop-targets nil
                                                                      @dragging-body panel-key nil
                                                                      ww hh]))
                                            [re-com/box :style
                                            ...
                                            [re-com/box :style
                                             {:filter      (if overlay?
                                              ...
                                              :margin-top  (if (and overlay? is-layout?)
                                                             "-25px"
                                                             "inherit") ;; not sure why the pos gets
                                                                        ;; fucked up on
                                              :margin-left (if (and overlay? is-layout?)
                                                             "-3px"
                                                             "inherit")} :child ;(if (=
                                                                                ;selected-view
                                                                                ;:layered-viz)
      ...
      (and no-view? selected-view-is-sql?) (if (and fw fh) ;(js/alert (str selected-view))
                                             [rc/catch
                                              [magic-table panel-key [selected-view] fw fh]]
                                             [rc/catch [magic-table panel-key [selected-view]]]) ;; block-sizes
                                                                                                 ;; block-width
...
(re-frame/reg-sub ::lookup-root-key-tab
                  (fn [db [_ brick-vec tab]]
                    (let [lookup-map (into {}
                                           (for [[k v] (into {}
                                                             (filter #(= tab (get (val %) :tab ""))
                                                               (get db :panels)))]
                                             {(get v :root) k}))]
                      (get lookup-map brick-vec))))
...
(re-frame/reg-sub ::panel-px-width
                  (fn [db [_ panel-key]] (* brick-size (get-in db [:panels panel-key :w]))))

(re-frame/reg-sub ::panel-px-height
                  (fn [db [_ panel-key]] (* brick-size (get-in db [:panels panel-key :h]))))
...
(re-frame/reg-sub ::connection-id
                  (fn [db [_ panel-key]] (get-in db [:panels panel-key :connection-id])))
...
(re-frame/reg-sub ::ghosted?
                  (fn [db [_ panel-key]]
                    (if (get db :peek?) false (get-in db [:panels panel-key :ghosted?] false))))
...
(re-frame/reg-sub ::no-ui?
                  (fn [db [_ panel-key]]
                    (if (get db :peek?)
                      false
                      (or (get db :no-ui? false) (get-in db [:panels panel-key :no-ui?] false)))))

(re-frame/reg-sub ::panel-style
                  (fn [db [_ panel-key]]
                    (if (get db :peek?) {} (get-in db [:panels panel-key :style] {}))))

(re-frame/reg-sub ::hidden?
                  (fn [db [_ panel-key]]
                    (if (get db :peek?) false (get-in db [:panels panel-key :hidden?] false))))

(re-frame/reg-sub ::minimized?
                  (fn [db [_ panel-key]]
                    (if (get db :peek?) false (get-in db [:panels panel-key :minimized?] false))))
...
(defn squareql-logo
  ...
  (let [left (* x brick-size)
        top  (* y brick-size)]
    [re-com/h-box :children
     [[re-com/box :child [:img {:src "images/squareql-logo-100a.png"}] :height "102px" :width
       "102px" :size "none" :attr
...
(re-frame/reg-event-db ::move-to-current-tab
                       (undoable)
                       (fn [db [_ panel-id]]
                         (assoc-in db [:panels panel-id :tab] (get db :selected-tab))))
...
(re-frame/reg-sub ::is-pinned?
                  (fn [db [_ panel-key]] (get-in db [:panels panel-key :pinned?] false)))
    ...
    (if (cstr/includes? icon "zmdi")
      [re-com/md-icon-button :src (at) :md-icon-name icon :style
       {;:color bcolor :cursor "grab"
        :font-size "15px"} :attr {}]
      [re-com/box :size "none" :height (px (* num brick-size)) :child
    ...
    (when (get-in db [:panels panel-key :iconized?] false)
      (let [;iconized? (get-in db [:panels panel-key :iconized?] false)
            ...
            default-icon-map {:h    1
                              :w    1
                              :view [:box :align :center :justify :center :width
                                     (px (* brick-size 1)) :height (px (* brick-size 1)) :padding
                                     "5px" :size "auto" :style
                                     {:font-size "10px"
                                      :border    (str "1px solid "
                                                      (theme-pull :theme/editor-outer-rim-color nil)
                                      ...
                                      :filter    (str "drop-shadow(0 0 0.75rem "
                                                      (theme-pull :theme/editor-outer-rim-color nil)
                                                      ")")} :child
                                     (if icon [render-icon icon 1] (str name))]}
...
(re-frame/reg-sub ::panel-icon
                  (fn [db [_ panel-key]]
                    (let [selected-view   (get-in db [:panels panel-key :selected-view])
                          is-grid?        @(ut/tracked-subscribe [::is-grid? panel-key
                                                                  selected-view])
                          selected-query? (some #(= % selected-view)
                                                (keys (get-in db [:panels panel-key :queries])))
                          default         (cond selected-query? "zmdi-grid"
                                                is-grid?        "zmdi-view-dashboard"
                                                :else           "zmdi-widgets") ;; depends on what
                                                                                ;; view
...
(re-frame/reg-sub ::has-a-running-flow?
                  (fn [db [_ panel-key]]
                    (let [panel-map (get-in db [:panels panel-key])
                          pile      (filter #(cstr/starts-with? (str %) ":flow/")
                                      (ut/deep-flatten panel-map))]
                      (some true?
                            (for [e pile]
...
(re-frame/reg-sub ::has-a-flow-view?
                  (fn [db [_ panel-key view]]
                    (let [panel-map (get-in db [:panels panel-key :views view])
                          ...
                          finds     (get (group-by #(first (first %)) @drop-last-tracker-refs)
                                         panel-key)
...
(re-frame/reg-sub ::resolved-block-body-shell
...
(defn maybedoall
  []
  (let [hover-highlight? (or @param-hover @query-hover)] (if hover-highlight? doall seq)))
        ...
        (for [[bw bh brick-vec-key] brick-roots] ;diff-grid1] ;(if @dragging? current-grid
          (let [bricksw (* bw brick-size)
                ...
                trunc-name (when (not (nil? name))
                             (let [charpx      5.75
                               ...
                               (if (> pixel-width panel-width)
                                 ...
                                 name)))
                block-width
                  (if root? (+ 1 (* brick-size (if (= w 0) (- bricks-wide 1) w))) brick-size)
                block-height
                  (if root? (+ 1 (* brick-size (if (= h 0) (- bricks-high 1) h))) brick-size)
                selected? (= brick-vec-key selected-block)
                block-selected? selected?
                subq-blocks
                  (if root? @(ut/tracked-sub ::subq-panels-alpha {:panel-id selected-block}) [])
                ...
                reco-selected (let [;;rr @(ut/tracked-subscribe [::conn/clicked-parameter-key
                                    rr @(rfa/sub ::conn/clicked-parameter-key-alpha
                ...
                viz-reco? (and (or (= selected-block "none!") (nil? selected-block))
                               ...
                               editor?
                               (= @db/editor-mode :vvv))
                {:keys [ghosted? no-ui? hidden? minimized?]} body-shell
                no-ui? (or no-ui? (not (nil? tab)) full-no-ui?)
                panel-style (get body-shell :style)
                tab-color (cond selected?           "#9973e0"
                                viz-reco?           "#ffb400"
                                hover-q?            "#c7005d"
                ...
                theme-base-block-style-map (when (map? theme-base-block-style)
                                             theme-base-block-style)]
            (if (and (not (nil? iconization)) (ut/not-empty? iconization)) ;; remove all this
              (let [icon-h (get iconization :h 1)
                ...
                [re-com/box :size "auto" :width (px (* brick-size icon-w)) :height
                 (px (* brick-size icon-h)) :align :center :justify :center :attr
                 {;:on-click #(js/alert (if tab "in container" "not in container"))
                  :on-context-menu #(when (not tab)
                                      (ut/tracked-dispatch [::toggle-icon-block brick-vec-key]))
                  :on-mouse-down   #(mouse-down-handler % brick-vec-key tab-offset true)
                  :on-mouse-over   #(when (and (not @over-block?)
                                               (not (= brick-vec-key @over-block)))
                  ...
                  :on-double-click #(do (tag-screen-position %)
                                        (ut/tracked-dispatch [::launch-clone brick-vec-key]))}
                 :style
                 (merge {:position         "fixed"
                         :user-select      "none"
                         :border           (cond selected? "2px solid #9973e0"
                 ...
                 (merge
                   {:position "fixed" ;"absolute" ;"fixed"
                      ...
                      (when (= brick-vec-key @db/speaking)
                        (let [block-id       brick-vec-key ;:audio
                          ...
                          (cond (and audio-playing? talking-block?)
                                  (str "1px 1px " (px (* 90 (+ 0.1 (get @db/audio-data block-id))))
                                       " "        (theme-pull :theme/editor-outer-rim-color nil))
                                :else "none")))
                    ...
                    :background-color (cond ;drag-action? "#00000000"
                                        ...
                                        selected?     (theme-pull :theme/base-block-color-selected
                                                                  nil) ;"#0b031b"
                 ...
                 [re-com/v-box :gap "1px" :size "1" :justify :between :children
                  [;(when (not editor-panel?)
                   (if ;(and (not ghosted?)
                     ...
                     [re-com/h-box :height "20px" :padding "3px" :justify :between :align :center
                      :style
                      {:background-color (cond ;drag-action? "#58A27977"
                                           selected?           "#9973e022" ; "#3b528b" ;
                                           parent-of-selected? "#9973e022"
                                           upstream?           "#9973e022"
                                           downstream?         "#05dfff22"
                                           :else               "inherit")
                       :z-index          (if (or is-grid? selected?) (+ zz 10) zz)
                       :color            (cond selected? (theme-pull
                                                           :theme/block-title-selected-font-color
                                                           nil) ;"#ffffff"
                          ...
                          {:on-click      #(ut/tracked-dispatch [::select-block brick-vec-key])
                           :on-mouse-down (fn [e]
                                            ...
                                            (js/setTimeout
                                              ...
                                              click-delay))
                           :on-mouse-up   (fn [_] (reset! mouse-down? false))} :child " "])
                       (when (> w 5)
                           ...
                           :on-mouse-down (fn [e]
                                            ...
                                            (js/setTimeout
                                              #(when @mouse-down?
                                                 (mouse-down-handler e brick-vec-key tab-offset))
                                              click-delay))
                           :on-mouse-up   (fn [_] (reset! mouse-down? false))} :style
                         ...
                         [re-com/h-box :gap "5px" :children
                          [;;  (when (not selected?)
                           (when col-selected?
                             ...
                             [re-com/md-icon-button :md-icon-name
                              (if col-selected? "zmdi-pause" "zmdi-play") :style
                              {:font-size "15px" :opacity 0.25 :cursor "pointer"}])
                           (when hovered-on?
                             ^{:key (str "brick-" brick-vec "-header-pin")}
                             [re-com/md-icon-button :md-icon-name
                              (if is-pinned? "zmdi-pin-off" "zmdi-pin") :on-click
                           ...
                           (when hovered-on?
                             ...
                             [re-com/md-icon-button :md-icon-name "zmdi-window-minimize" :on-click
                              #(do (ut/tracked-dispatch [::toggle-minimize-block brick-vec-key])
                                   ...
                                   (do (reset! over-block? false))) :style
                              {:font-size "15px" :opacity 0.33 :cursor "pointer"}])
                           (when hovered-on?
                             ^{:key (str "brick-" brick-vec "-header-icon")}
                             [re-com/md-icon-button :md-icon-name "zmdi-photo-size-select-small"
                              :on-click
                         ...
                         [;[re-com/gap :size "12px"]
                          (doall
                            (if (empty? mixed-keys)
                              ...
                              [re-com/h-box :gap "0px" :children
                               (doall
                                 (for [s mixed-keys]
                                   (let [selected?        (= (if (= s base-view-name) nil s)
                                                             selected-view)
                                         not-view?        (not (some #(= % s) (keys views)))
                                         reco-count       (when not-view?
                                                            @(ut/tracked-subscribe [::reco-count s
                                                                                    :reco]))
                                         [_ single-wait?] (if not-view?
                                                            @(ut/tracked-subscribe [::query-waitings
                                                                                    s])
                                                            [0 0])
                                         reco-ready?      (and not-view? (> reco-count 0)) ; (not
                                         query-running?   single-wait?
                                         flow-running?    @(ut/tracked-subscribe
                                                             [::has-a-running-flow-view?
                                                              brick-vec-key s])
                                         param-keyword    [(keyword (str "param/"
                                                                         (ut/safe-name s)))]
                                         is-param? ;@(ut/tracked-subscribe
                                           @(rfa/sub ::conn/clicked-parameter-key-alpha
                                                     {:keypath param-keyword})
                                         param-dtype      (when is-param? (ut/data-typer is-param?))
                                         param-dtype      (try (if (and (= param-dtype "vector")
                                                                        (every? string? is-param?))
                                                                 "string"
                                                                 param-dtype)
                                                               (catch :default _ param-dtype)) ;; since
                                         param-color      (get (theme-pull :theme/data-colors
                                                                           db/data-colors)
                                                               param-dtype
                                                               "orange")
                                         fcolor           (cond is-param? param-color
                                                                ...
                                                                :else
                                                                  (theme-pull
                                                                    :theme/block-title-font-color
                                                                    "#ffffff50"))
                                         bcolor           (if is-param?
                                                            (str fcolor "22")
                                                            (if selected? tab-color "inherit"))]
                                     (draggable
                                        ...
                                        [[re-com/box :child
                                         ...
                                         (when (or flow-running?
                                           ...
                                           [re-com/md-icon-button :md-icon-name "zmdi-refresh"
                                            :class "rotate linear infinite" :style
                                            {:font-size        "20px"
                                         ...
                                         (when reco-ready?
                                           [re-com/box :style {:margin-top "-6px"} :child
                                            [re-com/md-icon-button :md-icon-name
                                             "zmdi-view-dashboard" :on-click
                                             #(do (clear-preview2-recos)
                                                  (ut/tracked-dispatch
                                                    [::select-view brick-vec-key
                                                     (if (= s base-view-name) nil s)])
                                                  (reset! mad-libs-view (if (= s @mad-libs-view)
                                                                          nil
                                                                          s))
                                                  (when (not
                                                          (= s
                                             ...
                                             {:font-size        "14px"
                                              ...
                                              :padding-left     "2px"
                                              :padding-right    "2px"}]])]]))))]))
                          (cond selected? ^{:key (str "brick-" brick-vec "-resize-handle")}
                                          [re-com/box :size "none" :justify :end :align :end :width
                                           "18px" :height "18px" :attr
                                           (if selected?
                                             {:on-mouse-down resize-mouse-down-handler}
                                             {:on-click #(ut/tracked-dispatch [::select-block
                                                                               brick-vec-key])})
                                           :style
                                           {:margin-top    "-2px"
                                            :margin-right  "-2px"
                                ...
                                (and (or upstream? parent-of-selected? downstream?) (>= w 5))
                                  [re-com/box :style
                                   ...
                                   (cond parent-of-selected? "parent"
                                         ...
                                         :else               "upstream")]
                                :else ^{:key (str "brick-" brick-vec "-resize-handle-gap")}
                                      [re-com/gap :size "12px"])]]])
