...
(defn theme-pull [cmp-key fallback & test-fn] @(rfa/sub ::theme-pull-sub {:cmp-key cmp-key :fallback fallback :test-fn test-fn}))
  ...
  (fn [db _]
    (let [create-runner-listeners (fn [obody]
      ...
      (filterv #(not (cstr/includes? (str %) "||")) ;; live running subs ignore
        (vec (distinct
               (concat signal-subs clover-solvers solver solvers drop-refs runstream-refs runstreams theme-refs flow-refs)))))))
                         ...
                         (let [;;new @(ut/tracked-subscribe [::get-flow-subs])
                           ...
                           (doall (doseq [n deads]
                                    ...
                                    (ut/tracked-dispatch [::conn/declick-parameter
                                                          (vec (map keyword (ut/splitter (ut/replacer (str n) ":" "") #"/")))]) ;; remove
                                  ))
...
(defn partition-keywords [keywords] (let [str-keywords (map str keywords)] (group-by #(cstr/includes? % "||") str-keywords)))
...
(re-frame/reg-sub
  ...
  (fn [db] (and (get db :flow? false) (= (get @db/flow-editor-system-mode 0) "flows running") (get db :flow-editor? false))))
...
(re-frame/reg-event-db ::update-workspace (undoable) (fn [db [_ keypath value]] (assoc-in db (cons :panels keypath) value)))
...
(re-frame/reg-event-db ::delete-selected-panel
                       (undoable)
                       (fn [db _] (ut/tracked-dispatch [::delete-panel (get db :selected-block)]) db))

(re-frame/reg-event-db ::delete-panel
                       (undoable)
                       (fn [db [_ & [panel-key]]]
...
(re-frame/reg-event-db ::quick-delete-panel (fn [db [_ panel-key]] (ut/dissoc-in db [:panels panel-key])))
...
(re-frame/reg-event-db ::unalias-query
                       (undoable)
                       (fn [db [_ query-key]]
...
(re-frame/reg-sub ::page-num (fn [db [_ panel-key query-key]] (get-in db [:panels panel-key :queries query-key :page] 1)))

(re-frame/reg-event-db ::change-page
                       (undoable)
...
(re-frame/reg-sub ::selected-block? (fn [db {:keys [panel-key]}] (true? (= panel-key (get-in db [:selected-block] "none!")))))
...
(re-frame/reg-event-db ::change-drop-type ;; experiment
...
(re-frame/reg-sub ::query (fn [db [_ panel-key query-key]] (get-in db [:panels panel-key :queries query-key])))
                   ...
                   (let [psplit        (ut/splitter (ut/safe-name (str k)) "/")
                     ...
                     (draggable ;(sql-spawner-filter :param [k table field])
                       ...
                       [re-com/v-box :size "auto" :attr
                        {:on-mouse-enter #(reset! param-hover [k table field]) :on-mouse-leave #(reset! param-hover nil)} :style
                        ...
                        [[re-com/h-box :justify :between :children
                          [[re-com/box :child
                            (str (if hovered?
                                   ...
                                   (str k))
                                 (when param-has-fn? " *ƒ")) :style
                            {:color dcolor :font-weight 700 :font-size "13px" :padding-left "4px" :padding-right "4px"}]
                           (if hovered?
                             [re-com/md-icon-button :md-icon-name "fa-regular fa-trash-can" ;; "zmdi-close"
                              ...
                              {:color        dcolor
                               ...
                               :margin-right "3px"
                               :font-size    "14px"
                               :height       "15px"} :on-click #(ut/tracked-dispatch [::conn/declick-parameter [table field]])]
...
(defn sql-spawner-meta
  ...
  (let [selected-meta-db      @(ut/tracked-subscribe [::conn/clicked-parameter [:connections-sys]]) ;;
        ...
        selected-fields       (vec (map ut/sql-keyword
                                     (map :field_name @(rfa/sub ::conn/sql-data-alpha {:keypath [:fields-sys]}))))
        parent-sql-sql-alias  (ut/gen-sql-sql-alias)
        ...
        value-walks         (into {}
                                  (for [k value-walks-targets] ;all-sql-call-keys]
                                    (let [fs    (ut/splitter (ut/safe-name k) "/")
                                      ...
                                      {k (get-in @(rfa/sub ::conn/sql-data-alpha {:keypath [ds]}) [row field])})))
                ...
                (and not-sys? view-based? (not block-panel?))
                  (merge
                    ...
                    (if (and self? block-panel?)
                      {:block-ops
                         [(if (= drag-type :query) (if is-child? [:delete-query :promote-query] [:delete-query]) [:delete-view])]}
                      {}))
...
(re-frame/reg-sub ::stylers
                  (fn [db [_ panel-key query-key]]
                    (into {}
                          (for [[[cols name] logic-map] (get-in db [:sql-source query-key :style-rules])] ;; moved to fit
                                                                                                          ;; non-panel
          ...
          (fn [preview-maps pk pnum]
            (try
              (let [panel-key (nth pk pnum)
                    ...
                    body
                      [re-com/v-box :size "none" :width (px (* ww brick-size)) :height (px (* hh brick-size)) :style
                       {:zoom         0.44
                        :padding-left "10px"
              ...
              (catch :default _
                [re-com/box :height (px (* (js/Math.floor (* (/ (/ height-int charts-high) brick-size) h-multi)) brick-size))
                 :width (px (* (js/Math.floor (* (/ (/ width-int charts-wide) brick-size) w-multi)) brick-size)) :size "auto"
                 :align :center :justify :center :style
...
(re-frame/reg-sub ::installed-kits
  ...
  (let [viewer-sourced? (try (and (cstr/starts-with? (first data-keypath) "::") (string? (first data-keypath)))
        ...
        child-parts (into kit-callout-fields
                          (cond parent-of-selected? (vec ;;
                                                      (ut/deep-flatten (merge @(rfa/sub ::panel-sql-calls
           ...
           (vec
             (for [c fields]
               (let [cwidth     (cond (cstr/ends-with? (str query-key) "-sys") ;; TODO, convert to
                                      ...
                                      (and (some #(= % c) (keys custom-col-widths))) (get custom-col-widths c) ;; :col-wdiths
                                                                                                               ;; keys exists
                                                                                                               ;; in
                                      :else                                          equal-width-final)
             ...
             (if (and (not (get-in metadata [:fields (first @hover-field) :group-by?])) ;agg?
               ...
               (let [query-key       (keyword (str (ut/replacer (str query-key) #":" "")
                 ...
                 (js/setTimeout #(do ;; change view tab
                                   ...
                                   (reset! animate? false))
                                1000))
               (ut/tracked-dispatch-sync [::conn/click-parameter data-keypath (get @rowset x)]))) :cell-style
                   ...
                   [[re-com/box :child rowcount-string] [re-com/gap :size "0px"] ;; the 2 gaps are
                    ...
                    (when (and (> full-rowcount 5) ;; TODO, gets messed up sometimes?
                      ...
                      [re-com/md-icon-button :md-icon-name "zmdi-toll" :on-click
                       #(ut/tracked-dispatch [::conn/run-sql-deep-meta-for panel-key query-key (sql-alias-replace-sub query)])
                       :attr
                       (when hover-field-enable?
                  ...
                  (if (and has-pages? (not last-page?))
                    [re-com/md-icon-button :md-icon-name "zmdi-caret-right" :on-click
                     #(do (ut/tracked-dispatch [::change-page panel-key query-key (+ page-num 1) ttl-pages])
                          (ut/tracked-dispatch [::conn/clear-query-history query-key])) :style
                     {:font-size "22px" :cursor "pointer" :opacity 0.5 :padding "0px" :margin-top "-1px"}]
                    [re-com/box :child " " :width "22px"])]]) [re-com/gap :size "0px"]]]]]]]) ;; just in case so we don't
                                                                                              ;; render empty child
...
(re-frame/reg-sub ::selected-view
                  (fn [db [_ panel-key]]
                    (let [body    (get-in db [:panels panel-key :views])
                          ...
                          vq      (conj (conj (into (keys body) (keys queries)) :layered-viz) :dyn-tab)] ;; pseudo
...
(re-frame/reg-sub ::something-running?
                  (fn [db _]
                    (let [status (get db :status)] (true? (some #(or (= % :started) (= % :queued)) (ut/deep-flatten status))))))
...
(re-frame/reg-event-db
  ...
  (fn [db [_ alert-id]]
    ...
    (if alert-id (let [alerts (get db :alerts)] (assoc db :alerts (vec (remove #(= (last %) alert-id) alerts)))) db)))
  ...
  (fn [db [_]]
    (let [lkp         (fn [query-key]
      ...
      (doseq [[k v] all-queries]
        (conn/sql-style-meta [k] (sql-alias-replace-sub v) (get-in db [:panels (lkp k) :connection-id]))))))
                       ...
                       (fn [db [_]]
                         (let [tab-rules (into {}
                           ...
                           (doseq [[[tab-name rule-name] {:keys [logic panel-key]}] tab-rules]
                             (conn/sql-tab-meta [panel-key] [[tab-name rule-name] (sql-alias-replace-sub logic)] panel-key)))))
                                             ...
                                             (for [[k {:keys [value source]}] values
                                               ...
                                               {k (if (= source :input)
                                                    ...
                                                    (let [;;vv @(ut/tracked-subscribe [::rs-value
                                                          ;;flow-id k])
                                                          vv @(rfa/sub ::rs-value {:flow-id flow-id :kkey k})] ;;; dupe
                                                                                                               ;;; from
                                                                                                               ;;; buffy
  ...
  (let [panel-key :virtual-panel ;:block-4752 ;:hello-there-brother
    ...
    (cond (= type :view)  (let [view {key data_d} w (or w 11) h (or h 9)] [honeycomb panel-key :virtual-view w h view nil])
          (= type :query) (let [temp-key (get data_d :_query-id (keyword (str "kick-" (hash c))))
                            ...
                            [re-com/box :size "none" :width (px (* w brick-size)) :height (px (- (* h brick-size) 30)) :child
      ...
      (let [walk-map
              (merge
                {;:re-com/box re-com/box ;; basics
                            ...
                            (fn []
                              (when (not running?) ;(not (some (fn [x] (= x text))
                                (let [fstr (str "running flow " flow-id (when overrides? " (with overrides)"))
                                  ...
                                  ;;                                        {:overrides overrides})
                                  ;;                                 base-opts)
                                  ;;                  :client-name client-name :keypath     [:panels panel-key :views
                                  ;;                                selected-view]}
                 ...
                 :scrubber (fn [[kk pm & [opts]]] [scrubber-panel true @(rfa/sub ::keypaths-in-params {:key-type :param}) kk pm
                 ...
                 :v-box re-com/v-box}
                (walk-map-sizes (/ px-width-int brick-size) (/ px-height-int brick-size) nil nil nil nil))]
...
(def fake-body [:box
                ...
                :child
                [:color-theft
                 "./images/gen/b64-image3-61714-61215.png"]])

;;(ut/tapp>> [:fofof (replace-templates clover-templates fake-body)])
...
(defn honeycomb
  ...
  (let [;block-map panel-map ;@(ut/tracked-subscribe [::panel-map panel-key]) ;(get workspace
        ...
        override-view (cond (= override-view :*)                                                        selected-view ;; default
                                                                                                                      ;; editor
                                                                                                                      ;; "everything"
                            (and (not (nil? override-view)) (some #(= % override-view) all-keys))       override-view
                            (and (not (nil? override-view)) (not (some #(= % override-view) all-keys))) (first all-keys)
                            :else                                                                       nil)
                                                ...
                                                (let [[_ that this] v]
                                                  {v ;(resolver/logic-and-params
                                                     (resolver/logic-and-params (if (vector? this)
                                                                                  ...
                                                                                  (vec (for [r @(rfa/sub ::conn/sql-data-alpha
                                                                                                         {:keypath [this]})]
                                                                                         (last (get r that)))))
                                                                                panel-key)})))]
                          (walk/postwalk-replace logic-kps obody)))
...
(re-frame/reg-sub ::panel-px-height (fn [db [_ panel-key]] (* brick-size (get-in db [:panels panel-key :h]))))

(re-frame/reg-sub ::panel-px-root
                  (fn [db [_ panel-key]]
                    (let [r (get-in db [:panels panel-key :root]) x (* brick-size (first r)) y (* brick-size (last r))] [x y])))
    ...
    (if (cstr/includes? icon "zmdi")
      [re-com/md-icon-button :src (at) :md-icon-name icon :style
       {;:color bcolor :cursor "grab"
        :font-size "15px"} :attr {}]
      [re-com/box :size "none" :height (px (* num brick-size)) :child [:img {:src icon :width "100%"}]])
    " "))
    ...
    (when (get-in db [:panels panel-key :iconized?] false)
      (let [;iconized? (get-in db [:panels panel-key :iconized?] false)
            ...
            default-icon-map {:h    1
                              :w    1
                              :view [:box :align :center :justify :center :width (px (* brick-size 1)) :height
                                     (px (* brick-size 1)) :padding "5px" :size "auto" :style
                                     {:font-size "10px"
          ...
          (let [bricksw                                      (* bw brick-size)
                ...
                trunc-name                                   (when (not (nil? name))
                                                               (let [charpx      5.75
                                                                 ...
                                                                 (if (> pixel-width panel-width)
                                                                   ...
                                                                   name)))
                block-width                                  (if root?
                                                               (+ 1 (* brick-size (if (= w 0) (- bricks-wide 1) w)))
                                                               brick-size)
                block-height                                 (if root?
                                                               (+ 1 (* brick-size (if (= h 0) (- bricks-high 1) h)))
                                                               brick-size)
                ...
                hover-q?                                     (if (and editor? root?)
                                                               (or @(ut/tracked-subscribe [::has-query? brick-vec-key
                                                                                           (try (nth @param-hover 1)
                                                                                                (catch :default _ nil))])
                                                                   @(ut/tracked-subscribe [::has-query? brick-vec-key
                                                                                           @query-hover])
                                                                   (and (= @query-hover brick-vec-key)
                       ...
                       [re-com/box :size "none" :width (px (- block-width 4)) :height "15px" :style
                        ...
                        [re-com/h-box :size "auto" :justify :between :children
                         [;[re-com/gap :size "12px"]
                          (doall
                            (if (empty? mixed-keys)
                              ^{:key (str "brick-" brick-vec "-footer-gap")} [re-com/gap :size "12px"]
