...
(defn point-inside-block? [x y block]
...
(defn compute-expanded-size [view-ref block all-blocks-map pw-int ph-int]
  (let [{:keys [root w h]} block
       ...
       ; all-blocks (select-keys all-blocks-map (seq processed-blocks))
        ...
        all-blocks (vec (for [p all-blocks
                              :let [x (get-in p [:root 0])
                                    y (get-in p [:root 1])
                                    h (get p :h)
                                    w (get p :w)]]
                          (-> p
                              ;(assoc :view-ref view-ref)
                              (assoc :root [(if (and (< x 1) (ut/is-float? x)) ;; treat as percent of total
                                              (js/Math.floor (* x pw-int)) x)
                                            (if (and (< y 1) (ut/is-float? y)) ;; treat as percent of total
                                              (js/Math.floor (* y ph-int)) y)])
                              (assoc :h (if (and (< h 1) (ut/is-float? h)) ;; treat as percent of total
                                          (js/Math.floor (* h ph-int)) h))
                              (assoc :w (if (and (< w 1) (ut/is-float? w)) ;; treat as percent of total
                                          (js/Math.floor (* w pw-int)) w)))))]
...
(defn resolve-percents [block-map pw-int ph-int]
  (into {}
        (for [[k v] block-map]
          (let [x (get-in v [:root 0])
                ...
                h (if (and (= h 0.99) (ut/is-float? h)) ;; override as 1.0
                    (js/Math.floor (* 1 ph-int)) h)
                w (if (and (= w 0.99) (ut/is-float? w)) ;; override as 1.0
                    (js/Math.floor (* 1 pw-int)) w)

                x (if (and (< x 1) (ut/is-float? x)) ;; treat as percent of total
                    (round-down-half (* x pw-int)) x)
                y (if (and (< y 1) (ut/is-float? y)) ;; treat as percent of total
                    (round-down-half (* y ph-int)) y)

                h (if (and (< h 1) (ut/is-float? h) (not (zero? h))) ;; treat as percent of total
                    (round-down-half (* h ph-int)) h)
                w (if (and (< w 1) (ut/is-float? w) (not (zero? w))) ;; treat as percent of total
                    (round-down-half (* w pw-int)) w)]
...
(defn process-layout [blocks-map pw-int ph-int]
  (let [ph-int (- ph-int 1)
        bks (reagent/atom {})
        statics (into {} (for [[k v] blocks-map
                               :when (and (> (get v :h) 1) (> (get v :w) 1))]
                           {k (assoc v :id k)}))
        percents (into {} (for [[k v] blocks-map
    ...
    ;(doseq [[k v] (sort-by)])
    ;(into (sorted-map-by (fn [[k v]] (apply + (get v :root)))) (resolve-percents dyns pw-int ph-int))
    ...
    (into {} (for [[k v] @bks] (if (= (+ (get v :h) (get-in v [:root 1])) ph-int)
