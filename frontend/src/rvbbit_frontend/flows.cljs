(ns rvbbit-frontend.flows
  (:require
    ...
    [day8.re-frame.undo :as    undo
                        :refer [undoable]]
    [garden.color :as    c
                  :refer [analogous color? complement hex->hsl hex? hsl hsl->hex hsla invert mix
                          rgb->hex rgb->hsl shades split-complement tetrad triad]]
    [goog.dom :as gdom]
...
(re-frame/reg-sub ::flowmap-raw-block
                  (fn [db [_ bid]] (get-in db [:flows (get db :selected-flow) :map bid] {})))
    ...
    (let [curr-flow-id        (get db :selected-flow)
      ...
      (if (not (some #(= new-bid %) block-ids)) ;; only if is unique, else do nothing
        (do (ut/tracked-dispatch
              [::http/set-flow-results
               (ut/postwalk-replacer pw-replace-map @(ut/tracked-subscribe [::http/flow-results]))])
            (-> db
                (assoc :selected-flow-block new-bid)
                (assoc-in [:flows curr-flow-id]
                          (ut/postwalk-replacer pw-replace-map (get-in db [:flows curr-flow-id])))))
        db))))

(re-frame/reg-sub ::flowmap-connections
                  (fn [db] (vec (get-in db [:flows (get db :selected-flow) :connections] []))))
                             ...
                             (for [[k v] (get-in db [:flows (get db :selected-flow) :map])]
                               {k (merge {:out :out}
                                         (let [oo (get-in v [:ports :out] {})]
                                           (if (map? (get oo :out)) (get oo :out) oo)))})) ;; weird
          ...
          clean-conns  (vec
                         (for [[o i] conns
                               :let  [ns-i?  (gns i)
                                      ...
                                      bid-o  (if ns-o? (keyword (gns o)) o)
                                      pid-o  (if ns-o? (keyword (gn o)) :out)
                                      valid? (or (and (or (get-in port-map-in [bid-i pid-i])
                                                          (= bid-i :done))
                                                      (get-in port-map-out [bid-o pid-o]))
                                                 (get-in db
                                                         [:flows (get db :selected-flow) :map bid-o
                                                          :cond pid-o]) ;; condi port
                                                 (get-in db
                                                         [:flows (get db :selected-flow) :map bid-o
                                                          :push pid-o]) ;; push port
                                             )
                                      _ (when (not valid?)
                                          (ut/tapp>> [:removed-invalid-connection [o i]
                                                      [[bid-i pid-i] [bid-o pid-o]]
                                                      (or (get-in port-map-in [bid-i pid-i])
                                                          (= bid-i :done))
                                                      (get-in port-map-out [bid-o pid-o])
...
(re-frame/reg-event-db ::refresh-live-dat
                       (fn [db _]
                         (let [kks (filter #(cstr/starts-with? (str %) ":live-dat-")
                                     (keys (get db :data)))]
    ...
    (for [[k v] fmap
      ...
      (cond (= ttype :query)
            ...
            (= ttype :param) (let [pfull (get-in v [:data :drag-meta :param-full])
                                   ddata @(ut/tracked-subscribe [::conn/clicked-parameter-key
                                                                 [pfull]])
                                   dtype (ut/data-typer ddata)
                                   full  (-> v
                                             (assoc-in [:data :user-input] ddata)
            ...
            (= ttype :open-block)
              (let [pfull (if (cstr/includes? (str (get-in v [:data :syntax] "clojure")) "clojure")
                            ...
                            (try (cstr/join "\n" (get-in v [:data :user-input]))
                                 (catch :default _
                                   (do (ut/tapp>> [:error-convering-string-types v :line 347
                                                   :flows-cljs])
  ...
  (let [canvas-key (into {}
        ...
        components-key
          (into
            {}
            (for [[k
                   {:keys [data ports view file-path flow-path raw-fn sub-flow-id flow-id
                           sub-flow]}]
                    flowmap ;; <-- flow-id refers to the subflow embed, not the parent
                  :let [ttype       (or (get-in data [:flow-item :type]) ;; ^^-- fid is the
                        ...
                        view-swap   (fn [obody flow-id bid push-key]
                                      (let [pkey      (keyword (str
                                                                 (ut/replacer (str push-key) ":" "")
                                                                 ">"))
                        ...
                        view-swaps  (fn [obody flow-id push-key-bid-pairs]
                                      (reduce (fn [body [push-key bid]]
                                        ...
                                        push-key-bid-pairs))
                        view        (when view
                                      (let [conns              (vec (filter #(cstr/includes?
                                                                               (str (first %))
                                                                               "/push-path")
                                                                      flowmaps-connections))
                                            push-key-bid-pairs (vec (for [[c1 c2] conns]
                                                                      [(keyword (last (ut/splitter
                                                                                        (str c1)
                                                                                        #"/")))
                                                                       c2]))
                                            view               (view-swaps view
                                                                           fid
                                                                           push-key-bid-pairs)]
                                        view))
              ...
              (cond (and (= ttype :open-block) (ut/ne? (get ports :in))) ;; open block with inputs
                    ...
                    (or (= ttype :open-block) (= ttype :cell) (= ttype :param))
                      {k (if (= (get data :syntax "clojure") "clojure")
                           ...
                           (get data :user-input))}
                    (= ttype :open-fn)
                      {k (merge (when view {:view view})
                                {:fn                raw-fn
                                 :raw-fn            raw-fn
                                 :default-overrides (get-in data [:flow-item :defaults] {})
                                 :inputs            (vec (keys (get ports :in)))})}
                    (= ttype :query) {k {:fn     (get data :user-input) ;'(fn [x] x) ;:raw-fn '(fn
                                                                        ;[x] x)
                                         :inputs (vec (keys (get ports :in)))}}
                    ...
                    (= ttype :sub-flow)
                      {k (-> (process-flowmap2 (get sub-flow :map) (get sub-flow :connections) fid)
                             ...
                             (assoc :flow-id flow-id))} ;; flow-id of the embdeeded flow, NOT the
                    :else {k {:fn-key            [fn-category fn-key]
                              :default-overrides (get-in data [:flow-item :defaults] {})
                              :inputs            (if false ;expandable-in?
                                                   [(vec (keys (get ports :in)))] ;; send as single
                                                   (vec (keys (get ports :in))))}})))
        components-key
          (into {} ;; deconstruct conditional paths to single condi key
                (for [[k v] flowmap]
                  (if (not (empty? (get v :cond)))
                    (let [ccond (into {}
                                      (for [[c1 c2] flowmaps-connections
                                            :let    [link  (keyword (str
                                                                      (ut/replacer (str k) ":" "")
                                                                      "/cond-path"))
        ...
        flowmaps-connections (vec (filter #(and (not (cstr/includes? (str (first %)) "/cond-path"))
                                    ...
                                    flowmaps-connections))
        server-flowmap
          {:canvas canvas-key :components components-key :connections flowmaps-connections}]
    server-flowmap))


(defn has-done?
  []
  (true? (some #(= % :done) (apply concat @(ut/tracked-subscribe [::flowmap-connections])))))

(re-frame/reg-event-db ::add-live-flow-subs
                       (fn [db [_ running-subs]]
                         (assoc db :flow-subs (vec (into running-subs (get db :flow-subs))))))
      ...
      (doall
        (let [flowmap              @(ut/tracked-subscribe [::flowmap])
              ...
              running-view-subs    (vec (for [[k v] comps
                                              :when (get v :view)]
                                          [flow-id
                                           (keyword (str (ut/replacer (str k) ":" "") "-vw"))]))
              running-subs         (vec (for [k (keys comps)] [flow-id k]))
              running-subs         (vec (into running-subs running-view-subs))
              watched?             (ut/ne? (get @(ut/tracked-subscribe
                                                   [::bricks/flow-watcher-subs-grouped])
                                                flow-id))
          ...
          (ut/tracked-dispatch
            [::wfx/request :default
             {:message     {:kind        :run-flow
                            ...
                            :file-image  {:flowmaps             @(ut/tracked-subscribe
                                                                   [::flowmap-raw])
...
(defn is-running?
  ...
  (let [;started? @(ut/tracked-subscribe [::flow-runner flow-id bid])
        ...
        server-running? (or (if only? false chans-open?)
                            @(ut/tracked-subscribe [::conn/clicked-parameter-key
                                                    [(keyword
                                                       (str "flow/" flow-id ">*running?"))]]))
...
(defn calc-block-widths-fn
  ...
  (let [safe-w      70 ;(- w 50)
        ...
        longesto    (try (apply max (map #(count (str %)) (conj (keys outputs) 10)))
                         (catch :default _ 0))
...
(defn draggable-pill
  [m _ element]
  (draggable-port element (get m :flow-id) (get m :from) m _ nil nil)) ;; stub
      ...
      (let [coords
              (for [[o i] conns
                    :let  [ns-i?          (gns i)
                           ...
                           ik             (keys ii)
                           ok             (keys oo)
                           [ix iy]        [(get-in flow-map [bid-i :x])
                                           (get-in flow-map [bid-i :y])]
                           [ox oy ow]     [(get-in flow-map [bid-o :x]) (get-in flow-map [bid-o :y])
                                           w]
...
(defn coord-cacher
  [conns flow-map]
  (let [results-hash (hash (get @(ut/tracked-subscribe [::http/flow-results])
                                @(ut/tracked-subscribe [::selected-flow])))
              ...
              (for
                [[o i] conns
                 :let  [ns-i?         (gns i)
                        ...
                        oo            (merge ooo oop)
                        ik            (keys ii)
                        ok            (try (into (into (vec (sort-by str (keys ooo)))
                                                       (vec (keys oop)))
                                                 (vec (keys oopp)))
                                           (catch :default e
                                             (do (ut/tapp>> [:error-sorting-keys e (keys oo)
                                                             :flows.cljs :ln 1148])
                                                 (into (into (vec (sort-by str (keys ooo)))
                                                             (vec (keys oop)))
                                                       (vec (keys oopp))))))
  ...
  (doall
    (for [[x1 y1 x2 y2 involved? color z1 z2 z1a z2a] coords]
      ...
      (let [flow-id       @(ut/tracked-subscribe [::selected-flow])
            ...
            selected-i    (get-in @flow-details-block-container-atom
                                  [flow-id flow-select :port-panel :in])
            selected-o    (get-in @flow-details-block-container-atom
                                  [flow-id flow-select :port-panel :out])
            not-selected? (true? (and (nil? selected-i) (nil? selected-o)))
            selected?     (true? (or (= selected-i [z2 z2a])
                                     (= selected-o [z1 z1a])
                                     (= selected-i [z1 z1a])
                                     (= selected-o [z2 z2a])))
...
(re-frame/reg-event-db ::remove-flowmap-bid
                       (fn [db [_ bid]]
...
(re-frame/reg-event-db
  ...
  (fn [db [_ bid direction]]
    (let [ports         (get-in db [:flows (get db :selected-flow) :map bid :ports direction] {})
          ...
          inputs        (try (edn/read-string (get-in db
                                                      [:flows (get db :selected-flow) :map bid :data
                                                       :flow-item :inputs]
                                                      []))
                             (catch :default _ []))
...
(defn zoom-to-element
  ...
  (let [element (js/document.getElementById element-id)]
    (when-let [z @db/zoomer] (.zoomToElement z element padding))))
...
(defn set-zoom-pan
  ...
  (when-let [z @db/zoomer] (.setTransform z positionX positionY scale 0)))
...
(re-frame/reg-sub
  ::get-flow-brick
  (fn [db _]
    (let [ordered1 (reverse (sort-by last
                                     (for [[k v] (get-in db [:flows (get db :selected-flow) :map])]
                                       [k (get v :x)])))]
...
(defn center-to-saved-coords
  ...
  (when-let [z @db/zoomer]
    (let [[positionX positionY scale] @(ut/tracked-subscribe [::zoom-start])]
      ...
      (js/setTimeout (fn [] (.zoomToElement z @(ut/tracked-subscribe [::get-flow-brick]) 0.8))
                     1000))))
...
(defn draggable-port
  ...
  [(reagent/adapt-react-class rdnd/Draggable)
   ...
   [re-com/box :size "auto" :child element :style
    {:cursor (if @dragging-port? "grabbing" "grab")}]])
   ...
   (let [data (let [] ;subkey (str flow-id ">" (ut/replacer (str bid) #":" ""))
     ...
     {:type          :flow-port
      ...
      :on-drag-start #(do (reset! dragging-port? true)
                          ...
                          (reset! bricks/dragging-body data))
      :data          (pr-str data)}) ;[bid pid]
   [re-com/box :size "none" :child element :style
    {:cursor (if @dragging-port? "grabbing" "grab")}]])
...
(defn remove-connections
  ...
  (let [flowmaps-connections @(ut/tracked-subscribe [::flowmap-connections])
        ...
        conns-count          (count (filter #(or (= (side-fn %)
    ...
    (if (not (nil? icon))
      (if (not (cstr/starts-with? (str icon) "http")) ;(cstr/includes? icon "zmdi")
        [re-com/md-icon-button :src (at) :md-icon-name icon :style
         {:color bcolor :font-size (px fsize)} :attr {}]
        [re-com/box :style {:margin-left "-10px"} :size "none" :width (px size) :child
...
(defn valid-drop-ports
  ...
  (let [filtered-inputs (filter #(or (try (some (fn [x] (= x (last @dragged-port))) (val %))
...
(re-frame/reg-event-db ::set-flow-opts
                       (fn [db [_ opts-map]]
    ...
    [re-com/v-box :size "none" :width (px (- width-int 24)) :height (px (- height-int 24)) :style
     ...
     [(if scrubber?
      ...
      [re-com/box :width "100%" :align :end :attr
       {:on-click #(swap! flow-details-block-container-atom assoc-in
                     [flow-id :opts-scrubber?]
                     (not scrubber?))} :style
       {:font-size    "11px"
        :padding-left "10px"
        :cursor       "pointer"
        :color        (str (theme-pull :theme/editor-outer-rim-color nil) 98)} :child
       (str "scrubber " (if scrubber? "on" "off"))]]]))
...
(defn reset-delay!
  [atom value delay-ms]
  (reset! atom value)
  (async/go (async/<! (async/timeout delay-ms)) (reset! atom nil)))

(defn set-delay!
  [atom value delay-ms]
  (async/go (async/<! (async/timeout delay-ms)) (reset! atom value)))
...
(re-frame/reg-sub
  ...
  (fn [_ [_ flow-id bid & [pid return-type]]]
    (let [flow-map       @(ut/tracked-subscribe [::flowmap])
          override-value (get-in @(ut/tracked-subscribe [::http/flow-results])
                                 [:return-maps (str flow-id) bid])
        ...
        (let [flow-select     @(ut/tracked-subscribe [::selected-flow-block])
              ...
              bcolor          (try (cond error?  "red"
                                         ...
                                         :else   (or (get styler :color nil)
                                                     (get (theme-pull :theme/data-colors
                                                                      db/data-colors)
                                                          dt
                                                          (get (theme-pull :theme/data-colors
                                                                           db/data-colors)
        ...
        (let [pid   (if (and port-multi? (= pid :out)) :* pid) ;; change up the default out for
              value (if (and port-multi? (not (= pid :*))) ;; if :* just pass the whole value
                      (try (let [idx? (cstr/starts-with? (str pid) ":idx")
                                 pkey (if idx?
                                        (edn/read-string (ut/replacer (str pid) ":idx" ""))
                                        pid)]
                             (get value pkey))
...
(re-frame/reg-sub
  ...
  (fn [db [_ bid pid]]
    {:meta    (get-in db [:flows (get db :selected-flow) :map bid :data :flow-item :meta pid] {})
     :default (get-in db
                      [:flows (get db :selected-flow) :map bid :data :flow-item :defaults pid]
                      {})}))
...
(defn step-ports
  ...
  (let [;;ports (into {} (for [e (range (+ 1 (rand-int 6)))] {(keyword (str "a" (rand-int
        ...
        outport-keys (into (into (vec (sort-by str (keys ports))) (keys condi-ports))
                           (keys push-ports))
        ports        (if (not input?)
                       (let [cports (into {}
                                          (for [n (into (keys condi-ports) (keys push-ports))]
                                            {n (get ports :out :any)}))]
                         (merge ports cports))
                       ports)
    ...
    [re-com/h-box :size "none" :height "15px" :justify :between :attr
     {:on-mouse-enter #(reset! port-hover2 bid)
      ...
      :on-mouse-leave #(reset! port-hover2 nil)} :align :center :style
     {:font-size "10px" :z-index 687} :children
     (for [[k v] (if input?
                   (sort-map-keys (vec (or inputs-vec (keys ports))) ports)
                   (sort-map-keys (try outport-keys ;(sort (keys ports))
                                       (catch :default e
                                         (do (ut/tapp>> [:key-sorting-error e outport-keys
                                                         :flows.cljs :ln 1802])
                                             outport-keys)))
                                  ports))
           :let  [hover-key [bid input? k (str " " k)]
                  ...
                  filled? (if input?
                            @(ut/tracked-subscribe [::get-incoming-port bid :in k])
                            (ut/ne? @(ut/tracked-subscribe [::get-outgoing-port bid
                                                            (if input? :in :out) k])))
                  multi? (cstr/ends-with? (str k) "+")
                  pval1 (when (> (count ports) 1)
                          @(ut/tracked-subscribe [::port-color flow-id (or ibid bid) (or ivk k)
                                                  :data-value]))
                  condi? (and (not input?) (cstr/starts-with? (str k) ":cond-path"))
                  push? (and (not input?) (cstr/starts-with? (str k) ":push-path"))
                  valve-open? (when condi?
                                @(ut/tracked-subscribe [::bricks/condi-valve-open? flow-id bid k])) ;; TODO
                  icon
                    [re-com/md-icon-button :src (at) :md-icon-name
                     ...
                     (if valid-drop?
                       {:on-drag-over  #(do ;(ut/tapp>> [:h hovered? hover-key @port-hover])
                        ...
                        :on-drop       #(let [connected-string (str (first @dragged-port)
                                                                    "/"
                                                                    (second @dragged-port))
                     ...
                     {:color       (if panel-hovered? port-color (str port-color 99)) ;(get
                      :cursor      (if (and (not (and input? has-default?)) input?)
                                     "pointer"
                                     "grab")
                      ...
                      :margin-left (cond valid-drop? "-5px"
                                         hovered? "-9px"
                                         panel-hovered? "-2px"
                                         (and @dragging-port? (not (= bid (first @dragged-port))))
                                           "3px"
                                         :else "inherit")
       ...
       [re-com/box :size "none" :align (if input? :end :start) :width (px port-width) :height "15px"
        :child
  ...
  (fn [db [_ bid]]
    (if (= (get-in db [:flows (get db :selected-flow) :map bid :data :drag-meta :type]) :open-block)
      (let [user-input   (get-in db [:flows (get db :selected-flow) :map bid :data :user-input])
            resolved-val @(ut/tracked-subscribe [::port-color (get db :selected-flow) bid :out
                                                 :data-value])]
        (not (= user-input resolved-val)))
      false)))

(re-frame/reg-sub ::syntax
                  (fn [db [_ bid]]
                    (get-in db [:flows (get db :selected-flow) :map bid :data :syntax] "clojure")))

(re-frame/reg-sub ::image-check
                  (fn [db _]
                    (vec (for [kp (ut/kvpaths db) :when (ut/is-base64? (get-in db kp))] kp))))
...
(defn step-center
  ...
  (let [hovered?      (and (= bid (first @port-hover)) (not @dragging-flow?))
        ...
        shades-color  (try (vec (for [s (shades bcolor)] (hsl->hex s)))
                           (catch :default _ ["#ffffff"]))
        syntax        @(ut/tracked-subscribe [::syntax bid])
        tetrad-color  (try (vec (for [s (tetrad bcolor)] (hsl->hex s)))
                           (catch :default _ ["#ffffff"])) ;; bad colors will crash it.
    ...
    [re-com/v-box :height (px (- h 36)) :size "none" :padding "4px" :justify :center :children
     [[re-com/h-box :height "21px" :style {:margin-top "-4px"} :justify :between :align :center
       :children
       [[re-com/box :size "auto" :attr {:on-mouse-down mouse-down-fn :on-click #(select-block bid)}
         :style
         {:font-size   "10px"
          :font-weight 700
          :color       (if selected? (first shades-color) (last shades-color))} :child
         (if (and @sniffy-sniff (= @port-hover2 bid))
           [re-com/h-box :size "auto" :justify :between :align :center :children
            (for [e [(if (first @sniffy-sniff) :in-port :out-port) (get @sniffy-sniff 1)]]
           ...
           (let [;pval (try (if (or (vector? pval) (map? pval) (list? pval))
             ...
             (cond rabbit-code-view? [re-com/box :padding "5px" :child
                   ...
                   rabbit-code? [re-com/box :padding "5px" :child
                                 [buffy/render-honey-comb-fragments pval (/ (- w 22) 50) (/ h 50) ; nil
                                                                                                  ; ;
                                                                                                  ; (/
                                                                                                  ; h
                                                                                                  ; 50)
                                 ]]
                   simple-value? (let [;ss (ut/auto-font-size pval (- h 50) (- w 25))
                                       ss 10]
                   ...
                   (and (try (not (empty? pval)) (catch :default _ false)) b64?)
                     [re-com/v-box :size "auto" :align :center :justify :center :children
                      [;[re-com/box :child (str "(**base64 string: ~" (.toFixed (/ (* (count (str
                       [re-com/box :child
                        [:img {:src (str "data:image/png;base64," pval) :width (px (- w 10))}]]]]
...
(defn flow-grid
  ...
  (let [selected-bid         @(ut/tracked-subscribe [::selected-flow-block])
        ...
        bounds-x             (when read-only-flow?
                               (apply min (for [[_ {:keys [x]}] flow-map :let []] x)))
        bounds-y             (when read-only-flow?
                               (- (apply min (for [[_ {:keys [y]}] flow-map :let []] y)) 30))
        bounds-x2            (when read-only-flow?
                               (apply max (for [[_ {:keys [x w]}] flow-map :let []] (+ w x))))
        bounds-y2            (when read-only-flow?
                               (apply max (for [[_ {:keys [y h]}] flow-map :let []] (+ y h))))
        done-data-type-color (when read-only-flow?
                               (get (theme-pull :theme/data-colors db/data-colors)
                                    (if (some #(= % :*)
                                              (keys (get-in flow-map [done-block :ports :out])))
                                      :map
                                      (gn (first (vals (get-in flow-map
                                                               [done-block :ports :out])))))
                                    (get (theme-pull :theme/data-colors db/data-colors)
                                         "unknown"
                                         "#FFA500")))]
    [re-com/h-box :size "none" :align :center :justify :center :width (px canvas-width) ;;"3200px"
     ...
     {:overflow         "hidden"
      ...
      :background-size  (when true ;zoom-unlocked?
                          "75px 75px")
      :border           (when zoom-unlocked?
                          (str "3px dashed " (theme-pull :theme/editor-outer-rim-color nil)))}
     :children
     (conj
       (for [[bid {:keys [x y h w z data icon ports]} :as fmap] flow-map
             :let
               [in-ports-map    (get ports :in)
         ...
         [re-com/v-box :attr
          ...
          {:position "fixed"
           ...
           :left x
           :top y} :children
          [[step-ports bid in-ports-map w true flow-id x y]
           ...
           (when selected?
             [re-com/box ;; close box
              :style {:position "fixed" :z-index 985 :size "none" :left (- (+ x w) 17) :top (- y 2)}
              :height "12px" :width "12px" :child
              [re-com/md-icon-button :src (at) :md-icon-name "zmdi-close" :on-click
               #(remove-block bid) :style
  ...
  (let [menu-pills ["thiessen" "missles" "hope" "cope" "rope"]]
    [re-com/h-box :children
     [(when (not @hide-menu-panel?)
        [re-com/v-box :style {:position "fixed" :left 0 :top 35 :z-index 700} :gap "5px" :width
         "110px" :children
         (for [p menu-pills]
           ^{:key (str "pills" p)}
      ...
      [re-com/box :style
       {:color    "#ffffff"
        :cursor   "pointer"
        :position "fixed"
        :left     (if @hide-menu-panel? 15 125)
        :top      300} :attr {:on-click #(reset! hide-menu-panel? (not @hide-menu-panel?))} :child
       (if @hide-menu-panel? ">" "<")]]]))
...
(defn get-flow-deps
  ...
  (let [;;all-sql-call-keys      @(ut/tracked-subscribe [::bricks/all-sql-call-keys])
        ...
        sql-aliases-used       @(ut/tracked-sub ::bricks/panel-sql-aliases-in-views-body
                                                {:panel-key panel-key :body body})
        valid-body-params      (vec (ut/deep-flatten @(ut/tracked-subscribe
                                                        [::bricks/valid-body-params-in body])))
        ...
        all-params             (into (into (into valid-body-params used-datasets)
                                           value-walks-targets)
                                     condi-walks-targets)
...
(defn flow-droppable
  ...
  (if true ;(not @dragging-port?)  ;; for. fucks. sake.
    [(reagent/adapt-react-class rdnd/Droppable)
     {:types types-vec
      :on-drag-enter
        #(when (or (and (= (get-in @bricks/dragging-body [:drag-meta :source-query]) :flows-sys)
                        @drop-toggle?)
      ...
      :on-drop
        #(when (or (empty? @port-hover) (nil? @port-hover)) ;; is not a port drop and intended
           (ut/tapp>> [:dropped? @port-hover @dragged-port types-vec root @bricks/dragging-body
                       element])
           (if (and (= (get-in @bricks/dragging-body [:drag-meta :source-query]) :flows-sys)
                    (not @drop-toggle?))
             (let [file-path @(ut/tracked-subscribe [::conn/clicked-parameter-key
                                                     [:flows-sys/file_path]])]
               (ut/tracked-dispatch [::http/load-flow file-path])
               (ut/tapp>> [:load-flow file-path]))
             (let [incoming (js->clj %)
                   _ (ut/tapp>> [:incoming? incoming (first incoming)
                                 (get (first incoming) "meta-menu") (get incoming "meta-menu")
                                 (read-string (last (last incoming)))])
                   ...
                   [x y] root
                   sub-flow-part? (not (empty? (get-in @bricks/dragging-body
                                                       [:flow-item :file-path])))
                   sub-flow-drop? (and ;(not from-port?)
                                    (or (and (= (get-in @bricks/dragging-body
                   ...
                   _ (ut/tapp>> [:dropeed-offsets root dm-type zoom-multi zoom-offset-x
                                 zoom-offset-y drop-x drop-y rooted-data])
                     ...
                     (cond
                       port-meta? (let []
                                    {:w     125
                                     ...
                                     :y     drop-y})
                       from-port?
                         (let [;kp (get-in rooted-data [:drag-meta :keypath])
                       ...
                       sub-flow-drop?
                         (let [;poss-inputs (vec (for [[k v] (get sub-flow :map)]
                               ...
                               base-conns  (set (for [[_ v2] (get sub-flow :connections)]
                                                  (keyword (gns v2))))
                       ...
                       flow-item ;; item picked from grid rows
                         (let []
                           ...
                           (merge {:w     200
                                  ...
                                  (if (= (get flow-item :type) :open-fn)
                                    {:raw-fn (get flow-item :fn)}
                                    {})))
                       (= dm-type :view) ;; prarm dragged from editor bar
                         (let [view-ref             (keyword
                                                      (str "view/" (gn (get-in rooted-data
                                                                               [:drag-meta
                                                                                :source-panel-key]))
                                                           "."     (gn (get-in rooted-data
                                                                               [:drag-meta
                                                                                :source-table]))))
                               panel-key            (get-in rooted-data
                                                            [:drag-meta :source-panel-key])
                               body                 @(ut/tracked-subscribe
                                                       [::bricks/resolve-view-alias view-ref])
                           ...
                           {:w     200
                            ...
                            :data  (-> @bricks/dragging-body
                                       ...
                                       (assoc-in [:drag-meta :keypath]
                                                 [(get-in rooted-data
                                                          [:drag-meta :source-panel-key]) :views
                       ...
                       (and (= dm-type :where) ;; cell value pull from query grid (acts as a
                         ...
                         (let [cell-ref (keyword
                           ...
                           {:w     200
                            ...
                            :icon  "zmdi-grid"
                            :ports {:in  {}
                                    :out {:out (keyword (ut/data-typer (first
                                                                         (resolver/logic-and-params
                                                                           [cell-ref]
                                                                           nil))))}}})
                       (= dm-type :query)
                         (let [panel-key            (get-in rooted-data
                                                            [:drag-meta :source-panel-key])
                               qid                  (get-in rooted-data [:drag-meta :source-table])
                               body                 @(ut/tracked-subscribe [::bricks/find-query
                                                                            qid])
                               res-params-map-types (get-flow-deps panel-key body)]
                           (ut/tapp>> [:qbody body])
                           {:w     200
                       ...
                       :else {:w     200
                              ...
                              :ports {:in {} :out {}}})
                   flow-id
                     (cond sub-flow-drop? (ut/safe-key
                                            (try (edn/read-string (get sub-flow :flow-id))
                                                 (catch :default _ :name-issue!))
                                            @(ut/tracked-subscribe [::conn/reserved-type-keywords]))
                           flow-item      (ut/safe-key (try (edn/read-string (get flow-item :name))
                                                            (catch :default _ :name-issue!))
                                                       @(ut/tracked-subscribe
                                                          [::conn/reserved-type-keywords]))
                   ...
                   check-port-drop (cond port-meta?   (ut/safe-key (keyword (str "<" src-port))
                                                                   safe-keys-reserved)
                                         get-in-drag? ;; use the get-in keypath to name
                                           (let [;kp-str (ut/replacer (str (apply (comp merge str)
                                                 ...
                                                 kp-str (-> kp-str
                                                            ...
                                                            (ut/replacer " " "")
                                                            (ut/replacer "'" ""))]
                                             (ut/safe-key (keyword (str src-block "_" kp-str))
                                                          safe-keys-reserved))
                                         :else        (ut/safe-key (keyword
                                                                     (str src-block "_" src-port))
                                                                   safe-keys-reserved))]
               (reset! dragging? false)
               (reset! bricks/dragging? false)
               (cond false "hey"
                     :else (do ;;(ut/tapp>> [:flow-inherits-drag-body? @bricks/dragging-body data
                             (conn/add-flow-block drop-x
                                                  drop-y
                                                  flow-body
                                                  (if from-port? check-port-drop flow-id)
                                                  browser-pull?)
                             (when from-port?
                               (if port-meta? ;; in to out
                                 (connect-ports ;(keyword (if (= src-port "out") src-block (str
                                   check-port-drop ;(keyword (str (ut/replacer (str
                                   (keyword (if (= src-port "out")
                                              src-block
                                              (str src-block "/" src-port))))
                                 (connect-ports
                                   (keyword
                                     (if (= src-port "out") src-block (str src-block "/" src-port)))
                                   (keyword (str (ut/replacer (str check-port-drop) ":" "")
                                                 "/x"))))))))))} [re-com/box :child element]]
    element))
...
(defn smooth-scroll-to-element
  ...
  (let [container      (gdom/getElement container-id)
        ...
        element-left   (- (.-offsetLeft element)
                          (if @(ut/tracked-subscribe [::bricks/flow-editor?]) 640 40))]
      ...
      (let [start    (atom nil)
        ...
        (letfn [(step [timestamp]
                  ...
                  (let [progress        (/ (- timestamp @start) duration)
                        flow-select     @(ut/tracked-subscribe [::selected-flow-block])
                        new-scroll-left (+ container-left
                                           (* progress (- element-left container-left)))]
...
(defn scroll-to-element
  ...
  (let [container      (gdom/getElement container-id)
        ...
        container-left (.-scrollLeft container)
        element-left   (- (.-offsetLeft element)
                          (if @(ut/tracked-subscribe [::bricks/flow-editor?]) 610 10))]
     ...
     [(reagent/adapt-react-class cm/UnControlled)
      {:value   value
       :onBlur  #(try (let [new-vals (ut/cm-deep-values %)]
                      ...
                      (catch :default _
                        (ut/tapp>> [:issue-saving bid :text-rw (ut/cm-deep-values %)])))
       :options {:mode              "text"
                 :lineWrapping      true
                 :lineNumbers       false ;true
                 :matchBrackets     true
     ...
     [[(reagent/adapt-react-class cm/UnControlled)
      ...
      (when using-opts?
        [re-com/box :style {:opacity 0.5} :child
         [code-box width-int nil
          (str ";; using flow opts:  " (pr-str (select-keys opts uses)))]])]]))
...
(defn code-box-condi
  ...
  (let [value-keys  (filter keyword? (ut/deep-flatten value))
    ...
    [re-com/v-box :size "none" :width (px (- width-int 24)) :height (px (- height-int 24)) :style
     ...
     [[(reagent/adapt-react-class cm/UnControlled)
       {:value   (ut/format-map (- width-int 50) (str value))
        :onBlur  #(ut/tracked-dispatch [::edit-condi-port bid name
     ...
     [re-com/v-box :size "none" :height (px (- height-int 24)) :style
      ...
      [[re-com/box :width (px (- width-int 24)) :child
        [(reagent/adapt-react-class cm/UnControlled)
         {:value   (ut/format-map (- width-int 50) (str value))
          :onBlur  #(do (ut/tapp>> [:edit-view-fn bid (cstr/join " " (ut/cm-deep-values %))
                        ...
                        (ut/tracked-dispatch [::edit-view-fn bid
                                              (read-string (cstr/join " " (ut/cm-deep-values %)))]))
          :options {:mode              "clojure"
                    :lineWrapping      true
                    :lineNumbers       true
       ...
       (let [condi-ports @(ut/tracked-subscribe [::push-ports bid])
             ...
             open?       (true? (get-in @flow-details-block-container-atom
                                        [flow-id bid :push-paths :open?]
                                        false))]
         [re-com/v-box :style
          {:background    "#00000055"
           :padding-top   "8px"
          ...
          [[re-com/h-box :size "auto" :padding "4px" :justify :between :align :center :style
            ...
            [[re-com/h-box :align :center :justify :center :style {:padding-left "3px"} :gap "5px" ;:style
              ...
              {:on-click #(swap! flow-details-block-container-atom assoc-in
                            [flow-id bid :push-paths :open?]
                            (not open?))} :children
              [[re-com/md-icon-button :md-icon-name "fa-solid fa-carrot" :style
                {:font-size "17px" :cursor "pointer"}]
               [re-com/box :child (str "push paths (" cnt ")")]]]
             [re-com/md-icon-button :on-click #(ut/tracked-dispatch [::add-push-port bid])
              :md-icon-name "zmdi-plus" :style {:font-size "22px" :cursor "pointer"}]]]
           (when (and open? (> cnt 0)) [re-com/gap :size "8px"])
           (when (and open? (> cnt 0))
             [re-com/v-box :style {:border-radius "8px" :background-color "#00000044"} :gap "8px"
              :padding "4px" :children
              (for [[k {:keys [dest]}] condi-ports
                    :let               [select-vec [bid k]
                                        valve-open? false ;@(ut/tracked-subscribe
                                        selected? (or (and @sniffy-sniff
                                                           (= @port-hover2 bid)
                                                           (= k (get @sniffy-sniff 1)))
                                                      (= select-vec selected))
                                        is-first? (= k :push-path)
                                        short-code
                                          (str ";; use for an on-click fn                      ["
                                               k
                                               "> \"*some-value*\"] or  "
                                               (when is-first? "  ")
                                               "          ["
                ...
                [re-com/v-box :padding "4px" :attr
                 ...
                 [[re-com/h-box :size "auto" :justify :between :align :center :style
                   ...
                   [[re-com/box :style {:font-size "15px" :font-weight 700} :child (str k)]
                    [re-com/h-box :width "300px" :height "100%" :justify :between :align :center
                     :children
                     [[re-com/v-box :children (for [d dest] [re-com/box :child (str d)])]
                      [re-com/md-icon-button :src (at) :md-icon-name "zmdi-close" :on-click
                       #(ut/tracked-dispatch [::remove-push-port bid k]) :style
          ...
          #(if stringify?
             (let [ddata     (ut/cm-deep-values %)
                   ...
                   input-map (into {}
                                   (for [e    (range (count inputs))
                                         :let [name (keyword (ut/replacer (str (ut/safe-name
                                                                                 (get inputs e)))
                                                                          "/"
                                                                          "-"))]]
                                     {name :any}))]
               ...
               (ut/tracked-dispatch [::update-flowmap-key-in bid [:data :flow-item :inputs]
                                     (vec inputs)])
             ...
             (try
               (let [ddata           (read-string (cstr/join " " (ut/cm-deep-values %)))
                     ...
                     flow-id         @(ut/tracked-subscribe [::selected-flow])
                     existing-inputs (get-in @(ut/tracked-subscribe [::flowmap])
                                             [flow-id bid :ports :in])
                 ...
                 (ut/tracked-dispatch [::clean-connections]))
               (catch :default e
                 (ut/tapp>> [:saving-issue :code-box-rwo bid (str e) :parse-issue?]))))
        :options {:mode              syntax ;"clojure"
                  :lineWrapping      true
                  :lineNumbers       true
                  :matchBrackets     true
       ...
       [;[re-com/box :child "header1"]
        [re-com/h-box :gap "3px" :align :center :justify :center :children
         (into
           (if icon
             (cond
               (string? icon) [[re-com/md-icon-button :src (at) :md-icon-name icon :style
                                {:color     (if open?
                                ...
                                (vec
                                  (for [i icon]
                                    [re-com/md-icon-button :src (at) :md-icon-name i :style
                                     {:margin-left "-1px"
                                      :color       (if open?
                                                     (str (theme-pull :theme/editor-outer-rim-color
                                                                      nil)
                                                          99)
                                                     (str (theme-pull :theme/editor-outer-rim-color
                                                                      nil)
        ...
        [re-com/md-icon-button :src (at) :md-icon-name "zmdi-more" :on-click
         #(swap! flow-details-block-container-atom assoc-in [flow-id bid title :open?] (not open?))
         :style
         {:color     (if open?
...
(defn flow-details-block
  ...
  (let [;honeycomb? (try (and (keyword? (first body)) (vector? body)) (catch :default _ false))
        ...
        flow-id         @(ut/tracked-subscribe [::selected-flow])
        cvalue          (get-in @(ut/tracked-subscribe [::http/flow-results])
                                [:return-maps flow-id id]
                                "(not run yet)")
        error?          (get cvalue :error false)
        styler          (get-in fmap [:data :flow-item :style] {})
        outputs         (get-in fmap [:ports :out])
        override-value  (get-in @(ut/tracked-subscribe [::http/flow-results])
                                [:return-maps (str flow-id) id])
        outputs         (if (and override-value (= (vec (keys outputs)) [:out])) ;; if we have
                                                                                 ;; return
                          {:out (keyword (ut/data-typer override-value))} ;; change output data
                                                                          ;; types
                          outputs)
        ...
        block-renamer   (if (= @rename-block id)
                          [re-com/input-text :src (at) :model str-id ;(str id)
                           ...
                           #(do (if (= str-id (str %)) ;; regex doesnt allow : so we need to
                                  ...
                                  (do (ut/tracked-dispatch [::rename-block id %])
                                      (reset! rename-block nil)))) :validation-regex flow-id-regex
                          ...
                          [re-com/box :attr
                           (if selected? {:on-double-click #(reset! rename-block id)} {}) :child
                           (str id)])
        output-toggle   [re-com/md-icon-button :src (at) :md-icon-name "zmdi-more" :on-click
                         #(ut/tracked-dispatch [::flow-block-meta id :view-output?
                                                (not show-output?)]) :style
        ...
        default-output  [re-com/v-box :gap "9px" :padding "8px" :width (px ccw) ;"350px"
                         :height "380px" :size "none" :align :center :justify :center :style
                         {:margin-top "-9px"} :children
                         [;[re-com/box :child (str id " (viewer)")]
                          [re-com/h-box :width "350px" :size "none" :padding "6px" :justify :between
                           :align :center :padding "8px" :style {:margin-top "17px"} :children
                           [;[re-com/box :child (str id)]
                            block-renamer [re-com/box :child "(viewer)"]]]
                          [render-icon (get fmap :icon) ccolor]
                          [re-com/box :padding "6px" :style {:font-size "11px"} :child
                           (get-in fmap [:data :flow-item :description] " ")]
                          (cond (and (vector? return-val) (keyword? (first return-val))) ;; try to
                                                                                         ;; render
                                                                                         ;; as
                                  [re-com/box :size "none" :height "280px" :width (px ccw) ;"350px"
                                   :style {:overflow "auto"} :child
                                   [buffy/render-honey-comb-fragments return-val 7 5]]
                                (vector? return-val)
                                  [re-com/box :size "none" :height "280px" :width (px ccw) ;"350px"
                                   :style {:overflow "auto"} :child
                                ...
                                (map? return-val)
                                  [re-com/box :size "none" :height "280px" :width (px ccw) ;"350px"
                                   :style {:overflow "auto"} :child
                                   [map-boxes2 return-val :sub-flows-used nil [] :output "map"]]
                                :else
                                  [re-com/box :size "none" :height "280px" :width (px ccw) ;"350px"
                                   :child
                                   [code-box ccw 280
                                    (if (string? return-val) (pr-str return-val) return-val)]])]]]
    [re-com/v-box :attr
     {:id           (str id)
      :on-drag-over #(when @bricks/swap-layers? (reset! bricks/swap-layers? false))
      :on-click     #(select-block id)} :height (px cch) :size "none" :padding "10px" :width
     (px ccw) :align :center :justify :center :style
     ...
     [;; flow-details-block-container [content title flow-id bid]
      (cond
        ...
        (= ttype :open-fn) [re-com/v-box :size "none" :gap "9px" :padding "6px" :align :center
                            :justify :center :width (px ccw) ;"350px"
                            :height "380px" :style {:margin-top "-5px"} :children
                            [[re-com/h-box :width "350px" :size "none" :justify :between :align
                              :center :padding "8px" :style {:margin-top "17px"} :children
                              [;[re-com/box :child (str id)]
                               block-renamer [re-com/box :child "clojure"]]]
                             [re-com/box :child
                              [code-box-fn ccw 378 (str (get fmap :raw-fn "(fn [x] x)")) id]]]]
        (= type :sub-flow)
          (let [;flowmaps-connections @(ut/tracked-subscribe [::flowmap-connections])
                subby-results   (get-in @(ut/tracked-subscribe [::http/flow-results])
                                        [:return-maps])
                sub-flow-lookup (into {}
                                      (for [k (keys subby-results)]
                                        {(keyword (last (ut/splitter (str k) "/"))) k}))
                sfl             (get sub-flow-lookup id)]
            [re-com/box :height "380px" :size "none" :style
             {;:border "1px solid pink"
              :overflow "auto"} :child
             [re-com/v-box :width (px ccw) :height "380px" :size "none" :align :center :justify
              :between :children
              [[re-com/h-box :width "440px" :justify :between :align :center :children
                [[re-com/box :child (str type) :style {:color out-color}]
                 [re-com/box :child (str sfl) :style {:color out-color}]] :padding "6px"]
                 ...
                 (vec (for [i (filter #(not (= (str %) (str flow-id))) (keys subby-results))]
                        [re-com/v-box :children
                         [[re-com/box :child (str i) :style {:font-size "19px"}]
                          [map-boxes2 (get subby-results i) :sub-flows-used nil [] :sub-flows-used
                           "map"]]]))]]]]])
        (= type :view)
          (let [qid  (get-in fmap [:data :drag-meta :source-table])
                body (get-in fmap [:data :drag-meta :body])]
            [re-com/box :width "500px" :height "380px" :size "none" :style
             {;:border "1px solid pink"
              :overflow "auto"} :child
             [re-com/v-box :width "495px" :height "400px" :size "auto" :align :center :justify
              :between :children
              [[re-com/h-box :width "490px" :justify :between :align :center :children
               ...
               [re-com/box :align :center :justify :center :width "490px" :height "360px" :size
                "none" :child [buffy/render-honey-comb-fragments body 9 7]]]]])
        (= type :open-block)
          (let [;edit-style :fragment
            ...
            [re-com/v-box :size "none" :gap "9px" :padding "6px" :align :center :justify :center
             :width (px width) :height "380px" :style
             {;:border "1px solid green"
              :margin-top "-5px"} :children
             [[re-com/h-box :width (px width) :size "none" :justify :between :align :center :padding
               "8px" :style {:margin-top "17px"} :children
               [;[re-com/box :child (str id)]
              ...
              [re-com/h-box :width (px width) :height "40px" :size "none" :justify :between :align
               :center :gap "8px" :children
               [[re-com/h-box :gap "8px" :style {:font-size "11px"} :children
                 (for [e    widths
                       :let [selected? (= e width)]]
                   [re-com/box :attr
                    (when (not selected?)
                      {:on-click #(ut/tracked-dispatch [::flow-block-meta id :width e])}) :style
                    (if selected? {} {:opacity 0.4 :cursor "pointer"}) :child (str e)])]
                [re-com/h-box :gap "8px" :style {:font-size "11px"} :children
                 (for [e    syntaxes
                       :let [selected? (= e syntax)]]
                   [re-com/box :attr
                    (when (not selected?)
                      {:on-click #(ut/tracked-dispatch [::flow-block-meta id :syntax e])}) :style
                    (if selected? {} {:opacity 0.4 :cursor "pointer"}) :child (str e)])]]]]])
        ...
        (= type :query) (let [qid           (get-in fmap [:data :drag-meta :source-table]) ;; original
                          ...
                          [re-com/box :width "500px" :height "380px" :size "none" :style
                           {;:border "1px solid pink"
                            :overflow "auto"} :child
                           [re-com/v-box :width "495px" :height "368px" :size "none" :align :center
                            :justify :center :children
                            [[re-com/h-box :width "490px" :justify :between :align :center :children
                              [[re-com/box :child (str qid) :style {:color out-color}]
                               [re-com/box :child (str type) :style {:color out-color}]] :padding
                              "6px"] [buffy/render-honey-comb-fragments packed-query]
               ...
               {:data       {:values :live-dat}
                ...
                :encoding   {:x       {:field    :start
                             ...
                             :tooltip [{:field    "start"
                                       ...
                                       {:field    "ms"
                                        :type     "temporal"
                                        :timeUnit "minutessecondsmilliseconds"} {:field "type"}
                                       {:field "path"} {:field "dest"}
                                       {:field "end"
                                        :type "temporal"
                                        :timeUnit
    ...
    (if (not @title-edit-idx)
      [re-com/box :size "none" :height "45px" :width (px w) :align :center :justify :end :attr
       ...
       {:cursor        "pointer"
        ...
        :font-size     "30px"} :child (str flow-id)]
      [re-com/input-text :src (at) :model (str flow-id) :width (px w) :height "45px" :on-change
       #(do (ut/tracked-dispatch [::rename-flow flow-id %]) (reset! title-edit-idx nil))
       :validation-regex flow-id-regex :change-on-blur? true :style
...
(defn run-history-bar
  ...
  [re-com/box :style {:margin-left "25px" :margin-top "15px"} :child
   [buffy/render-honey-comb-fragments
    {:queries {:runstream-chart-simple-python-exec {:select        [[[[:min :elapsed]] :elapsed]
                                                                    :started]
     ...
     :view    [:> :ResponsiveContainer {:width "100%" :height :panel-height+50}
               [:> :BarChart
                ...
                [:> :CartesianGrid {:strokeDasharray "1 4" :opacity 0.33}] [:> :Tooltip]
                [:> :XAxis {:dataKey :started}]
                [:> :Bar
                 {:dataKey :elapsed
                  :stroke  :theme/editor-outer-rim-color
                  :fill    :theme/editor-outer-rim-color}]]]} 5.5 11]])
...
(defn settings-block
  ...
  (let [flow-select         @(ut/tracked-subscribe [::selected-flow-block])
    ...
    (cond
      (= ttype :run-history)
        (let [caller @(ut/tracked-subscribe [::conn/clicked-parameter-key
                                             [:virtual-panel/client_name]])
              status @(ut/tracked-subscribe [::conn/clicked-parameter-key
                                             [:virtual-panel/return_status]])
              dropdown1
                {:view    [:dropdown
              ...
              dropdown2
                [:dropdown
                 {:choices     [{:id 1 :label "error"} {:id 0 :label "success"}
                                {:id -1 :label "timeout"}]
                  :width       "200px"
              ...
              waffle1 {:view    [:nivo-waffle-chart
                       ...
                       :queries {:waffle-hour   {:select        [[[:cast [:substr :start_ts 11 3]
                                                                   :text] :id]
                                                                 [[[:count 1]] :value]]
                                                 :connection-id "flows-db"
                                 ...
                                 :waffle-totals {:select [[:sum :value] :summy]
                                                 :from   [{:select        [[[:cast
                                                           ...
                                                           :from          [[:fn_history :cc393]]
                                                           :group-by      [1]}]}}}
              viz1 {:queries {:flow-history-calendar-sys* {:select [[[[:count [:distinct :run_id]]]
                                                                     :value]
              ...
              vselected? @(ut/tracked-subscribe [::conn/clicked-parameter-key
                                                 [:virtual-panel/flow-day]])
              grid-menu
                {:select        [:flow_id [[:count [:distinct :run_id]] :runs]]
                 ...
                 :where         [:and
                                 (if vselected?
                                   [:= [:substr :start_ts 0 11] :virtual-panel/flow-day]
                                   [:= 1 1]) (if caller [:= :client_name (str caller)] [:= 1 1])
              ...
              selected? @(ut/tracked-subscribe [::conn/clicked-parameter-key
                                                [(keyword (str gm-kw "/flow_id"))]])
              grid1
                {:select        [:flow_id :run_id :start_ts [:elapsed_seconds :seconds]
                                 :human_elapsed
                 ...
                 :where         [:and
                                 (if selected?
                                   [:= :flow_id (keyword (str gm-kw "/flow_id"))]
                                   [:= 1 1])
                                 (if vselected?
                                   [:= [:substr :start_ts 0 11] :virtual-panel/flow-day]
                                   [:= 1 1]) (if caller [:= :client_name (str caller)] [:= 1 1]) ;; since
                                                                                                 ;; we
                                 (if status [:= :in_error :virtual-panel/return_status] [:= 1 1])]
                 :order-by      [[:start_ts :desc]]
                 :from          [[:flow_history :tt336a]]}
              callout1
                {:select        [[[:count [:distinct :run_id]] :cnt]]
                 :connection-id "flows-db"
                 :where         [:and
                                 (if selected?
                                   [:= :flow_id (keyword (str gm-kw "/flow_id"))]
              ...
              selected-run-id @(ut/tracked-subscribe [::conn/clicked-parameter-key
                                                      [(keyword (str grid-kw "/run_id"))]])
              selected-start-ts @(ut/tracked-subscribe [::conn/clicked-parameter-key
                                                        [(keyword (str grid-kw "/start_ts"))]])
              ...
              grid2 {:select        [:*]
                     ...
                     :where         [:and
                                     (if run-selected?
                                       [:= :run_id (keyword (str grid-kw "/run_id"))]
                                       [:= 1 0])]
                     :order-by      [[:start_ts :desc]]
          ...
          [re-com/v-box :children
           [[re-com/h-box :size "auto" :height "60px" :width "750px" :justify :between :align
             :center :gap "40px" :style {:font-size "15px"} :children
            ...
            [re-com/h-box :children
             [[re-com/box :size "none" :align :center :justify :center :style {:font-size "15px"}
               :child
               [buffy/render-honey-comb-fragments grid-menu (* (/ dyn-width 50) 0.25) 10
                "grid-menu-sys*"]]
              [re-com/box :size "none" :align :center :justify :center :style {:font-size "15px"}
               :child
               [buffy/render-honey-comb-fragments grid1 (* (/ dyn-width 50) 0.75) 10
                "grid1-sys*"]]]]
            [buffy/render-honey-comb-fragments
             [:h-box :size "auto" :justify :between :align :center :gap "10px" :padding "8px" :style
              ...
              [;"filters: "
               [:string :virtual-panel/flow-day] [:string [(keyword (str gm-kw "/flow_id")) " "]]
               caller [:string [(keyword (str grid-kw "/run_id")) " "]]]] (- (/ dyn-width 50) 1) 6
             "filter-row-sys*"] [re-com/gap :size "10px"]
            (when run-selected?
              [flow-details-block-container "flow block results log*" :system :system
               [re-com/box :size "none" :align :center :justify :center :style {:font-size "15px"}
                :child
                [buffy/render-honey-comb-fragments grid2 (- (/ dyn-width 50) 1)
            ...
            (when run-selected?
              [flow-details-block-container "flow channel results log*" :system :system
               [re-com/box :size "none" :align :center :justify :center :style {:font-size "15px"}
                :child
                [buffy/render-honey-comb-fragments grid3 (- (/ dyn-width 50) 1)
                 (- (/ panel-height-bricks 2) 1) ;;10
                 "grid3-sys*"]] ; flow-id orderb true 570 nil ;366
               "zmdi-dns"]) [re-com/gap :size "10px"]
            (let [pval @(ut/tracked-subscribe [::conn/clicked-parameter-key
                                               [(keyword (str grid2-kw "/value"))]])
                  blk  @(ut/tracked-subscribe [::conn/clicked-parameter-key
                                               [(keyword (str grid2-kw "/block"))]])
                  flw  @(ut/tracked-subscribe [::conn/clicked-parameter-key
                                               [(keyword (str grid2-kw "/flow_id"))]])
                ...
                [re-com/box :height "300px" :size "none" :style
                 ...
                 (when pval
                   (if (or (map? pval) (vector? pval))
                     [map-boxes2 pval blk (str flw " / " blk) [] :output
                      (if (vector? pval) "vector" "map") true]
                     [re-com/box :child
                      (pr-str @(ut/tracked-subscribe [::conn/clicked-parameter-key
                                                      [(keyword (str grid2-kw
                                                                     "/value"))]]))]))]))]])
      (= ttype :flow-browser) [re-com/h-box :children
                               [[re-com/box :size "auto" :child
                                 [bricks/magic-table :flow-list* [:flows-sys] 12 19 []]]]]
      (= ttype :part-browser) [re-com/h-box :children
                               [[re-com/box :size "auto" :child ;[bricks/magic-table
                                 ...
                                 [bricks/magic-table :flow-cat-list* [:flow-fn-categories-sys] 3 19
                                  []]]
                                [re-com/box :size "auto" :child
                                 [bricks/magic-table :flow-fn-list* [:flow-fn-sys] 9 19
                                  [:full_map :category :file_path]]]]]
      (= ttype :debug) (if flow-select
                         [re-com/box :child
                          [(if read-only-flow? code-box code-box-rw) 600 440
                           (get flowmaps flow-select) flow-select] :width "600px" :height "430px"
                          :size "1"]
      ...
      (= ttype :scheduler)
        (let [grid1       {:select        [:flow_id :override :schedule :ts]
              ...
              comps       (keys flowmaps)
              flow-names  (vec (distinct (map :flow_id
                                           @(ut/tracked-subscribe [::conn/sql-data [:flows-sys]]))))
              ddown       (fn [x xdef w kkey]
                            [re-com/box :size "none" :align :center :width (px (or w 100)) :child
                             [re-com/single-dropdown :width (px (or w 100)) :style
                              {:border    (str "1px solid "
                               ...
                               :color     "inherit"
                               :focus     {:outline "none"}
                               :font-size "18px"} :parts {:chosen-drop {:style {:color "green"}}}
                              :choices (vec2choices x) :on-change
                              #(swap! scheduler-atom assoc-in [flow-id kkey] %) :model
                              (get-in @scheduler-atom [flow-id kkey] xdef)]])
              date-input  (fn [input-regex w dd kkey]
                            [re-com/input-text :src (at) :model
                             (get-in @scheduler-atom [flow-id kkey] dd) :width (px (or w 90))
                             :on-change #(swap! scheduler-atom assoc-in [flow-id kkey] %)
                             :change-on-blur? true :style
                             {;:font-size "20px" :margin-top "28px"
                              :opacity          (if (nil? (get-in @scheduler-atom [flow-id kkey]))
                                                  0.4
                                                  1.0)
                              :font-size        "16px"
          ...
          [re-com/v-box :align :center :justify :center :children
           [[re-com/h-box :size "none" :width "595px" :padding "10px" :height "55px" :align :center
             :gap "6px" :style
             {:color      (theme-pull :theme/editor-outer-rim-color nil)
              :margin-top "15px"
              :font-size  "20px"} :children
             [[re-com/md-icon-button :src (at) :md-icon-name "zmdi-timer" :style
               {:font-size "26px"}]
              [re-com/box :child "every" :style
               {:opacity (if (nil? (get-in @scheduler-atom [flow-id :every])) 0.4 1.0)}]
              [date-input #"^\d{2}$" 45 "10" :every]
              [ddown ["seconds" "minutes" "hours" "days"] "hours" 110 :period]
              [re-com/box :child "starting" :style
               {:opacity (if (nil? (get-in @scheduler-atom [flow-id :starting])) 0.4 1.0)}]
              [date-input #"^\d{0,4}-?\d{0,2}-?\d{0,2}$" 130 (ut/today) :starting]
              [re-com/box :child "at" :style
               {:opacity (if (nil? (get-in @scheduler-atom [flow-id :at])) 0.4 1.0)}]
              [date-input #"^\d{4}$" 70 "1500" :at]]]
            [re-com/h-box :size "none" :width "595px" :padding "10px" :height "44px" :align :center
             :gap "6px" :style
             {:color (theme-pull :theme/editor-outer-rim-color nil) :opacity 0.2 :font-size "20px"}
             :children
             [[re-com/md-icon-button :src (at) :md-icon-name "zmdi-alarm-plus" :style
               {:font-size "26px"}] [re-com/box :child "when"]
              [ddown flow-names (first flow-names) 233 :trigger-flow-id] [re-com/box :child "is"]
            ...
            [re-com/h-box :size "none" :width "595px" :padding "10px" :height "44px" :align :center
             ...
             [[re-com/md-icon-button :src (at) :md-icon-name "zmdi-hearing" :style
               {:font-size "26px"}] [re-com/box :child "called with "]
              [date-input #"." 158 nil :trigger-words] [re-com/box :child "sent to"]
              [ddown comps (first comps) 180 :trigger-word-insert]]]
            [re-com/h-box :justify :between ;:align :center
             :padding "10px" :children
             [[re-com/box :size "auto" :padding "6px" :style
               {:border "0px dashed lime" :opacity 0.22} :child (str @scheduler-atom)]
              [re-com/gap :size "20px"]
              [re-com/h-box :gap "14px" :children
               (for [b ["+" "schedule!"]]
                 [re-com/box :padding "5px" :align :center :justify :center :min-width "30px"
                  :height "36px" :attr
                  (if (= b "schedule!")
                    {:on-click
                       #(do (ut/tracked-dispatch
                  ...
                  {:border           (str "2px solid "
                   ...
                   :font-size        "18px"
                   :color            (theme-pull :theme/editor-outer-rim-color nil)}])]] :height
             "58px" :width "565px" :size "none"]
      ...
      (for [[fid v] ss ;; could use key destructuring, but some keys are weird. meh, its fine w
        ...
        [re-com/v-box :style
         ...
         [[re-com/h-box :padding "4px" :height "35px" :align :center :justify :between :children
           [[re-com/box :child
             (if (cstr/includes? (str fid) "/")
               (let [spl    (ut/splitter (str fid) #"/")
                     ...
                     sub    (last spl)]
                 [re-com/v-box :children
                  [[re-com/box :child (str "(sub-flow called from) " parent) :style
                    {:font-size "9px"}] [re-com/box :child (str sub)]]])
               (str fid)) :width "50%" :style
             {:padding-left "4px" :font-size "15px" :font-weight 700}]
            [re-com/box :child
          ...
          [re-com/h-box :padding "4px" :height "35px" :align :center :style
           {:font-size "10px" :padding-right "20px"} :justify :between :children
           [[re-com/box :child
             (if process? (str "(external process) " command) (str "started by: " started-by))
             :style {:padding-left "4px" :font-weight 300}]
...
(defn flow-editor
  ...
  (let [react-hack [@editor-mode @trig-atom-test @db/flow-editor-system-mode]
        ...
        flow-id    @(ut/tracked-subscribe [::selected-flow])
        dyn-width  (last @db/flow-editor-system-mode)
        o-modes    [["flows running" 800] ["flow browser" 600] ["flow parts" 600] ["metrics" 990]
                    ["KPIs" 990] ["signals" 990] ["rules" 990] ["flow history" 1200]]
        sql-calls  {:flow-fn-categories-sys {:select   [:category]
                                             :from     [:flow_functions]
                                             :group-by [1]}
                    :flow-fn-all-sys        {:select [:name] :from [:flow_functions] :group-by [1]}
                    :flow-fn-sys            {:select     [:name :description :full_map :inputs :icon
                                                          :input_types :output_types :category]
                                             :from       [:flow_functions]
                                             :col-widths {:name 215 :description 500}
                                             :where      (if (get sql-params
    ...
    [re-com/v-box :size "none" :width (px w) :height (px h) :gap "10px" :style
     ...
     [[re-com/h-box :size "none" :height "40px" :width (px w) :align :start :justify :between :style
       ...
       [[re-com/h-box :align :center :justify :center :style {:font-size "14px"} :size "auto"
         :children
         (for [o    o-modes
           ...
           [re-com/box :height "50px" :align :center :justify :center :size "auto" :attr
            ...
            {;:border "1px solid white"
             ...
             :background-color (if selected?
                                 ...
                                 "#00000000")} :child (str (first o))])] [re-com/gap :size "10px"]]]
      [re-com/gap :size "8px"]
      (cond
        (= (first @db/flow-editor-system-mode) "flows running") [flow-details-block-container
                                                                 "server flow statuses" :system
                                                                 :system
                                                                 [re-com/box :height (px (* h 0.6))
                                                                  :child [server-flows (* h 0.6)]] ; flow-id
                                                                                                   ; orderb
                                                                                                   ; true
                                                                                                   ; 570
                                                                                                   ; nil
        ...
        (= (first @db/flow-editor-system-mode) "flow browser")  [flow-details-block-container
                                                                 (first @db/flow-editor-system-mode)
                                                                 :system :system
                                                                 [settings-block flow-id
                                                                  :flow-browser] "zmdi-chart-donut"]
        (= (first @db/flow-editor-system-mode) "flow parts")    [flow-details-block-container
                                                                 ...
                                                                 "zmdi-chart-donut"])
      (cond
        (some #(= (first @db/flow-editor-system-mode) %) ["flow browser" "flow parts"])
           ...
           [(let [qq1 {:queries {:gen-viz-192 {:select   [[[[:sum :threads]] :threads] :ts]
                       ...
                       :view    [:> :ResponsiveContainer {:width (/ dyn-width 2) :height 200}
                                 [:> :AreaChart
                                  ...
                                  [:> :CartesianGrid {:strokeDasharray "1 5" :opacity 0.1}]
                                  [:> :Tooltip] ;[:> :XAxis {:dataKey :ts}]
                                  [:> :Area
                                   {:type              "monotone"
                                    :dataKey           :threads
                                    :isAnimationActive false
                                    :stroke            (str (theme-pull
                                                              :theme/editor-outer-rim-color
                                                              nil)) ;;"#8884d8"
                                    :fill              (str (theme-pull
                                                              :theme/editor-outer-rim-color
              ...
              [re-com/h-box :children
               [[re-com/v-box :width "50%" :children
                 [[re-com/box :child "threads" :align :center :justify :center :size "auto" :style
                   {:opacity 0.7}]
                  [re-com/box :child [buffy/render-honey-comb-fragments qq1 (/ dyn-width 50) 2]]]]
                [re-com/v-box :width "50%" :children
                 [[re-com/box :child "memory" :align :center :justify :center :size "auto" :style
                   {:opacity 0.7}]
                  [re-com/box :child
                   [buffy/render-honey-comb-fragments qq2 (/ dyn-width 50) 2]]]]]])
            [re-com/box :style {:padding-left "12px"} :child
             (let [qq {:select        [:ts [:thread_count :threads] [:used_memory_mb :used_mb]
      ...
      :layer      [; modify chart to use layer
                   ...
                   {:data     {:values chart-data}
                    ...
                    :encoding {:color   {:value "black"}
                               ...
                               :text    {:field     "number"
                                         :type      "quantitative"
                                         :condition {:test "datum.number <= 1" :value ""}}}}]
...
(re-frame/reg-event-db
  ...
  (fn [db [_ conn direction]] ;; direction = :up or :down
    (let [conn  (vec (map #(ut/replacer (str %) ":" "") conn))
          conn  [(keyword (str (first conn)
                               (when (not= (second conn) "out") (str "/" (second conn)))))
      ...
      (if (nil? idx)
        ...
        (cond (= direction :up)   (when (> idx 0)
                                    (assoc-in db
                                      [:flows (get db :selected-flow) :connections]
...
(re-frame/reg-sub ::condi-ports
                  (fn [db [_ bid]] (get-in db [:flows (get db :selected-flow) :map bid :cond] {})))

(re-frame/reg-sub ::push-ports
                  (fn [db [_ bid]] (get-in db [:flows (get db :selected-flow) :map bid :push] {})))
  ...
  (fn [db [_ bid name]]
    (-> db
        (assoc-in [:flows (get db :selected-flow) :connections]
                  (vec (remove #(cstr/ends-with? (str (first %))
                                                 (str "/" (ut/replacer (str name) ":" "")))
...
(re-frame/reg-event-db ::edit-condi-port
                       (undoable)
                       (fn [db [_ bid name fn]]
                         (assoc-in db
                           [:flows (get db :selected-flow) :map (get db :selected-flow-block) :cond
                            name]
                           {:fn fn :dest []})))
...
(defn port-panel
  ...
  (let [;blocks @(ut/tracked-subscribe [::flowmap])
    ...
    [re-com/v-box :gap "8px" :children
     [[re-com/v-box :style
       {:background "#00000055" :padding-top "8px" :padding-bottom "8px" :border-radius "8px"}
       :children
          ...
          (into
            [[re-com/h-box :justify :between :align :center :height "17px" :style
              ...
              [[re-com/box :child "name" :padding "4px" :width "160px"]
               [re-com/box :child (if (= type :in) "allowed" "expected") :padding "4px" :width
                "124px"]
               [re-com/box :padding "4px" :width "203px" :child
                (if (= type :in) "coming from" "going to")]
            ...
            (for [[vk vv] (if (= type :in)
                            ...
                            (sort-map-keys (try (sort-by str (keys v))
                                                (catch :default e
                                                  (do (ut/tapp>> [:fucking-sorting-error e
                                                                  :flows.cljs :ln 4216 (keys v)])
                                                      (keys v))))
                                           v))
                  :let    [select-vec          [bid vk]
                           ...
                           [ibid ivk]          (if (= type :in)
                                                 @(ut/tracked-subscribe [::get-incoming-port bid
                                                                         type vk])
                                                 [nil nil])
                           pcolor              @(ut/tracked-subscribe [::port-color flow-id
                                                                       (or ibid bid) (or ivk vk)])
                           ptype               @(ut/tracked-subscribe [::port-color flow-id
                                                                       (or ibid bid) (or ivk vk)
                                                                       :data-type])
                           pval1               @(ut/tracked-subscribe [::port-color flow-id
                                                                       (or ibid bid) (or ivk vk)
                                                                       :data-value])
                           vopen?              (true? (get-in @flow-details-block-container-atom
                                                              [flow-id bid :port-values type vk
                                                               :open?]
                                                              false))
                           ...
                           hover-bundle-only   {:on-mouse-enter
                                                  #(do (swap! flow-details-block-container-atom
                                                         assoc-in
                                                         [flow-id :involved]
                                                         involved)
                                                       (swap! flow-details-block-container-atom
                                                         assoc-in
               ...
               [[re-com/h-box :attr hover-bundle-clicks :style
                 {;:margin-left "4px" :margin-right "4px"
                  ...
                  :background-color (str pcolor
                                         (if (and (not connected?) (not selected?) (= type :out))
                                           "18"
                                           "30"))} :align :center :size "auto" :min-height "42px"
                 :children
                 [[re-com/box :child (str vk) :padding "4px" :style
                   ...
                   (if true ;selected?
                     [re-com/tag-dropdown ;:width "95px"
                       ...
                       :main            {:style {:background-color "#00000000" ;(str (theme-pull
                                                 ...
                                                 :border           (str
                                                                     ...
                                                                     (theme-pull
                                                                       ...
                                                                       nil))
                                                 :outline          "none"}}
                       :list-group-item {:style {:background-color (str (theme-pull
                                                                          :theme/editor-rim-color
                                                                          nil)
                                                                        "99")
                      ...
                      (fn [m] [re-com/box :child (subs (str (get m :label)) 1 2) :style
                               {:font-weight 700 :color (get m :color)}]) :required? true :max-width
                      "130px" :disabled? (not selected?) :label-fn
                      (fn [m] [re-com/box :child (str (get m :label)) :style
                               {:font-weight 700 :color (get m :color)}]) :style
                      {:font-size   "12px"
                       :font-family (theme-pull :theme/base-font nil)
                       :color       (theme-pull :theme/editor-font-color nil)} :abbrev-threshold 10
                      :choices
                      (vec (for [[k v] (ut/sort-map-by-key (merge {"any" "#c32148"}
                                                                  (theme-pull :theme/data-colors
                                                                              db/data-colors)))]
                             {:id               (keyword k)
                              :label            (str (keyword k))
                              :color            (ut/choose-text-color v)
                              :background-color v})) :on-change
                      #(ut/tracked-dispatch [::set-raw-port-types flow-id bid type vk %])]
                     (str vv)) :padding "4px" :width "130px"]
                  (if (not (empty? conns))
                    [re-com/v-box :padding "4px" :width "200px" :children
                     (for [e conns] [re-com/box :child (str e)])]
                    (if default?
                      ...
                      [re-com/box :width "200px" :child
                       [re-com/md-icon-button :style
                        {:font-size "10px" :color pcolor :margin-right "8px"} :md-icon-name
                        "zmdi-close"]]))
                  ...
                  [re-com/v-box :width "20px" :size "none" :style {:margin-top "3px"} :children
                   [[re-com/md-icon-button :style
                     {:font-size "13px" :color pcolor :margin-right "8px"} :md-icon-name
                ...
                (when true ;(= type :in)
                  (for [conn (cond (and (= type :in) (empty? conns)) [nil]
                    ...
                    (let [[b1 p1]        conn ;(first conns) ;; discarding others, since we can
                          pval           (if (= type :in)
                                           @(ut/tracked-subscribe [::port-color flow-id b1 p1
                                                                   :data-value])
                                           pval1)
                        ...
                        [re-com/v-box :children
                         [[re-com/h-box :children
                           [[re-com/box :width "535px" :style
                             ...
                             (let [pval (if (or (vector? pval) (map? pval) (list? pval))
                                          (ut/replace-large-base64 pval)
                                          pval)
                                   b64? (ut/is-base64? (str pval))]
                               (if (and ;(try (not (empty? pval)) (catch :default _ false))
                                 ...
                                 [re-com/v-box :padding "16px" :size "auto" :gap "10px" :children
                                  [[re-com/box :child
                                    (str "(**base64 string: ~"
                                         (.toFixed (/ (* (count (str pval)) 6) 8 1024 1024) 2)
                                         " MB)")]
                                   [re-com/box :size "auto" :child
                                    [:img {:src (str "data:image/png;base64," pval)}]]]]
                                 [code-box 555 nil (pr-str pval)]))]
                            (when multi?
                              [re-com/v-box :width "31px" :align :center :justify :center :children
                               [[re-com/md-icon-button :attr
                                 {:on-click #(ut/tracked-dispatch [::re-order-connection c-line
                                                                   :up])} :style
                                 {:font-size "12px"
                          ...
                          (when show-scrubber? ;; scrubber?
                            [re-com/box :style {:padding-top "8px" :padding-bottom "8px"} :child
                             [bricks/scrubber-panel true
                              @(ut/tracked-subscribe [::bricks/keypaths-in-flow bid]) [:flow bid] vk ;[:flow-item
                                                                                                     ;:defaults
                                                                                                     ;vk]
                                                                                                     ;;;kk
                                                                                                     ;pm
                          ...
                          (when desc ;; :meta :desc for the port
                            [re-com/box :style
                             {:margin-left    "15px"
                              ...
                              :padding-bottom "10px"} :width "535px" :child
                             (str desc)])]]))))]]))])]
      (when (and (= type :out) (not is-open-in?)) ;; if we can have conditional ports!
        (let [condi-ports @(ut/tracked-subscribe [::condi-ports bid])
              cnt         (count (keys condi-ports))
              open?       (true? (get-in @flow-details-block-container-atom
                                         [flow-id bid :condi-paths :open?]
                                         false))]
          [re-com/v-box :style {:background "#00000055" :padding-top "8px" :border-radius "8px"}
           :children
           [[re-com/h-box :size "auto" :padding "4px" :justify :between :align :center :style
             {:color ;; (str (theme-pull :theme/editor-outer-rim-color nil) 88)
                (if open?
             ...
             [[re-com/h-box :align :center :justify :center :style {:padding-left "3px"} :gap "5px" ;:style
              ...
              [re-com/md-icon-button :src (at) :on-click
               #(ut/tracked-dispatch [::add-condi-port bid]) :md-icon-name "zmdi-plus" :style
               {;:color (theme-pull :theme/editor-outer-rim-color nil)
                :font-size "22px"
            ...
            (when (and open? (> cnt 0))
              [re-com/v-box :style
               ...
               (for [[k {:keys [fn dest]}] condi-ports
                     :let                  [select-vec  [bid k]
                                            valve-open? @(ut/tracked-subscribe
                                                           [::bricks/condi-valve-open? flow-id bid
                                                            k])
                                            selected?   (or (and @sniffy-sniff
                                                                 (= @port-hover2 bid)
                                                                 (= k (get @sniffy-sniff 1)))
                                                            (= select-vec selected))
                                            pcolor      (theme-pull :theme/editor-outer-rim-color
                                                                    nil) ;; @(ut/tracked-subscribe
                                           ]]
                 [re-com/v-box :padding "4px" :attr
                  {:on-mouse-enter #(do (swap! flow-details-block-container-atom assoc-in
                                          [flow-id bid :port-panel type]
                  ...
                  [[re-com/h-box :size "auto" :justify :between :align :center :style
                   ...
                   [re-com/h-box :justify :between :align :center :children ;;(str fn)
                    [;[code-box 500 nil fn]
                     [code-box-condi 500 nil fn bid k]
                     [re-com/box :justify :center :align :center :width "40px" :size "none" :height
                      "100%" :style
                      {;:border "1px solid yellow"
                       ...
                       :font-weight      700
                       :border-radius    "8px"
                       :opacity          (if valve-open? 1 0.4)} :child
                      (str valve-open?)]]]]])])]]))]]))
...
(defn channel-box
  ...
  (let [read-only-flow? (true? (cstr/includes? (str flow-select) "/"))
        connections     (filter #(= flow-select (second %))
                          @(ut/tracked-subscribe [::flowmap-connections-parsed flow-select]))
        cc              (count connections)]
    [re-com/v-box :gap "10px" :children
     [[re-com/h-box :width "570px" :style
       {;:border "1px solid white"
        :color (str (theme-pull :theme/editor-outer-rim-color nil) 88)} :size "auto" :padding "4px"
       :align :center :justify :center :gap "10px" :children
      ...
      [re-com/box :width "570px" :style
       {:border        (str "3px dashed " (theme-pull :theme/editor-outer-rim-color nil) 66)
        :border-radius "12px"} :child [code-box-channels 570 nil (pr-str val) flow-id flow-select]]
      [re-com/box :width "570px" :style
       {:border           (str "1px solid " (str (theme-pull :theme/editor-outer-rim-color nil) 66))
        :border-radius    "10px"
        :font-size        "18px"
        :background-color (str (theme-pull :theme/editor-outer-rim-color nil) 25)} :size "auto"
       :padding "4px" :align :center :justify :center :child
       [re-com/h-box :gap "10px" :attr
        {:on-click #(ut/tracked-dispatch [::bricks/push-value flow-id flow-select val true])} :style
        {:color (theme-pull :theme/editor-outer-rim-color nil) :cursor "pointer"} :padding "6px"
        :children
...
(re-frame/reg-sub ::get-raw-data-input
                  (fn [db [_ flow-id bid]] (get-in db [:flows flow-id :map bid :data :user-input])))

(re-frame/reg-sub ::get-raw-block-map
                  (fn [db [_ flow-id bid]] (get-in db [:flows flow-id :map bid])))
...
(defn flow-code-editor-block
  ...
  (let [read-only-flow? (true? (cstr/includes? (str flow-select) "/"))
    ...
    (if (= ttype :open-fn)
      [re-com/box :style
       {:border-radius "9px" :padding-top "8px" :padding-bottom "8px" :background-color bg-shade}
       :child
       [code-box-fn ;;w h
         ...
         [(when has-override?
            [re-com/box :justify :end :align :center :style
             {:background-color (theme-pull :theme/editor-outer-rim-color nil)
              :color            (ut/choose-text-color (theme-pull :theme/editor-outer-rim-color
                                                                  nil))
              :font-size        "14px"
              :font-weight      700
              :padding          "4px"
              :border-radius    "9px"} :child
             "Note! This is overridden at run time! See flow options :overrides map above."])
          [re-com/box :style
           {:border-radius    "9px"
            :padding-top      "8px"
            :padding-bottom   "8px"
            :background-color bg-shade} :child
          ...
          [re-com/h-box :width (px (- w 10)) :height "25px" :size "none" :justify :between :align
           :center :gap "8px" :children
           [;[re-com/gap :size "5px"]
            [re-com/h-box :gap "8px" :style
             {:font-size "11px" :padding-left "10px" :padding-right "10px"} :children
             (for [e    syntaxes
                   :let [selected? (= e syntax)]]
               [re-com/box :attr
                (when (not selected?)
                  {:on-click #(ut/tracked-dispatch [::flow-block-meta flow-select :syntax e])})
                :style
                (if selected?
...
(defn output-viewer
  ...
  (let [value    @(ut/tracked-subscribe [::port-color flow-id bid nil :data-value])
        bg-shade "#00000055"
        value    (if (or (vector? value) (map? value) (list? value))
                   (ut/replace-large-base64 value)
                   value)
    ...
    [re-com/box :style
     ...
     (cond (and (try (not (empty? value)) (catch :default _ false)) b64?)
             [re-com/v-box :size "auto" :gap "10px" :children
              [[re-com/box :child
                (str "(**base64 string: ~"
                     (.toFixed (/ (* (count (str value)) 6) 8 1024 1024) 2)
                     " MB)")]
    ...
    (if (= @rename-block bid)
      [re-com/input-text :src (at) :model str-bid :width "200px" :on-change
       #(do (if (= str-bid (str %)) ;; regex doesnt allow : so we need to remove before
              (reset! rename-block nil)
              (do (ut/tracked-dispatch [::rename-block bid %]) (reset! rename-block nil))))
       :validation-regex flow-id-regex :change-on-blur? true :style
       {:text-decoration  "underline"
        :color            "inherit"
        :margin-top       "3px"
        ...
        :text-align       "left"
        :background-color "#00000000"}]
      [re-com/box :style {:cursor "pointer" :color (theme-pull :theme/editor-outer-rim-color nil)}
       :attr {:on-double-click #(reset! rename-block bid)} :child (str bid)])))

(re-frame/reg-sub ::block-details
                  (fn [db [_ bid]]
                    (get-in db [:flows (get db :selected-flow) :map bid :description] "")))

(defn details-panel
  [bid]
  (let [value @(ut/tracked-subscribe [::block-details bid])] [text-box-rw nil nil value bid]))
...
(defn transform-data-old
  ...
  (let [log-scale?        @gantt-log ;false ;true
        react!            [@(ut/tracked-subscribe [::http/flow-results])
                           @(ut/tracked-subscribe [::http/flow-results]) @gantt-log]
        ...
        data-pairs        (remove #(or (= (first %) :done) (cstr/includes? (str (first %)) "/"))
                            data)
    ...
    (mapv (fn [[step {:keys [start end fake?]}]]
            (let [end              (if (nil? end) (+ (.now js/Date) 1000) end)
                  ...
                  normalized-width (int (* (/ scaled-duration
                                              (if log-scale?
                                                (Math/log1p (+ total-duration log-offset))
                                                total-duration))
                                           svg-width))
  ...
  (let [log-scale?            @gantt-log
    ...
    (mapv (fn [[step run]]
            (let [{:keys [start in-chan? end fake?]} run
                                                       ...
                                                       (if (nil? end) (+ (.now js/Date) 1000) end)
                                                       end)
                  start                              (if running?
                                                       (if (nil? start) (.now js/Date) start)
                                                       start)
                  duration-ms                        (- end start)
  ...
  (let [gap              11
    ...
    (let [flow-select      @(ut/tracked-subscribe [::selected-flow-block])
      ...
      [:svg
       {:width  "110%"
        :height (str
                  (+ 20 (* height (count grouped-by-stepc)) (* gap (dec (count grouped-by-stepc))))
                  "px")}
       (try
         (doall
           (map-indexed
             (fn [idx [step runs]]
               (doall
                 (map-indexed
                   (fn [run-idx {:keys [x color width last? raw-duration]}]
                        ...
                        [:rect
                         {:x              x
                          ...
                          :filter         (when selected?
                                            (str "drop-shadow(0 0 0.75rem " color ")"))
...
(defn gantt-container
  ...
  (let [opw       pw ;; orig
    ...
    (if in-sidebar?
      (let [blocks      @(ut/tracked-subscribe [::flowmap])
            flow-select @(ut/tracked-subscribe [::selected-flow-block])]
        [re-com/h-box :size "none" :gap "6px" :height
         (if max-height (px max-height) (px (* (count (keys blocks)) 36))) :style
         (when max-height {:overflow "auto"}) :children
         [[re-com/v-box :style
           {;:border "1px solid white"
            :margin-left   "3px"
            :padding-left  "5px"
            :padding-right "5px"} :width "28px" :gap "10px" :children
           (block-icons blocks flow-id flow-select)] [gantt-chart data (* 0.47 opw) flow-id]] :width
         (px opw)])
      [re-com/v-box :children
       [[re-com/box :width (px pw) :height (px bg-height) :style
        ...
        [re-com/h-box :justify :between :align :center :height "30px" :size "none" :width
         (px (+ 3 pw)) :children
         [[re-com/box :style
           {:cursor "pointer" :text-decoration (if @gantt-log "none" "line-through")} :attr
    ...
    (for ;[[k {:keys [icon]}] (ordered-for orderb blocks)]
      ...
      (let [bcolor    @(ut/tracked-subscribe [::port-color flow-id k])
        ...
        [re-com/box :size "none" :height "27px" :width "27px" :align :center :justify :center :class
         ...
         :child
         [re-com/md-icon-button :src (at) :md-icon-name (or icon "zmdi-view-quilt") :on-click
          #(do (select-block k)) :attr
...
(defn save-custom-block
  [flow-id flow-select]
  [re-com/v-box :gap "4px" :style
   {:font-size "10px" :font-weight 400 :color (theme-pull :theme/editor-outer-rim-color nil)}
   :children
   [[re-com/box :child
          ...
          #(ut/tracked-dispatch
             [::wfx/request :default
              {:message     {:kind        :save-custom-flow-block
                             :name        flow-select
                             :block-map   (let [flowmap              @(ut/tracked-subscribe
                                                                        [::flowmap])
                                                flowmap-raw          @(ut/tracked-subscribe
                                                                        [::flowmap-raw])
                                                flow-id              @(ut/tracked-subscribe
                                                                        [::selected-flow])
                                                flowmaps-connections @(ut/tracked-subscribe
                                                                        [::flowmap-connections])
                                                server-flowmap       (process-flowmap2
                                                                       flowmap
                                                                       flowmaps-connections
                                                                       flow-id)]
                                            (merge
                                              {:types       (get-in flowmap-raw
                                                                    [flow-select :data :flow-item
                                                                     :types])
                                               :description (cstr/join " "
                                                                       (get-in flowmap-raw
                                                                               [flow-select
                                                                                :description]))}
                                              (select-keys (get flowmap-raw flow-select)
                                                           [:description])
                                              (get-in server-flowmap [:components flow-select])))
                             :client-name @(ut/tracked-subscribe [::bricks/client-name])}
               :on-response [::simple-response]
               :timeout     15000000}])} :align :center :justify :center :size "auto" :child "save"
       :padding "5px" :style
       {:font-weight      700
        :border-radius    "6px"
        :cursor           "pointer"
...
(defn flow-details-panel
  ...
  (let [react-hacks         [@flow-hover @scroll-id @editor-mode
    ...
    [re-com/v-box :children
     [[re-com/h-box :children
       [;[re-com/gap :size "8px"]
        (when (and @(ut/tracked-subscribe [::bricks/flow-editor?]) (not flow-select))
          [re-com/box :style {:margin-top "-2px"} :child
           [flow-editor browser-panel-width full-height] ; browser-panel-height]
        ...
        (when (and @(ut/tracked-subscribe [::bricks/flow-editor?]) flow-select)
          (let [;body (get @(ut/tracked-subscribe [::flowmap]) flow-select)
                ...
                pval              @(ut/tracked-subscribe [::port-color flow-id flow-select :out
                                                          :data-value])
                view-pval         @(ut/tracked-subscribe
                                     [::port-color flow-id
                                      (keyword (str (ut/replacer (str flow-select) ":" "") "-vw"))
                                      :out :data-value])
                ...
                opts-map          @(ut/tracked-subscribe [::opts-map])]
            [re-com/v-box :align :center :height (px full-height) :width (px browser-panel-width)
             :size "none" :style {:font-size "15px" :background-color "#00000088"} :children
             [[re-com/h-box :children
               [[block-renamer flow-select]
                [re-com/h-box :gap "8px" :children
                 [[re-com/box :attr
                   {:on-click #(zoom-to-element (str "flow-brick-" flow-select) 1.3)} :child
                   [render-icon (str (get fmap :icon "zmdi-layers")) "inherit" 29 20]]
                  [re-com/box :child (str block-type)]]]] :width "601px" :height
               (px header-row-height) ;; width overflow on purpose to
               :style
               {:margin-left      "1px"
                :padding-right    "16px"
                :font-weight      700
              ...
              [re-com/v-box :size "none" :gap "6px" :height (px (- full-height header-row-height))
               :style
               {:position   "fixed"
                :overflow-y "auto"
                :overflow-x "hidden"
                :left       6
                :top        header-row-height} :children
               [;[re-com/box :child (str "debug some shit: "  ttype open?)]
                [flow-details-block-container "flow options*" :system :system
                 [code-box-options flow-id 570 nil opts-map] "zmdi-code"]
                (when (not gantt?)
                  ...
                  [flow-details-block-container "input scrubber" flow-id flow-select
                   [re-com/box :size "auto" :style {:padding-top "8px" :padding-bottom "8px"} :child
                    [bricks/scrubber-panel true
                     ...
                     [:flow flow-select] :* {:fm true :canvas? true :flow? true}]]
                   ["zmdi-long-arrow-right" "zmdi-circle-o"]])
                (if open?
                  [flow-details-block-container
                   (if (= ttype :open-fn) "code editor" "code / value editor") flow-id flow-select
                   [flow-code-editor-block (- browser-panel-width 20) ;; 580
                    378 fmap flow-select ttype flow-id]
                   (if (= ttype :open-fn) "zmdi-functions" "zmdi-view-subtitles")]
                ...
                (when rabbit-code-view?
                  [flow-details-block-container "rabbit render (view)" flow-id flow-select
                   [re-com/box :align :center :justify :center :child
                    [buffy/render-honey-comb-fragments view-pval nil nil]] "zmdi-aspect-ratio"])
                ...
                (when browsable?
                  [flow-details-block-container "output browser" flow-id flow-select
                   [map-boxes2 pval flow-select flow-id [] :output
                    (if (vector? pval) "vector" "map") true] "zmdi-shape"])
                (when rabbit-code?
                  [flow-details-block-container "rabbit render" flow-id flow-select
                   [re-com/box :align :center :justify :center :child
                    [buffy/render-honey-comb-fragments pval nil nil]] "zmdi-aspect-ratio"])
                (when (= ttype :open-fn)
                ...
                [flow-details-block-container "flow block map - debugger*" flow-id flow-select
                 [debug-box flow-select
                  @(ut/tracked-subscribe [::get-raw-block-map flow-id flow-select])] "zmdi-bug"]
                [flow-details-block-container "channel push - debugger*" flow-id flow-select
                 (let [last-output (ut/replace-large-base64 @(ut/tracked-subscribe
                                                               [::port-color flow-id flow-select nil
                                                                :data-value]))
                       val         (get-in @channel-holster
                                           [flow-id flow-select :value]
                                           last-output)
                       copen?      @(ut/tracked-subscribe [::bricks/flow-channels-open? flow-id])]
                   (if copen?
                     [channel-box flow-id flow-select val]
                     [re-com/v-box :align :center :justify :center :style
                      {:color (theme-pull :theme/editor-outer-rim-color nil)} :children
                      [[re-com/box :style {:font-size "15px"} :child "channels are all closed"]
                       [re-com/box :style {:font-size "11px"} :child
                        "if you'd like to push channel values, run again with option :close-on-done? false"]]]))
                 "zmdi-bug"]
                [flow-details-block-container "connections - debugger*" flow-id flow-select
                 [re-com/box ;;; edit-conns
                  ...
                  [code-box-rwc (- browser-panel-width 20) ;; 580
                   nil (str @(ut/tracked-subscribe [::flowmap-connections]))]] "zmdi-bug"]
        ...
        [re-com/v-box :gap "10px" ;; (if (or gantt? flow-select) "10px" "20px")
         ...
         {:background-color "#00000099"
          ...
          :top              -1 ;28
          :padding-top      "10px"
          :padding-right    (if (not @(ut/tracked-subscribe [::bricks/flow-editor?]))
                              "5px"
                              "inherit")
          :border-top       (str "6px solid " (theme-pull :theme/editor-outer-rim-color nil))
          :border-right     (str "1px solid " (theme-pull :theme/editor-outer-rim-color nil) 25)}
         :width "40px" :align :center :justify :start :children
         (into
           [[re-com/md-icon-button :src (at) :md-icon-name
             (if @(ut/tracked-subscribe [::bricks/flow-editor?])
               "zmdi-chevron-left"
               "zmdi-chevron-right") :on-click
             #(do (ut/tracked-dispatch [::bricks/toggle-flow-editor])
                  (reset! editor-tooltip-atom nil)) :attr
             {:on-mouse-enter #(reset! editor-tooltip-atom (str "toggle flow work panel "
                                                                (if @(ut/tracked-subscribe
                                                                       [::bricks/flow-editor?])
                                                                  "open"
                                                                  "closed")))
              :on-mouse-leave #(reset! editor-tooltip-atom nil)} :style
            ...
            [re-com/md-icon-button :src (at) :md-icon-name "zmdi-chart" :on-click
             #(do (ut/tracked-dispatch [::bricks/toggle-flow-gantt])
                  (reset! editor-tooltip-atom nil)) :attr
             {:on-mouse-enter #(reset! editor-tooltip-atom (str "toggle run gantt chart "
     ...
     :height (px full-height) :width
     (if @(ut/tracked-subscribe [::bricks/flow-editor?]) (px browser-panel-width) (px 45))]))
           ...
           (if (> rs-running 0)
             (conj
               ...
               [[:v-box :children
                 (vec
                   (into
                     [[:box :child (str rs-running " flow" (when (> rs-running 1) "s") " running")]]
                     (vec
                       (for [e    rs-running-list
                         ...
                         [:v-box :padding "3px" :width (px max-w) ;;"215px"
                          ...
                          [[:box :size "auto" :style {:padding-left "5px"} :child (str fid)]
                           (when est? ;true ;est?
                             [:box :child [:progress-bar [(- max-w 15) est (str e run-id)]] :height
                              "25px" :padding "3px"])
                           (when est? ;true ;est?
                             [:box :align :end :style
     ...
     [re-com/box :child
      [re-com/h-box :attr
       ...
       [;[:img {:src "images/test-kick-icon.png" :width 30 :height 30}]
        [re-com/md-icon-button :src (at) :md-icon-name
         (if @db/kick-alert "zmdi-star" "zmdi-star-outline") :style
         {;:color "red"
          :transition   "all 0.4s ease-in-out"
          :color        (if @db/pause-alerts
                          (theme-pull :theme/editor-outer-rim-color nil)
                          "inherit")
          :position     "fixed"
          :bottom       17
        ...
        [re-com/v-box :children ;(into (for [e (range rs-running)] [re-com/box :child "o"])
         (for [a    alerts
               :let [abody       (first a)
                     push-codes? (try (some #(or (= % :push) (= % :dialog-push))
                                            (ut/deep-flatten abody))
                                      (catch :default _ false))
           ...
           [re-com/box :size "none" :attr
            (when (not push-codes?)
              (if @db/kick-alert
                ...
                {})) :width (when (> width 0) (px width)) :height (when (> height 0) (px height))
            :child [buffy/render-honey-comb-fragments abody (get a 1) (get a 2)]])]]] :width
      (px (+ 0 box-width)) :height (px (if @db/kick-alert all-h box-height)) :padding "9px" :style
      {:position         "fixed"
       :font-size        "18px"
...
(defn lookup-modal
  ...
  (let [[left top] @db/context-modal-pos
        ...
        sql-calls  {:fflows-sys     {:select        [[":flow" :category] ["" :description]
                                                     [:flow_id :name] :file_path [:body :full_map]
                                                     ["zmdi-developer-board" :icon]]
                                     :from          [:flows]
                                     :connection-id "flows-db"
                                     :order-by      [[3 :asc]]}
                    :fflowparts-sys {:select   [:category :description :name :file_path :full_map
                                                :icon]
                                     :order-by [[3 :asc]]
                                     :from     [:flow_functions]}}]
    ...
    [re-com/modal-panel :style {:z-index 999 :padding "0px"} :parts
     {:child-container {:style {:left      left
                                ...
                                :top       top
                                :font-size "8px"
                                :border    (str "5px solid "
                                                (theme-pull :theme/editor-outer-rim-color nil))
                                :position  "fixed"}}} :backdrop-opacity 0.63 :backdrop-on-click
     #(reset! lookup-modal? false) :child
     [re-com/h-box :justify :between :align :center :gap "10px" :style
      {:color "#000000" :font-size "18px"} :children
      [;[re-com/box :child "topper"]
       [re-com/typeahead :width "250px" :suggestion-to-string
        (fn [item] (str (get item :category) "/" (ut/replacer (str (get item :name)) ":" ""))) ;; render
                                                                                               ;; in
        :render-suggestion
        (fn [ss _] ;; render in dropdown
          [re-com/h-box :style {:border-top "1px solid #00000009"} :size "none" :width "240px"
           :justify :between :align :center :children
           [[re-com/v-box :children
             [[re-com/h-box :justify :between :align :center :children
               [[re-com/box :style {:font-weight 700 :font-size "12px"} :child
                 (str (get ss :name))]]]
              (when (ut/ne? (get ss :description))
                [re-com/box :style {:font-size "7px"} :child (str (get ss :description))])]]
            [re-com/v-box :align :end :justify :end :children
             [[render-icon (str (get ss :icon)) "#00000044" 20 15]
              [re-com/box :style {:font-size "6px" :color "#00000066" :font-weight 700} :child
               (str (get ss :category))]]]]]) :on-change #(reset! lookup-val %) :rigid? false
        :placeholder "what do you want to add?" :data-source
        (fn [x]
                       ...
                       (fn [{:keys [db]} [_ flow-id]]
                         {:db       (ut/tracked-dispatch [::unload-flow-db flow-id])
                          :dispatch [::wfx/request :default
                                     {:message {:kind        :remove-flow-watcher ;; just in case we
                                                                                  ;; are
                                                :client-name @(ut/tracked-subscribe
                                                                [::conn/client-name])
                                                :flow-id     flow-id}
...
(re-frame/reg-event-db ::toggle-flow-drop-down
                       (fn [db _]
...
(re-frame/reg-event-db ::toggle-lock
                       (fn [db _] (assoc db :alt-key-held? (not (get db :alt-key-held?)))))
...
(defn flow-panel
  ...
  (let [[x y]                @db/flow-detached-coords
        ...
        watched?             (ut/ne? (get @(ut/tracked-sub ::bricks/flow-watcher-subs-grouped {})
                                          flow-id))
      ...
      (if (and (not @db/dragging-flow-editor?) (not @bricks/dragging-editor?))
        {;:on-mouse-enter #(do (reset! bricks/over-flow? true)
         :on-mouse-over  #(when (not @bricks/over-flow?)
                            (do (reset! bricks/over-flow? true) (reset! bricks/over-block? true)))
      ...
      {:position         "fixed"
       ...
       :box-shadow       (let [block-id       :audio
                           ...
                           (cond (and audio-playing? talking-block?)
                                   (str "1px 1px " (px (* 80 (+ 0.1 (get @db/audio-data block-id))))
                                        " "        (theme-pull :theme/editor-outer-rim-color nil))
                                 :else "none"))
             ...
             [;; [re-com/h-box
              [re-com/md-icon-button :style
               {;:font-size "13px" :margin-top "-16px"
                :width "28px"} :on-click #(ut/tracked-dispatch [::toggle-flow-drop-down])
              ...
              [re-com/md-icon-button :md-icon-name "zmdi-window-minimize" :on-click
               #(ut/tracked-dispatch [::bricks/toggle-flow]) :style
              ...
              (when flow-drop-down?
                [re-com/v-box :padding "8px" :size "auto" ;:width "400px"
                 ...
                 {:background-color "#000000"
                  ...
                  :font-size        "19px"
                  :right            0} :children
                 (for [[v [file-path name]] (vec (sort (distinct
                                                         (into (for [[k v] @(ut/tracked-subscribe
                                                                              [::http/flows])]
                                                                 [k nil])))))
                       :let                 [selected? (= (str v) (str flow-id))]]
                   [re-com/h-box :gap "10px" :justify :between :children
                    [[re-com/box :attr
                      {:on-click #(let [sub-flow-exec? (and (string? file-path)
                                                            (not (nil? file-path)))]
                                    (if sub-flow-exec?
                                      (ut/tracked-dispatch [::http/load-flow-w-alias file-path v])
                                      (ut/tracked-dispatch [::set-selected-flow (str v)])))} :child
                      (str v)]
                     [re-com/md-icon-button :src (at) :md-icon-name "zmdi-close" :on-click
                    ...
                    (if selected?
                      ...
                      {:cursor "pointer" :opacity 0.6}) :padding "4px"])])]]] :size "none" :width
           (px (- panel-width 12)) :height "25px" :style ;{:background-color (theme-pull
            ...
            [re-com/box :style
             ...
             {:id "flow-canvas"
              ...
              :on-context-menu (re-com/handler-fn #_{:clj-kondo/ignore [:unresolved-symbol]}
                                                  (bricks/tag-screen-position event) ;; event is
                                                                                     ;; magic in
                                                  (when (and (empty? @port-hover2)
                                                             (empty? @flow-hover)) ; (nil?
                                                                                   ; @flow-hover)
                                                                                   ; ;(and
                                                    (reset! lookup-modal? true))
                                                  #_{:clj-kondo/ignore [:unresolved-symbol]}
                                                  (.preventDefault event))} :size "none" :width
             (px (- panel-width 12)) :height (px (- ppanel-height 35)) :child
             [re-com/v-box :size "1" :style
              (merge ;(theme-pull :theme/canvas-background-css nil)
                {:font-family (theme-pull :theme/base-font nil) :z-index 200 :border-radius "12px"}) ; "0px
                                                                                                     ; 0px
                                                                                                     ; 12px
                                                                                                     ; 12px"
              :children
              [;;   [tab-menu]
               (when (and @bricks/dragging?
                          @bricks/over-flow?
                          (not @dragging-port?)
                 ...
                 (let [rel-x      (first @db/context-modal-pos) ;(* (js/Math.floor (/ (first
                       ...
                       load-flow? (= (get-in @bricks/dragging-body [:drag-meta :source-query])
                                     :flows-sys)
                   ...
                   (when in-bounds? ;; draw that shit
                     [re-com/box :child
                      (if load-flow?
                        (if @drop-toggle? " *ADD to flow " " *LOAD this flow ")
                        " add to flow canvas ") :align :center :justify :center :style
               ...
               [re-com/box :size "none" :style
                ...
                [(reagent/adapt-react-class zpan/TransformWrapper)
                 {;:key (str @db/pan-zoom-offsets)
                  ...
                  :onPanningStop    #(do (reset! db/pan-zoom-offsets [(.-positionX (.-state %))
                                                                      (.-positionY (.-state %))
                                                                      (.-scale (.-state %))])
                                         (reset! db/panning? false))
                  :onPanningStart   #(reset! db/panning? true)
                  :onWheelStop      #(do (reset! db/pan-zoom-offsets [(.-positionX (.-state %))
                                                                      (.-positionY (.-state %))
                                                                      (.-scale (.-state %))])
               ...
               [re-com/h-box :gap "6px" :width
                ...
                [(when has-done?
                   (if running? ;; (or running? chans-open?)
                     [re-com/box :child
                      [re-com/md-icon-button :src (at) :md-icon-name "zmdi-refresh-sync" :class
                       (if (not running?) "rotate-reverse linear infinite" "rotate linear infinite")
                       :style
                       {:color            (str (theme-pull :theme/editor-outer-rim-color nil))
                        :cursor           "pointer"
                        :transform-origin "21.5px 22px"
                        :filter           "drop-shadow(0.25rem 0.35rem 0.4rem rgba(0, 0, 0, 0.44))"
                        :font-size        "43px"} :on-click
                                         ...
                                         [::wfx/request :default
                                          {:message {:kind        :kill-flow
                                                     ...
                                                     :client-name @(ut/tracked-subscribe
                                                                     [::bricks/client-name])}
                                           :timeout 15000000}])
                      :on-mouse-enter #(reset! editor-tooltip-atom
                                         (str "kill flow (on server) and all open channels"))
                      :on-mouse-leave #(reset! editor-tooltip-atom nil)} :style
                     {:color     (str (theme-pull :theme/editor-outer-rim-color nil))
                      :cursor    "pointer"
                      :filter    "drop-shadow(0.25rem 0.35rem 0.4rem rgba(0, 0, 0, 0.44))"
                 ...
                 (when (not running?)
                   [re-com/box :child
                    [re-com/md-icon-button :src (at) :md-icon-name "zmdi-time-interval" :on-click
                     #(reset! editor-mode :scheduler) :attr
                     {:on-mouse-enter #(reset! editor-tooltip-atom
                                         (str "set flow schedule (on server)"))
                      :on-mouse-leave #(reset! editor-tooltip-atom nil)} :style
                     {:color     (str (theme-pull :theme/editor-outer-rim-color nil))
                      :cursor    "pointer"
                      :filter    "drop-shadow(0.25rem 0.35rem 0.4rem rgba(0, 0, 0, 0.44))"
                   ...
                   [re-com/box :child
                    [re-com/md-icon-button :src (at) :attr
                     {:on-mouse-enter #(reset! editor-tooltip-atom (str "new flow"))
                      :on-mouse-leave #(reset! editor-tooltip-atom nil)} :on-click
                     #(ut/tracked-dispatch [::new-flow]) :md-icon-name "zmdi-widgets" :style
                     {:color     (str (theme-pull :theme/editor-outer-rim-color nil))
                      :cursor    "pointer"
                      :filter    "drop-shadow(0.25rem 0.35rem 0.4rem rgba(0, 0, 0, 0.44))"
                      :font-size "43px"}] :style
                    ...
                    (let [ccolor (theme-pull :theme/editor-outer-rim-color nil)]
                      [re-com/h-box :attr
                       ...
                       [[re-com/box :child "drop here for: "]
                        [re-com/box :style {:font-weight 700} :child "all done & return value"]]
                       :size "none" :align :center :justify :center :style
                       {:font-size        "27px"
                       ...
                       (px (- (if @(ut/tracked-subscribe [::bricks/flow-editor?])
                                (* ww 0.5035)
                                (* ww 0.793))
                              210))])]
                   [re-com/box :child (str (or @editor-tooltip-atom "")) :size "none" :height "80%"
                    :width
                    (px (- (if @(ut/tracked-subscribe [::bricks/flow-editor?])
                             (* ww 0.5035)
                             (* ww 0.793))
                           210)) :align :center :justify :center :style
               ...
               [re-com/h-box :style
                ...
                [[edit-flow-title flow-id 1450]
                 [re-com/md-icon-button :src (at) :md-icon-name
                  (if watched? "zmdi-eye" "zmdi-eye-off") :on-click
                  #(if watched?
                     (ut/tracked-dispatch [::wfx/request :default
                     ...
                     (let [flowmap              @(ut/tracked-subscribe [::flowmap])
                           flowmaps-connections @(ut/tracked-subscribe [::flowmap-connections])
                           client-name          @(ut/tracked-subscribe [::conn/client-name])
                           server-flowmap       (process-flowmap2 flowmap
                                                                  flowmaps-connections
                                                                  flow-id)
                           comps                (get server-flowmap :components) ;; (assoc (get
                           running-view-subs    (vec (for [[k v] comps
               ...
               [re-com/box :child
                [re-com/md-icon-button :src (at) :md-icon-name
                 (if zoom-unlocked? "zmdi-lock-open" "zmdi-lock") :on-click
                 #(ut/tracked-dispatch [::toggle-lock]) :attr
                 {:on-mouse-enter #(reset! editor-tooltip-atom
                                     (str "hold C to unlock flow pan & zoom"))
                  :on-mouse-leave #(reset! editor-tooltip-atom nil)} :style
                 {:color     (if zoom-unlocked?
                               "red"
                               (str (theme-pull :theme/editor-outer-rim-color nil)))
                  :cursor    "pointer"
                  :font-size "43px"}] :style
