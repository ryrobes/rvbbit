...
(re-frame/reg-sub
  ...
  (fn [db [_ _]]
    (let [rs-scrubber-meta   (when (and (get db :buffy?)
                                        (= (get @db/chat-mode ["none!" nil nil]) :runstreams))
                               (apply merge
                               ...
                               (into {}
                                     (for [[k v] (get-in db
                                                         [:flows (get db :selected-flow) :map
                                                          (get db :selected-flow-block) :data
                                                          :flow-item :meta])
...
(re-frame/reg-sub ::user-scrubbers
                  (fn [db [_ _]]
                    (merge (get-in db [:click-param :theme :*scrubbers])
  ...
  (fn [db _]
    (let [obody      (get db :panels)
          ...
          logic-kps2 (into {}
                           (for [v kps2]
                             (let [[_ param opts] v
                                   pkey-vec       [(keyword (last
                                                              (ut/splitter
                                                                (ut/replacer (str param) #":" "")
                                                                #"/")))]]
                               {pkey-vec opts})))]
      logic-kps2)))
  ...
  (let [theme-parameters @(ut/tracked-subscribe [::user-scrubber-options])
    ...
    (merge
      {[:mark :type] {:message "vega-lite viz mark type"
                      ...
                      :values  ["area" "bar" "circle" "line" "point" "rect" "rule" "square" "text"
                                "tick"]}
       [1 :syntax] {:message
                      "codemirror syntax highlighting - also changes how the data is stored(!)"
         ...
         {:message "vega-lite color schemes / palettes"
          ...
          :values
            [{:id "accent" :label "accent" :group "Categorical Schemes"}
             ...
             {:id    "yelloworangebrown"
              :label "yelloworangebrown"
              :group "Sequential Multi-Hue Schemes"}
       ...
       [:aggregate] {:message "vega-lite axis aggregation fn used"
                     ...
                     :values  ["count" "distinct" "sum" ;"product" "mean"
                               "average" "variance" "variancep" "stdev" "median" "q1" "q3" "ci0"
                               "ci1" "min" "max"]}
       ...
       [[:font-family] [:titleFont] [:monospaced-font] [:base-font] [:labelFont] [:legendFont]]
         {:message "font face!"
          :values  (distinct (sort ["VikingNormal" "WolfpackHalloweed" "Roboto" "Roboto Condensed"
                                    "Ubuntu" "Alata" "Oxygen Mono" "Lato" "Poppins" "Montserrat"
                                    "Open Sans" "Victor Mono" "Sudo Var" "Overpass Mono" "ZT Gatha"
                                    "Instagram Sans Condensed" "Instagram Sans" "Nova Square"
                                    "Noto Sans Linear A" "Nova Mono" "Share Tech Mono" "Oswald"
                                    "Raleway" "Homemade Apple" "Kulim Park" "JetBrains Mono"
                                    "IBM Plex Mono" "Roboto Slab" "Source Code Pro"
                                    "Pastor of Muppets" "Odor Mean Chey" "Pastor of Muppets Flipped"
                                    "Roboto Condensed" "Fira Sans" "Fira Code" "Sono"
                                    "Chivo Mono"]))}
       [[:codemirror-theme]]
         {:message "codemirror theme"
          :values  (distinct
                     (sort ["3024-day" "bespin" "gruvbox-dark" "material" "panda-syntax"
                            "tomorrow-night-bright" "3024-night" "blackboard" "hopscotch" "mbo"
                            "paraiso-dark" "tomorrow-night-eighties" "abcdef" "cobalt" "icecoder"
                            "mdn-like" "paraiso-light" "ttcn" "ambiance-mobile" "colorforth" "idea"
                            "midnight" "pastel-on-dark" "twilight" "ambiance" "darcula" "isotope"
                            "monokai" "railscasts" "vibrant-ink" "ayu-dark-smaller" "dracula"
                            "lesser-dark" "moxer" "rubyblue" "xq-dark" "rvbbit-dynamic" "ayu-dark"
                            "duotone-dark" "liquibyte" "neat" "seti" "xq-light" "ayu-mirage"
                            "duotone-light" "lucario" "neo" "shadowfox" "yeti" "ayu-yellow"
                            "eclipse" "material-darker" "night" "solarized" "yonce" "base16-dark"
                            "elegant" "material-ocean" "nord" "ssms" "zenburn" "base16-light"
                            "erlang-dark" "material-palenight" "oceanic-next" "the-matrix"]))}}
      canvas-overrides
      theme-parameters)))
...
(re-frame/reg-event-db
  ...
  (fn [db [_ kp v view-key type-key]]
    (let [selected-block (get db :selected-block)
          ...
          full-kp        (cond (and (vector? view-key) (= (first view-key) :opts))
                                 [:flows (get db :selected-flow) :opts]
                               (or (nil? view-key) (= view-key :base)) [:panels selected-block
                                                                        type-key]
                               ...
                               (and (vector? view-key) (= (first view-key) :flow))
                                 (if (= kp [:*])
                                   [:flows (get db :selected-flow) :map (last view-key) :data
                                    :user-input]
                                   [:flows (get db :selected-flow) :map (last view-key) :data
                                    :flow-item :defaults])
   ...
   (let [v           (if (= ext :px) (int (first (clojure.string/split v "px"))) v)
         values      (get (first (remove nil? ;;; slider values should be [:min :step :max]
                                   (for [[k v] (friendly-text)]
                                     (when (ordered-keys-in-kp? k kp) v))))
                          :values)
     ...
     [re-com/h-box :src (at) :size "auto" :justify :between ;:size "1"
      ...
      [[re-com/md-icon-button :md-icon-name "zmdi-caret-left" :on-click
        #(ut/tracked-dispatch [::scrubber-view-update kp (if (= ext :px) (str (- v 1) "px") (- v 1))
       ...
       [re-com/slider :src (at) :model v :min min-val :step step-val :max max-val :width
        ...
        #(ut/tracked-dispatch [::scrubber-view-update kp (if (= ext :px) (str % "px") %) view-key
                               type-key]) :disabled? false]
  ...
  [re-com/v-box :gap "7px" :padding "3px" :children
   (conj (vec (for [c colors]
                [re-com/box :child " " ; (str c)
                 ...
                 {:on-click #(ut/tracked-dispatch [::scrubber-view-update kp c view-key type-key])}
                 :style
                 {:background-color c
                  :font-size        "9px"
                  :font-weight      500
  ...
  (when (= kp @color-picker-enabled?)
    [re-com/h-box :children
     [[re-com/box :src (at) :height "500px" :padding "5px" :child
      ...
      (try [color-farmer kp v view-key type-key]
    ...
    [re-com/box :size "none" :width (px (- width-int 24)) :height (px (- height-int 24)) :style
     ...
     [(reagent/adapt-react-class cm/UnControlled)
      {:value   (ut/format-map (- width-int 24) (str value))
       :onBlur  #(ut/tracked-dispatch-sync [::scrubber-view-update kp
...
(defn shapes
  ...
  (let [zmdi-icons ["zmdi-triangle-down" "zmdi-triangle-up" "zmdi-square-o" "zmdi-circle-o"
    ...
    [re-com/h-box :size "auto" :style
     {:background-color (str color 11)
      :border           (if new? (str "1px dashed " color) (str "1px solid " color))} :justify
     :between :children
     [body
      [re-com/box :child
      ...
      [re-com/v-box :size "auto" :children
       [;[re-com/box :child (str (get-in @lookup-atom-f [view-key idx]))]
        [shapes [clause-vec-loop kp v view-key 0 idx]
         (get @(ut/tracked-subscribe [::conn/data-colors])
              (get-in @lookup-atom [view-key idx])
              "#ffffff22") (get-in @lookup-atom [view-key idx]) v]]])
    (if (try (> (count v) 1) (catch :default e false))
      [re-com/v-box :justify :between :size "auto" :padding "1px" :style
       {:padding-left "5px" :padding-right "5px"} :children
       (for [e (range (count v))] [clause-vec-loop (conj kp e) (get v e) view-key 0 idx])]
      (let [pos (- (count kp) 1)
            lpos (last kp)
            src-table (first (flatten (get @(ut/tracked-subscribe [::query-by-query-key view-key])
                                           :from)))
            ...
            pos-meaning (cond (or (and (= pos 0)
                                       (or
                                         (or (= lpos :group-by) (= lpos :order-by) (= lpos :select))
                                         (= lpos 0)))
                                  (and (= pos 1) (= lpos 0)))
                                :base-field
                              ...
                              :else :?)
            mdata? (true? (or (= pos-meaning :base-field)
                              (= pos-meaning :alias)
                              (= pos-meaning :fn-val)))
            data-type
              (cond (and mdata? (keyword? v) (= pos-meaning :fn-val))       (get-in src-metad
                                                                                    [:fields v
                                                                                     :data-type])
                    ...
                    mdata?                                                  (get-in metad
                                                                                    [:fields v
                                                                                     :data-type]))
            data-ex (cond (and mdata? (keyword? v) (= pos-meaning :fn-val))
                            (cstr/join ", " (keys (get-in src-metad [:fields v :commons])))
                          (and mdata? (not (keyword? v)) (= pos-meaning :fn-val)) (str v)
                          mdata? (cstr/join ", " (keys (get-in metad [:fields v :commons]))))
            base-box
              [re-com/v-box :justify :between :size "auto" :children
            ...
            [re-com/box :size "auto" :padding "2px" :width "400px" :child
             [re-com/v-box :size "auto" :gap "3px" :children
              [[clause-vec-loop kp vv view-key 1 (.indexOf v vv)]
               [code-box "panel-key" view-key 524 55 (str vv) (conj kp (.indexOf v vv)) view-key
                type-key]]]])])]
...
(defn button-panel
  ...
  (let [values (get (first (remove nil?
                             (for [[k v] (friendly-text)]
                               (when (ordered-keys-in-kp-multi? k kp) v))))
                    :values)]
    [re-com/h-box :src (at) :gap "3px" :size "auto" :children
     (for [vv values]
       [re-com/box :src (at) :align :center :justify :center :size "auto" :padding "3px" :child
        (str vv) :attr
...
(defn pixel-controller
  ...
  (let [hovered? (= @db/pixel-pusher-hover [id ico kp])]
    [re-com/md-icon-button :src (at) :attr
     {:on-mouse-enter #(reset! db/pixel-pusher-hover [id ico kp])
      :on-mouse-leave #(reset! db/pixel-pusher-hover nil)} :md-icon-name ico ; "zmdi-chevron-down"
     :style
     {:color (if hovered? "#ffffff" "#ffffff55") :border (when hovered? "3px solid #201d2a")}]))
...
(defn layout-box
  ...
  (let [;reacthack @pixel-pusher-hover ;; imp! Use-percents? @use-percents? ;; keep the
        ...
        x+                 #(ut/tracked-dispatch [::scrubber-view-update kp
                                                  (assoc v :root [(trunc (+ x inc-val)) y]) view-key
                                                  type-key])
        x-                 #(ut/tracked-dispatch [::scrubber-view-update kp
                                                  (assoc v :root [(trunc (- x inc-val)) y]) view-key
                                                  type-key])
        y+                 #(ut/tracked-dispatch [::scrubber-view-update kp
                                                  (assoc v :root [x (trunc (+ y inc-val))]) view-key
                                                  type-key])
        y-                 #(ut/tracked-dispatch [::scrubber-view-update kp
                                                  (assoc v :root [x (trunc (- y inc-val))]) view-key
                                                  type-key])
        h+                 #(ut/tracked-dispatch [::scrubber-view-update kp
                                                  (assoc v :h (trunc (+ h inc-val))) view-key
                                                  type-key])
        h-                 #(ut/tracked-dispatch [::scrubber-view-update kp
                                                  (assoc v :h (trunc (- h inc-val))) view-key
                                                  type-key])
        w+                 #(ut/tracked-dispatch [::scrubber-view-update kp
                                                  (assoc v :w (trunc (+ w inc-val))) view-key
                                                  type-key])
        w-                 #(ut/tracked-dispatch [::scrubber-view-update kp
                                                  (assoc v :w (trunc (- w inc-val))) view-key
                                                  type-key])
        z+                 #(ut/tracked-dispatch [::scrubber-view-update kp
                                                  (assoc v :z (trunc (+ z 1))) view-key type-key])
        z-                 #(ut/tracked-dispatch [::scrubber-view-update kp
                                                  (assoc v :z (trunc (- z 1))) view-key type-key])
        editor-box         [re-com/box :padding "10px" :size "auto" :child
                            [clojure-box 220 100 v kp v view-key type-key]]
        percent-toggle     [re-com/v-box :style {:margin-top "-8px"} :children
                            [[re-com/box :height "21px" :align :center :justify :center :child
                              ...
                              {:font-family  "Kulim Park"
                               ...
                               :color        "#ffffff66"
                               :padding-left "40px"}]
                             [re-com/box :height "21px" :style
                              {:font-weight 200 :margin-left "41px" :opacity 0.7} :align :center
                              :justify :center :child "(< 1.0 = %)"]]]
        button-block-style {:border           "1px solid #201d2a"
    ...
    [re-com/h-box :attr
     {:on-mouse-enter #(reset! db/pixel-pusher-key kp)
      :on-mouse-leave #(reset! db/pixel-pusher-key nil)} :children
        ...
        [[re-com/h-box :justify :between :size "auto" :children
          [[re-com/box ;:size "160px"
            ...
            [re-com/v-box ;;:align :center  :justify :center
             :style
             {:font-family "Roboto Mono" :font-size "10px" :font-weight 500 :color "#ffffff77"}
               ...
               [[re-com/box :child
                 [re-com/v-box :align :center :justify :center :children
                  [[re-com/box :child (str x)]
                   [re-com/box :child "x" :style {:font-size "8px" :font-weight 400}]]] ;(str x)
                 :align :center :width "33px" :height "31px" :justify :center                       ; :size
                                                                                                    ; "30px"
                 :style blank-block-style]
                [re-com/box :child [pixel-controller :y "zmdi-chevron-up" kp] :align :center :width
                 "33px" :height "31px" :justify :center                                             ; :size
                                                                                                    ; "30px"
                ...
                [re-com/box :child " " :align :center :width "33px" :height "31px" :justify :center ; :size
                                                                                                    ; "30px"
                 :style blank-block-style]
                [re-com/box :child [pixel-controller :h "zmdi-chevron-up" kp] :align :center :width
                 "33px" :height "31px" :justify :center                                             ; :size
                                                                                                    ; "30px"
                ...
                [re-com/box :child " " :align :center :width "33px" :height "31px" :justify :center ; :size
                                                                                                    ; "30px"
                 :style blank-block-style]
                [re-com/box :child [pixel-controller :z "zmdi-plus" kp] :align :center :width "33px"
                 :height "31px" :justify :center                                                    ; :size
                                                                                                    ; "30px"
                 :attr {:on-click z+} :style filled-block-style]]]
              [re-com/h-box :children
               [[re-com/box :child [pixel-controller :x "zmdi-chevron-left" kp] :align :center
                 :width "33px" :height "31px" :justify :center                                      ; :size
                                                                                                    ; "30px"
                 :attr {:on-click x-} :style button-block-style]
                [re-com/box :child " o " :align :center :width "33px" :height "31px" :justify
                 :center                                                                            ; :size
                                                                                                    ; "30px"
                 :style filled-block-style]
                [re-com/box :child [pixel-controller :x "zmdi-chevron-right" kp] :align :center
                 :width "33px" :height "31px" :justify :center                                      ; :size
                                                                                                    ; "30px"
                 :attr {:on-click x+} :style button-block-style]
                [re-com/box :child " " :align :center :width "33px" :height "31px" :justify :center ; :size
                ...
                [re-com/box :child
                 [re-com/v-box :align :center :justify :center :children
                  [[re-com/box :child (str h)]
                   [re-com/box :child "height" :style {:font-size "8px" :font-weight 400}]]] ;(str
                                                                                             ;h)
                 :align :center :width "33px" :height "31px" :justify :center                       ; :size
                                                                                                    ; "30px"
                 :style blank-block-style]
                ...
                [re-com/box :child
                 [re-com/v-box :align :center :justify :center :children
                  [[re-com/box :child (str z)]
                   [re-com/box :child "order" :style {:font-size "8px" :font-weight 400}]]] ;(str o)
                 :align :center :width "33px" :height "31px" :justify :center                       ; :size
                                                                                                    ; "30px"
                 :style blank-block-style]]]
              [re-com/h-box :children
               [[re-com/box :child " " :align :center :width "33px" :height "31px" :justify :center ; :size
                                                                                                    ; "30px"
                 :style blank-block-style]
                [re-com/box :child [pixel-controller :y "zmdi-chevron-down" kp] :align :center
                 :width "33px" :height "31px" :justify :center                                      ; :size
                                                                                                    ; "30px"
                 :attr {:on-click y+} :style button-block-style]
                [re-com/box :child
                 [re-com/v-box :align :center :justify :center :children
                  [[re-com/box :child (str y)]
                   [re-com/box :child "y" :style {:font-size "8px" :font-weight 400}]]] ;(str y)
                 :align :center :width "33px" :height "31px" :justify :center                       ; :size
                                                                                                    ; "30px"
                 :style blank-block-style]
                [re-com/box :child " " :align :center :width "33px" :height "31px" :justify :center ; :size
                                                                                                    ; "30px"
                 :style blank-block-style]
                [re-com/box :child [pixel-controller :h "zmdi-chevron-down" kp] :align :center
                 :width "33px" :height "31px" :justify :center                                      ; :size
                                                                                                    ; "30px"
                 :attr {:on-click h-} :style button-block-style]
                [re-com/box :child " " :align :center :width "33px" :height "31px" :justify :center ; :size
                                                                                                    ; "30px"
                 :style blank-block-style]
                [re-com/box :child [pixel-controller :z "zmdi-minus" kp] :align :center :width
                 "33px" :height "31px" :justify :center                                             ; :size
                                                                                                    ; "30px"
                 :attr {:on-click z-} :style filled-block-style]]]
              [re-com/h-box :children
               [[re-com/box :child " " :align :center :width "33px" :height "31px" :justify :center ; :size
                ...
                [re-com/box :child " " :align :center :width "33px" :height "31px" :justify :center ; :size
                 ...
                 :style blank-block-style]]]
              [re-com/h-box :children
               [[re-com/box :child [pixel-controller :w "zmdi-chevron-left" kp] :align :center
                 :width "33px" :height "31px" :justify :center                                      ; :size
                                                                                                    ; "30px"
                 :attr {:on-click w-} :style button-block-style]
                [re-com/box :child
                 [re-com/v-box :align :center :justify :center :children
                  [[re-com/box :child (str w)]
                   [re-com/box :child "width" :style {:font-size "8px" :font-weight 400}]]] ;(str w)
                 :align :center :width "33px" :height "31px" :justify :center ; :size "30px"
                 :style blank-block-style]
                [re-com/box :child [pixel-controller :w "zmdi-chevron-right" kp] :align :center
                 :width "33px" :height "31px" :justify :center                ; :size "30px"
                 :attr {:on-click w+} :style button-block-style] percent-toggle]]]]]]]]]]
      editor-box]]))
...
(defn scrubbers
  ...
  (let [layout-box-maps @(ut/tracked-subscribe [::keypaths-of-layout-panels])
        ...
        user-scrubbers  (ut/postwalk-replacer
    ...
    (merge
      {:select-clause      {:test-fn          vector?
       ...
       :button-panel       {:test-fn          string? ;#(or (integer? %) (cstr/ends-with? %
                            :keypath-contains (into (vec (for [[k v] (friendly-text)
                                                               :when (string? (get-in v
                                                                                      [:values 0]))]
                                                           k))
                                                    [[:font-weight] [:mark :type]
                                                     [:encoding :x :type] [:encoding :y :type]
                                                     [:aggregate] [:sort] [:encoding :color :type]])
                            :renderer-fn      button-panel
                            :renderer-fn-ext  []}
       :dropdown           {:test-fn          string? ;#(or (integer? %) (cstr/ends-with? %
                            :keypath-contains [[:font-family] [:scheme] [:titleFont]
                                               [:monospaced-font] [:base-font] [:codemirror-theme]
       ...
       :color-picker       {:test-fn          #(true? (and (string? %)
                                                           (cstr/starts-with? (str %) "#")))
                            :keypath-contains []
...
(defn get-type
  ...
  (first (sort (remove nil?
                 (for [[k {:keys [test-fn keypath-contains renderer-fn renderer-fn-ext]}]
                         (scrubbers)]
                   (when (and (test-fn v) ;; run the checker fn
                              (if (or (nil? keypath-contains) ;; see if the keys given are an
                                                              ;; ordered
