...
(defn transform-pre-deep-nest
  ...
  (if (get @transform-cache [(hash dsl) (hash data)])
    ...
    (let [{:keys [transform-select from fill-gaps? pivot-by order-nest-by nest-by]} dsl
          agg-specs (filter #(vector? (try (first %) (catch :default _ nil))) transform-select)
          dim-specs (filter #(not (vector? (try (first %) (catch :default _ nil)))) transform-select)
                             ...
                             :data (vec (map (fn [val]
                                               (let [dim-maps (apply merge
                                                                (map-indexed (fn [idx dim-spec]
                                                                               (let [[dim-key dim-alias] (process-dim-spec
                                                                                                           dim-spec)]
          ...
          output (cond single-nest-string? (assoc-in output [0 :id] nest-key)
                       nest-key            (let [x-vals (vec (sort (distinct (apply concat
                                                                               (for [{:keys [data]} output]
                                                                                 (distinct (map :x data)))))))]
...
(defn deep-nest
  ...
  (if (empty? fields)
    ...
    (let [field   (first fields)
      ...
      (map (fn [[group vals]] {:name group :children (vec (deep-nest (rest fields) vals agg-specs dim-specs))}) grouped))))
...
(defn transform
  ...
  (if (get @transform-cache [(hash dsl) (hash data)])
    ...
    (let [{:keys [transform-select from fill-gaps? pivot-by order-nest-by nest-by]} dsl
          ...
          all-pivot-keys (distinct (map #(get % pivot-key) data))
          group-fn (fn [row] (mapv (fn [dim-spec] (let [[dim-key _] (process-dim-spec dim-spec)] (get row dim-key))) dim-specs))
          output (cond (and nest-key (> (count nest-by) 1))
                         (let [top-level-nest-key (try (if (string? (first nest-key))
                                                         (first nest-key)
                                                         (str "(all " (name (first nest-key)) ")"))
                                                       (catch :default _ "(all)"))
                               nest-keys          (vec (remove string? nest-by))]
                           {:name top-level-nest-key :children (vec (deep-nest nest-keys data agg-specs dim-specs))})
                       nest-key (let [grouped (group-by #(get % nest-key) data)]
                                  (vec (map (fn [[group vals]]
                                              {:id   group
                                               :data (vec (map (fn [val]
                                                                 (let [dim-maps (apply merge
                                                            ...
                                                            vals))})
                                         grouped)))
                       :else (let [grouped     (group-by group-fn data)
                                   single-agg? (= 1 (count agg-specs))]
                               (vec
                                 (map
                                   (fn [[category vals]]
...
(def dsl-deep-nest {:transform-select [[[:sum :sales] :total-sales] :product] :from [:data] :nest-by [:region :category :rep]})
