...
(defn transform-pre-deep-nest
  ...
  (if (get @transform-cache [(hash dsl) (hash data)])
    ...
    (let [{:keys [transform-select from fill-gaps? pivot-by order-nest-by nest-by]} dsl
                         ...
                         (let [pivot-vals (vals-by-pivot-key vals pivot-key)
                               pivot-map  (apply merge
                                            (map (fn [pivot-val-group]
                                                   (let [color    (get (first pivot-val-group)
                                                                       pivot-key)
                                                         color    (if (nil? color) "IS_NULL" color)
                                                         agg-maps (calculate-aggregations
                                                                    agg-specs
                                                                    pivot-val-group)]
                                                     (into {}
          ...
          output
            (cond
              single-nest-string? (assoc-in output [0 :id] nest-key)
              nest-key            (let [x-vals (vec (sort (distinct (apply concat
                                    ...
                                    (vec (for [{:keys [id data]} output]
                                           {:id   id
                                            :data (let [this-x  (vec (sort (distinct (map :x
                                                                                       data))))
                                                        whole?  (= this-x x-vals)
                                                        missing (vec (cset/difference (set x-vals)
                                                                                      (set this-x)))
...
(defn deep-nest
  ...
  (if (empty? fields)
    ...
    (let [field   (first fields)
      ...
      (map (fn [[group vals]]
             {:name group :children (vec (deep-nest (rest fields) vals agg-specs dim-specs))})
        grouped))))
