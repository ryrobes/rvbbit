...
(defn debounce
  [f interval]
  (let [dbnc (Debouncer. f interval)] (fn [& args] (.apply (.-fire dbnc) dbnc (to-array args)))))
...
(defn editor-panel-metadata
  ...
  (let [sql-calls  {:connections-sys {:select [;:database_name
                                               [[:case [:like :connection_str "%csv%"]
                                                 "*csv-import-db*" [:like :connection_str "%cache%"]
                                                 "*cache-db*" :else :database_name] :database_name]
                                               :connection_id]
                                      :from   [:connections]}
                    :tables-sys      {:select   [:db_schema :db_catalog :connection_id
                                                 [[:|| :db_catalog "/" :db_schema] :schema_cat]
                                                 :table_name [[:count 1] :fields]]
                                      :from     [:fields]
    ...
    [re-com/h-box :size "auto" :style {:color (str (theme-pull :theme/editor-font-color nil) 35)} ;; rows
                                                                                                  ;; label
                                                                                                  ;; under
     :children
     [[re-com/box :size "auto" :child
       [bricks/magic-table :system-connections-list* [:connections-sys] 3 10 [:database_name]]]
      [re-com/box :size "auto" :child
       [bricks/magic-table :system-tables-list* [:tables-sys] 9 10
        [:db_schema :connection_id :db_catalog]]]]]))
...
(defn search-panel-metadata
  ...
  (let [client-name (ut/replacer (str @(ut/tracked-subscribe [::bricks/client-name])) ":" "")
    ...
    [re-com/h-box :size "auto" :style {:color (str (theme-pull :theme/editor-font-color nil) 35)} ;; rows
     ...
     :children
     [[re-com/box :size "auto" :child
       [bricks/magic-table :searches-types-list* [:searches-types-sys] 3 10 []]] ;; last
      [re-com/box :size "auto" :child
       [bricks/magic-table :searches-sub-types-sys-list* [:searches-sub-types-sys] 9 10
        [:is_live]]]]]))
...
(defn search-panel-metadata-ext
  ...
  (let [sql-params         (into {}
        ...
        sql-calls          {:searches-rows-sys
                              {:select [:item_sub_type :item_type :item_key :display_name :sample
                                        :value :is_live :block_meta] ;; :searches-types-sys/item_type
    ...
    (dorun (for [[k v] sql-calls]
             (let [query        (ut/postwalk-replacer sql-params v)
                   ...
                   unrun-sql?   @(ut/tracked-subscribe [::conn/sql-query-not-run? [k] query])]
               (when (or (not data-exists?) unrun-sql?) (conn/sql-data [k] query)))))
    [re-com/h-box :size "auto" :style {:color (str (theme-pull :theme/editor-font-color nil) 35)}
     :children
     [[re-com/v-box :size "auto" :children
       [[re-com/box :size "auto" :child
         [bricks/magic-table :searches-rows-sys-list* [:searches-rows-sys] 11
          (if selected-shortcode 8.5 10) [:value :is_live :item_type :block_meta :item_key]]]
        (when selected-shortcode
          [re-com/box :child
           [bricks/shortcode-box 560 85 (str selected-shortcode " ;; rabbit-code parameter")
            "clojure"]])]]]]))
  ...
  (let [all-sql-call-keys @(ut/tracked-subscribe [::bricks/all-sql-call-keys])
        ...
        sql-params (into {}
                         (for [k [:viz-tables-sys/table_name :viz-shapes0-sys/shape
                                  :viz-tables-sys/table_name :viz-shapes0-sys/shape
                                  :viz-shapes-sys/combo_edn :user-dropdown-sys/req-field]]
                           {k ;;@(ut/tracked-subscribe [::conn/clicked-parameter-key [k]])
        ...
        sql-calls
          {:viz-tables-sys  {:select   [:table_name [[:count 1] :recos]]
                             :from     [:viz_recos_vw]
        ...
        current-tab-queries (try (map #(-> %
                                 ...
                                 (catch :default _ []))
        block-list (vec (filter (fn [x] (some #(= % (get x :table_name)) current-tab-queries))
                          block-list)) ;; overwrite above with curr tab only
        recos-count (count full-recos)
        block-list-boxes
          (for [{:keys [table_name recos]} block-list
                :let                       [sel  (get sql-params :viz-tables-sys/table_name)
                                            sel? (= table_name sel)]]
            [re-com/box :attr
             {:on-click #(do (ut/tracked-dispatch [::conn/click-parameter
                                                   [:viz-tables-sys :table_name] table_name])
        ...
        preview-maps (into {}
                           (for [{:keys [combo_hash shape_name combo_edn query_map viz_map condis h
                                         w selected_view]}
                                   full-recos]
          ...
          (fn [preview-maps pk pnum]
            (try
              (let [panel-key (nth pk pnum)
                    ...
                    selected-view (try (edn/read-string
                                         (get-in preview-maps [panel-key :selected_view] nil))
                                       (catch :default _ nil))
                    condis (get-in preview-maps [panel-key :condis])
                    combo_hash (get-in preview-maps [panel-key :combo_hash])
                    combo_edn (try (when (not (nil? ce)) (ut/replacer ce #"\"" ""))
                    ...
                    body [re-com/v-box :size "none" :attr
                          {:on-click
                             #(do (ut/tracked-dispatch [::conn/click-parameter
                                                        [:recos-sys :combo_hash] combo_hash])
                                  (ut/tracked-dispatch [::conn/click-parameter
                                                        [:recos-sys :combo_edn] combo_edn])
                                  (ut/tracked-dispatch [::conn/click-parameter
                                                        [:recos-sys :shape_name] shape_name])
                                  (ut/tracked-dispatch [::conn/click-parameter
                                                        [:recos-sys :selected_view] selected-view])
                                  (ut/tracked-dispatch [::conn/click-parameter [:recos-sys :h] h])
                                  (ut/tracked-dispatch [::conn/click-parameter [:recos-sys :w] w])
                                  (ut/tracked-dispatch [::conn/click-parameter
                                                        [:recos-sys :query_map] query_map])
                                  (ut/tracked-dispatch [::conn/click-parameter [:recos-sys :viz_map]
                                                        viz_map])
                                  (ut/tracked-dispatch [::conn/click-parameter [:recos-sys :condis]
                                                        condis]))} :width "581px" :height "434px" ;"200px"
                          :style
                          {:zoom      0.44
                          ...
                          [[re-com/box :size "auto" :height "40px" :style
                           ...
                           [re-com/box :size "auto" :height "40px" :justify :end :align :end :style
                            {:padding-left  "14px"
                             :padding-right "14px"
                             :font-weight   400
                             :font-size     "22px"} :child (str shape_name)]]]]
                (if sel?
                  (bricks/draggable
                    (bricks/sql-spawner-meta :viz-reco)
                    "meta-menu"
                    [re-com/box :style
              ...
              (catch :default _
                [re-com/box :width "240px" :size "auto" :height "400px" :align :center :justify
                 :center :style
        ...
        pages (/ recos-count 6)]
    (dorun
      (let [prev-preview-keys (for [k @(ut/tracked-subscribe [::bricks/preview-keys])]
    ...
    [re-com/v-box :height "433px" :children
     [[re-com/h-box :height "39px" :gap "4px" :style
       {:overflow "auto" :padding-bottom "5px" :padding-left "7px"} :children block-list-boxes]
      [re-com/h-box :height "40px" :gap "12px" :align :center :justify :start :style
       ...
       [[re-com/single-dropdown :choices
         (conj (for [c combo-singles] {:id c :label c}) {:id nil :label "(all possible fields)"})
         ...
         #(ut/tracked-dispatch [::conn/click-parameter [:user-dropdown-sys :req-field] %]) :width
         "285px"]
        [re-com/single-dropdown :choices
         (conj (for [{:keys [combo_edn recos]} combo-list]
                 {:id combo_edn :label (str combo_edn " (" recos ")")})
               {:id nil :label "(all possible field combos)"}) :max-height "320px" :placeholder
         "(all possible field combos)" :style {:font-size "13px"} :model
         (get sql-params :viz-shapes-sys/combo_edn) :on-change
         #(ut/tracked-dispatch [::conn/click-parameter [:viz-shapes-sys :combo_edn] %]) :width
         "660px"]
        [re-com/box :child "grid" :attr
         {:on-click #(ut/tracked-dispatch [::bricks/set-grid-reco? true])} :style
         {:color       (if grid-reco? (theme-pull :theme/editor-font-color nil) "inherit")
          :cursor      "pointer"
          :margin-top  "-9px"
          :font-weight 700}]
        [re-com/box :child "previews" :attr
         {:on-click #(ut/tracked-dispatch [::bricks/set-grid-reco? false])} :style
      ...
      [re-com/h-box :size "auto" :style {:color (str (theme-pull :theme/editor-font-color nil) 35)}
       :children
       [[re-com/box :size "none" :width "300px" :child
         [bricks/magic-table :viz-shapes0-list* [:viz-shapes0-sys] 7 8 []]]
        (if grid-reco?
          [re-com/box :size "none" :width "880px" :child
           [bricks/magic-table :recos-list* [:recos-sys] 19 8
            [;(when shape-picked? :shape_name)
             :query_map_str :table_name :context_hash :connection_id :h :w :condis :viz_map
             :combo_hash]]]
          (let [] ;; get the next 6 graphs and render?
            [re-com/h-box :children
             [[re-com/v-box :size "auto" :height "380px" :width "770px" :children
              ...
              [re-com/v-box :size "auto" :height "380px" :style
               {:border-top "1px solid #9973e066" :overflow "hidden"} :children
               (let []
                 (for [c (range pages)]
  ...
  (let [all-sql-call-keys @(ut/tracked-subscribe [::bricks/all-sql-call-keys])
    ...
    [re-com/h-box :size "auto" :style {:color (str (theme-pull :theme/editor-font-color nil) 35)}
     ...
     [[re-com/v-box :children
       [[re-com/box :size "auto" :child
         [bricks/magic-table :viz-tables-list* [:viz-tables-sys] 7 5 []]]
        [re-com/box :size "auto" :child
         [bricks/magic-table :viz-shapes0-list* [:viz-shapes0-sys] 7 5 []]]]]
      [re-com/box :size "auto" :child
       [bricks/magic-table :viz-shapes-list* [:viz-shapes-sys] 7 10 []]]]]))
...
(defn editor-panel-status
  ...
  (let [;all-sql-call-keys @(ut/tracked-subscribe [::bricks/all-sql-call-keys])
        ...
        sql-calls   {:status-sys
                       {:select [:*] :from [:latest_status] :where [:= :client_name client-name]}}]
    (dorun (for [[k v] sql-calls]
             (let [query        (ut/postwalk-replacer sql-params v)
                   data-exists? @(ut/tracked-subscribe [::conn/sql-data-exists? [k]])
                   unrun-sql?   @(ut/tracked-subscribe [::conn/sql-query-not-run? [k] query])]
               (when (or (not data-exists?) unrun-sql?) (conn/sql-data [k] query)))))
    [re-com/box :size "auto" :child
     [bricks/magic-table :status-sys-list* [:status-sys] 11 20 [:ts :client_name]]]))
...
(defn editor-panel-metadata-files
  ...
  (let [sql-calls  {:files-sys {:select [:*] :order-by [[:screen_name :asc]] :from [:screens]}}
    ...
    [re-com/h-box :size "auto" :style {:color (str (theme-pull :theme/editor-font-color nil) 35)}
     ...
     [;[re-com/box
      [re-com/box :size "auto" :child
       [bricks/magic-table :files-list* [:files-sys] 11.5 10 [:file_path :ts]]]]]))
...
(defn editor-panel-metadata-kits
  ...
  (let [;market (group-by :package-name @(ut/tracked-subscribe [::bricks/all-kits])) ;
        ...
        curr       @(ut/tracked-sub ::conn/clicked-parameter-key-alpha
                                    {:keypath [:kits-sys/enabled]})
        kit-market {:installed-packages pkits1 :available-packages market}
        hackreact  [@db/kit-browser]]
    [re-com/v-box :size "auto" :style
     {:color (str (theme-pull :theme/editor-font-color nil)) :margin-top "6px" :overflow "auto"}
     :children
     (conj
             ...
             [(str (try (name typer) (catch :default _ (str typer))))
              [re-com/box :style {:opacity 0.33 :font-size "15px"} :child
               (str " ("
                    (count pkits)
                    " package"
                    (when (> (count pkits) 1) "s")
                    ", "
                    (count (flatten (vals pkits)))
                    " kits)")]]]]
           (if (empty? pkits)
             [re-com/box :size "none" :align :center :justify :center :height "60px" :child
              (if (= typer :installed-packages)
                "(no kits installed)"
                "(no installable kits found)")]
                ...
                [re-com/v-box :children
                 [[re-com/box :padding "9px" :style {:font-size "20px" :font-weight 700} :child
                  ...
                  [re-com/v-box :style {:margin-left "19px"} :children
                   (for [{:keys [description fn when name icon author run-on repl installed?
                     ...
                     [re-com/h-box :padding "5px" :gap "22px" :attr
                      {:on-click
                         #(reset! db/kit-browser (if selected? nil [package-name kit-name]))}
                      :justify :between :align :center :style
                      ...
                      [[re-com/box :min-width "100px" :style
                       ...
                       [re-com/box :size "auto" :style
                        {:opacity (if (and (not selected?) disabled?) 0.3 1)} :child
                        (str description)]]])]]])])]])
...
(defn editor-panel-metadata-ext-kits
  ...
  (let [pkits1 (group-by :package-name @(ut/tracked-subscribe [::bricks/installed-kits]))
        ...
        kit-body (filter #(= (get % :kit-name) (second @db/kit-browser))
                   (get pkits (first @db/kit-browser) {}))
        display-map (dissoc (first kit-body) :fn :when)
        curr @(ut/tracked-sub ::conn/clicked-parameter-key-alpha {:keypath [:kits-sys/enabled]})
        install-box [re-com/h-box :height "60px" :size "none" :padding "10px" :justify :between
                     :children
                     [[re-com/box :size "auto" :align :center :justify :center :style
                       {:border "1px solid white" :cursor "pointer"} :attr {:on-click #()} :child
                       "download and install?" :width "240px"]]]
        enabled? (some (fn [x] (= x @db/kit-browser)) curr)
        enable-box
          [re-com/h-box :height "60px" :size "none" :padding "10px" :justify :between :children
           [[re-com/box :size "auto" :align :center :justify :center :style
             ...
             :child "enabled" :width "240px"]
            [re-com/box :size "auto" :align :center :justify :center :style
             {;:border "1px solid white"
              :cursor          "pointer"
              :font-size       "28px"
              :text-decoration (when (not enabled?) "line-through")
              :color           (when (not enabled?)
                                 (theme-pull :theme/editor-grid-selected-background-color nil))
              :border          (when (not enabled?)
                                 (str "3px solid "
                                      (theme-pull :theme/editor-grid-selected-background-color
                                                  nil)))} :attr
             {:on-click #(ut/tracked-dispatch [::conn/click-parameter settings-param-kp
                                               {:enabled (vec (remove (fn [x] (= x @db/kit-browser))
                                                                curr))}])} :child "disabled" :width
             "240px"]]]]
    (if (empty? kit-body)
      [re-com/box :size "auto" :align :center :justify :center :child
       "(select a kit function or package name)"]
      [re-com/v-box :size "auto" :padding "6px" :style
       ...
       [(if (get display-map :installed?) enable-box install-box)
        [re-com/v-box :children
         [[re-com/v-box :children
           [[re-com/gap :size "8px"]
            [re-com/v-box :padding "8px" :gap "4px" :style
             {:font-size "27px" :color (theme-pull :theme/editor-font-color nil)} :children
             [[re-com/h-box :justify :between :align :center :children
               [[re-com/box :child (get display-map :name)]
                (cond (cstr/starts-with? (str (get display-map :icon)) "zmdi")
                        [re-com/md-icon-button :src (at) :md-icon-name (get display-map :icon)
                         :style
                         {;:background-color (theme-pull :theme/editor-rim-color "#a3a3a3")
                         }]
                      :else [re-com/box :style {:font-size "13px"} :child "no-icon"])]]
              [re-com/box :style {:font-size "17px"} :child (get display-map :description)]]]
            [re-com/gap :size "12px"]
            (when (get display-map :image-url)
              [re-com/box :align :center :justify :center :style {} :child
               [:img
                {:src (get display-map :image-url) :style {:border-radius "20px"} :width "480px"}]])
     ...
     [[re-com/h-box :justify :between :children
       [[re-com/box :child (str key-type "/*") :align :center :style
        ...
        (when (get @param-scrubber? key-type)
          [re-com/box :child
           [re-com/input-text :model (get @param-search key-type) :on-change
            #(swap! param-search assoc key-type %) :height "24px" :width "200px" :style
      ...
      (if (get @param-scrubber? key-type) ;theme-scrubber?
        [re-com/box :size "none" :width (px single-width) :height (px (- single-height 90)) ;;; [view?
         ...
         [bricks/scrubber-panel true ; view?
          @(rfa/sub ::bricks/keypaths-in-params {:key-type key-type}) key-type
          (get @param-search key-type) {:fm true}] ; view-key (update kp)
         :style {:overflow "auto"}]
      ...
      [re-com/box :child (str "scrubber " (if (get @param-scrubber? key-type) "on" "off")) :attr
       {:on-click
          #(swap! param-scrubber? assoc key-type (not (get @param-scrubber? key-type false)))}
       :style
       {:padding-left "18px"
...
(defn editor-panel-metadata-ext-files
  ...
  (let [sql-params (into {}
                         (for [k [:files-sys/file_path]]
                           {k ;;@(ut/tracked-subscribe [::conn/clicked-parameter-key [k]])
                              @(ut/tracked-sub ::conn/clicked-parameter-key-alpha {:keypath [k]})}))
        sql-calls  {:blocks-sys {:select   [:block_key :block_name :queries :views :block_data
                                            :view_names :query_names]
                                 :from     [:blocks]
    ...
    [re-com/h-box :size "auto" :style {:color (str (theme-pull :theme/editor-font-color nil) 35)}
     ...
     [;[re-com/box
      [re-com/v-box :size "auto" :children
       [[re-com/box :size "auto" :child
         [bricks/magic-table :blocks-list* [:blocks-sys] 11 10
  ...
  (let [sql-params (into {}
        ...
        sql-calls  {:fields-sys {:select   [:field_name :field_type :data_type]
                                 ...
                                 :where    [:and [:= :connection_id :connections-sys/connection_id]
                                            ...
                                            [:<> :field_type "derived"] [:<> :field_type "special"]]
                                 :order-by [:field_name]}}
      ...
      [re-com/v-box :size "auto" :children
       [[re-com/box :size "auto" :child
         [bricks/magic-table :system-fields-list*
          ...
          [:db_type :db_schema :table_type :database_name :db_catalog :table_name :connection_id
     ...
     [[re-com/v-box :size "auto" :children
       [[re-com/box :size "auto" :child
         [bricks/magic-table :recos-list* [:recos-sys] 11 10
          [(when shape-picked? :shape_name) :query_map_str :table_name :context_hash :connection_id
           :h :w :condis :viz_map (when combo-picked? :combo_edn)]]]]]]]))
...
(defn editor-panel-metadata-status
  ...
  (let [pending     @(ut/tracked-subscribe [::wfx/pending-requests http/socket-id])
        ...
        pending     (remove #(cstr/ends-with? (str (get-in % [:message :ui-keypath 0])) "-sys")
                      pending)]
         ...
         [[re-com/v-box :size "auto" :align :start :children
           (if (and (nil? l) (nil? ff))
             ...
             [[re-com/box :child (str ":s - " l)]
              [re-com/box :child (str ":f - " ff) :style {:color "#ffffff88"}]])]
  ...
  (let [;bw 33 bh 11
        ...
        params     (merge sched
                          {"client-name"                     @(ut/tracked-subscribe
                                                                [::bricks/client-name])
                           "[::bricks/last-heartbeat]"       @(ut/tracked-subscribe
                                                                [::bricks/last-heartbeat])
                           ...
                           "[::bricks/selected-block-map]"   @(ut/tracked-subscribe
                                                                [::bricks/selected-block-map])
  ...
  (let [client-name @(ut/tracked-subscribe [::bricks/client-name])
    ...
    [re-com/v-box :children
     [;[re-com/box
             ...
             (when (ut/ne? (cstr/trim (str %)))
               (ut/tracked-dispatch [::wfx/request :default
                                     {:message     {:kind        :search-groups
                                      ...
                                      :timeout     500000}]))) :height "50px" :width
        (px (- single-width 25)) :style
        {:background-color (theme-pull :theme/editor-background-color nil)
         :font-size        "25px"
         :font-weight      700
      ...
      [re-com/box :height (px (- single-height 120)) :style
       {:border "1px solid cyan" :overflow "auto"} :size "none" :child
       [re-com/v-box :padding "10px" :style {:color (theme-pull :theme/editor-font-color nil)}
        :children
        (for [[r cnt] results]
          [re-com/h-box :padding "4px" :justify :between :align :center :height "30px" :style
           {:border "1px solid darkcyan"} :children
           [[re-com/box :child (str r)] [re-com/box :child (str (ut/nf cnt))]]])]]] :size "auto"
     :width (px single-width) :height (px single-height)]))

(defn search-panel-right
  [single-width single-height]
  [re-com/box :child "search" :size "auto" :width (px single-width) :height (px single-height)
...
(defn editor-panel
  ...
  (let [selected-panel-map  @(ut/tracked-subscribe [::bricks/selected-block-map])
        ...
        views?              (ut/ne? views)
        view-selected?      (true? (or (and views?
                                            (some #(= % (get @db/data-browser-query selected-block))
                                                  (keys views)))
                                       (not queries?)))
        panel-count         (if system-panel? 3 (if (or views? queries?) 3 2))
        bricks-wide         (if (or system-panel? queries? views?)
                              bricks-wide
                              (js/Math.ceil (* bricks-wide 0.66)))
        ...
        hh1                 [@db/mad-libs-waiting-room @db/data-browser-query-con
                             @db/item-browser-mode @db/scrubbers @db/value-spy] ;; reactivity hack!
                                                                                ;; react!
        ...
        reco-selected       @(ut/tracked-sub ::conn/clicked-parameter-key-alpha
                                             {:keypath [:recos-sys/combo_hash]})
        reco-combo          @(ut/tracked-sub ::conn/clicked-parameter-key-alpha
                                             {:keypath [:recos-sys/combo_edn]})
        shape-name          @(ut/tracked-sub ::conn/clicked-parameter-key-alpha
                                             {:keypath [:recos-sys/shape_name]})
        reco-query          @(ut/tracked-sub ::conn/clicked-parameter-key-alpha
                                             {:keypath [:recos-sys/query_map]})
        reco-viz            @(ut/tracked-sub ::conn/clicked-parameter-key-alpha
                                             {:keypath [:recos-sys/viz_map]})
        reco-condis         @(ut/tracked-sub ::conn/clicked-parameter-key-alpha
                                             {:keypath [:recos-sys/condis]})
        ...
        mad-libs-combos     (when mad-libs-combo?
                              @(ut/tracked-subscribe [::bricks/get-combo-rows selected-block
                                                      (get selected-panel-map
                                                           :mad-libs-combo-hash)]))
        screen-name         (ut/safe-name @(ut/tracked-subscribe [::bricks/screen-name]))
        screen-name-regex   #"(.|\s)*\S(.|\s)*"
        websocket-status    (select-keys @(ut/tracked-subscribe [::http/websocket-status])
                                         [:status :datasets :panels :waiting])]
    ...
    (when (and (not (= reco-selected reco-preview)) reco-selected?)
      (insert-hidden-reco-preview reco-selected
                                  reco-viz
                                  reco-query
                                  reco-condis
                                  reco-combo
    ...
    [rc/catch
     [re-com/h-box :size "none" :children
      [[re-com/box :size "none" :child
         ...
         [[re-com/box :padding "4px" :child
                ...
                (if system-panel?
                  [re-com/h-box :align :center :justify :center :gap "15px" :children
                   [[re-com/box :child (str "db browser") :style
                    ...
                    [re-com/box :child (str "viz recos") :style
                     {:opacity (if (= @db/editor-mode :vvv) 1.0 0.4) :cursor "pointer"} :attr
                     {:on-click #(reset! db/editor-mode :vvv)}]
                    ...
                    [re-com/box :child (str "params") :style
                     {:opacity (if (= @db/editor-mode :params) 1.0 0.4) :cursor "pointer"} :attr
                     {:on-click #(reset! db/editor-mode :params)}]
                    ...
                    [re-com/box :child ;(str "*")
                     ...
                     {:opacity (if (= @db/editor-mode :status) 1.0 0.4) :cursor "pointer"} :attr
                    ...
                    [re-com/md-icon-button :src (at) :md-icon-name "zmdi-refresh" :style
                     {;:background-color (theme-pull :theme/editor-rim-color "#a3a3a3")
                      ...
                      :margin-top    "-6px"
                      :font-size     "19px"} :attr
                     {:on-click #(ut/tracked-dispatch [::bricks/refresh-meta-tables
                                                       @db/editor-mode])}]]]
          ...
          (cond
            system-panel? (condp = @db/editor-mode ; @file-mode?
                            :files  [re-com/box :child [editor-panel-metadata-files] :style
                                     {:padding-top "10px"} :height (px (- ttl-height 40))]
                            :kits   [re-com/box :child [editor-panel-metadata-kits] :style
                                     {:padding-top "10px"} :height (px (- ttl-height 40))]
                            :params [re-com/box :child
                                     [editor-panel-metadata-params single-width single-height
                                      :param] :style {:padding-top "10px"} :height
                                     (px (- ttl-height 40))]
                            ...
                            :search [re-com/box :child [search-panel-metadata] ;;[search-panel-left
                                                                               ;;single-width
                                     :style {:padding-top "10px"} :height (px (- ttl-height 40))]
                            :vvv    [re-com/box :child [editor-panel-viz2] :style
                                     {:padding-top "10px"} :height (px (- ttl-height 40))]
                            :status [re-com/box :child [ui-debugger] :style {:padding-top "10px"}
            ...
            (= :viz-gen (get @db/data-browser-query selected-block)) ;; viz-gen?
              (let [src-table-id-str (last (get selected-panel-map :mad-libs-combo-hash))
                    ...
                    opts             @(ut/tracked-subscribe [::bricks/get-mad-libs-options
                                                             selected-block src-table-id-str
                                                             combo-hash])
                 ...
                 [[re-com/h-box :justify :between :children
                   [[re-com/box ;:align :end  ;:align :center ;:justify :end
                    ...
                    [re-com/md-icon-button :src (at) :md-icon-name
                     (if open-shape? "zmdi-chevron-down" "zmdi-chevron-up") :attr
                     {:on-click #(swap! db/mad-libs-box assoc-in
                                   [selected-block shape-name]
                                   (not open-shape?))} :style
                     {:color         (theme-pull :theme/editor-outer-rim-color nil)
                      :padding-right "16px"
                      :font-size     "25px"}]]]
                  (when open-shape?
                  ...
                  [re-com/v-box :padding "6px" :gap "8px" :height (px (- ttl-height 34)) :style
                   ...
                   (for [[k o] (group-by #(first (first %)) opts-data)
                         :let  [short-axes (-> (str k)
                                               (ut/replacer #"-field" "")
                                               (ut/replacer #":" ""))
                                open?      (get-in @db/mad-libs-box
                                                   [selected-block short-axes]
                                                   false)]]
                     [re-com/v-box ;:size "auto"
                      :padding "9px" :justify :between :style
                      {:border         (str "2px dashed "
                                            (theme-pull :theme/editor-outer-rim-color nil)
                                            44)
                       :transition_NOT "all 0.2s ease-in-out"
                       :border-radius  "16px"} :children
                      [[re-com/v-box :children
                        [[re-com/h-box :style {:transition_NOT "all 0.2s ease-in-out"} :align
                          :center :justify :between :children
                                 ...
                                 (bricks/draggable
                                   {:h 6
                                    ...
                                    :name (str "show me why - fields " short-axes
                                               ...
                                               " / "                   shape-name)
                                    :queries
                                      {q1 {:select     [:axes_key :connection_id :db_type
                                                        [[:case [:= :derived_name nil] :field_name
                                                          :else :derived_name] :field] :field_name
                                                        :derived_calc :derived_name :shape_name
                                                        :table_name]
                                           :from       [[:found_fields]]
                                           :where      [:and [:= :table_name src-table-id-str]
                                                        [:= :shape_name shape-name]
                                                        [:= :axes_key short-axes]]
                                           :col-widths {:axes_key      90
                                                        :table_name    180
                                                        :connection_id 90}}
                                       q2 {:select     [:attribute_name :attribute_value
                                                        :connection_id ;:context_hash
                                                        :db_type
                                                        [[:case [:= :derived_name nil] :field_name
                                                          :else :derived_name] :field] :field_name
                                                        :derived_calc :derived_name :table_name ;:table_type
                                                                                                ;:updated
                                                       ]
                                           :from       [[:attributes :nn359]]
                                           :where      [:and [:= :table_name src-table-id-str]]
                                           :col-widths {:attribute_name 227}}
                                       q3 {:transform-select [[[:sum :attribute_value] :val] :field
                                                              :derived_name :derived_calc
                                                              :table_name]
                                           :from             [q2-ref]
                                           :pivot-by         [:attribute_name]
                                           :col-widths       {:field 240 :table_name 200}}}
                                    :tab @(ut/tracked-subscribe [::bricks/selected-tab])
                                    :drag-meta {:block-name (str "show-me-" (rand-int 1234))
                                                :type       :viz-reco}}
                                   "meta-menu"
                                   [re-com/box :child "show me why" :padding "5px" :style
                                    {:background-color (str (theme-pull
                                     ...
                                     :color            (theme-pull :theme/editor-outer-rim-color
                                                                   nil)
                                     :border-radius    "10px"}])))
                             [re-com/md-icon-button :src (at) :md-icon-name
                              (if open? "zmdi-chevron-down" "zmdi-chevron-up") :attr
                              {:on-click #(swap! db/mad-libs-box assoc-in
                                            [selected-block short-axes]
                                            (not open?))} :style
                              {:color     (theme-pull :theme/editor-outer-rim-color nil)
                                ...
                                :let    [fname              (get (last ov) :field_name)
                                         ...
                                         cur-axes-keyhashes (into {}
                                                                  (for [[k v] (get opts
                                                                                   :current-combo)]
                                                                    {k (get v :key_hash)}))
                                         cur-axes-rev       (for [r (vals (dissoc
                                                                            (get-in
                                                                              opts
                                                                              [:current-combo])
                                                                            axes))]
                                         ...
                                         new-curr-axe       (into {}
                                                                  (sort-by key
                                                                           (merge cur-axes-keyhashes
                                                                                  new-axe)))
                                         heat-color         (value-to-color all-score2
                                                                            colors1
                                                                            score2)
                                         heat-color         (if (nil? heat-color)
                                                              zero-color
                                                              heat-color)
                                         loading?           (= new-curr-axe
                                                               @db/mad-libs-waiting-room)]]
                            [re-com/h-box :style
                             {;:margin-left "3px"
                              :transition_NOT  "all 0.2s ease-in-out"
                             ...
                             [[re-com/box :child " " :height "12px" :width "12px" :style
                              ...
                              [re-com/h-box :gap "8px" :size "auto" :align :center :justify :between
                               ...
                               {:font-size    "13px"
                                :font-weight  700
                                :padding-left "5px"
                                :font-style   (if (or (not (nil? drname)) (= fname "rows"))
                                                "italic"
                                                "inherit")
                                :font-family  (theme-pull :theme/base-font nil)} :children
                               [[re-com/box :style
                                 {:background-color (if loading? "#ffffff21" "inherit")
                                  :border-radius    "14px"} :child
                                 (str (if (nil? drname) fname drname))]
                                [re-com/h-box :gap "5px" :children
                                 [(when open?
                                    [re-com/box :style {:font-weight 200 :opacity 0.7} :child
                                     (str "score: " (rounder score2))])
                                  (when loading? ;(= new-curr-axe @db/mad-libs-waiting-room)
                                    [:img
                                     {:src    "images/loading-cropped.gif"
                                      :width  "19px"
                                      :height "19px"}])]]]]] :attr
                             {:on-click #(when (not (or cur? cur-rev?))
                                           (let []
                                             (ut/tracked-dispatch [::bricks/get-combo-hash
                                                                   ...
                                                                   new-curr-axe])))}])]]]]])]]])
            :else
              (let [selected-kp @(ut/tracked-subscribe [::bricks/editor-panel-selected-view])
                    selected-kp (if (nil? (first selected-kp)) nil selected-kp)] ;; if viz-gen or
                (if ;(and ;view-selected?
                  ...
                  [re-com/box :size "none" :width (px single-width) :height
                   ...
                   [bricks/scrubber-panel view-selected?
                    (if view-selected?
                      @(ut/tracked-subscribe [::bricks/keypaths-in-view selected-block data-key])
                      @(ut/tracked-subscribe [::bricks/keypaths-in-query selected-block data-key]))
                    data-key nil {:fm true}] :style {:overflow "auto"}]
                  [bricks/panel-code-box selected-block selected-kp (+ 17 single-width)
                   (- single-height 20)
                   (if (nil? selected-kp)
                     selected-panel-map
        ...
        (if (and (= @db/editor-mode :vvv) system-panel?) (px (* 2 single-width)) single-width-px)
       ...
       (cond
         (or views? queries?) ; data-key
           (let []
             [re-com/box :size "none" :child
              [re-com/v-box :size "1" :children
               [[re-com/box :padding "4px" :child (str "queries & views") :style
                 {:font-weight 500
                  :color       (theme-pull :theme/editor-font-color nil)
                  :background  (str "linear-gradient("
                                    (theme-pull :theme/editor-rim-color nil)
                ...
                [re-com/h-box :justify :between :children
                 [[re-com/h-box :children
                   [(when mad-libs-combos
                      [re-com/h-box :gap "7px" :padding "5px" :style
                       {:font-weight 700 :color "#dddddd"} :children
                    ...
                    [re-com/h-box :gap "7px" :padding "5px" :style
                     ...
                     [[re-com/h-box :style
                       {:margin-left      "-5px"
                        ...
                        :padding-left     "5px"} :gap "4px" :children
                       [[re-com/box :padding "3px" :attr
                         {:on-click #(do (swap! db/data-browser-query assoc selected-block :*))}
                         :style
                         {:cursor        "pointer"
                          :font-size     "13px"
                          :padding-left  "5px"
                          :padding-right "5px"} :child (str :*)]]]]]
                    (when queries?
                      [re-com/h-box :gap "7px" :padding "5px" :style
                       {:font-weight 700 :color "#dddddd"} :children
                           ...
                           [re-com/h-box :style
                            ...
                            [[re-com/box :padding "3px" :attr
                              ...
                              {:cursor        "pointer"
                               ...
                               :padding-right "5px"} :child (str k)]]]))])]]
                  [re-com/h-box :gap "7px" :padding "5px" :style
                   {:font-weight 700 :color "#dddddd66" :margin-top "3px"} :children
                   [[re-com/box :attr
                               ...
                               [{:data '(vec
                                         (for
                                          ...
                                          {:row_id i
                                           ...
                                           :name   (str "row " i)}))}]}])} :style
                     {:cursor "pointer"} :child "+code"]
                    [re-com/box :attr
                     {:on-click #(ut/tracked-dispatch [::bricks/add-new selected-block :views
                                                       [:box :child "heya" :align :center :justify
                                                        :center :size "auto" :style
                                                        {:font-size "22px"}]])} :style
                     {:cursor "pointer"} :child "+view"]]]]]
                (if @db/bad-form?
                  [re-com/v-box :padding "12px" :style
                   ...
                   [[re-com/h-box :size "auto" :children
                     [[re-com/md-icon-button :src (at) :md-icon-name "zmdi-chevron-left" :style
                      ...
                      [re-com/md-icon-button :src (at) :md-icon-name "zmdi-help" :tooltip "help?"
                       :style
                       {:color     (theme-pull :theme/editor-outer-rim-color nil)
                        :font-size "15px"
                        :opacity   0.22}]]]
                  ...
                  (let [query-box? (or (some #(= % (get @db/data-browser-query selected-block))
                                             (keys sql-calls))
                                       (some #(= % data-key) (keys sql-calls)))
                        is-layout? @(ut/tracked-subscribe [::bricks/is-layout? selected-block
                                                           data-key])
                        viz-gen?   (= :viz-gen (get @db/data-browser-query selected-block))]
                    [re-com/v-box :children
                     [(cond
                        query-box? [re-com/v-box :justify :between :children
                                    (if (get @db/data-browser-query-con data-key)
                                      [(let [repl-output (dissoc @(ut/tracked-subscribe
                                                                    [::bricks/repl-output data-key])
                                                           :status)
                                             console     (vec (remove empty?
                                                                (get repl-output :out [])))]
                                         [re-com/v-box :padding "9px" :size "none" :height
                                          (px (- single-height 107)) :style
                                          {;:border "1px solid pink"
                                           :overflow "auto"} :children
                                          [(when (not (empty? console))
                                             [re-com/box :child
                                              "console output (limited to 50 items)"])
                                           (when (not (empty? console))
                                             [re-com/v-box :children
                                              (for [r console] [re-com/box :child (str r)])])
                                           [shape/map-boxes2 (dissoc repl-output :out) "no-block"
                                            "no-flow" [] "no-block" "map"]]])]
                                      [@(ut/tracked-subscribe [::bricks/sql-data-table-editor
                                                               selected-block [data-key]
                                                               single-width-bricks 6.05])
                                       (when (not @bricks/dragging?)
                                         [bricks/read-only-sql-box single-width
                                          (/ single-height 3.2) (str sql-string)])])]
                        viz-gen? ;[re-com/box :child (str (count mad-libs-combos) " rows. ")]
                          (let [;default-combo-view @(ut/tracked-subscribe
                                src-table-id-str   (last (get selected-panel-map
                                                              :mad-libs-combo-hash))
                                combo-hash         (first (get selected-panel-map
                                                               :mad-libs-combo-hash))
                                opts               @(ut/tracked-subscribe
                                                      [::bricks/get-mad-libs-options selected-block
                                                       src-table-id-str combo-hash])
                                shape-name         (get opts :shape_name)
                                viz-shape          @(ut/tracked-subscribe [::bricks/get-viz-shape
                                                                           shape-name])
                                default-shape-view (get viz-shape :selected_view)
                                default-shape-view (try (when (not (empty? default-shape-view))
                                                          (keyword (ut/replacer default-shape-view
                                                                                #":"
                                                                                "")))
                                                        (catch :default _ nil))]
                            (ut/tapp>> [:def-sel default-shape-view viz-shape])
                            [re-com/box :size "none" :style {:transform "translate(0)"} ;; a known
                                                                                        ;; CSS hack
                                                                                        ;; for fixed
                             :height (px (- single-height 100)) :child
                             [bricks/honeycomb selected-block (or default-shape-view :oz)]])
                        :else      [re-com/box :size "none" :style {:transform "translate(0)"} ;; a
                                                                                               ;; known
                                                                                               ;; CSS
                                                                                               ;; hack
                                                                                               ;; for
                                                                                               ;; fixed
                                    :height (px (- single-height 100))                         ;; (get
                      ...
                      (if viz-gen? ;; dont show scrubber boolean if on viz-gen mode
                        ...
                        (let [view-scrubbers? (get-in @db/scrubbers [selected-block data-key] false)
                              value-spy?      (get-in @db/value-spy
                                                      [selected-block data-key]
                                                      false)]
                          [re-com/h-box :gap "10px" :children
                           [[re-com/box :size "none" :width "90px" :child
                             (if view-scrubbers? "scrubber on" "scrubber off") :attr
                             {:on-click #(swap! db/scrubbers assoc-in
                                           [selected-block data-key]
                                           (not view-scrubbers?))} :style
                            ...
                            [re-com/box :size "none" :width "90px" :child "value spy" :attr
                             {:on-click #(swap! db/value-spy assoc-in
                                           [selected-block data-key]
                                           (not value-spy?))} :style
                             {:color           (if value-spy? "yellow" "grey")
                              ...
                              :cursor          "pointer"
                              :font-size       "11px"
                              :font-weight     700}]]]))]]))]] :height (px (- ttl-height 24)) :width
              single-width-px :style {:overflow "hidden"}])
                                ...
                                [[re-com/box :padding "4px" :child (str "(fields, data-types)")
                                 ...
                                 [re-com/box :style {:padding-top "10px"} :child
                                  (condp = @db/editor-mode ; @file-mode?
                                    ...
                                    :params [editor-panel-metadata-params single-width single-height
                                             :theme]
                                  ...
                                  ) :height (px (- ttl-height 40))]]] :height single-height-px
                               :width (if (= @db/editor-mode :vvv) "0px" single-width-px) :style
                ...
                [re-com/h-box :width (px (* single-width 0.538)) :justify :between :children
                 [[re-com/box :child "parameters"]
                  [re-com/h-box :gap "3px" :style {:font-size "11px" :margin-top "3px"} :children
                   (vec
                     (for [f (keys @db/param-filter)]
                       [re-com/box :child (ut/safe-name f) :attr
                        {:on-click
                           #(swap! db/param-filter assoc f (not (get @db/param-filter f false)))}
                        :style
                        {:text-decoration (if (get @db/param-filter f true) "none" "line-through")
                         ...
                         :padding-left    "4px"
                         :padding-right   "4px"}]))]]]]
               [bricks/click-param-browser click-params (* single-width 0.6)
                (- single-height 1.3)]]]
                  ...
                  (for [b    ["queries" "blocks"]
                    ...
                    [re-com/box :attr {:on-click #(reset! db/item-browser-mode (keyword b))} :style
                     (if selected?
                       ...
                       {:cursor "pointer" :user-select "none" :opacity 0.3}) :child (str b)])]
                 [re-com/md-icon-button :md-icon-name "zmdi-window-minimize" :on-click
                  #(ut/tracked-dispatch [::bricks/toggle-editor]) :style
               ...
               [re-com/box :style {:padding-right "3px"} :child
                (cond (= @db/item-browser-mode :queries) [bricks/screen-query-browser
                      ...
                      (= @db/item-browser-mode :blocks)  [bricks/screen-block-browser
                                                          (* single-width 0.379)
                                                          (- single-height 1.3)]
                      :else                              [re-com/box :child
                                                          "nothing selected above?"])]]]] :height
            single-height-px :width single-width-px :style
            {;:border "1px solid white" :background-color "#1f2430"
             :overflow "hidden"}])
         [re-com/h-box :height "42px" :width (px (- (* single-width 0.98) 1)) :size "none" :align
          :center :justify :between :gap "9px" :children
          [(if (not (and (= @db/editor-mode :viz) reco-selected?))
             [re-com/h-box ;:padding "8px"
              :justify :between :align :center :size "auto" :children
              [;[re-com/box
               [re-com/input-text :src (at) :model (str screen-name) :width "487px" :height "30px"
                :on-change #(ut/tracked-dispatch [::bricks/set-screen-name %]) :validation-regex
                screen-name-regex :change-on-blur? true :style
                {:font-size        "20px"
             ...
             [re-com/box :align :center :justify :center :child (str "viz preview: " reco-combo)
              :style
              {:font-size        "16px"
               :font-weight      700
               :font-style       "underline"
          ...
          {:padding-left   "8px"
           ...
           :background     (str "linear-gradient("
                                (theme-pull :theme/editor-rim-color nil)
                                ", transparent)")}]]]] :width ttl-width-px :height ttl-height-px
      :margin "-2px" :style
      {:position         "fixed"
       :top              y-px
       :left             x-px
...
(defn tab-menu
  ...
  (let [;tabs @(ut/tracked-subscribe [::bricks/tabs])
             ...
             (conj
               (vec
                 (conj
                                          ...
                                          [[re-com/input-text :model (str t) :on-change
                                            #(do (ut/tapp>> [:changed-tab-name (str t) :to (str %)])
                                                 (when (and (ut/ne? (cstr/trim (str %)))
                                                   ...
                                                   (ut/tracked-dispatch [::bricks/rename-tab (str t)
                                                                         (str %)])) ;; dont
                                                 (reset! title-edit-idx nil)) :change-on-blur? true
                                            :width (px (+ 15 (* (count (str t)) 11))) ;"inherit" ;
                                            :style
                                            {:background-color "#00000000"
                                             ...
                                             :color            (theme-pull
                                                                 ...
                                                                 nil)}]
                                           [re-com/md-icon-button :src (at) :md-icon-name
                                            "zmdi-delete" :tooltip "delete this tab" :style
                                            {:color       (theme-pull :theme/editor-outer-rim-color
                                                                      nil)
                                             :font-size   "14px"
                                             :padding-top "5px"
                                             :width       "10px"
                                             :height      "20px"} :attr
                                            {:on-click #(ut/tracked-dispatch [::bricks/delete-tab
                                                                              (str t)])}]]]
                                         (str t))) :size "none" :align :center :justify
                                     (when (not @db/show-tabs?) :center) :padding "5px" :width
                                     (when (not @db/show-tabs?) "5px") :style
                                     {:transition_NOT  "all 0.2s ease-in-out"
                                      :backdrop-filter "blur(8px)"}] :attr
                                    (merge (when (not (= @title-edit-idx t))
                                             {:on-click #(do (ut/tracked-dispatch
                                                               [::bricks/select-tab (str t)])
                                                             (reset! title-edit-idx nil))})
                                           (if selected?
                                             {:on-double-click #(reset! title-edit-idx (str t))}
                                             {})
                                           {:on-context-menu #(ut/tracked-dispatch
                                                                [::bricks/toggle-tab-visibility
                                                                 (str t)])}) :style
                                    {:cursor           "pointer"
                                     :background-color (theme-pull :theme/base-block-color-selected
                                                                   nil)
                                     :color            (theme-pull :theme/editor-outer-rim-color
                                                                   nil)
                                                         ...
                                                         (if (= @title-edit-idx t)
                                                           ...
                                                           (str
                                                             ...
                                                             (theme-pull
                                                               ...
                                                               nil))))}]]]
                       (if (not selected?)
                         (bricks/draggable (let [[_ _ w h] @(ut/tracked-subscribe
                                                              [::bricks/tab-recenter t])]
                                             {:w             w
                                              :selected-view :vv
                                              :name          (str t)
                                              :h             h
                   ...
                   (if hidden?
                     [re-com/md-icon-button :src (at) :md-icon-name
                      (if @db/show-hidden-tabs? "zmdi-tab" "zmdi-tab-unselected") :style
                      {:background-color "#00000099" ;(theme-pull
                       ...
                       :width            "30px"
                       :height           "30px"} :attr
                      {:on-click #(reset! db/show-hidden-tabs? (not @db/show-hidden-tabs?))}]
                     [re-com/md-icon-button :src (at) :md-icon-name
                      (if @db/show-tabs? "zmdi-chevron-left" "zmdi-chevron-right") :style
                      {:background-color (theme-pull :theme/base-block-color-selected nil)
                       ...
                       :width            "30px"
                       :height           "30px"} :attr
                      {:on-click #(reset! db/show-tabs? (not @db/show-tabs?))}])))
               (when (and @db/show-tabs? (not hidden?))
                 [re-com/md-icon-button :src (at) :md-icon-name "zmdi-plus" :tooltip "add new tab"
                  :style
                  {:color        (theme-pull :theme/editor-outer-rim-color nil)
                   :padding-left "4px"
                   :margin-left  "-3px"
                  ...
                  {:on-click #(let [new-tab (ut/make-tab-name)]
                                ...
                                (ut/tracked-dispatch [::bricks/select-tab new-tab]))}])) :height
             "30px" :style
             {:position        "fixed"
              :top             top
              :left            0
    ...
    [re-com/v-box :children
     [[tab-row 0 tabs] (when (ut/not-empty? hidden-tabs) [tab-row 30 hidden-tabs true])]]))
...
(defn snapshot-menu
  ...
  (let [;tabs @(ut/tracked-subscribe [::bricks/tabs])
        ...
        tabs       (vec (map :key
                          (sort-by
                            :key
                            (vec (for [[k v] snapshots :when (get v :menu?)] (assoc v :key k))))))
           ...
           (vec
             (conj
               (for [t (reverse tabs)]
                 [re-com/box :child
                  [re-com/box :child
                   (if (not @db/show-snaps?)
                     (str (try (.indexOf tabs t) (catch :default _ "err!")))
                     (str t)) :size "none" :align :center :justify
                   (when (not @db/show-snaps?) :center) :padding "5px" :width
                   (when (not @db/show-snaps?) "5px") :attr
                   {:on-click #(ut/tracked-dispatch [::bricks/swap-snapshot t])} :style
               ...
               [(if (and (not @db/show-snaps?) @db/vertical-snaps?) re-com/v-box re-com/h-box)
                :align :end :justify :end :children
                ((if (and (not @db/show-snaps?) @db/vertical-snaps?) reverse vec)
                  [[re-com/md-icon-button :src (at) :md-icon-name
                    (if @db/vertical-snaps? "zmdi-chevron-up" "zmdi-chevron-down") :tooltip
                    "minimize tab menu" :style
                    {:background-color (theme-pull :theme/base-block-color-selected nil)
                     :color            (theme-pull :theme/editor-outer-rim-color nil)
                     :padding-left     "4px"
                     :margin-left      "-3px"
                     ...
                     :width            "30px"
                     :height           "30px"} :attr
                    {:on-click #(reset! db/vertical-snaps? (not @db/vertical-snaps?))}]
                   [re-com/md-icon-button :src (at) :md-icon-name
                    (if @db/show-snaps? "zmdi-chevron-right" "zmdi-chevron-left") :tooltip
                    "minimize tab menu" :style
                    {;:transform (when @db/show-tabs? "rotate(90deg)")
                     ...
                     :width            "30px"
                     :height           "30px"} :attr
                    {:on-click #(reset! db/show-snaps? (not @db/show-snaps?))}]])]))
...
(defn mouse-up-handler2
  [on-move]
  (fn me [evt] (do (gevents/unlisten js/window EventType.MOUSEMOVE on-move))))
                   ...
                   (for [[k v] data]
                     (if (or (map? v) (vector? v))
                       (conj (for [ee (ut/kvpaths v)]
                               (str (keyword
                                      (ut/replacer (str prefix k ">" (cstr/join ">" ee)) ":" ""))))
...
(re-frame/reg-event-db
  ...
  (fn [db _]
    (let [fs                (vec (for [kk   (get db :flow-subs)
          ...
          new-autocompletes (vec (into (create-clover-keys-from-data (get pp-without-fs :param)
                                                                     "param/")
                                       (create-clover-keys-from-data (get pp-without-fs :theme)
                                                                     "theme/")))
...
(re-frame/reg-event-db ::set-rs-overrides-hashmap
                       (fn [db [_ new]] (assoc db :rs-overrides-hashmap new)))

(re-frame/reg-sub ::theme-colors-hashmap (fn [db] (get db :theme-colors-hashmap)))

(re-frame/reg-event-db ::set-theme-colors-hashmap
                       (fn [db [_ new]] (assoc db :theme-colors-hashmap new)))
...
(re-frame/reg-sub ::view-data
                  (fn [db [_ kp]] (get-in db (into [:panels (get db :selected-block)] kp))))

(def panels [:flow? :buffy? :editor?])

(re-frame/reg-sub ::minimized-system-panels
                  (fn [db _] (vec (for [p panels :when (not (get db p))] [p p]))))

(re-frame/reg-event-db ::toggle-sys-panel (fn [db [_ kkey]] (assoc db kkey (not (get db kkey)))))

(re-frame/reg-sub ::is-fire-on-change?
                  (fn [db [_ flow-id]] (get-in db [:runstreams flow-id :fire?] false)))
  ...
  (if @(ut/tracked-subscribe [::is-fire-on-change? flow-id])
    (let [client-name  @(ut/tracked-subscribe [::bricks/client-name])
          ...
          overrides    (if runstreamed?
                         @(ut/tracked-subscribe [::bricks/runstream-overrides flow-id])
                         overrides)
...
(defn task-bar
  ...
  (try
    (let [min-panels              @(ut/tracked-subscribe [::bricks/all-panels-minimized])
          ...
          sselected-view          @(ut/tracked-subscribe [::conn/clicked-parameter
                                                          [:param :selected-view]])
         ...
         (for [[e name] min-panels
           ...
           [re-com/h-box :align :center :justify :center :height "30px" :gap "6px" :attr
            ...
            {:background-color "#00000099"
             ...
             :cursor           "pointer"
             :color            (theme-pull :theme/editor-outer-rim-color nil)
             :backdrop-filter  "blur(2px)"} :children
            [[re-com/box :child (str nn)] [render-icon icon]]])]))
    (catch :default e
      (do (ut/tapp>> [:TASK-BAR-ERROR! (str e) (.-message e) :stack (.-stack e)])
          (js/console.log (str :TASK-BAR-ERROR! (str e) (.-message e) :stack (.-stack e)))))))

(re-frame/reg-event-db ::set-session-loading
                       (fn [db [_ session]] (assoc db :loading-session session)))
    ...
    (fn []
      (let [url         (str "snaps/" (ut/replacer (str s) ".edn" ".jpg") "?timestamp=" @timestamp)
        ...
        [re-com/v-box :size "auto" :style
         {:cursor        "pointer"
          :border-radius "8px"
          :border        (cond loading? (str "3px dashed "
                                             (theme-pull :theme/editor-outer-rim-color nil)
                                             88)
                               current? (str "3px solid "
                                             (theme-pull :theme/editor-outer-rim-color nil))
           ...
           [[re-com/box :size "auto" :child (str t " ago")]
            (if loading?
              [re-com/md-icon-button :md-icon-name "zmdi-refresh" :class "rotate linear infinite"
               :style {:font-size "15px" :transform-origin "7.5px 12px" :margin-bottom "-8px"}]
  ...
  (let [hh          @(ut/tracked-subscribe [::subs/h])
    ...
    [re-com/modal-panel :style
     ...
     {:child-container {:style {:left             left
                                ...
                                :position         "fixed"}}} :backdrop-opacity 0.75
     :backdrop-on-click #(ut/tracked-dispatch [::bricks/disable-session-modal]) :child
     [re-com/v-box :size "auto" :gap "4px" :style {:background-color "#000000"} :width (px w)
      :height (px h) :children
      (for [sess sessions]
                                ...
                                (let [src-r     @(ut/tracked-subscribe [::bricks/panel-px-root k])
                                      ...
                                      dd        (if (not (cstr/starts-with? (str d) ":block"))
                                                  @(ut/tracked-subscribe
                                                     [::bricks/lookup-panel-key-by-query-key d])
                                                  d)
                                      involved? (some #(= % dd) involved)
                                      dest-r    @(ut/tracked-subscribe [::bricks/panel-px-root dd])
                                      dest-h    @(ut/tracked-subscribe [::bricks/panel-px-height
                                                                        dd])
                                      ...
                                      (flatten
                                        [(if peek? (- x1 (* src-w 0.15)) x1) y1  ;(if peek? (* y1
                                         ...
                                         (cond (= dd selected-block) "#9973e0"
                                               ...
                                               (some #(= % dd)
                                                     (ut/cached-upstream-search subq-mapping
                                                                                selected-block))
                                                 "#7be073" ;; upstream?
                                               (some #(= % dd)
                                                     (ut/cached-downstream-search subq-mapping
                                                                                  selected-block))
                                                 "#05dfff" ;; downstream?
                                               :else "orange") k d nil]))))))))))]
              lmap)
      ...
      [re-com/box :style {:background-color "black"} :attr
                 ...
                 (when (and (not @bricks/over-block?) (not @bricks/over-flow?))
                   (let [x  (keyword
                     ...
                     (let [default "new parameter value!" ;(if (nil? default) "new parameter
                           ...
                           exists? (not (nil? vv))
                           _ (when (not exists?)
                               (ut/tracked-dispatch [::conn/click-parameter kp default]))])))
                 #_{:clj-kondo/ignore [:unresolved-symbol]}
       ...
       [re-com/v-box :size "1" :style
        ...
        [[rc/catch [tab-menu]] [rc/catch [snapshot-menu]] (when session? [session-modal])
         ...
         (when (or (and @bricks/dragging? (not @bricks/on-block?) (not @bricks/over-flow?))
           ...
           [re-com/box :child " " :style
            {:background-color (str (theme-pull :theme/editor-background-color nil) 22) ;; "#00000022"
             ...
             :position         "fixed"
             :left             (* (js/Math.floor (/ (first @db/context-modal-pos)
                                                    bricks/brick-size))
                                  bricks/brick-size)
             :top              (* (js/Math.floor (/ (last @db/context-modal-pos) bricks/brick-size))
         ...
         [re-com/box :child
          [re-com/md-icon-button :src (at) :md-icon-name "zmdi-pizza" :tooltip
           ...
           {:color        "#ffffff"
            ...
            :margin-top   "-2px"
            :padding-left "2px"
            :font-size    "15px"} :attr {:on-click #(ut/tracked-dispatch [::bricks/toggle-editor])}]
          ...
          {:position         "fixed"
           ...
           :color            "white"}]
         [re-com/box :child
          [re-com/md-icon-button :src (at) :md-icon-name (if lines? "zmdi-layers" "zmdi-layers-off")
           :tooltip "show lineage / sub-query lines? (L)" :style
           {:color        "#ffffff"
            :cursor       "pointer"
            :opacity      (if lines? 1.0 0.3)
            :margin-top   "-2px"
            :padding-left "2px"
            :font-size    "15px"} :attr {:on-click #(ut/tracked-dispatch [::bricks/toggle-lines])}]
          :width "20px" :height "20px" :style
          {:position         "fixed"
           :z-index          98
           ...
           :color            "white"}]
         [re-com/box :child
          [re-com/md-icon-button :src (at) :md-icon-name (if peek? "zmdi-eye" "zmdi-eye-off")
           :tooltip "toggle peek mode (P)" :style
           {:color        "#ffffff"
            :cursor       "pointer"
            :opacity      (if peek? 1.0 0.3)
            :margin-top   "-2px"
            :padding-left "2px"
            :font-size    "15px"} :attr {:on-click #(ut/tracked-dispatch [::bricks/toggle-peek])}]
          :width "20px" :height "20px" :style
          {:position         "fixed"
           :z-index          98
           :border-radius    "0px 7px 0px 0px"
         ...
         [re-com/box :child
          [re-com/md-icon-button :src (at) :md-icon-name
           (if auto-run? "zmdi-refresh-sync" "zmdi-refresh-sync-off") :tooltip
           ...
           {:color        "#ffffff"
            ...
            :padding-left "2px"
            :font-size    "15px"} :attr
           {:on-click #(ut/tracked-dispatch [::bricks/toggle-auto-run])}] :width "20px" :height
          "20px" :style
          {:position         "fixed"
           :z-index          98
           :border-radius    "0px 7px 0px 0px"
           ...
           :color            "white"}]
         [re-com/box :child
          [re-com/md-icon-button :src (at) :md-icon-name "zmdi-developer-board" :tooltip
           "  toggle external editing" :style
           {:color        (if external? "red" "#ffffff")
            ...
            :padding-left "2px"
            :font-size    "15px"} :attr
           {:on-click #(ut/tracked-dispatch [::bricks/toggle-external])}] :width "20px" :height
          "20px" :style
          {:position         "fixed"
           :z-index          98
           :bottom           0
           ...
           :color            "white"}]
         [re-com/box :child
          [re-com/md-icon-button :src (at) :md-icon-name "zmdi-close" :tooltip
           "un-select block (ESC)" :style
           {:color        "#ffffff"
            :cursor       "pointer"
            ...
            :padding-left "2px"
            :font-size    "15px"} :attr
           {:on-click #(ut/tracked-dispatch [::bricks/select-block "none!"])}] :width "20px" :height
          "20px" :style
          {:position         "fixed"
           :z-index          98
           :border-radius    "0px 7px 0px 0px"
              ...
              (for [[kk cnt] flow-watcher-subs-grouped]
                [re-com/h-box :style
                 {:background-color (theme-pull :theme/editor-outer-rim-color nil)
                  ...
                  :padding-right    "6px"
                  :color            (theme-pull :theme/editor-background-color nil)} :gap "5px"
                 :children
         ...
         [re-com/box :child
          ...
          {:position         "fixed"
           ...
           :color            "white"}]
         [re-com/box :child
          [re-com/md-icon-button :src (at) :md-icon-name "zmdi-refresh" :tooltip "refresh all"
           :on-click #(do (ut/tracked-dispatch [::bricks/refresh-all])) :style
           {:color        "#ffffff"
            :cursor       "pointer"
            :margin-top   "-2px"
            :padding-left "2px"
          ...
          {:position         "fixed"
           ...
           :color            "white"}]
         [re-com/box :child
          [re-com/md-icon-button :src (at) :md-icon-name "zmdi-save" :tooltip "save board (Ctrl-S)"
           :on-click #(do (ut/tracked-dispatch [::http/save :skinny screen-name])) :style
           {:color        "#ffffff"
            :cursor       "pointer"
            :margin-top   "-2px"
            :padding-left "2px"
