                       ...
                       (fn [db [_ content adding]]
                         ...
                         (let [curr (get-in db [:signals-map (get db :selected-warren-item) :signal])]
                           (assoc-in db
                             ...
                             (ut/postwalk-replacer {content (vec (conj content (if (vector? adding) adding [adding])))} curr)))))
...
(re-frame/reg-event-db ::edit-signal-drop-kill
                       ...
                       (fn [db [_ content]]
                         (let [curr (get-in db [:signals-map (get db :selected-warren-item) :signal])]
                           ...
                           (assoc-in db [:signals-map (get db :selected-warren-item) :signal] (ut/remove-subvec curr content)))))
...
(defn kill-nest
  [op]
  [re-com/box :child
   [re-com/md-icon-button :src (at) :md-icon-name "zmdi-close" :style {:font-size "14px" :opacity 0.3 :padding-right "5px"}]
   :style {:cursor "pointer"} :attr
   {:on-click #(do (ut/tapp>> [:kill-nested op]) (ut/tracked-dispatch [::edit-signal-drop-kill op]))}])
  ...
  (let [hovered?             (= @hover-tools-atom coll)
    ...
    [re-com/v-box :attr
     {:on-mouse-enter #(reset! hover-tools-atom coll)
      ...
      :on-mouse-leave #(do (unhighlight-code) (reset! hover-tools-atom nil))} :align :center :justify :center :width "40px"
     :children
     [[re-com/box :style
...
(defn visualize-clause
  ...
  (let [is-operator?     (fn [c] (and (vector? c) (keyword? (first c)) (contains? #{:and :or :not} (first c))))
                           ...
                           (let [body     (vec (cons operator contents))
                             ...
                             [re-com/h-box :style
                              ...
                              [[re-com/v-box :size "auto" :width (px (- 540 (* lvl 35))) :children
                                (cons [droppable-area [re-com/box :child (str operator)] [:operator] contents operator]
                                      (map #(visualize-clause % (inc lvl) contents full-clause) contents)) :gap "8px" :style
                                (merge
                                  {:border           (str "3px solid " (theme-pull :theme/editor-outer-rim-color nil) 45)
                                   ...
                                   :font-weight      700
                                   :margin-left      (if (is-true? body full-clause) (str (* lvl 11) "px") (str (* lvl 10) "px"))}
                                  (when (is-true? body full-clause)
        ...
        render-condition (fn [condition level contents]
                           (let [bbox [re-com/h-box :children [[re-com/box :child (str condition) :size "auto"]] :style
          ...
          [re-com/h-box :style
           (merge {;:transition "all 0.2s" :border-radius "8px"
                  ...
                  {;:font-size "17px" :overflow "hidden" :max-width 200
                   :font-weight 300
                   :margin-left (if (is-true? clause full-clause) (str (* level 11) "px") (str (* level 10) "px"))}) :justify
           :between :align :center ;:padding "6px"
           :children
           [[re-com/h-box :children (map #(visualize-clause % level clause full-clause) clause) :gap "10px" :style
             {;:border "1px solid #ccc"
              :border-radius    "8px"
              :border           (str "3px solid " (theme-pull :theme/editor-outer-rim-color nil) 25)
              :background-color (str (theme-pull :theme/editor-outer-rim-color nil) "08")
              :padding          "5px"}] [hover-tools clause full-clause]]])) ; Keep pairings on the same row
...
(re-frame/reg-event-db ::select-signal (fn [db [_ signal-name]] (assoc db :selected-warren-item signal-name)))

(re-frame/reg-sub ::operators
                  (fn [db _] (get-in db [:signal-items :operators] [:= :<> :> :< :>= :<= :and :or :not :like :changed?])))

(re-frame/reg-sub ::conditions (fn [db _] (get-in db [:signal-items :conditions] [:and :or :not])))

(re-frame/reg-sub ::time-items
                  (fn [db _] (get-in db [:signal-items :time-items] [[:= :day 1] [:= :hour 13] [:= :month 1] [:= :year 2021]])))

(re-frame/reg-sub ::selected-warren-item (fn [db _] (get db :selected-warren-item)))

(re-frame/reg-event-db ::edit-signal (undoable) (fn [db [_ signal name]] (assoc-in db [:signals-map name :signal] signal)))

(re-frame/reg-event-db ::edit-basic
                       (undoable)
...
(re-frame/reg-event-db
  ...
  (fn [db [_ item-type]]
    (let [item-type     (keyword (ut/drop-last-char item-type))
          ...
          existing-keys (vec (into (into (keys (get db :signals-map {})) (keys (get db :solvers-map {})))
          ...
          starting-data (cond (= item-type :signal) [:and [:= :day 1] [:= :hour 9]]
                              (= item-type :solver) {:signal false :persist? false :type :clojure :default 0 :data (+ 1 2 3)}
                              :else                 {:flow-id nil :signal nil :overrides {}})
          kp            (if (= item-type :signal) [:signals-map signal-name :signal] [item-type-key signal-name])]
...
(defn items-list
  ...
  (let [;signals @(ut/tracked-subscribe [::signals-map])
        ...
        react!           [@waiting? @db/selectors-items @db/selectors-open]
          ...
          [re-com/v-box :padding "6px" :size "none" :width "100%" :attr
           ...
           {:border           (if selected?
            ...
            :background-color (cond selected? ;;(or selected? (true? vv))
                                    ...
                                    (true? vv) (str (theme-pull :theme/editor-outer-rim-color nil) 30) ;; "rgba(0,
                                    :else      "rgba(0, 0, 0, 0.1)")
            :cursor           "pointer"} :children
           [[re-com/h-box :style {:font-size "19px"} :justify :between :children
             [[re-com/box :style {:font-size "15px" :font-weight 700} :padding "6px" :size "auto" :child (str name)]
...
(defn selector-panel
  ...
  (let [open?       (some #(= name %) @db/selectors-open)
        ...
        react!      [@db/selectors-items @db/selectors-open]
        partition?  (integer? wide?)]
    [re-com/v-box :padding "6px" :style
     (merge {:border (str "2px solid " (theme-pull :theme/editor-outer-rim-color nil) 55) :border-radius "4px"} style) :children
     [[re-com/h-box :height "28px" :style
       {:cursor "pointer" :user-select "none" :color (theme-pull :theme/editor-outer-rim-color nil)} :justify :between :align
       :center :attr
       {:on-click #(if open?
                     (do (reset! db/selectors-open (remove (fn [x] (= x name)) @db/selectors-open))
                         (swap! db/selectors-items dissoc name))
                     (do (swap! db/selectors-open conj name)
                         (swap! db/selectors-items assoc name (vec (keys items)))))}
       :children [[re-com/h-box :style {:font-size "22px"} :gap "8px" :align :center :justify :center :children
                   [[re-com/box :child name] [re-com/box :style {:opacity 0.55 :font-size "15px"} :child (str "(" items-count ")")]]]
      ...
      (when open?
        (if (or ;(= name "solvers")
          ...
          (let [;signals @(ut/tracked-subscribe [::signals-map]) ;; they get passed from the
                solver? (cstr/includes? (str name) "solvers")
                selected-warren-item @(ut/tracked-sub ::selected-warren-item {})
                ;; itmz (cond (not solver?)
                ;;            (into {}
                ;;                  (for [[name _] items]
                ;;                    (let [sigkw (keyword (str "signal/" (ut/replacer (str name) ":" "")))
                ;;                          vv    @(ut/tracked-sub ::conn/clicked-parameter-key-alpha
                ...
                ;;_ (ut/tapp>> [:itmz itmz])
                ph                   200]
            [items-list (if selected-warren-item (* ph 0.3) ph)
             items selected-warren-item name])
          (if partition?
                  ...
                  [draggable-item
                   [re-com/box :width (px width) :padding "6px" :style
                    {:border           "1px solid purple"
                     ...
                     :font-family      (theme-pull :theme/monospaced-font nil)} :child (str item)] :operator item])])]
            [(if wide? re-com/h-box re-com/v-box)
             :children
             (for [item items]
               [draggable-item
                [re-com/box :padding "6px" :style
                 {:border "1px solid purple" :font-family (theme-pull :theme/monospaced-font nil)} :child (str item)] :operator
                item])])))]]))

(re-frame/reg-sub ::signal-flow-id (fn [db _] (get-in db [:signals-map (get db :selected-warren-item) :flow-id])))

(re-frame/reg-sub ::signal-open-inputs (fn [db _] (get-in db [:signals-map (get db :selected-warren-item) :open-inputs])))

(re-frame/reg-event-db ::edit-signal-flow-id
                       (undoable)
                       (fn [db [_ flow-id]] (assoc-in db [:signals-map (get db :selected-warren-item) :flow-id] flow-id)))
  ...
  (let [server-flows (map :flow_id @(ut/tracked-subscribe [::conn/sql-data [:flows-sys]]))
    ...
    [re-com/v-box :padding "6px" :children
     [[re-com/box :padding "6px" :child "flow to run when signals are met:"]
      (if fid
        [re-com/h-box :align :center :justify :between :padding "6px" :width "320px" :height "35px" :style
         {;:border "1px solid #ffffff33"
          :font-family (theme-pull :theme/monospaced-font nil)} :children
         [[re-com/box :style {:overflow "hidden" :font-size "17px" :white-space "nowrap" :max-width "292px"} :child (str fid)]
          [re-com/box :style {:cursor "pointer"} :attr {:on-click #(ut/tracked-dispatch [::edit-signal-flow-id nil])} :child
           "x"]]]
        [re-com/typeahead :suggestion-to-string (fn [item] (str (get item :label))) :render-suggestion
         ...
         #(do (ut/tracked-dispatch [::refresh-open-blocks (get % :id)]) (ut/tracked-dispatch [::edit-signal-flow-id (get % :id)]))
         :rigid? true :style {:font-family (theme-pull :theme/monospaced-font nil)} :width "320px" :change-on-blur? true
       ...
       (for [btn ["input values" "when logic"]]
         [re-com/v-box :height "50px" :align :center :justify :center :size "auto" :style
          (merge {;:border "1px solid pink"
                  :cursor "pointer"}
                 (if (= btn @mode-atom) {:background-color "rgba(0, 0, 255, 0.3)"} {:background-color "rgba(0, 0, 0, 0.1)"}))
...
(re-frame/reg-event-db
  ...
  (fn [db [_ old new item-type]]
    (let [sver          (fn [x] (ut/replacer (str x) ":" ""))
          item-type-str (sver item-type)
          item-type-key (keyword (str item-type-str "s-map"))
          refs          {(keyword (str item-type-str "/" (sver old))) (keyword (str item-type-str "/" (sver new)))}]
      (ut/tapp>> [:rename-item! old new item-type item-type-str item-type-key refs])
      (-> db
          (assoc :signals-map (ut/postwalk-replacer (if (= item-type-key :signals-map) (merge {old new} refs) refs)
                                                    (get db :signals-map {}))) ;; change all fqns refs
          (assoc :rules-map (ut/postwalk-replacer (if (= item-type-key :rules-map) (merge {old new} refs) refs)
                                                  (get db :rules-map {})))
          (assoc :solvers-map (ut/postwalk-replacer (if (= item-type-key :solvers-map) (merge {old new} refs) refs)
                                                    (get db :solvers-map {})))
  ...
  (let [;read-only-flow? (true? (cstr/includes? flow-id "/"))
    ...
    (if (not @title-edit-idx)
      [re-com/box :size "none" :height "45px" :width (px w) :align :center :justify :start :attr
       ...
       {:cursor "pointer" :padding-right "12px" :padding-top "1px" :border "2px solid transparent" :font-size "26px"} :child
       (str item-name)]
      [re-com/input-text :src (at) :model (ut/replacer (str item-name) ":" "") :width (px (- w 6)) :height "45px" :on-change
       #(do (ut/tracked-dispatch [::rename-item item-name (try (keyword (str %)) (catch :default _ (str %))) item-type])
            (reset! title-edit-idx nil)) :validation-regex flow-id-regex :change-on-blur? true :style
...
(re-frame/reg-sub
  ...
  (fn [db _]
    (let [signals (vec (keys (get db :signals-map)))
      ...
      (and (get db :flow?) (not (nil? sel)) (some #(= sel %) signals) (= (get @db/flow-editor-system-mode 0) "signals")))))
     ...
     [(when selected-warren-item
      ...
      (when signal?
        (let [reaktsz [@db/signal-history-ticker?]]
          [re-com/h-box :gap "10px" :align :center :style {:font-size "13px" :padding-left "21px"} :children
           [[re-com/box :child (str (if @db/signal-history-ticker? "stop" "play")) :style
             {:color (if @db/signal-history-ticker? "red" "green") :cursor "pointer" :font-weight 700} :attr
             {:on-click #(reset! db/signal-history-ticker? (not @db/signal-history-ticker?))}]
            [re-com/box :child " live history ticker"]
            [re-com/md-icon-button :src (at) :md-icon-name "zmdi-time-countdown" :style {:font-size "17px"}]]]))

      (let [signals-history2 (when signal?
                               (into {}
                                     (for [tt (range (count (get signals-history (first (keys signals-history)))))]
                                       {tt (vec (for [kk (keys signals-history)] [kk (get-in signals-history [kk tt])]))})))]
         ...
         (cond (and signal? (not (nil? selected-warren-item)))
                 [re-com/h-box :width "600px" :children
                  (for [[e vv] signals-history2
                        :let   [main       (reduce (fn [a b] (if (> (count a) (count b)) a b)) (map first vv))
                     ...
                     (for [[kk [ee tt]] vv
                       ...
                       [re-com/box :attr
                        {:on-mouse-enter #(history-mouse-enter kk)
                         ...
                         :on-mouse-leave history-mouse-leave} :size "auto" :align :center :justify :center :style
                        (merge (when (= kk @hover-tools-atom) {:filter "invert(233%)"})
                               (when (not main-true?)
                                 {:border (str "1px solid " (str (theme-pull :theme/editor-outer-rim-color nil) 45))})
                               {:font-family (theme-pull :theme/monospaced-font nil) :color "#000000" :font-size "10px"}
  ...
  (let [item-type-str (ut/replacer (str item-type) ":" "")
    ...
    [re-com/h-box :padding "6px" :justify :between :align :center :children
     [[re-com/h-box :align :center :justify :center :gap "8px" :style {:opacity 0.15} :children
       [[re-com/h-box :attr {:on-click #(ut/tracked-dispatch get-map-evt-vec)} :style {:cursor "pointer"} :gap "4px" :children
      ...
      [re-com/h-box :align :center :justify :center :attr
       {:on-click #(ut/tracked-dispatch [::wfx/request :default
                                         {:message {:kind        server-fn ;;:save-signals-map
                                                    :data-map    data-map
                                                    :client-name db/client-name}
                                          :timeout 15000000}])} :style {:cursor "pointer" :opacity 0.15} :gap "4px" :children
       [[re-com/box :child (str "push this " item-type-str "s set to server?")]
        [re-com/md-icon-button :src (at) :md-icon-name "zmdi-upload" :style {:font-size "17px"}]]]]]))
  ...
  (let [[_ hpct]             db/flow-panel-pcts ;; [0.85 0.50]
        ...
        get-map-evt-vec      [::wfx/request :default
                              {:message     {:kind        item-type-key ;;:signals-map
                                             :client-name db/client-name}
                               :on-response (cond (= item-type-key :rules-map)   [::rules-map-response]
    ...
    [re-com/v-box :padding "6px" :width (px (- (last @db/flow-editor-system-mode) 14)) :size "none" :height (px ph) :style
