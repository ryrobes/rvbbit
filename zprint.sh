...
find "$DIRECTORY" -type f \( -name "*.clj" -o -name "*.cljs" -o -name "*.cljc" \) -print0 | while IFS= read -r -d '' file; do
  ...
  #zprint '{:style [:sort-require :ns-justify :justified-original] :comment {:smart-wrap? false} :width 100 :map {:comma? false :sort? false}}' < "$file.nocomments" > "$file.tmp" && mv "$file.tmp" "$file"
  zprint '{:style [:sort-require :ns-justify :respect-nl :respect-bl :justified-original] :comment {:smart-wrap? false} :width 100 :map {:comma? false :sort? false}}' < "$file.nocomments" > "$file.tmp" && mv "$file.tmp" "$file"
